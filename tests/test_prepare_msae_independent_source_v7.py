...
SCRIPT = Path(__file__).parents[1] / "scripts/prepare_msae_independent_source_v7.py"
ACQUISITION_SCRIPT = Path(__file__).parents[1] / "scripts/acquire_msae_independent_source_v7.py"
_spec = importlib.util.spec_from_file_location("msae_v7", SCRIPT)
assert _spec and _spec.loader
m = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = m
_spec.loader.exec_module(m)
_acq_spec = importlib.util.spec_from_file_location("msae_v7_acquisition", ACQUISITION_SCRIPT)
...
def acquisition_fixture(tmp_path: Path, monkeypatch):
    root=tmp_path;prov=root/"reports/provenance/msae_independent_source_v7";prov.mkdir(parents=True)
    config=root/"configs/msae_independent_source_v7/acquisition.json";config.parent.mkdir(parents=True)
    ...
    registry=prov/"historical_source_registry.json";registry.write_text(json.dumps({
        "schema_version":"msae_independent_source_v7_historical_source_registry_v1",
        "status":"frozen_preacquisition","domain_utf8":"msae-v7/pedigree","inputs":[],
    ...
    screen=prov/"preacquisition_alias_screen.json";screen.write_text(json.dumps({
        "schema_version":"msae_independent_source_v7_preacquisition_alias_screen_v2",
    ...
    baseline=prov/"baseline_inventory.json";baseline.write_bytes(acq.canonical({
        "schema_version":"msae_independent_source_v7_baseline_inventory_v1",
        "training_root_entries":[],"entries":[],"history_inputs_sha256":empty_digest,
        "authority_expected_absent":sorted(acq.V7_AUTHORITY)}))
    ...
    authority.write_bytes(acq.canonical({"status":"approved_for_exact_source_acquisition","history_inputs_sha256":empty_digest,"artifact_sha256":{
        "scripts/acquire_msae_independent_source_v7.py":acq.sha_file(ACQUISITION_SCRIPT),
        "configs/msae_independent_source_v7/acquisition.json":acq.sha_file(config)}}))
    review=root/"reports/adversarial/msae_independent_source_v7_preacquisition_authority_review.md";review.parent.mkdir(parents=True)
    ...
    monkeypatch.setattr(acq,"RAW",root/"data/msae_independent_source_v7/raw"/acq.COMMIT)
...
def acquisition_success_fixture(tmp_path: Path,monkeypatch):
    ...
    entry={"schema_version":"msae_independent_source_v7_source_acquisition_entry_v1","status":"entered",
    ...
    clone_dir="/tmp/msae-v7-acquire-fixture/repo";empty_home="/tmp/msae-v7-acquire-fixture/home"
    argvs=[[*config["ignore_argv"][:-1],"data/msae_independent_source_v7/raw/sentinel"],
    ...
    report={"schema_version":"msae_independent_source_v7_source_acquisition_v1","source_repo":acq.REPO,
        ...
        "ignore_checks":[{"path":"data/msae_independent_source_v7/raw/sentinel","stdout_sha256":by["ignore_raw"]["stdout_sha256"],
                          "stderr_sha256":by["ignore_raw"]["stderr_sha256"],"exit_status":0},
                         {"path":"data/msae_independent_source_v7/private/sentinel","stdout_sha256":by["ignore_private"]["stdout_sha256"],
...
def test_split_and_manifest_golden():
    ...
    golden = {
        "schema_version": "msae_independent_source_v7_split_manifest_v1",
        "source_commit": "0" * 40,
    ...
    assert hashlib.sha256(m.canonical_file_bytes(golden)).hexdigest() == "ce0680d5fcd2a5f26a4c03b957539061c4d5a70dab584974cce880d28ae1f7af"
    assert m.group_split_key("doc-a", "0" * 40) == hashlib.sha256(
        m.canonical_bytes(["msae-independent-source-v7/C1C2-group", "0" * 40, "doc-a"])).hexdigest()


def test_create_once_private_payload_and_no_source_output(tmp_path: Path, capsys):
    private = tmp_path / "private"
    records = [{"canary": "MSAE_V7_SECRET_7f6b6aa8", "id": 1}]
...
def test_pedigree_normalization_git_equivalence_and_hash_domain():
    ...
    assert m.pedigree_hash(value) == hashlib.sha256(b"msae-v7/pedigree\0" + value.encode()).hexdigest()


def test_pedigree_json_whitespace_bound_is_exact():
    accepted=b'"dataset"'+b" "*256+b":"+b"\t"*256+b'"UD_Finnish-TDT"'
    assert m.PEDIGREE_JSON.search(accepted).group(1)==b"UD_Finnish-TDT"
...
def test_registry_history_binding_is_exact():
    ...
    registry={"schema_version":"msae_independent_source_v7_historical_source_registry_v1",
              "status":"frozen_preacquisition","domain_utf8":"msae-v7/pedigree",
    ...
    screen={"schema_version":"msae_independent_source_v7_preacquisition_alias_screen_v2",
...
def test_frozen_v7_plan_chain_matches_reviewed_source_free_artifacts():
    assert m.verify_frozen_v7_authority_bindings()=={
...
def _acquisition_baseline_entry(root: Path, path: Path, disposition: str) -> dict[str, object]:
    ...
    return {"path":path.relative_to(root).as_posix(),"size":st.st_size,
        ...
        "adapter":"authority" if disposition=="v7_authority" else "opaque_binary",
        "content_reads":1,"extracted_unit_count":0,"hardlink_aliases":[]}


def test_acquisition_history_binding_rejects_unknown_v7_provenance(tmp_path: Path, monkeypatch):
...
@pytest.mark.parametrize("disposition,relative",(
    ...
    ("v7_authority","docs/plan-msae-independent-source-v7.md"),
))
...
def test_acquisition_history_binding_returns_registry_array_digest_and_reaches_entry(
    ...
    registry={"schema_version":"msae_independent_source_v7_historical_source_registry_v1",
        "status":"frozen_preacquisition","domain_utf8":"msae-v7/pedigree","inputs":records,
    ...
    baseline.update(entries=[text_entry,binary_entry],history_inputs_sha256=history_digest)
    acq.BASELINE.write_bytes(acq.canonical(baseline))
...
def test_source_family_alias_scan_catches_chunk_boundary(tmp_path: Path, monkeypatch):
    ...
    path.write_bytes(b" "*(1024*1024-3)+b"fin"+b"nish tdt ")
    ...
    assert result["content_occurrences"]==[{"path":"history.txt","aliases":["finnish tdt"]}]
...
def test_process_snapshot_schema_and_hash_only():
    ...
    assert snapshot["schema_version"] == "msae_independent_source_v7_process_snapshot_v1"
...
def test_inventory_rejects_populated_v7_and_directory_symlink_without_opening(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path)
    raw = tmp_path / "data/msae_independent_source_v7/raw"
    raw.mkdir(parents=True); secret = raw / "secret"; secret.write_text("must not open")
    with pytest.raises(m.GateFailure, match="preexisting_v7_namespace"):
        m.inventory_repository()
    secret.unlink(); raw.rmdir(); (tmp_path/"data/msae_independent_source_v7").rmdir()
...
def test_frozen_acquisition_config():
    config=SCRIPT.parents[1]/"configs/msae_independent_source_v7/acquisition.json"
...
def test_training_root_reconstruction_uses_global_path_order(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(m,"_walk_paths_allow_v7",lambda:iter([(z,"results/z.txt"),(a,"results/a.txt")]))
...
def test_authority_manifest_is_create_once_and_review_bound(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path)
    prov=tmp_path/"reports/provenance/msae_independent_source_v7";prov.mkdir(parents=True)
    ...
    impl=tmp_path/"reports/adversarial/msae_independent_source_v7_preacquisition_implementation_review.md"
    ...
    for path in sorted(item for item in tmp_path.rglob("*") if item.is_file()):
        ...
        entries.append({"path":rel,"disposition":"v7_authority","size":st.st_size,"mode":st.st_mode&0o777,
    ...
    with pytest.raises(m.GateFailure,match="authority_manifest_already_exists"):
        m.build_authority_manifest()
    review=tmp_path/"reports/adversarial/msae_independent_source_v7_preacquisition_authority_review.md"
...
def test_baseline_requires_ship_review_before_publication(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path);prov=tmp_path/"reports/provenance/msae_independent_source_v7";prov.mkdir(parents=True)
    monkeypatch.setattr(m,"PROV",prov)
    paths=["docs/plan-msae-independent-source-v7.md","reports/adversarial/msae_independent_source_v7_plan_review.md",
           "reports/provenance/msae_independent_source_v7/preacquisition_alias_screen.json",
           "reports/provenance/msae_independent_source_v7/historical_source_registry.json","tests/test_prepare_msae_independent_source_v7.py",
           "configs/msae_independent_source_v7/acquisition.json","reports/verification/msae_independent_source_v7_source_free_checks.log",
           "scripts/acquire_msae_independent_source_v7.py"]
    for rel in paths:
        path=tmp_path/rel;path.parent.mkdir(parents=True,exist_ok=True);path.write_text(rel)
    review=tmp_path/"reports/adversarial/msae_independent_source_v7_preacquisition_implementation_review.md"
...
def test_acquisition_lstat_preflight_stops_before_process(tmp_path: Path, monkeypatch, blocked: str):
    ...
    raw=tmp_path/"data/msae_independent_source_v7/raw"/acq.COMMIT
...
def test_pre_payload_namespace_is_exact(tmp_path: Path, monkeypatch, mutation: str):
    raw=tmp_path/"data/msae_independent_source_v7/raw"/m.COMMIT;raw.mkdir(parents=True)
...
def test_scientific_rejection_publisher_fault_recovers_through_real_terminal_verifier(
    ...
    prov=tmp_path/"reports/provenance/msae_independent_source_v7";prov.mkdir(parents=True)
    private=tmp_path/"data/msae_independent_source_v7/private";private.mkdir(parents=True)
    monkeypatch.setattr(m,"ROOT",tmp_path);monkeypatch.setattr(m,"PROV",prov);monkeypatch.setattr(m,"PRIVATE",private)
    rejection={"schema_version":"msae_independent_source_v7_rejection_v1","status":"rejected",
        ...
        "k2_or_branch_training_authorized":False,"stage_c_authorized":False,
        "terminal_process_snapshot":m.process_snapshot(),"training_root_status":"unavailable",
    ...
    publish_scientific_terminal_with_recoverable_fault(prov/"rejection.json",rejection,fault,monkeypatch)
    m.verify_terminal()
    (prov/".rejection.json.building").write_text("partial",encoding="utf-8")
...
def test_verify_terminal_rejection_enforces_payload_absence(tmp_path: Path, monkeypatch):
    ...
    os.chmod(prov/"rejection.json",0o666)
    with pytest.raises(m.GateFailure,match="terminal_final_custody"):m.verify_terminal()
    os.chmod(prov/"rejection.json",0o644)
    alias=tmp_path/"rejection-alias.json";os.link(prov/"rejection.json",alias)
    with pytest.raises(m.GateFailure,match="terminal_final_custody"):m.verify_terminal()
    alias.unlink()
...
def test_verify_terminal_complete_synthetic_success(tmp_path: Path, monkeypatch, terminal_publish_fault: str):
    ...
    monkeypatch.setattr(m,"ROOT",tmp_path);monkeypatch.setattr(m,"PROV",prov);monkeypatch.setattr(m,"PRIVATE",private)
    for rel in ("docs/plan-msae-independent-source-v7.md","reports/adversarial/msae_independent_source_v7_plan_review.md",
                "scripts/acquire_msae_independent_source_v7.py","tests/test_prepare_msae_independent_source_v7.py"):
        path=tmp_path/rel;path.parent.mkdir(parents=True,exist_ok=True);path.write_text(rel)
    ...
    split={"schema_version":"msae_independent_source_v7_split_manifest_v1","source_commit":m.COMMIT,
    ...
    role_entries=lambda prefix:[{"zero_based_rank":index,"sent_id":f"{prefix}{index}",
                                "source_record_sha256":h} for index in range(20)]
    roles={"schema_version":"msae_independent_source_v7_role_manifest_v1","source_commit":m.COMMIT,"roles":{
    ...
    source={"schema_version":"msae_independent_source_v7_source_manifest_v1","source_commit":m.COMMIT,
    ...
    dedup={"schema_version":"msae_independent_source_v7_dedup_v1","roles":{
    ...
    support={"schema_version":"msae_independent_source_v7_support_v1","floor_distinct_utterances":20,
    ...
    snapshot={"schema_version":"msae_independent_source_v7_process_snapshot_v1",
    ...
    baseline={"schema_version":"msae_independent_source_v7_baseline_inventory_v1",
    ...
    (raw/"README.md").write_text("fixture readme",encoding="utf-8")
    census_files={}
    ...
    census={"schema_version":"msae_independent_source_v7_document_group_census_v1","newdoc_marker_count":0,
    ...
    license_value={"schema_version":"msae_independent_source_v7_license_v1","license_id":"CC-BY-SA-4.0",
        ...
        "evidence":{"LICENSE.txt":evidence,"README.md":evidence},"obligations":["attribution","share_alike"],
        "raw_committed":False,"payload_committed":False,"contradiction_count":0,"status":"eligible"}
    family={"schema_version":"msae_independent_source_v7_source_family_v1","status":"eligible",
        "disposition":"project_source_use_unseen_before_v7","content_occurrence_count":0,"content_occurrences":[],
    ...
    registry={"schema_version":"msae_independent_source_v7_historical_source_registry_v1",
        "status":"frozen_preacquisition","domain_utf8":"msae-v7/pedigree","inputs":[],"input_count":0,
        "inputs_sha256":empty_sha,"overbound_json_source_key_construct_count":0}
    pedigree={"schema_version":"msae_independent_source_v7_candidate_pedigree_v1",
    ...
    values={
        ...
        "source_acquisition_entry.json":{"schema_version":"msae_independent_source_v7_source_acquisition_entry_v1"},
        "source_acquisition.json":{"schema_version":"msae_independent_source_v7_source_acquisition_v1"},
        ...
        "history_overlap.json":{"schema_version":"msae_independent_source_v7_history_overlap_v1",
        ...
        "cross_role_overlap.json":{"schema_version":"msae_independent_source_v7_cross_role_overlap_v1",
        ...
        "preacquisition_authority_manifest.json":{"schema_version":"msae_independent_source_v7_preacquisition_authority_v1"},
    }
    ...
    seal={"schema_version":"msae_independent_source_v7_seal_v1",
          "status":"independent_source_ready_for_future_prescore_protocol",
          "source_status":"project_source_use_unseen_before_v7",
          "plan_sha256":m.sha_file(tmp_path/"docs/plan-msae-independent-source-v7.md"),
          "plan_review_sha256":m.sha_file(tmp_path/"reports/adversarial/msae_independent_source_v7_plan_review.md"),
          "program_sha256":m.sha_file(SCRIPT),
          "acquisition_runner_sha256":m.sha_file(tmp_path/"scripts/acquire_msae_independent_source_v7.py"),
          "test_sha256":m.sha_file(tmp_path/"tests/test_prepare_msae_independent_source_v7.py"),
          "artifact_sha256":bindings,
          "payload":{"path":"data/msae_independent_source_v7/private/blind_payload.jsonl",
    ...
    m.write_json(prov/"no_training_gate.json",{"schema_version":"msae_independent_source_v7_no_training_gate_v1",
    ...
    monkeypatch.setattr(m,"reconstruct_scientific_artifacts",lambda:{"sha256":m.sha_file(payload),"size":ps.st_size,"record_count":40})
    closure_calls=[]
    monkeypatch.setattr(m,"_verify_history_snapshot",lambda *_a,**_k:closure_calls.append("history"))
    monkeypatch.setattr(m,"verify_training_roots",lambda *_a,**_k:closure_calls.append("training"))
    m.verify_terminal()
    assert closure_calls==["history","training"]
    ...
    monkeypatch.setattr(m,"_verify_history_snapshot",
        lambda *_a,**_k:(_ for _ in ()).throw(m.GateFailure("history_structural_addition")))
    with pytest.raises(m.GateFailure,match="history_structural_addition"):m.verify_terminal()
    monkeypatch.setattr(m,"_verify_history_snapshot",lambda *_a,**_k:None)
    monkeypatch.setattr(m,"verify_training_roots",
        lambda *_a,**_k:(_ for _ in ()).throw(m.GateFailure("training_root_delta")))
    with pytest.raises(m.GateFailure,match="training_root_delta"):m.verify_terminal()
