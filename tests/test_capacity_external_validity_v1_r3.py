...
spec=importlib.util.spec_from_file_location('cev1',ROOT/'scripts/capacity_external_validity_v1_r3.py');m=importlib.util.module_from_spec(spec);sys.modules['cev1']=m;assert spec.loader;spec.loader.exec_module(m)
...
def test_plan_declares_k2_closed():
 text=(ROOT/'PLAN_CAPACITY_EXTERNAL_VALIDITY_V1_R3.md').read_text();assert 'No K2 path exists' in text


def test_r3_config_and_source_parity():
 z=m.verify_parity();assert z['status']=='PASS';assert z['restore_paths_forbidden']
 with tempfile.TemporaryDirectory() as td:
  bad=Path(td)/'bad.py';bad.write_text((ROOT/'scripts/capacity_external_validity_v1_r3.py').read_text().replace("rcond=c['methods']['svd_rtol'])@Y.astype",'rcond=0.5)@Y.astype',1));old=m.SCRIPT;m.SCRIPT=bad
  try:
   try:m.verify_parity()
...
def test_all_jsonl_guard_blocks_access_surfaces():
 ...
 with m.no_jsonl_access_guard():
  for fn in (lambda:p.read_text(),lambda:p.read_bytes(),lambda:p.stat(),lambda:p.open(),lambda:list(Path('/tmp').glob('*.jsonl'))):
   try:fn()
