...
for _local_module in (
    ...
    "msae_independent_measurement_v3_post_m2_gen8",
    "msae_independent_measurement_v3_post_m2_gen8_runtime",
    "msae_measurement_remediation_v1", "msae_measurement_v2",
    "run_msae_independent_calibration_v3", "run_msae_independent_calibration_v3_gen8",
):
    sys.modules.pop(_local_module, None)
_controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen8.py"
_controller = types.ModuleType("msae_independent_measurement_v3_post_m2_gen8")
...
SPEC = _controller.source_only_module_spec(
    ...
    ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen8_runtime.py")
assert SPEC and SPEC.loader
runtime = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(runtime)


def test_gen8_reviewed_authority_constants_and_public_entry_are_exact():
    ...
    assert entries.index("build_changed_regions") + 1 == \
        entries.index("probe_gen8_capability")
    assert runtime.GEN8_SUBSTITUTIONS == (
        (b"post_m7_gen7", b"post_m8_gen8"),
        (b"post-m7-gen7", b"post-m8-gen8"),
        (b"msae-independent-v3-gen7", b"msae-independent-v3-gen8"),
        (b"m7t_", b"m8t_"), (b"m7b_", b"m8b_"),
        (b"m7p_", b"m8p_"), (b"m7x_", b"m8x_"),
        (b"m7a_", b"m8a_"), (b"m7c_", b"m8c_"),
        (b"m7s_", b"m8s_"),
        (b"Gen7", b"Gen8"), (b"gen7", b"gen8"),
    )
...
def test_gen8_changed_region_manifest_is_structural_and_rejects_unregistered_drift(
        tmp_path):
    manifest = runtime._gen7_to_gen8_change_manifest()
    assert set(manifest) == {
        "schema_version", "baseline_subjects", "candidate_subjects",
        "policy", "rows", "manifest_sha256_excluded",
    }
    assert manifest["schema_version"] == "msae_v3_gen8_changed_regions_v1"
    assert manifest["manifest_sha256_excluded"] is True
    assert manifest == runtime._gen7_to_gen8_change_manifest()
    assert all(set(row) == {
    ...
    relatives = (
        "scripts/msae_independent_measurement_v3_post_m2_gen8.py",
        "scripts/msae_independent_measurement_v3_post_m2_gen8_runtime.py",
        "scripts/run_msae_independent_calibration_v3_gen8.py",
        "scripts/launch_msae_independent_calibration_v3_gen8.sh",
        "scripts/run_msae_independent_measurement_v3_post_m2_gen8_tmux_test.py",
        "docs/rfc-msae-independent-measurement-v3-post-m8-gen8.md",
        "tests/test_msae_independent_measurement_v3_post_m8_gen8.py",
    )
    ...
    with pytest.raises(ValueError, match="byte-identical|outside allowed"):
        runtime._gen7_to_gen8_change_manifest(root=candidate)


def test_gen8_changed_region_rows_bind_new_nodes_and_reject_unknown_nodes():
    baseline = b"VALUE = 1\n\ndef keep():\n    return 1\n"
    candidate = (
        b"VALUE = 2\n\n"
        b"def allowed_new():\n    return 3\n\n"
        b"def keep():\n    return 1\n")
    rows = runtime._gen8_changed_region_rows(
        "fixture.py", baseline, candidate,
        allowed_assignments={"VALUE": "R8_CONSTANTS"},
        allowed_definitions={"allowed_new": "R8_CHANGE_POLICY"},
    )
    assert [(row["qualified_name"], row["region_id"]) for row in rows] == [
        ("VALUE", "R8_CONSTANTS"),
        ("allowed_new", "R8_CHANGE_POLICY"),
    ]
    bad = candidate.replace(b"return 1", b"return 9")
    with pytest.raises(ValueError, match="outside allowed"):
        runtime._gen8_changed_region_rows(
...
def test_changed_regions_descriptor_embeds_exact_reconstructable_manifest():
    ...
    assert descriptor["schema_version"] == \
        "msae_v3_gen8_changed_regions_transaction_v1"
...
def test_review_control_parser_requires_exact_contiguous_block_and_rows(tmp_path):
    check = {
        "schema_version": "msae_v3_gen8_review_check_v1", "id": "fixture",
...
def test_failed_gen7_payload_is_honest_typed_and_independently_reconstructed():
    ...
    assert set(payload) == {
        ...
        "sealed_payload_metadata", "gen8_plan_authority",
    }
...
def test_containment_attempt_and_case_journal_schemas_are_exact():
    ...
    assert descriptor["schema_version"] == "msae_v3_gen8_containment_attempt_v1"
...
def test_containment_pass_report_requires_complete_ordered_cases_and_zero_tripwires():
    ...
    for index in range(10):
        ...
        before = {"schema_version": "msae_v3_gen8_outer_before_v1",
                  "case_id": case_id}
        action = {"schema_version": "msae_v3_gen8_outer_action_v1",
                  "case_id": case_id}
        owner = {"schema_version": "fixture-owner-ledger-v1", "case_id": case_id}
        outer = {
            "schema_version": "msae_v3_gen8_case_outer_result_v1",
            ...
            "tripwires": {
                "schema_version": "msae_v3_gen8_tripwire_counts_v1",
    ...
    report = {
        "schema_version": "msae_v3_gen8_tmux_containment_v1",
...
def test_nfs_publisher_fault_matrix():
    ...
    try:
        ...
        raw = runtime.base.canonical_bytes({
            "schema_version": "msae_v3_gen8_publisher_baseline_v1"})
...
def test_derived_publication_requires_two_independent_reproductions(
    ...
    fake_mount = {
        "schema_version": "msae_v3_gen8_mount_identity_v1",
...
def _copy_source_only_cli_fixture(root: Path) -> None:
    ...
    for name in (
        ...
        "msae_independent_measurement_v3_post_m2_gen8.py",
        "msae_independent_measurement_v3_post_m2_gen8_runtime.py",
        "msae_measurement_remediation_v1.py",
        "msae_measurement_v2.py",
        "run_msae_independent_calibration_v3.py",
        "run_msae_independent_calibration_v3_gen8.py",
    ):
...
def test_controller_ignores_valid_timestamp_local_pyc_before_all_gates(tmp_path):
    ...
    result = subprocess.run([
        python, "-S", "-B", "-I",
        str(candidate / "scripts/msae_independent_measurement_v3_post_m2_gen8.py"),
        "not-a-command",
    ], text=True, capture_output=True)
...
def test_runner_ignores_valid_timestamp_controller_pyc_before_auth_gate(tmp_path):
    candidate = tmp_path / "candidate"
    _copy_source_only_cli_fixture(candidate)
    controller = candidate / "scripts/msae_independent_measurement_v3_post_m2_gen8.py"
    ...
    result = subprocess.run([
        ...
        str(candidate / "scripts/run_msae_independent_calibration_v3_gen8.py"),
...
def test_runner_removes_scripts_path_before_deferred_third_party_import(tmp_path):
    ...
    runner = candidate / "scripts/run_msae_independent_calibration_v3_gen8.py"
    result = subprocess.run([
        python, "-S", "-B", "-I", "-c",
        "import runpy,sys;"
...
def _minimal_config_candidate_root(root: Path) -> tuple[str, str, str]:
    ...
    relatives = (
        ...
        "scripts/launch_msae_independent_calibration_v3_gen8.sh",
        "docs/rfc-msae-independent-measurement-v3-post-m8-gen8.md",
        "docs/plan-msae-independent-measurement-v3-post-m8-gen8.md",
        "tests/test_msae_independent_measurement_v3_post_m8_gen8.py",
    ...
    return (runtime.base.sha_file(
        root / "reports/adversarial/msae_independent_measurement_v3_post_m2_gen8_plan.md"),
...
def test_real_protocol_config_is_byte_equal_across_two_copied_sparse_roots(tmp_path):
    ...
    for row in left_value["replay"]["strata"]:
        assert row["code_sha256"] == runtime.base.sha_file(
            left / "scripts/run_msae_independent_calibration_v3_gen8.py")
...
def test_frozen_plan_review_is_canonical_and_machine_bound():
    value = runtime._review_control_lines(
        runtime.PLAN_REVIEW, scope="gen8_plan", expected_controls={
            "VERDICT": "SHIP", "REVIEW_SCOPE": "gen8_plan",
            "PLAN_SHA256": runtime.FROZEN_PLAN_SHA256,
            "SEALED_CONTENT_READS": "0",
        }, expected_sha256=runtime.FROZEN_PLAN_REVIEW_SHA256)
    assert value["scope"] == "gen8_plan"
...
def test_gen3_review_namespace_rejects_undeclared_prefixed_sibling(
    ...
    names = {
        "plan": "msae_independent_measurement_v3_post_m2_gen8_plan.md",
        "pre": "msae_independent_measurement_v3_post_m2_gen8_pre_capability.md",
        "implementation":
            "msae_independent_measurement_v3_post_m2_gen8_implementation.md",
        "post": "msae_independent_measurement_v3_post_m2_gen8_post_m3.md",
        "prescore": "msae_independent_measurement_v3_post_m2_gen8_prescore.md",
    }
    ...
    (root / "msae_independent_measurement_v3_post_m2_gen8_backup.md").write_text(
        "unexpected\n")
...
def test_failed_gen2_is_wrapped_in_gen8_without_cross_generation_write():
...
def test_legacy_whole_file_delta_is_dead_and_structural_manifest_is_authority(
    ...
    manifest = runtime._gen7_to_gen8_change_manifest()
    monkeypatch.setattr(
        runtime, "gen6_to_gen8_substitution_evidence",
        lambda: (_ for _ in ()).throw(AssertionError("legacy comparator called")))
    monkeypatch.setattr(
        runtime, "_typed_gen8_delta_projection",
    ...
    monkeypatch.setattr(
        runtime, "_gen7_to_gen8_change_manifest",
        lambda *args, **kwargs: (_ for _ in ()).throw(
...
def test_closure_registry_contains_every_gen8_review_and_failure_input(monkeypatch):
...
def test_post_m3_review_binds_every_realized_directory_and_downstream_absence(
    ...
    assert result["review"]["scope"] == "gen8_post_m3"
    assert set(captured) == {
...
def test_setup_receipt_schema_is_exact_and_rejects_bad_digest():
    value = runtime._setup_receipt_value(
        1, "gen8_provenance_root", "mkdir", str(runtime.ACTIVE_PROV),
        "a" * 64, None, None)
    assert set(value) == {
...
def test_transition_cli_requires_both_review_gates_before_dispatch():
    ...
    setup = subprocess.run(
        [python, "-S", "-B", "-I", controller, "setup-m3-gen8",
         "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256],
        text=True, capture_output=True)
    assert setup.returncode != 0
    assert "noncanonical gen8 command shape" in setup.stderr
    m4 = subprocess.run(
        [python, "-S", "-B", "-I", controller, "build-m4-gen8"],
        text=True, capture_output=True)
    assert m4.returncode != 0
    assert "noncanonical gen8 command shape" in m4.stderr
...
def test_m3_rejects_every_nonreviewed_m2_digest_before_writing():
    with pytest.raises(ValueError, match="M2 pin drift"):
        runtime.setup_m3_gen8("0" * 64, "a" * 64, "b" * 64,
                              "c" * 64, "d" * 64, "e" * 64,
                              "f" * 64, "1" * 64)
...
def test_review_verdict_parser_rejects_conflicts_and_duplicate_digest_lines(tmp_path, monkeypatch):
    ...
    good = "\n".join(["VERDICT: SHIP", "REVIEW_SCOPE: gen8_prescore",
...
def test_recursive_static_closure_rejects_planted_surface_escapes(tmp_path):
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    runner = tmp_path / "scripts/run_msae_independent_calibration_v3_gen8.py"
    planted = [
...
def test_static_dynamic_imports_have_exact_site_specific_targets(tmp_path):
    ...
    gen3_dynamic = [row for row in value["dynamic_import_mappings"]
                    if row["path"].endswith("post_m2_gen8_runtime.py")]
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    module = tmp_path / "scripts/msae_independent_measurement_v3_post_m2_gen8_runtime.py"
    ...
    module.write_text(original.replace(
        ...
        '"scripts/run_msae_independent_calibration_v3_gen8.py")', 1))
...
def test_static_value_flow_ledger_closes_caller_argv_and_helper_return_bypasses(tmp_path):
    ...
    module.write_text(original.replace(
        ...
        '            str(continuation.CONTROLLER_PATH), "broker-gen8",',
        'argv = ["/bin/false", "-S", "-B", "-I",\n'
        '            str(continuation.CONTROLLER_PATH), "broker-gen8",', 1))
...
def test_internal_broker_requires_live_launcher_capability_before_socket_or_gpu(tmp_path, monkeypatch):
    ...
    socket_path = "/tmp/m8b_" + "a" * 64 + ".sock"
...
def test_tmux_client_timeout_is_bounded_and_extinguishes_its_captured_group(monkeypatch):
    vector = ["/usr/bin/tmux", "-f", "/dev/null", "-S",
              "/tmp/m8t_" + "a" * 64 + ".sock", "has-session", "-t",
              "msae-independent-v3-gen8-calibration"]
    observed = []
...
def _gen8_monitor_record_fixture() -> dict:
    stage = "a" * 64
    tmux_path, broker_path, pane_path = runtime._tmux_paths(stage)
    return {
        "schema_version": "msae_v3_gen8_tmux_monitor_v2",
        "protocol_id": runtime.PROTOCOL,
...
def _gen8_monitor_payload(sequence: int) -> dict:
    digest = "a" * 64
    payloads = {
        ...
        4: {"record_sha256": digest, "record_ack_sha256": digest,
            "tmux_socket_path": "/tmp/m8t_" + digest + ".sock",
            "broker_socket_path": "/tmp/m8b_" + digest + ".sock",
            "pane_socket_path": "/tmp/m8p_" + digest + ".sock"},
        5: {"server_pid": 2003, "server_start_ticks": "13",
            ...
            "tmux_socket_binding": {"device": 1, "inode": 2,
                                    "uid": os.getuid(), "gid": os.getgid(),
                                    "mode": 0o700, "nlink": 1},
            "session": "msae-independent-v3-gen8-calibration",
...
def test_monitor_record_and_short_socket_contract_reject_every_mutation():
    record = _gen8_monitor_record_fixture()
...
def test_monitor_frame_chain_is_canonical_exact_and_datagram_bounded():
    ...
    for sequence, (frame_type, _keys) in enumerate(runtime.MONITOR_FRAME_SPECS, 1):
        frame = runtime._monitor_frame(
            frame_type, sequence, previous, _gen8_monitor_payload(sequence))
...
def test_parent_death_watchdog_kills_worker_when_broker_exits(tmp_path):
    runtime_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen8_runtime.py"
    controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen8.py"
    script = f'''\
import importlib.util, os, sys, time, types
controller_path={str(controller_path)!r}
controller=types.ModuleType("msae_independent_measurement_v3_post_m2_gen8")
...
def _tmux_test_process_fixture() -> dict[str, int | str]:
    return {
    "schema_version": "msae_v3_gen8_tmux_test_process_v1",
...
def test_tmux_supervisor_cleans_exact_crash_left_identity_pairs(tmp_path):
    module_name = "_msae_gen8_tmux_supervisor_pair_test"
...
def test_tmux_supervisor_cleanup_rejects_wrong_bytes_and_third_links(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_cleanup_poison", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    ...
    value = {"schema_version": "msae_v3_gen8_tmux_test_identity_bound_v1",
...
def test_tmux_supervisor_cleanup_rechecks_second_pair_member_before_unlink(
        tmp_path, monkeypatch):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_cleanup_substitution",
...
def test_tmux_supervisor_cleanup_accepts_valid_destination_only_record(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_cleanup_destination", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    final = tmp_path / "identity_bound.json"
    final.write_bytes(supervisor._canonical_bytes({
        "schema_version": "msae_v3_gen8_tmux_test_identity_bound_v1",
...
def test_tmux_supervisor_rejects_invocation_drift_before_namespace(drift):
    before = sorted(Path("/tmp").glob("msae-independent-v3-gen8-test-*"))
    argv = [str(ROOT / ".venv-atlas/bin/python"), "-S", "-B", "-I",
    ...
    assert sorted(Path("/tmp").glob("msae-independent-v3-gen8-test-*")) == before


def test_tmux_supervisor_child_timeout_terminates_real_resistant_group():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_child_timeout", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    source = (
...
def test_tmux_supervisor_extinguishes_group_after_pytest_leader_exit():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_dead_leader", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    source = (
        "import os,signal,time\n"
...
def test_tmux_supervisor_rejects_late_duplicate_identity_connection(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_duplicate_connection",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_publication_fault_boundaries_cleanup(
    ...
    module_name = f"_msae_gen8_tmux_supervisor_fault_{boundary}"
    spec = runtime.continuation.source_only_module_spec(
        module_name, runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_publication_retries_short_writes(tmp_path, monkeypatch):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_short_write", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_test_terminal_is_exact_create_once_and_owned(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_terminal", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_pre_capability_review_requires_exact_contiguous_command_evidence(
        monkeypatch):
    rows = [{"schema_version": "msae_v3_gen8_pre_capability_check_v1",
             "id": "one", "argv": ["/registered/one"], "cwd": str(ROOT),
             "environment": {}, "subject_sha256": "1" * 64,
             "exit_code": 0, "summary": "pass"},
            {"schema_version": "msae_v3_gen8_pre_capability_check_v1",
    ...
    encoded = ["CHECK_ROW_JSON: " + runtime.base.canonical_bytes(row).decode(
        "utf-8").rstrip("\n") for row in rows]
    prefix = ["VERDICT: SHIP", "REVIEW_SCOPE: gen8_pre_capability",
              "CHECKS RUN"]
    ...
    monkeypatch.setattr(runtime, "_current_gen8_implementation_entries",
                        lambda: {})
...
def test_pre_capability_namespace_gate_blocks_extra_and_downstream_before_write(
    ...
    monkeypatch.setattr(runtime, "_validate_gen3_review_namespace",
                        lambda _phase: None)
    monkeypatch.setattr(runtime, "_all_gen8_short_family_absence",
...
def test_tmux_fault_event_variants_reject_every_unregistered_control_mutation():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_event_schema",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(supervisor)
    ...
    hook = {
        "schema_version": "msae_v3_gen8_fault_event_v1",
...
def test_tmux_outer_reconstructs_scenario_action_projection():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen8_tmux_supervisor_action_projection",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(supervisor)
    ...
    action_sha256 = "c" * 64
    target = {
        "schema_version": "msae_v3_gen8_scenario_monitor_frame_v1",
    ...
    expected = {
        "schema_version": "msae_v3_gen8_scenario_action_observation_v1",
...
def test_cleanup_side_effect_mutations_fail_for_crash_and_exception(
    ...
    config = {
        ...
        "session": "msae-independent-v3-gen8-containment",
...
def test_failed_handoff_extinction_kills_real_processes_and_socket(
    ...
    if raw_control_fd is None:
        pytest.skip("the reviewed gen8 outer supervisor is required")
    ...
    token_preimage = {
        "schema_version": "msae_v3_gen8_test_token_preimage_v1",
    ...
    def frame(case_id, sequence, prior, frame_type, payload):
        return {"schema_version": "msae_v3_gen8_test_control_frame_v1",
...
def test_gen8_terminal_payload_preserves_two_failed_gen3_invocations_honestly():
...
def test_exact_gen8_cli_shape_rejects_reordering_duplicates_and_old_routes():
    setup = [
        "setup-m3-gen8",
        "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256,
        ...
        "--gen8-plan-review-sha256", runtime.FROZEN_PLAN_REVIEW_SHA256,
        "--gen8-pre-capability-review-sha256", "b" * 64,
        "--gen8-implementation-review-sha256", "a" * 64,
    ]
    runtime._require_exact_command_shape(setup)
    for defective in (
        [*setup[:3], *setup[5:7], *setup[3:5], *setup[7:]],
        [*setup, "--gen8-implementation-review-sha256", "a" * 64],
        ["setup-m3", *setup[1:]],
        ["build-m4-gen8", "--post-m3-review-sha256", "A" * 64],
        ["sign-gen8", "extra"],
    ):
        with pytest.raises(ValueError):
...
def test_capability_reentry_rejects_undeclared_children_before_mutation(
    ...
    report = analysis / "gen8_capability_reproduction.json"
    ...
    with pytest.raises(ValueError, match=(
        ...
        runtime.probe_gen8_capability(report, "a" * 64)
...
def test_capability_reauthorizes_report_and_outer_namespace_before_descriptor_delete(
    ...
    row = runtime._publication_row(
        ...
        report_raw, 0o644, "gen8_capability_report")
    ...
    monkeypatch.setattr(runtime, "_failed_gen4_entries", lambda: {})
    monkeypatch.setattr(runtime, "_failed_gen5_entries", lambda: {})
    monkeypatch.setattr(runtime, "_current_gen8_implementation_entries", lambda: subject)
    monkeypatch.setattr(runtime, "_validate_capability_report", lambda _value: None)
    monkeypatch.setattr(
        runtime, "_pre_capability_namespace_gate",
        lambda *, allow_capability_root: {
...
def test_setup_prefix_validator_rejects_receipt_hole(tmp_path, monkeypatch):
    prov = tmp_path / "prov"; prov.mkdir(mode=0o700)
    journal = prov / ".setup_m3_gen8_transaction"; journal.mkdir(mode=0o700)
    ...
    monkeypatch.setattr(runtime, "SETUP_MANIFEST", prov / "setup_m3_gen8_manifest.json")
    monkeypatch.setattr(runtime, "M4_NEW", tuple())
...
def test_setup_m3_gen8_builds_and_cleans_exact_24_receipt_chain(
        tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime, "_current_gen8_entries", lambda: {})
    ...
    completion = runtime.setup_m3_gen8(
        runtime.M2_REVIEWED_COMPLETION_SHA256,
        ...
        "7" * 64,
        "8" * 64)
    assert completion["schema_version"] == "msae_v3_gen8_m3_completion_v1"
