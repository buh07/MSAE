...
SCRIPT = Path(__file__).parents[1] / "scripts/prepare_msae_independent_source_v6.py"
ACQUISITION_SCRIPT = Path(__file__).parents[1] / "scripts/acquire_msae_independent_source_v6.py"
_spec = importlib.util.spec_from_file_location("msae_v6", SCRIPT)
assert _spec and _spec.loader
m = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = m
_spec.loader.exec_module(m)
_acq_spec = importlib.util.spec_from_file_location("msae_v6_acquisition", ACQUISITION_SCRIPT)
...
def acquisition_fixture(tmp_path: Path, monkeypatch):
    root=tmp_path;prov=root/"reports/provenance/msae_independent_source_v6";prov.mkdir(parents=True)
    config=root/"configs/msae_independent_source_v6/acquisition.json";config.parent.mkdir(parents=True)
    ...
    authority.write_bytes(acq.canonical({"status":"approved_for_exact_source_acquisition","artifact_sha256":{
        "scripts/acquire_msae_independent_source_v6.py":acq.sha_file(ACQUISITION_SCRIPT),
        "configs/msae_independent_source_v6/acquisition.json":acq.sha_file(config)}}))
    review=root/"reports/adversarial/msae_independent_source_v6_preacquisition_authority_review.md";review.parent.mkdir(parents=True)
    ...
    monkeypatch.setattr(acq,"RAW",root/"data/msae_independent_source_v6/raw"/acq.COMMIT)
...
def acquisition_success_fixture(tmp_path: Path,monkeypatch):
    ...
    entry={"schema_version":"msae_independent_source_v6_source_acquisition_entry_v1","status":"entered",
    ...
    clone_dir="/tmp/msae-v6-acquire-fixture/repo";empty_home="/tmp/msae-v6-acquire-fixture/home"
    argvs=[[*config["ignore_argv"][:-1],"data/msae_independent_source_v6/raw/sentinel"],
    ...
    report={"schema_version":"msae_independent_source_v6_source_acquisition_v1","source_repo":acq.REPO,
        ...
        "ignore_checks":[{"path":"data/msae_independent_source_v6/raw/sentinel","stdout_sha256":by["ignore_raw"]["stdout_sha256"],
                          "stderr_sha256":by["ignore_raw"]["stderr_sha256"],"exit_status":0},
                         {"path":"data/msae_independent_source_v6/private/sentinel","stdout_sha256":by["ignore_private"]["stdout_sha256"],
...
def test_split_and_manifest_golden():
    ...
    golden = {
        "schema_version": "msae_independent_source_v6_split_manifest_v1",
        "source_commit": "0" * 40,
    ...
    assert hashlib.sha256(m.canonical_file_bytes(golden)).hexdigest() == "3113f18eec64ba9aa0f8a47bd8d2cb8d55229f531d59d8b2a30053546427a1eb"
    assert m.split_key("x!", "x !") == hashlib.sha256(m.canonical_bytes(["msae-independent-source-v6/C1C2", "x!", "x !"])).hexdigest()


def test_create_once_private_payload_and_no_source_output(tmp_path: Path, capsys):
    private = tmp_path / "private"
    records = [{"canary": "MSAE_V6_SECRET_7f6b6aa8", "id": 1}]
...
def test_process_snapshot_schema_and_hash_only():
    ...
    assert snapshot["schema_version"] == "msae_independent_source_v6_process_snapshot_v1"
...
def test_inventory_rejects_populated_v6_and_directory_symlink_without_opening(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path)
    raw = tmp_path / "data/msae_independent_source_v6/raw"
    raw.mkdir(parents=True); secret = raw / "secret"; secret.write_text("must not open")
    with pytest.raises(m.GateFailure, match="preexisting_v6_namespace"):
        m.inventory_repository()
    secret.unlink(); raw.rmdir(); (tmp_path/"data/msae_independent_source_v6").rmdir()
...
def test_frozen_acquisition_config():
    config=SCRIPT.parents[1]/"configs/msae_independent_source_v6/acquisition.json"
...
def test_training_root_reconstruction_uses_global_path_order(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(m,"_walk_paths_allow_v6",lambda:iter([(z,"results/z.txt"),(a,"results/a.txt")]))
...
def test_authority_manifest_is_create_once_and_review_bound(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path)
    prov=tmp_path/"reports/provenance/msae_independent_source_v6";prov.mkdir(parents=True)
    ...
    impl=tmp_path/"reports/adversarial/msae_independent_source_v6_preacquisition_implementation_review.md"
    ...
    for path in sorted(item for item in tmp_path.rglob("*") if item.is_file()):
        ...
        entries.append({"path":rel,"disposition":"v6_authority","size":st.st_size,"mode":st.st_mode&0o777,
    ...
    with pytest.raises(m.GateFailure,match="authority_manifest_already_exists"):
        m.build_authority_manifest()
    review=tmp_path/"reports/adversarial/msae_independent_source_v6_preacquisition_authority_review.md"
...
def test_baseline_requires_ship_review_before_publication(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path);prov=tmp_path/"reports/provenance/msae_independent_source_v6";prov.mkdir(parents=True)
    monkeypatch.setattr(m,"PROV",prov)
    paths=["docs/plan-msae-independent-source-v6.md","reports/adversarial/msae_independent_source_v6_plan_review.md",
           "reports/provenance/msae_independent_source_v6/preacquisition_alias_screen.json",
           "reports/provenance/msae_independent_source_v6/historical_source_registry.json","tests/test_prepare_msae_independent_source_v6.py",
           "configs/msae_independent_source_v6/acquisition.json","reports/verification/msae_independent_source_v6_source_free_checks.log",
           "scripts/acquire_msae_independent_source_v6.py"]
    for rel in paths:
        path=tmp_path/rel;path.parent.mkdir(parents=True,exist_ok=True);path.write_text(rel)
    review=tmp_path/"reports/adversarial/msae_independent_source_v6_preacquisition_implementation_review.md"
...
def test_acquisition_lstat_preflight_stops_before_process(tmp_path: Path, monkeypatch, blocked: str):
    ...
    raw=tmp_path/"data/msae_independent_source_v6/raw"/acq.COMMIT
...
def test_pre_payload_namespace_is_exact(tmp_path: Path, monkeypatch, mutation: str):
    raw=tmp_path/"data/msae_independent_source_v6/raw"/m.COMMIT;raw.mkdir(parents=True)
...
def test_verify_terminal_rejection_enforces_payload_absence(tmp_path: Path, monkeypatch):
    prov=tmp_path/"reports/provenance/msae_independent_source_v6";prov.mkdir(parents=True)
    private=tmp_path/"data/msae_independent_source_v6/private";private.mkdir(parents=True)
    monkeypatch.setattr(m,"ROOT",tmp_path);monkeypatch.setattr(m,"PROV",prov);monkeypatch.setattr(m,"PRIVATE",private)
    rejection={"schema_version":"msae_independent_source_v6_rejection_v1","status":"rejected",
...
def test_verify_terminal_complete_synthetic_success(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(m,"ROOT",tmp_path);monkeypatch.setattr(m,"PROV",prov);monkeypatch.setattr(m,"PRIVATE",private)
    for rel in ("docs/plan-msae-independent-source-v6.md","reports/adversarial/msae_independent_source_v6_plan_review.md",
                "scripts/acquire_msae_independent_source_v6.py","tests/test_prepare_msae_independent_source_v6.py"):
        path=tmp_path/rel;path.parent.mkdir(parents=True,exist_ok=True);path.write_text(rel)
    h="a"*64
    split={"schema_version":"msae_independent_source_v6_split_manifest_v1","source_commit":m.COMMIT,
    ...
    roles={"schema_version":"msae_independent_source_v6_role_manifest_v1","source_commit":m.COMMIT,"roles":{
    ...
    source={"schema_version":"msae_independent_source_v6_source_manifest_v1","source_commit":m.COMMIT,
    ...
    dedup={"schema_version":"msae_independent_source_v6_dedup_v1","roles":{
    ...
    support={"schema_version":"msae_independent_source_v6_support_v1","floor_distinct_utterances":20,
    ...
    snapshot={"schema_version":"msae_independent_source_v6_process_snapshot_v1",
    ...
    baseline={"schema_version":"msae_independent_source_v6_baseline_inventory_v1",
    ...
    census={"schema_version":"msae_independent_source_v6_document_group_census_v1","newdoc_marker_count":0,
    ...
    license_value={"schema_version":"msae_independent_source_v6_license_v1","license_id":"CC-BY-SA-4.0",
    ...
    family={"schema_version":"msae_independent_source_v6_source_family_v1","status":"eligible",
    ...
    pedigree={"schema_version":"msae_independent_source_v6_candidate_pedigree_v1",
    ...
    values={
        ...
        "source_acquisition_entry.json":{"schema_version":"msae_independent_source_v6_source_acquisition_entry_v1"},
        "source_acquisition.json":{"schema_version":"msae_independent_source_v6_source_acquisition_v1"},
        ...
        "history_overlap.json":{"schema_version":"msae_independent_source_v6_history_overlap_v1",
        ...
        "cross_role_overlap.json":{"schema_version":"msae_independent_source_v6_cross_role_overlap_v1",
        ...
        "preacquisition_authority_manifest.json":{"schema_version":"msae_independent_source_v6_preacquisition_authority_v1"},
    }
    ...
    seal={"schema_version":"msae_independent_source_v6_seal_v1",
          ...
          "plan_sha256":m.sha_file(tmp_path/"docs/plan-msae-independent-source-v6.md"),
          "plan_review_sha256":m.sha_file(tmp_path/"reports/adversarial/msae_independent_source_v6_plan_review.md"),
          "program_sha256":m.sha_file(SCRIPT),
          "acquisition_runner_sha256":m.sha_file(tmp_path/"scripts/acquire_msae_independent_source_v6.py"),
          "test_sha256":m.sha_file(tmp_path/"tests/test_prepare_msae_independent_source_v6.py"),
          "artifact_sha256":bindings,
          "payload":{"path":"data/msae_independent_source_v6/private/blind_payload.jsonl",
    ...
    m.write_json(prov/"no_training_gate.json",{"schema_version":"msae_independent_source_v6_no_training_gate_v1","status":"pending_terminal_seal",
