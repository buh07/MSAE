...
spec = importlib.util.spec_from_file_location("behavioral_endpoint_v6_1", ROOT / "scripts/behavioral_endpoint_v6_1.py")
v6 = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(v6)
CFG_PATH = ROOT / "configs/behavioral_endpoint_v6_1/run.json"
...
def test_v6_1_recovery_and_recursive_dependency_closure():
    ...
    assert set(CFG["recovery"]["added_transitive_dependencies"]) <= closure
    assert CFG["recovery"]["scientific_protocol_changed"] is False
    assert CFG["recovery"]["new_model_forwards_before_recovery"] is False
...
def test_no_training_or_representation_method_execution_path():
    text = (ROOT / "scripts/behavioral_endpoint_v6_1.py").read_text().lower()
