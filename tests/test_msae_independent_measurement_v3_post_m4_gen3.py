...
for _local_module in (
    ...
    "msae_independent_measurement_v3_post_m2_gen3",
    "msae_independent_measurement_v3_post_m2_gen3_runtime",
    "msae_measurement_remediation_v1", "msae_measurement_v2",
    "run_msae_independent_calibration_v3", "run_msae_independent_calibration_v3_gen3",
):
    sys.modules.pop(_local_module, None)
_controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen3.py"
_controller = types.ModuleType("msae_independent_measurement_v3_post_m2_gen3")
...
SPEC = _controller.source_only_module_spec(
    ...
    ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen3_runtime.py")
...
def _copy_source_only_cli_fixture(root: Path) -> None:
    ...
    for name in (
        ...
        "msae_independent_measurement_v3_post_m2_gen3.py",
        "msae_independent_measurement_v3_post_m2_gen3_runtime.py",
        "msae_measurement_remediation_v1.py",
        "msae_measurement_v2.py",
        "run_msae_independent_calibration_v3.py",
        "run_msae_independent_calibration_v3_gen3.py",
    ):
...
def test_controller_ignores_valid_timestamp_local_pyc_before_all_gates(tmp_path):
    ...
    result = subprocess.run([
        python, "-S", "-B", "-I",
        str(candidate / "scripts/msae_independent_measurement_v3_post_m2_gen3.py"),
        "not-a-command",
    ], text=True, capture_output=True)
...
def test_runner_ignores_valid_timestamp_controller_pyc_before_auth_gate(tmp_path):
    candidate = tmp_path / "candidate"
    _copy_source_only_cli_fixture(candidate)
    controller = candidate / "scripts/msae_independent_measurement_v3_post_m2_gen3.py"
    ...
    result = subprocess.run([
        ...
        str(candidate / "scripts/run_msae_independent_calibration_v3_gen3.py"),
...
def test_runner_removes_scripts_path_before_deferred_third_party_import(tmp_path):
    ...
    runner = candidate / "scripts/run_msae_independent_calibration_v3_gen3.py"
    result = subprocess.run([
        python, "-S", "-B", "-I", "-c",
        "import runpy,sys;"
...
def _minimal_config_candidate_root(root: Path) -> tuple[str, str]:
    ...
    return runtime.base.sha_file(
        root / "reports/adversarial/msae_independent_measurement_v3_post_m2_gen3_plan.md"), \
...
def test_real_protocol_config_is_byte_equal_across_two_copied_sparse_roots(tmp_path):
    ...
    for row in left_value["replay"]["strata"]:
        assert row["code_sha256"] == runtime.base.sha_file(
            left / "scripts/run_msae_independent_calibration_v3_gen3.py")
...
def test_frozen_plan_review_is_canonical_and_machine_bound():
    ...
    assert value["digests"] == {"PLAN": runtime.FROZEN_PLAN_SHA256}
...
def test_gen3_review_namespace_rejects_undeclared_prefixed_sibling(
    ...
    names = {
        "plan": "msae_independent_measurement_v3_post_m2_gen3_plan.md",
        "implementation":
            "msae_independent_measurement_v3_post_m2_gen3_implementation.md",
        "post": "msae_independent_measurement_v3_post_m2_gen3_post_m3.md",
        "prescore": "msae_independent_measurement_v3_post_m2_gen3_prescore.md",
    }
    ...
    (root / "msae_independent_measurement_v3_post_m2_gen3_backup.md").write_text(
        "unexpected\n")
...
def test_post_m3_review_binds_every_realized_directory_and_downstream_absence(
    ...
    assert result["review"]["scope"] == "post_m3"
    assert {"CONFIG_DIRECTORY", "PROVENANCE_DIRECTORY", "M4_DATA_DIRECTORY",
            "STATE_DIRECTORY", "NONCE_DIRECTORY", "DOWNSTREAM_ABSENCE"} <= set(captured)
    assert captured["GEN2_FAILURE"] == "e" * 64
...
def test_transition_cli_requires_both_review_gates_before_dispatch():
    ...
    setup = subprocess.run(
        [python, "-S", "-B", "-I", controller, "setup-m3",
         "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256],
        text=True, capture_output=True)
    assert setup.returncode != 0
    assert "--plan-review-sha256" in setup.stderr
    assert "--implementation-review-sha256" in setup.stderr
    m4 = subprocess.run(
        [python, "-S", "-B", "-I", controller, "build-m4"],
        text=True, capture_output=True)
    assert m4.returncode != 0
    assert "--post-m3-review-sha256" in m4.stderr
...
def test_m3_rejects_every_nonreviewed_m2_digest_before_writing():
    with pytest.raises(ValueError, match="independently reviewed M2"):
        runtime.setup_m3("0" * 64, "a" * 64, "b" * 64)
...
def test_m3_existing_protocol_rejects_symlink_before_acceptance(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime, "ACTIVE_CONFIG", config)
    monkeypatch.setattr(runtime, "ACTIVE_M4_DATA", tmp_path / "m4_data")
    monkeypatch.setattr(runtime, "ACTIVE_PROV", tmp_path / "prov")
    monkeypatch.setattr(runtime, "ACTIVE_PRIVATE_KEY", private)
    monkeypatch.setattr(runtime, "CPU_NO_MODEL_TRACE", cpu_trace)
    monkeypatch.setattr(runtime, "verify_m2_completion", lambda digest: {})
    monkeypatch.setattr(runtime, "_plan_review_binding", lambda digest: {"sha256": digest})
    monkeypatch.setattr(runtime, "_gen2_failure_payload", lambda: {"status": "failed"})
    monkeypatch.setattr(runtime, "_implementation_review_binding",
                        lambda digest, predicted: {"sha256": digest})
    failure_parent = tmp_path / "gen2"; failure_parent.mkdir(mode=0o700)
    failure = failure_parent / "m4_failure.json"
    ...
    monkeypatch.setattr(runtime, "cpu_no_model_public_entry_trace", lambda **kwargs: {})
    monkeypatch.setattr(runtime, "_trace_review_gates", lambda *args: {})
    ...
    monkeypatch.setattr(runtime, "_verify_existing_authorization_state",
                        lambda **kwargs: {"contains_execution_authorization": False})
    monkeypatch.setattr(runtime, "_validate_setup_namespace_preconditions", lambda *args: None)
    with pytest.raises(OSError):
        runtime.setup_m3(runtime.M2_REVIEWED_COMPLETION_SHA256, "a" * 64, "b" * 64)
...
def test_setup_preflight_rejects_preplanted_namespace_before_any_write(
    ...
    provenance = tmp_path / "provenance"; provenance.mkdir()
    ...
    monkeypatch.setattr(runtime, "_validate_gen3_review_namespace", lambda phase: None)
    with pytest.raises(ValueError, match="partial/preplanted"):
...
def test_setup_existing_state_requires_private_agreement_before_first_write(
    ...
    assert calls == [{"verify_private_content": True}]
...
def test_review_verdict_parser_rejects_conflicts_and_duplicate_digest_lines(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError):
        runtime._strict_review_verdict(good + "\nSTAGE_A_SHA256: " + "0" * 64, manifest, "c" * 64)
    with pytest.raises(ValueError):
...
def test_recursive_static_closure_rejects_planted_surface_escapes(tmp_path):
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    runner = tmp_path / "scripts/run_msae_independent_calibration_v3_gen3.py"
    planted = [
...
def test_static_dynamic_imports_have_exact_site_specific_targets(tmp_path):
    ...
    gen3_dynamic = [row for row in value["dynamic_import_mappings"]
                    if row["path"].endswith("post_m2_gen3_runtime.py")]
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    module = tmp_path / "scripts/msae_independent_measurement_v3_post_m2_gen3_runtime.py"
    ...
    module.write_text(original.replace(
        ...
        '"scripts/run_msae_independent_calibration_v3_gen3.py")', 1))
...
def test_transaction_recovers_matching_partial_targets_after_transaction_root_loss(
        tmp_path, monkeypatch):
...
def test_two_root_m4_materialization_wires_real_extension_and_manifest_without_live_fallback(
    ...
    left_stage = runtime._materialize_complete_m4(left, [], [], directories)
    right_stage = runtime._materialize_complete_m4(right, [], [], directories)
...
def test_parent_death_watchdog_kills_worker_when_broker_exits(tmp_path):
    ...
    controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen3.py"
    script = f'''\
import importlib.util, os, sys, time, types
controller_path={str(controller_path)!r}
controller=types.ModuleType("msae_independent_measurement_v3_post_m2_gen3")
...
def test_candidate_set_is_unique_and_manifest_self_excluded():
    ...
    assert all("post_m2_gen3" in path for path in runtime.M4_NEW[3:])
...
def test_runner_uses_two_phase_model_gate_and_atomic_terminal_only():
    source = (ROOT / "scripts/run_msae_independent_calibration_v3_gen3.py").read_text()
...
def test_runner_rejects_alternate_failure_root_without_writing(tmp_path):
    ...
    result = subprocess.run([
        str(ROOT / ".venv-atlas/bin/python"), "-S", "-B", "-I",
        str(ROOT / "scripts/run_msae_independent_calibration_v3_gen3.py"),
