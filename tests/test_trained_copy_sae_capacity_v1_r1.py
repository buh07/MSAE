...
import torch
ROOT=Path(__file__).resolve().parents[1];SPEC=importlib.util.spec_from_file_location("saecap",ROOT/"scripts/trained_copy_sae_capacity_v1_r1.py");assert SPEC and SPEC.loader;m=importlib.util.module_from_spec(SPEC);sys.modules["saecap"]=m;SPEC.loader.exec_module(m)
