...
SPEC = importlib.util.spec_from_file_location("induction", ROOT / "scripts/canonical_induction_circuit_v1_1.py")
assert SPEC and SPEC.loader
ind = importlib.util.module_from_spec(SPEC); SPEC.loader.exec_module(ind)
CFG = json.loads((ROOT / "configs/canonical_induction_circuit_v1_1/run.json").read_text())


def test_recovery_changes_no_scientific_configuration() -> None:
    original = json.loads((ROOT / "configs/canonical_induction_circuit_v1/run.json").read_text())
    recovered = json.loads(json.dumps(CFG))
    for value in (original, recovered):
        for key in ("candidate_files", "namespace", "runtime", "schema_version", "recovery"):
            value.pop(key, None)
    assert recovered == original
    assert CFG["recovery"]["scientific_protocol_changed"] is False
    assert CFG["recovery"]["model_forwards_in_v1"] is False


def test_gpu_selector_consumes_stream_and_succeeds_under_pipefail() -> None:
    launcher = (ROOT / "scripts/launch_canonical_induction_circuit_v1_1_tmux.sh").read_text()
    selector = next(line for line in launcher.splitlines() if line.startswith('GPU_ROW='))
    assert "; exit" not in selector and "END{if(found) print row}" in selector
    command = "set -o pipefail; " + selector + "; test -n \"$GPU_ROW\""
...
def test_pre_gate_preflight_excludes_confirmation_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ind.preflight(ROOT / "configs/canonical_induction_circuit_v1_1/run.json", include_confirmation=False)
    assert seen == [False]
...
def test_review_binding_cannot_be_bypassed_by_python_optimization(tmp_path: Path) -> None:
    ...
    result = subprocess.run([sys.executable, "-O", "-c", code, str(ROOT / "scripts/canonical_induction_circuit_v1_1.py"), str(freeze), str(candidate), str(frozen), str(binding)], capture_output=True, text=True)
