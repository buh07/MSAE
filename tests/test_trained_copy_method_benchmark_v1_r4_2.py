...
SPEC=importlib.util.spec_from_file_location('tcm_r4_2',ROOT/'scripts/trained_copy_method_benchmark_v1_r4_2.py')
assert SPEC and SPEC.loader
tcm=importlib.util.module_from_spec(SPEC);sys.modules['tcm_r4_2']=tcm;SPEC.loader.exec_module(tcm)
...
def test_candidate_does_not_load_checkpoint(monkeypatch):
 ...
 assert x['test_sha256']==tcm.sha(tcm.TEST) and x['launcher_sha256']==tcm.sha(tcm.LAUNCHER)
 assert x['checks']['all_32_targets_jacobian_required']
 assert x['checks']['fitted_output_oracle_rank64_identity'] and x['checks']['fitted_output_oracle_rank_ordering']
...
def test_exact_identity_pass_all_gates_and_random_fails():
 ...
 zero=tcm.score(tcm.Method('zero','zero','zero',None),torch.zeros_like(z['delta']),torch.zeros_like(z['sham_delta']),z,rows,c,'development',model,'native');assert zero['support_pass'] and not zero['all_gates_pass'] and set(zero['metrics'])==set(c['gates'])
 perm=z['delta'].clone();ps=z['sham_delta'].clone()
 for target in range(c['model']['vocab']):
  ix=torch.nonzero(z['target']==target,as_tuple=False).flatten();perm[ix]=z['delta'][torch.roll(ix,1)];ps[ix]=z['sham_delta'][torch.roll(ix,1)]
 badperm=tcm.score(tcm.Method('within_target_permutation','within_target_permutation','matrix',None),perm,ps,z,rows,c,'development',model,'native');assert badperm['support_pass'] and not badperm['all_gates_pass']
...
def test_method_inventory_and_information_contracts_are_complete():
 ...
 assert c['methods']['parameter_count_semantics']['output_oracle_rank_k']=='width*k'
 expected={'exact_head_delta':0,'identity_full64':0,'output_oracle_rank4':256,'ambient_pca_rank8':512,'random_rank64':0,'target_mean_delta':2048,'within_target_permutation':4096,'topk_sae_unpaired_selector_seed8401':32832}
 assert {k:tcm.expected_parameter_count(k,c) for k in expected}==expected

def test_unpaired_ambient_and_sae_selectors_are_permutation_invariant():
 g=torch.Generator().manual_seed(91);states=torch.randn(101,64,generator=g);perm=torch.randperm(len(states),generator=g);_,a=tcm.fit_unpaired_ambient_basis(states);_,b=tcm.fit_unpaired_ambient_basis(states[perm]);assert np.allclose(a[:8].T@a[:8],b[:8].T@b[:8],atol=1e-6,rtol=1e-6)
 codes=torch.randn(101,256,generator=g);assert torch.equal(tcm.select_unpaired_sae_features(codes,16),tcm.select_unpaired_sae_features(codes[perm],16))

def test_rank_order_validator_rejects_reversed_basis():
 x=tcm.rng('rank-order-test').normal(size=(128,64));_,_,basis=tcm.canonical_svd(x);assert tcm.oracle_rank_order_valid(x,basis,[4,8,16,32,64]);assert not tcm.oracle_rank_order_valid(x,basis[::-1].copy(),[4,8,16,32,64])
...
def test_gpu_visibility_mismatch_fails_before_cuda(monkeypatch,tmp_path):
 monkeypatch.setenv('CUDA_VISIBLE_DEVICES','wrong');lock=tmp_path/'msae_trained_copy_methods_v1_r4_2_expected.lockdir';lock.mkdir();(lock/'token').write_text('x')
...
def test_proc_stat_ppid_parser_handles_spaces_and_closing_parentheses():
 assert tcm.parse_proc_stat_ppid('123 (python worker) S 77 1 2 3')==77
 assert tcm.parse_proc_stat_ppid('123 (worker ) with spaces) R 88 1 2 3')==88
 for bad in ['123 no-parens S 7','123 (x) S nope 1','0 (x) S 1 2']:
  try:tcm.parse_proc_stat_ppid(bad)
  except RuntimeError:pass
  else:raise AssertionError('malformed proc stat accepted')

def test_recovery_lineage_and_payload_bytes_match_unopened_r4():
 c=tcm.cfg();lineage=tcm.recovery_lineage(c);ps=tcm.panel_set(c)
 assert lineage['preservation_sha256']==c['recovery']['r4_failure_preservation_sha256']
 old={x['stage']:x for x in lineage['r4_payloads']}
 assert all(tcm.rows_jsonl_sha256(ps[st])==old[st]['sha256'] for st in ('fit','development','confirmation'))

def test_r4_formal_namespace_is_preserved_and_r4_2_is_isolated():
 c=tcm.cfg();term=json.loads((ROOT/'reports/provenance/trained_copy_method_benchmark_v1_r4_run_20260810/TERMINAL.json').read_text())
 assert term['status']=='TECHNICAL_FAILURE_BEFORE_ACCESS' and term['no_retry_authorized'] and not term['panel_accessed']
 assert 'r4_2' in c['runtime']['output_root'] and 'r4_2' in c['runtime']['provenance_root']


def test_launcher_lock_basename_matches_worker_contract(monkeypatch,tmp_path):
 c=tcm.cfg();uuid='GPU-test';lock=tmp_path/f'msae_trained_copy_methods_v1_r4_2_{uuid}.lockdir';lock.mkdir();(lock/'token').write_text('secret')
 monkeypatch.setenv('CUDA_VISIBLE_DEVICES',uuid)
 class FakeCuda:
  @staticmethod
  def synchronize():pass
 original_read_text=tcm.Path.read_text
 monkeypatch.setattr(tcm.torch,'device',lambda _x:'cuda:0');monkeypatch.setattr(tcm.torch,'ones',lambda *_a,**_k:object());monkeypatch.setattr(tcm.torch,'cuda',FakeCuda);monkeypatch.setattr(tcm.os,'getpid',lambda:123);monkeypatch.setattr(tcm.subprocess,'check_output',lambda *a,**k:f'{uuid}, 123\n'.encode());monkeypatch.setattr(tcm.Path,'read_text',lambda self: '123 (python worker) S 1 0 0' if str(self).endswith('/proc/123/stat') else original_read_text(self))
 assert tcm.gpu_validate(0,uuid,123,str(lock),'secret')=='cuda:0'
