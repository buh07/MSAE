...
SCRIPT = Path(__file__).parents[1] / "scripts/prepare_msae_independent_source_v8.py"
ACQUISITION_SCRIPT = Path(__file__).parents[1] / "scripts/acquire_msae_independent_source_v8.py"
_spec = importlib.util.spec_from_file_location("msae_v8", SCRIPT)
assert _spec and _spec.loader
m = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = m
_spec.loader.exec_module(m)
_acq_spec = importlib.util.spec_from_file_location("msae_v8_acquisition", ACQUISITION_SCRIPT)
...
def acquisition_fixture(tmp_path: Path, monkeypatch):
    root=tmp_path;prov=root/"reports/provenance/msae_independent_source_v8";prov.mkdir(parents=True)
    config=root/"configs/msae_independent_source_v8/acquisition.json";config.parent.mkdir(parents=True)
    ...
    registry=prov/"historical_source_registry.json";registry.write_text(json.dumps({
        "schema_version":"msae_independent_source_v8_historical_source_registry_v1",
        "status":"frozen_preacquisition","domain_utf8":"msae-v8/pedigree","inputs":[],
    ...
    screen=prov/"preacquisition_alias_screen.json";screen.write_text(json.dumps({
        "schema_version":"msae_independent_source_v8_preacquisition_alias_screen_v2",
    ...
    baseline=prov/"baseline_inventory.json";baseline.write_bytes(acq.canonical({
        "schema_version":"msae_independent_source_v8_baseline_inventory_v1",
        "training_root_entries":[],"entries":[],"history_inputs_sha256":empty_digest,
        "authority_expected_absent":sorted(acq.V8_AUTHORITY)}))
    ...
    authority.write_bytes(acq.canonical({"status":"approved_for_exact_source_acquisition","history_inputs_sha256":empty_digest,"artifact_sha256":{
        "scripts/acquire_msae_independent_source_v8.py":acq.sha_file(ACQUISITION_SCRIPT),
        "configs/msae_independent_source_v8/acquisition.json":acq.sha_file(config)}}))
    review=root/"reports/adversarial/msae_independent_source_v8_preacquisition_authority_review.md";review.parent.mkdir(parents=True)
    ...
    monkeypatch.setattr(acq,"RAW",root/"data/msae_independent_source_v8/raw"/acq.COMMIT)
...
def acquisition_success_fixture(tmp_path: Path,monkeypatch):
    ...
    entry={"schema_version":"msae_independent_source_v8_source_acquisition_entry_v1","status":"entered",
    ...
    clone_dir="/tmp/msae-v8-acquire-fixture/repo";empty_home="/tmp/msae-v8-acquire-fixture/home"
    argvs=[[*config["ignore_argv"][:-1],"data/msae_independent_source_v8/raw/sentinel"],
    ...
    report={"schema_version":"msae_independent_source_v8_source_acquisition_v1","source_repo":acq.REPO,
        ...
        "ignore_checks":[{"path":"data/msae_independent_source_v8/raw/sentinel","stdout_sha256":by["ignore_raw"]["stdout_sha256"],
                          "stderr_sha256":by["ignore_raw"]["stderr_sha256"],"exit_status":0},
                         {"path":"data/msae_independent_source_v8/private/sentinel","stdout_sha256":by["ignore_private"]["stdout_sha256"],
...
def test_split_and_manifest_golden():
    ...
    golden = {
        "schema_version": "msae_independent_source_v8_split_manifest_v1",
        "source_commit": "0" * 40,
    ...
    assert hashlib.sha256(m.canonical_file_bytes(golden)).hexdigest() == "b3b9154ba3075c9be024ca76296e19ff7fbe63dec915bfc1f6ba0bff7b726c84"
    assert m.group_split_key("doc-a", "0" * 40) == hashlib.sha256(
        m.canonical_bytes(["msae-independent-source-v8/C1C2-group", "0" * 40, "doc-a"])).hexdigest()


def test_create_once_private_payload_and_no_source_output(tmp_path: Path, capsys):
    private = tmp_path / "private"
    records = [{"canary": "MSAE_V8_SECRET_7f6b6aa8", "id": 1}]
...
def test_v8_license_gate_reads_only_license_txt(tmp_path: Path, monkeypatch):
...
def test_pedigree_normalization_git_equivalence_and_hash_domain():
    ...
    assert m.pedigree_hash(value) == hashlib.sha256(b"msae-v8/pedigree\0" + value.encode()).hexdigest()
...
def test_registry_history_binding_is_exact():
    ...
    registry={"schema_version":"msae_independent_source_v8_historical_source_registry_v1",
              "status":"frozen_preacquisition","domain_utf8":"msae-v8/pedigree",
    ...
    screen={"schema_version":"msae_independent_source_v8_preacquisition_alias_screen_v2",
...
def test_frozen_v8_plan_chain_matches_reviewed_source_free_artifacts():
    assert m.verify_frozen_v8_authority_bindings()=={
...
def test_predecessor_chain_includes_and_rehashes_terminal_v7():
    expected_v7={
    ...
    predecessors=m._predecessor_hashes()
    assert len(predecessors)==50
...
def _acquisition_baseline_entry(root: Path, path: Path, disposition: str) -> dict[str, object]:
    ...
    return {"path":path.relative_to(root).as_posix(),"size":st.st_size,
        ...
        "adapter":"authority" if disposition=="v8_authority" else "opaque_binary",
        "content_reads":1,"extracted_unit_count":0,"hardlink_aliases":[]}


def test_acquisition_history_binding_rejects_unknown_v8_provenance(tmp_path: Path, monkeypatch):
...
@pytest.mark.parametrize("disposition,relative",(
    ...
    ("v8_authority","docs/plan-msae-independent-source-v8.md"),
))
...
def test_acquisition_history_binding_returns_registry_array_digest_and_reaches_entry(
    ...
    registry={"schema_version":"msae_independent_source_v8_historical_source_registry_v1",
        "status":"frozen_preacquisition","domain_utf8":"msae-v8/pedigree","inputs":records,
...
def test_process_snapshot_schema_and_hash_only():
    ...
    assert snapshot["schema_version"] == "msae_independent_source_v8_process_snapshot_v1"
...
def test_inventory_rejects_populated_v8_and_directory_symlink_without_opening(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path)
    raw = tmp_path / "data/msae_independent_source_v8/raw"
    raw.mkdir(parents=True); secret = raw / "secret"; secret.write_text("must not open")
    with pytest.raises(m.GateFailure, match="preexisting_v8_namespace"):
        m.inventory_repository()
    secret.unlink(); raw.rmdir(); (tmp_path/"data/msae_independent_source_v8").rmdir()
...
def test_frozen_acquisition_config():
    config=SCRIPT.parents[1]/"configs/msae_independent_source_v8/acquisition.json"
...
def test_training_root_reconstruction_uses_global_path_order(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(m,"_walk_paths_allow_v8",lambda:iter([(z,"results/z.txt"),(a,"results/a.txt")]))
...
def test_authority_manifest_is_create_once_and_review_bound(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path)
    prov=tmp_path/"reports/provenance/msae_independent_source_v8";prov.mkdir(parents=True)
    ...
    impl=tmp_path/"reports/adversarial/msae_independent_source_v8_preacquisition_implementation_review.md"
    ...
    for path in sorted(item for item in tmp_path.rglob("*") if item.is_file()):
        ...
        entries.append({"path":rel,"disposition":"v8_authority","size":st.st_size,"mode":st.st_mode&0o777,
    ...
    with pytest.raises(m.GateFailure,match="authority_manifest_already_exists"):
        m.build_authority_manifest()
    review=tmp_path/"reports/adversarial/msae_independent_source_v8_preacquisition_authority_review.md"
...
def test_baseline_requires_ship_review_before_publication(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(m,"ROOT",tmp_path);prov=tmp_path/"reports/provenance/msae_independent_source_v8";prov.mkdir(parents=True)
    monkeypatch.setattr(m,"PROV",prov)
    paths=["docs/plan-msae-independent-source-v8.md","reports/adversarial/msae_independent_source_v8_plan_review.md",
           "reports/provenance/msae_independent_source_v8/preacquisition_alias_screen.json",
           "reports/provenance/msae_independent_source_v8/historical_source_registry.json","tests/test_prepare_msae_independent_source_v8.py",
           "configs/msae_independent_source_v8/acquisition.json","reports/verification/msae_independent_source_v8_source_free_checks.log",
           "scripts/acquire_msae_independent_source_v8.py"]
    for rel in paths:
        path=tmp_path/rel;path.parent.mkdir(parents=True,exist_ok=True);path.write_text(rel)
    review=tmp_path/"reports/adversarial/msae_independent_source_v8_preacquisition_implementation_review.md"
...
def test_acquisition_lstat_preflight_stops_before_process(tmp_path: Path, monkeypatch, blocked: str):
    ...
    raw=tmp_path/"data/msae_independent_source_v8/raw"/acq.COMMIT
...
def test_pre_payload_namespace_is_exact(tmp_path: Path, monkeypatch, mutation: str):
    raw=tmp_path/"data/msae_independent_source_v8/raw"/m.COMMIT;raw.mkdir(parents=True)
    monkeypatch.setattr(m,"RAW",raw)
...
def test_scientific_rejection_publisher_fault_recovers_through_real_terminal_verifier(
    ...
    prov=tmp_path/"reports/provenance/msae_independent_source_v8";prov.mkdir(parents=True)
    private=tmp_path/"data/msae_independent_source_v8/private";private.mkdir(parents=True)
    monkeypatch.setattr(m,"ROOT",tmp_path);monkeypatch.setattr(m,"PROV",prov);monkeypatch.setattr(m,"PRIVATE",private)
    rejection={"schema_version":"msae_independent_source_v8_rejection_v1","status":"rejected",
...
def test_verify_terminal_complete_synthetic_success(tmp_path: Path, monkeypatch, terminal_publish_fault: str):
    ...
    monkeypatch.setattr(m,"ROOT",tmp_path);monkeypatch.setattr(m,"PROV",prov);monkeypatch.setattr(m,"PRIVATE",private)
    for rel in ("docs/plan-msae-independent-source-v8.md","reports/adversarial/msae_independent_source_v8_plan_review.md",
                "scripts/acquire_msae_independent_source_v8.py","tests/test_prepare_msae_independent_source_v8.py"):
        path=tmp_path/rel;path.parent.mkdir(parents=True,exist_ok=True);path.write_text(rel)
    ...
    split={"schema_version":"msae_independent_source_v8_split_manifest_v1","source_commit":m.COMMIT,
    ...
    role_entries=lambda prefix:[{"zero_based_rank":index,"sent_id":f"{prefix}{index}",
                                "source_record_sha256":h} for index in range(20)]
    roles={"schema_version":"msae_independent_source_v8_role_manifest_v1","source_commit":m.COMMIT,"roles":{
    ...
    source={"schema_version":"msae_independent_source_v8_source_manifest_v1","source_commit":m.COMMIT,
    ...
    dedup={"schema_version":"msae_independent_source_v8_dedup_v1","roles":{
    ...
    support={"schema_version":"msae_independent_source_v8_support_v1","floor_distinct_utterances":20,
    ...
    snapshot={"schema_version":"msae_independent_source_v8_process_snapshot_v1",
    ...
    baseline={"schema_version":"msae_independent_source_v8_baseline_inventory_v1",
    ...
    census={"schema_version":"msae_independent_source_v8_document_group_census_v1","newdoc_marker_count":0,
    ...
    license_value={"schema_version":"msae_independent_source_v8_license_v1","license_id":"CC-BY-SA-4.0",
    ...
    family={"schema_version":"msae_independent_source_v8_source_family_v1","status":"eligible",
        "disposition":"project_source_use_unseen_before_v8","content_occurrence_count":0,"content_occurrences":[],
    ...
    registry={"schema_version":"msae_independent_source_v8_historical_source_registry_v1",
        "status":"frozen_preacquisition","domain_utf8":"msae-v8/pedigree","inputs":[],"input_count":0,
        "inputs_sha256":empty_sha,"overbound_json_source_key_construct_count":0}
    pedigree={"schema_version":"msae_independent_source_v8_candidate_pedigree_v1",
    ...
    values={
        ...
        "source_acquisition_entry.json":{"schema_version":"msae_independent_source_v8_source_acquisition_entry_v1"},
        "source_acquisition.json":{"schema_version":"msae_independent_source_v8_source_acquisition_v1"},
        ...
        "history_overlap.json":{"schema_version":"msae_independent_source_v8_history_overlap_v1",
        ...
        "cross_role_overlap.json":{"schema_version":"msae_independent_source_v8_cross_role_overlap_v1",
        ...
        "preacquisition_authority_manifest.json":{"schema_version":"msae_independent_source_v8_preacquisition_authority_v1"},
    }
    ...
    seal={"schema_version":"msae_independent_source_v8_seal_v1",
          "status":"independent_source_ready_for_future_prescore_protocol",
          "source_status":"project_source_use_unseen_before_v8",
          "plan_sha256":m.sha_file(tmp_path/"docs/plan-msae-independent-source-v8.md"),
          "plan_review_sha256":m.sha_file(tmp_path/"reports/adversarial/msae_independent_source_v8_plan_review.md"),
          "program_sha256":m.sha_file(SCRIPT),
          "acquisition_runner_sha256":m.sha_file(tmp_path/"scripts/acquire_msae_independent_source_v8.py"),
          "test_sha256":m.sha_file(tmp_path/"tests/test_prepare_msae_independent_source_v8.py"),
          "artifact_sha256":bindings,
          "payload":{"path":"data/msae_independent_source_v8/private/blind_payload.jsonl",
    ...
    m.write_json(prov/"no_training_gate.json",{"schema_version":"msae_independent_source_v8_no_training_gate_v1",
