...
import atlas_rope_v5 as core
import diagnose_atlas_rope_v5 as diagnostic
import run_atlas_rope_v5 as runner


def _ephemeral_signer(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    import base64
    import hashlib
    monkeypatch.setattr(core, "SIGNING_PUBLIC_KEY_BASE64", base64.b64encode(public).decode("ascii"))
...
def test_prescore_panels_are_reference_row_aligned() -> None:
    for source in ("EWT_calibration", "GUM_fresh_validation", "GENTLE_validation"):
        root = ROOT / "data/atlas_rope_v5_attempt9" / source
...
def test_cap_grid_boundary_and_out_of_grid() -> None:
    ...
    assert core.choose_grid_cap(8e-6, core.OLD_ATOL_GRID) is None
    assert core.choose_grid_cap(8e-6, core.ATOL_GRID) == 2e-5
    assert core.choose_grid_cap(1.3512020111083984e-5, core.ATOL_GRID) == 2e-5
    assert core.choose_grid_cap(2e-5, core.ATOL_GRID) is None
...
def test_actual_gentle_and_sentinel_support_contracts() -> None:
    gentle = ROOT / "data/atlas_rope_v5_attempt9/GENTLE_validation/panel.json"
    ...
    sentinel = core.read_json(ROOT / "data/atlas_rope_v5_attempt9/GUM_fresh_sentinel/panel.json")
...
def test_signed_attempt8_terminal_verifies_and_tampering_fails() -> None:
    terminal = core.read_json(ROOT / "pilot_runs/20260803_atlas_rope_technical_v4/TERMINAL.json")
    payload = core.verify_envelope(terminal)
    assert payload["status"] == "TERMINAL_EWT_CALIBRATION_INVALID"
...
def test_library_attestation_requires_exact_required_and_unique_sonames() -> None:
    ...
    assert all(isinstance(row, list) for row in result["extra_observed"])
    assert core.verify_library_attestation(required, [], forbidden_sonames=[])["status"] == "FAIL"


def test_signed_write_normalizes_nested_tuples_and_realistic_library_extras(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    signing_key = _ephemeral_signer(tmp_path, monkeypatch)
    monkeypatch.setattr(core, "RUN_ROOT", tmp_path / "run")
    path = tmp_path / "signed.json"
    payload = {
        "status": "PASS",
        "nested": {"rows": [("/a/lib.so", "lib.so", "aaa")]},
        "required_library_check": core.verify_library_attestation(
            [], [{"canonical_path": "/a/lib.so", "soname": "lib.so", "sha256": "aaa"}],
            forbidden_sonames=[],
        ),
    }
    digest = core.write_signed(path, payload, signing_key)
    observed = core.verify_envelope(core.read_json(path))
    assert observed == json.loads(core.canonical_json_bytes(payload))
    assert isinstance(observed["nested"]["rows"][0], list)
    assert digest == core.sha256_file(path)
    assert not list(tmp_path.glob(".signed.json.*.verified.tmp"))


def test_signed_write_preverification_failure_leaves_no_destination(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    signing_key = _ephemeral_signer(tmp_path, monkeypatch)
    monkeypatch.setattr(core, "RUN_ROOT", tmp_path / "run")
    path = tmp_path / "signed.json"
    monkeypatch.setattr(core, "verify_envelope",
                        lambda envelope: (_ for _ in ()).throw(RuntimeError("injected verification failure")))
    with pytest.raises(RuntimeError, match="injected verification failure"):
        core.write_signed(path, {"status": "PASS"}, signing_key)
    assert not path.exists()
    assert not list(tmp_path.glob(".signed.json.*.verified.tmp"))


def test_signed_write_is_atomic_no_clobber(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    signing_key = _ephemeral_signer(tmp_path, monkeypatch)
    monkeypatch.setattr(core, "RUN_ROOT", tmp_path / "run")
    path = tmp_path / "signed.json"
    core.write_signed(path, {"status": "FIRST"}, signing_key)
    original = path.read_bytes()
    with pytest.raises(RuntimeError, match="already exists"):
        core.write_signed(path, {"status": "SECOND"}, signing_key)
    assert path.read_bytes() == original
    assert core.verify_envelope(core.read_json(path))["status"] == "FIRST"


def test_signed_write_postlink_fault_hard_stops_and_preserves_evidence(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    signing_key = _ephemeral_signer(tmp_path, monkeypatch)
    monkeypatch.setattr(core, "RUN_ROOT", tmp_path / "run")
    path = tmp_path / "signed.json"
    real_verify = core.verify_envelope
    calls = 0

    def fail_second(envelope: dict[str, object]) -> dict[str, object]:
        nonlocal calls
        calls += 1
        if calls == 2:
            raise RuntimeError("injected post-link failure")
        return real_verify(envelope)

    monkeypatch.setattr(core, "verify_envelope", fail_second)
    with pytest.raises(RuntimeError, match="injected post-link failure"):
        core.write_signed(path, {"status": "PASS"}, signing_key)
    assert path.is_file()
    assert (core.RUN_ROOT / "SIGNING_FAULT.json").is_file()


def test_synthetic_grid_bundle_with_runtime_extras_signs_and_promotes_once(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    signing_key = _ephemeral_signer(tmp_path, monkeypatch)
    references, candidates, rows = core.make_grid_units("synthetic", _synthetic_bases())
    panel = {"logical_counts": {"bases": 100, "reference_units": 100, "candidate_units": 600,
                                 "reference_rows": 200, "candidate_rows": 1200, "cells": 30,
                                 "rows_per_cell": 40, "elements_per_cell": 30720}}
    monkeypatch.setattr(runner, "_load_panel", lambda source: (panel, references, candidates, rows))

    def forward(model: object, units: object, **kwargs: object) -> np.ndarray:
        count = sum(len(unit["positions"]) for unit in units)  # type: ignore[index]
        return np.zeros((count, core.WIDTH), dtype=np.float32)

    realistic_runtime = {
        "required_library_check": {"status": "PASS", "missing_required": [],
                                   "conflicting_duplicate_sonames": {}, "forbidden_present": [],
                                   "extra_observed": [["/extra/lib.so", "lib.so", "abc"]]},
        "observed_libraries": [{"canonical_path": "/extra/lib.so", "soname": "lib.so",
                                "sha256": "abc", "bytes": 1}],
    }
    monkeypatch.setattr(runner, "_forward_units", forward)
    monkeypatch.setattr(runner, "_runtime_attestation", lambda *args, **kwargs: realistic_runtime)
    monkeypatch.setattr(runner, "RUN_ROOT", tmp_path)
    monkeypatch.setattr(runner, "VALIDATION_AUTHORIZATION", ROOT / "PLAN_ATTEMPT9.md")
    monkeypatch.setattr(runner, "_verify_authorization", lambda *args, **kwargs: {})
    stage = tmp_path / "stage"
    monkeypatch.setattr(runner, "new_staging", lambda *args, **kwargs: (stage.mkdir(), stage)[1])
    final = tmp_path / "validation/GUM_grid"

    def verify(final_path: Path, **kwargs: object) -> dict[str, object]:
        observed = core.verify_envelope(core.read_json(final_path / "COMPLETE.json"))
        assert observed["runtime_preflight"] == realistic_runtime
        assert observed["runtime_postflight"] == realistic_runtime
        return observed

    monkeypatch.setattr(runner, "verify_grid_bundle", verify)
    result = runner._extract_grid(
        object(), {"model": {"name": "synthetic"}}, "GUM_fresh_validation", final, device=torch.device("cpu"),
        validation_caps={"atol": 2e-5, "relative_l2": 1e-5, "cosine_distance": 1e-11},
        authorization_path=ROOT / "PLAN_ATTEMPT9.md", signing_key=signing_key,
        runtime_preflight=realistic_runtime,
    )
    assert result["status"] == "PASS"
...
def test_controller_stage_order_matches_frozen_config_and_uses_gentle() -> None:
    config = core.read_json(ROOT / "configs/atlas_rope_v5/prescore.json")
...
def test_every_reviewed_path_mutation_fails_exact_inventory(tmp_path: Path) -> None:
    paths = {"a.txt": tmp_path / "a.txt", "b.txt": tmp_path / "b.txt"}
    for name, path in paths.items():
        path.write_text(name, encoding="utf-8")
    inventory = runner._exact_inventory(paths)
    runner._verify_exact_inventory(paths, inventory)
    for name, path in paths.items():
        original = path.read_bytes()
...
def test_runtime_preflight_failure_prevents_score_forward(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(runner, "_assert_not_terminal", lambda: None)
    monkeypatch.setattr(runner, "_verify_implementation_candidate", lambda: {})
    monkeypatch.setattr(runner, "_verify_prescore", lambda: ({"seed": 1}, {}))
    monkeypatch.setattr(runner, "_validation_caps", lambda: {"atol": 2e-5, "relative_l2": 1e-5,
                                                               "cosine_distance": 1e-11})
...
def test_attempt8_terminal_staging_recomputes_as_opened_calibration_history() -> None:
    reference, candidate, lineage = runner._verify_attempt8_history()
    assert reference.shape == (200, core.WIDTH)
    assert candidate.shape == (1200, core.WIDTH)
    assert lineage["status"] == "VERIFIED_OPENED_CALIBRATION_HISTORY"
    raw = lineage["old_raw_cap_inputs"]
    assert raw["status"] == "INELIGIBLE"
    assert raw["caps"]["atol"] is None
    assert raw["observed_maxima"]["required_atol"] == 1.3512020111083984e-5


def test_verification_report_must_bind_exact_candidate_inventory_hashes() -> None:
    inventory = {"PLAN_ATTEMPT9.md": {"bytes": 10, "sha256": "current"},
                 "scripts/example.py": {"bytes": 20, "sha256": "code"}}
    runner._verify_report_inventory_hashes(
        {"implementation_hashes": {"PLAN_ATTEMPT9.md": "current", "scripts/example.py": "code"}}, inventory)
    with pytest.raises(RuntimeError, match="does not bind the exact candidate inventory"):
        runner._verify_report_inventory_hashes(
            {"implementation_hashes": {"PLAN_ATTEMPT9.md": "stale", "scripts/example.py": "code"}}, inventory)


@pytest.mark.parametrize("caps", [None, {"atol": 2e-5, "relative_l2": 1e-5, "cosine_distance": 1e-11}])
def test_attempt9_forbids_new_score_bearing_ewt_calibration_forward(caps: dict[str, float] | None) -> None:
    with pytest.raises(RuntimeError, match="forbids a new EWT"):
        runner._extract_grid(
            object(), {}, "EWT_calibration", ROOT / "never-created",
            device=torch.device("cpu"), validation_caps=caps,
            authorization_path=ROOT / "PLAN_ATTEMPT9.md", signing_key=ROOT / "unused",
            runtime_preflight={},
        )
    with pytest.raises(RuntimeError, match="no promoted score-bearing calibration bundle"):
        runner.verify_grid_bundle(ROOT / "never-created", source="EWT_calibration", validation_caps=caps)


@pytest.mark.parametrize(
    "call",
    [
        lambda key, review: runner.make_implementation_candidate(ROOT / "PLAN_ATTEMPT9.md"),
        lambda key, review: runner.retire_attempt8(key, review),
        lambda key, review: runner.import_attempt8_calibration(key),
        lambda key, review: runner.make_calibration_cap_candidate(key),
        lambda key, review: runner.authorize_ewt(key, review),
        lambda key, review: runner.make_freeze_candidate(key),
        lambda key, review: runner.authorize_validation(key, review),
        lambda key, review: runner.run_gum(key),
        lambda key, review: runner.run_gentle(key),
        lambda key, review: runner.authorize_science(key),
    ],
)
def test_every_attempt9_controller_stage_rejects_terminal_before_work(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch, call: object) -> None:
    monkeypatch.setattr(runner, "TERMINAL_PATH", ROOT / "pilot_runs/20260803_atlas_rope_technical_v4/TERMINAL.json")
    key = tmp_path / "key.pem"
    review = ROOT / "reports/adversarial/atlas_v3_5_attempt9_plan_review.md"
    with pytest.raises(RuntimeError, match="terminal and cannot continue"):
        call(key, review)  # type: ignore[operator]
