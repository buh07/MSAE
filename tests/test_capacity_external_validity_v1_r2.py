...
spec=importlib.util.spec_from_file_location('cev1',ROOT/'scripts/capacity_external_validity_v1_r2.py');m=importlib.util.module_from_spec(spec);sys.modules['cev1']=m;assert spec.loader;spec.loader.exec_module(m)
...
def test_plan_declares_k2_closed():
 text=(ROOT/'PLAN_CAPACITY_EXTERNAL_VALIDITY_V1_R2.md').read_text();assert 'No K2 path exists' in text


def test_r2_config_and_source_parity():
 assert m.verify_parity()['status']=='PASS'
...
def test_real_target_mlp_jacobian_all_targets_cpu():
 c=m.cfg('capacity');rows=m.make_capacity_rows(98701,'smoke',c,16,16,98702);a=m.capacity_arrays(rows,98701,c);x=torch.tensor(a['x'],dtype=torch.float32);t=torch.tensor(a['target'],dtype=torch.long);model=m.TargetMLP(64,4,16,16,98703,torch.device('cpu'));r=m.empirical_jacobian_ranks(model,x,t,16,torch.device('cpu'));assert len(r)==16;assert all(isinstance(v,int) and v>=0 for v in r)
