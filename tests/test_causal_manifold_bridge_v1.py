...
ROOT=Path(__file__).resolve().parents[1];SPEC=importlib.util.spec_from_file_location("cmb",ROOT/"scripts/causal_manifold_bridge_v1.py");assert SPEC and SPEC.loader;m=importlib.util.module_from_spec(SPEC);sys.modules["cmb"]=m;SPEC.loader.exec_module(m)
