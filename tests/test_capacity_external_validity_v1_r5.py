...
import importlib.util,glob as globlib,json,os,sys,tempfile,signal
from unittest import mock
from pathlib import Path
import numpy as np, torch
ROOT=Path(__file__).resolve().parents[1]
spec=importlib.util.spec_from_file_location('cev1',ROOT/'scripts/capacity_external_validity_v1_r5.py');m=importlib.util.module_from_spec(spec);sys.modules['cev1']=m;assert spec.loader;spec.loader.exec_module(m)
...
def test_run_pipeline_gpu_validation_failure_writes_both_terminals():
 with tempfile.TemporaryDirectory() as td:
  root=Path(td);cc={'seed':1,'runtime':{'provenance_root':str(root/'cp'),'output_root':str(root/'co')}};ec={'runtime':{'provenance_root':str(root/'ep'),'output_root':str(root/'eo')}};old=dict(m._ACTIVE_RUN)
  with mock.patch.object(m,'cfg',side_effect=lambda kind:cc if kind=='capacity' else ec),mock.patch.object(m,'launch_preflight',return_value={'status':'PASS'}),mock.patch.object(m,'gpu_validate',side_effect=RuntimeError('forced gpu validation')),mock.patch.object(signal,'signal'):
   try:m.run_pipeline(0,'GPU-X','token',123,str(root/'lock'))
   except RuntimeError:pass
   else:raise AssertionError('forced GPU validation failure did not propagate')
  assert (root/'cp/events/999_TECHNICAL_TERMINAL.json').is_file();assert (root/'ep/events/999_TECHNICAL_TERMINAL.json').is_file();assert json.loads((root/'eo/final/result.json').read_text())['status']=='BLOCKED_UNOPENED';assert not (root/'ep/events/021_ACCESS_MAY_HAVE_OCCURRED.json').exists();m._ACTIVE_RUN.clear();m._ACTIVE_RUN.update(old)
...
def test_plan_declares_k2_closed():
 text=(ROOT/'PLAN_CAPACITY_EXTERNAL_VALIDITY_V1_R5.md').read_text();assert 'No K2 path exists' in text


def test_r5_config_and_source_parity():
 z=m.verify_parity();assert z['status']=='PASS';assert z['restore_paths_forbidden']
 with tempfile.TemporaryDirectory() as td:
  bad=Path(td)/'bad.py';bad.write_text((ROOT/'scripts/capacity_external_validity_v1_r5.py').read_text().replace("rcond=c['methods']['svd_rtol'])@Y.astype",'rcond=0.5)@Y.astype',1));old=m.SCRIPT;m.SCRIPT=bad
  try:
   try:m.verify_parity()
   except RuntimeError:pass
...
def test_all_jsonl_guard_blocks_access_surfaces():
 ...
 with m.no_jsonl_access_guard():
  for fn in (lambda:p.read_text(),lambda:p.read_bytes(),lambda:p.stat(),lambda:p.open(),lambda:open(os.fsencode(p), 'rb'),lambda:os.open(p,os.O_RDONLY),lambda:os.open(os.fsencode(p),os.O_RDONLY),lambda:list(Path('/tmp').glob('*.jsonl')),lambda:globlib.glob('/tmp/*.jsonl'),lambda:list(globlib.iglob('/tmp/*.jsonl')),lambda:os.scandir('/tmp/not-allowlisted'),lambda:os.listdir('/tmp')):
   try:fn()
