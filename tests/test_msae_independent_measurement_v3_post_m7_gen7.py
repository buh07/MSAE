...
for _local_module in (
    ...
    "run_msae_independent_calibration_v3", "run_msae_independent_calibration_v3_gen7",
):
    sys.modules.pop(_local_module, None)
_controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen7.py"
_controller = types.ModuleType("msae_independent_measurement_v3_post_m2_gen7")
...
def test_nfs_publisher_fault_matrix():
    ...
    try:
        ...
        raw = runtime.base.canonical_bytes({
            "schema_version": "msae_v3_gen7_publisher_baseline_v1"})
...
def test_derived_publication_requires_two_independent_reproductions(
    ...
    fake_mount = {
        "schema_version": "msae_v3_gen7_mount_identity_v1",
...
def _copy_source_only_cli_fixture(root: Path) -> None:
    ...
    for name in (
        ...
        "msae_independent_measurement_v3_post_m2_gen7.py",
        "msae_independent_measurement_v3_post_m2_gen7_runtime.py",
        "msae_measurement_remediation_v1.py",
        "msae_measurement_v2.py",
        "run_msae_independent_calibration_v3.py",
        "run_msae_independent_calibration_v3_gen7.py",
    ):
...
def test_controller_ignores_valid_timestamp_local_pyc_before_all_gates(tmp_path):
    ...
    result = subprocess.run([
        python, "-S", "-B", "-I",
        str(candidate / "scripts/msae_independent_measurement_v3_post_m2_gen7.py"),
        "not-a-command",
    ], text=True, capture_output=True)
...
def test_runner_ignores_valid_timestamp_controller_pyc_before_auth_gate(tmp_path):
    candidate = tmp_path / "candidate"
    _copy_source_only_cli_fixture(candidate)
    controller = candidate / "scripts/msae_independent_measurement_v3_post_m2_gen7.py"
    ...
    result = subprocess.run([
        ...
        str(candidate / "scripts/run_msae_independent_calibration_v3_gen7.py"),
...
def test_runner_removes_scripts_path_before_deferred_third_party_import(tmp_path):
    ...
    runner = candidate / "scripts/run_msae_independent_calibration_v3_gen7.py"
    result = subprocess.run([
        python, "-S", "-B", "-I", "-c",
        "import runpy,sys;"
...
def _minimal_config_candidate_root(root: Path) -> tuple[str, str, str]:
    ...
    relatives = (
        ...
        "scripts/run_msae_independent_calibration_v3_gen7.py",
        "scripts/msae_independent_measurement_v3_post_m2_gen7.py",
        "scripts/msae_independent_measurement_v3_post_m2_gen7_runtime.py",
        "scripts/launch_msae_independent_calibration_v3_gen7.sh",
        "docs/rfc-msae-independent-measurement-v3-post-m7-gen7.md",
        "docs/plan-msae-independent-measurement-v3-post-m7-gen7.md",
        "tests/test_msae_independent_measurement_v3_post_m7_gen7.py",
    ...
    return (runtime.base.sha_file(
        root / "reports/adversarial/msae_independent_measurement_v3_post_m2_gen7_plan.md"),
...
def test_real_protocol_config_is_byte_equal_across_two_copied_sparse_roots(tmp_path):
    ...
    for row in left_value["replay"]["strata"]:
        assert row["code_sha256"] == runtime.base.sha_file(
            left / "scripts/run_msae_independent_calibration_v3_gen7.py")
...
def test_frozen_plan_review_is_canonical_and_machine_bound():
    ...
    assert value["scope"] == "gen7_plan"
    assert value["digests"] == {
        "GEN6_PLAN": runtime.FAILED_GEN6_PLAN_SHA256,
        "GEN6_PLAN_REVIEW": runtime.FAILED_GEN6_PLAN_REVIEW_SHA256,
        "GEN6_FAILURE_REVIEW": runtime.FAILED_GEN6_FAILURE_REVIEW_SHA256,
        "GEN7_PLAN": runtime.FROZEN_PLAN_SHA256,
    }
...
def test_gen3_review_namespace_rejects_undeclared_prefixed_sibling(
    ...
    names = {
        "plan": "msae_independent_measurement_v3_post_m2_gen7_plan.md",
        "pre": "msae_independent_measurement_v3_post_m2_gen7_pre_capability.md",
        "implementation":
            "msae_independent_measurement_v3_post_m2_gen7_implementation.md",
        "post": "msae_independent_measurement_v3_post_m2_gen7_post_m3.md",
        "prescore": "msae_independent_measurement_v3_post_m2_gen7_prescore.md",
    }
    ...
    (root / "msae_independent_measurement_v3_post_m2_gen7_backup.md").write_text(
        "unexpected\n")
...
def test_failed_gen2_is_wrapped_in_gen7_without_cross_generation_write():
...
def test_gen6_to_gen7_typed_delta_projection_is_exact_and_rejects_byte_drift(
        monkeypatch):
    evidence = runtime.gen6_to_gen7_substitution_evidence()
    rows = evidence["entries"]
    assert evidence["schema_version"] == \
        "msae_v3_gen7_typed_substitution_evidence_v1"
    assert evidence["all_entries_typed_and_frozen"] is True
    assert evidence["scientific_ast_unchanged"] is True
    assert [row["name"] for row in rows] == [
        "controller", "runtime", "runner", "launcher",
        "tmux_test_supervisor", "rfc", "tests",
    ]
    assert {row["name"]: row["typed_delta_sha256"] for row in rows} == \
        dict(runtime.FROZEN_GEN7_TYPED_DELTA_SHA256S)
    assert {row["name"] for row in rows
            if row["byte_identical_after_substitution"]} == {
                "controller", "runner", "launcher",
            }

    original_regular_bytes = runtime._regular_bytes

    def drifted_regular_bytes(path, *, mode, owner=None):
        raw = original_regular_bytes(path, mode=mode, owner=owner)
        if Path(path) == runtime.RUNTIME:
            return raw + b"\n# planted unregistered control-plane byte\n"
        return raw

    monkeypatch.setattr(runtime, "_regular_bytes", drifted_regular_bytes)
    with pytest.raises(ValueError, match="gen7 typed delta drift for runtime"):
        runtime.gen6_to_gen7_substitution_evidence()


def test_closure_registry_contains_every_gen7_review_and_failure_input():
    required = {
        ...
        runtime.FAILED_GEN6_RECORD.relative_to(runtime.ROOT).as_posix(),
...
def test_post_m3_review_binds_every_realized_directory_and_downstream_absence(
    ...
    assert result["review"]["scope"] == "gen7_post_m3"
    assert set(captured) == {
...
def test_setup_receipt_schema_is_exact_and_rejects_bad_digest():
    value = runtime._setup_receipt_value(
        1, "gen7_provenance_root", "mkdir", str(runtime.ACTIVE_PROV),
        "a" * 64, None, None)
    assert set(value) == {
...
def test_transition_cli_requires_both_review_gates_before_dispatch():
    ...
    setup = subprocess.run(
        [python, "-S", "-B", "-I", controller, "setup-m3-gen7",
         "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256],
        text=True, capture_output=True)
    assert setup.returncode != 0
    assert "noncanonical gen7 command shape" in setup.stderr
    m4 = subprocess.run(
        [python, "-S", "-B", "-I", controller, "build-m4-gen7"],
        text=True, capture_output=True)
    assert m4.returncode != 0
    assert "noncanonical gen7 command shape" in m4.stderr
...
def test_m3_rejects_every_nonreviewed_m2_digest_before_writing():
    with pytest.raises(ValueError, match="M2 pin drift"):
        runtime.setup_m3_gen7("0" * 64, "a" * 64, "b" * 64,
                              "c" * 64, "d" * 64, "e" * 64,
                              "f" * 64, "1" * 64)
...
def test_review_verdict_parser_rejects_conflicts_and_duplicate_digest_lines(tmp_path, monkeypatch):
    ...
    good = "\n".join(["VERDICT: SHIP", "REVIEW_SCOPE: gen7_prescore",
...
def test_recursive_static_closure_rejects_planted_surface_escapes(tmp_path):
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    runner = tmp_path / "scripts/run_msae_independent_calibration_v3_gen7.py"
    planted = [
...
def test_static_dynamic_imports_have_exact_site_specific_targets(tmp_path):
    ...
    gen3_dynamic = [row for row in value["dynamic_import_mappings"]
                    if row["path"].endswith("post_m2_gen7_runtime.py")]
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    module = tmp_path / "scripts/msae_independent_measurement_v3_post_m2_gen7_runtime.py"
    ...
    module.write_text(original.replace(
        ...
        '"scripts/run_msae_independent_calibration_v3_gen7.py")', 1))
...
def test_static_value_flow_ledger_closes_caller_argv_and_helper_return_bypasses(tmp_path):
    ...
    module.write_text(original.replace(
        ...
        '            str(continuation.CONTROLLER_PATH), "broker-gen7",',
        'argv = ["/bin/false", "-S", "-B", "-I",\n'
        '            str(continuation.CONTROLLER_PATH), "broker-gen7",', 1))
...
def test_internal_broker_requires_live_launcher_capability_before_socket_or_gpu(tmp_path, monkeypatch):
    ...
    socket_path = "/tmp/m7b_" + "a" * 64 + ".sock"
...
def test_tmux_client_timeout_is_bounded_and_extinguishes_its_captured_group(monkeypatch):
    vector = ["/usr/bin/tmux", "-f", "/dev/null", "-S",
              "/tmp/m7t_" + "a" * 64 + ".sock", "has-session", "-t",
              "msae-independent-v3-gen7-calibration"]
    observed = []
...
def _gen7_monitor_record_fixture() -> dict:
    stage = "a" * 64
    tmux_path, broker_path, pane_path = runtime._tmux_paths(stage)
    return {
        "schema_version": "msae_v3_gen7_tmux_monitor_v2",
        "protocol_id": runtime.PROTOCOL,
...
def _gen7_monitor_payload(sequence: int) -> dict:
    digest = "a" * 64
    payloads = {
        ...
        4: {"record_sha256": digest, "record_ack_sha256": digest,
            "tmux_socket_path": "/tmp/m7t_" + digest + ".sock",
            "broker_socket_path": "/tmp/m7b_" + digest + ".sock",
            "pane_socket_path": "/tmp/m7p_" + digest + ".sock"},
        5: {"server_pid": 2003, "server_start_ticks": "13",
            ...
            "tmux_socket_binding": {"device": 1, "inode": 2,
                                    ...
                                    "mode": 0o700, "nlink": 1},
            "session": "msae-independent-v3-gen7-calibration",
...
def test_monitor_record_and_short_socket_contract_reject_every_mutation():
    record = _gen7_monitor_record_fixture()
...
def test_monitor_frame_chain_is_canonical_exact_and_datagram_bounded():
    ...
    for sequence, (frame_type, _keys) in enumerate(runtime.MONITOR_FRAME_SPECS, 1):
        frame = runtime._monitor_frame(
            frame_type, sequence, previous, _gen7_monitor_payload(sequence))
...
def test_parent_death_watchdog_kills_worker_when_broker_exits(tmp_path):
    runtime_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen7_runtime.py"
    controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen7.py"
    script = f'''\
import importlib.util, os, sys, time, types
controller_path={str(controller_path)!r}
controller=types.ModuleType("msae_independent_measurement_v3_post_m2_gen7")
...
def _tmux_test_process_fixture() -> dict[str, int | str]:
    return {
    "schema_version": "msae_v3_gen7_tmux_test_process_v1",
...
def test_tmux_supervisor_cleans_exact_crash_left_identity_pairs(tmp_path):
    module_name = "_msae_gen7_tmux_supervisor_pair_test"
...
def test_tmux_supervisor_cleanup_rejects_wrong_bytes_and_third_links(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_cleanup_poison", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    ...
    value = {"schema_version": "msae_v3_gen7_tmux_test_identity_bound_v1",
...
def test_tmux_supervisor_cleanup_rechecks_second_pair_member_before_unlink(
        tmp_path, monkeypatch):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_cleanup_substitution",
...
def test_tmux_supervisor_cleanup_accepts_valid_destination_only_record(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_cleanup_destination", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    final = tmp_path / "identity_bound.json"
    final.write_bytes(supervisor._canonical_bytes({
        "schema_version": "msae_v3_gen7_tmux_test_identity_bound_v1",
...
def test_tmux_supervisor_rejects_invocation_drift_before_namespace(drift):
    before = sorted(Path("/tmp").glob("msae-independent-v3-gen7-test-*"))
    argv = [str(ROOT / ".venv-atlas/bin/python"), "-S", "-B", "-I",
    ...
    assert sorted(Path("/tmp").glob("msae-independent-v3-gen7-test-*")) == before


def test_tmux_supervisor_child_timeout_terminates_real_resistant_group():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_child_timeout", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    source = (
...
def test_tmux_supervisor_extinguishes_group_after_pytest_leader_exit():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_dead_leader", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    source = (
        "import os,signal,time\n"
...
def test_tmux_supervisor_rejects_late_duplicate_identity_connection(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_duplicate_connection",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_publication_fault_boundaries_cleanup(
    ...
    module_name = f"_msae_gen7_tmux_supervisor_fault_{boundary}"
    spec = runtime.continuation.source_only_module_spec(
        module_name, runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_publication_retries_short_writes(tmp_path, monkeypatch):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_short_write", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_test_terminal_is_exact_create_once_and_owned(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_terminal", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_pre_capability_review_requires_exact_contiguous_command_evidence(
        monkeypatch):
    rows = [{"schema_version": "msae_v3_gen7_pre_capability_check_v1",
             "id": "one", "argv": ["/registered/one"], "cwd": str(ROOT),
             "environment": {}, "subject_sha256": "1" * 64,
             "exit_code": 0, "summary": "pass"},
            {"schema_version": "msae_v3_gen7_pre_capability_check_v1",
    ...
    encoded = ["CHECK_ROW_JSON: " + runtime.base.canonical_bytes(row).decode(
        "utf-8").rstrip("\n") for row in rows]
    prefix = ["VERDICT: SHIP", "REVIEW_SCOPE: gen7_pre_capability",
              "CHECKS RUN"]
    ...
    monkeypatch.setattr(runtime, "_current_gen7_implementation_entries",
                        lambda: {})
...
def test_pre_capability_namespace_gate_blocks_extra_and_downstream_before_write(
    ...
    monkeypatch.setattr(runtime, "_validate_gen3_review_namespace",
                        lambda _phase: None)
    monkeypatch.setattr(runtime, "_all_gen7_short_family_absence",
...
def test_tmux_fault_event_variants_reject_every_unregistered_control_mutation():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_event_schema",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(supervisor)
    ...
    hook = {
        "schema_version": "msae_v3_gen7_fault_event_v1",
...
def test_tmux_outer_reconstructs_scenario_action_projection():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen7_tmux_supervisor_action_projection",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(supervisor)
...
def test_failed_handoff_extinction_kills_real_processes_and_socket(
    ...
    if raw_control_fd is None:
        pytest.skip("the reviewed gen7 outer supervisor is required")
    ...
    token_preimage = {
        "schema_version": "msae_v3_gen7_test_token_preimage_v1",
    ...
    def frame(case_id, sequence, prior, frame_type, payload):
        return {"schema_version": "msae_v3_gen7_test_control_frame_v1",
...
def test_gen7_terminal_payload_preserves_two_failed_gen3_invocations_honestly():
...
def test_exact_gen7_cli_shape_rejects_reordering_duplicates_and_old_routes():
    setup = [
        "setup-m3-gen7",
        "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256,
        ...
        "--gen7-plan-review-sha256", runtime.FROZEN_PLAN_REVIEW_SHA256,
        "--gen7-pre-capability-review-sha256", "b" * 64,
        "--gen7-implementation-review-sha256", "a" * 64,
    ]
    runtime._require_exact_command_shape(setup)
    for defective in (
        [*setup[:3], *setup[5:7], *setup[3:5], *setup[7:]],
        [*setup, "--gen7-implementation-review-sha256", "a" * 64],
        ["setup-m3", *setup[1:]],
        ["build-m4-gen7", "--post-m3-review-sha256", "A" * 64],
        ["sign-gen7", "extra"],
    ):
        with pytest.raises(ValueError):
...
def test_capability_reentry_rejects_undeclared_children_before_mutation(
    ...
    report = analysis / "gen7_capability_reproduction.json"
    ...
    with pytest.raises(ValueError, match=(
        ...
        runtime.probe_gen7_capability(report, "a" * 64)
...
def test_capability_reauthorizes_report_and_outer_namespace_before_descriptor_delete(
    ...
    row = runtime._publication_row(
        ...
        report_raw, 0o644, "gen7_capability_report")
    ...
    monkeypatch.setattr(runtime, "_failed_gen4_entries", lambda: {})
    monkeypatch.setattr(runtime, "_failed_gen5_entries", lambda: {})
    monkeypatch.setattr(runtime, "_current_gen7_implementation_entries", lambda: subject)
    monkeypatch.setattr(runtime, "_validate_capability_report", lambda _value: None)
    monkeypatch.setattr(
        runtime, "_pre_capability_namespace_gate",
        lambda *, allow_capability_root: {
...
def test_setup_prefix_validator_rejects_receipt_hole(tmp_path, monkeypatch):
    prov = tmp_path / "prov"; prov.mkdir(mode=0o700)
    journal = prov / ".setup_m3_gen7_transaction"; journal.mkdir(mode=0o700)
    ...
    monkeypatch.setattr(runtime, "SETUP_MANIFEST", prov / "setup_m3_gen7_manifest.json")
    monkeypatch.setattr(runtime, "M4_NEW", tuple())
...
def test_setup_m3_gen7_builds_and_cleans_exact_24_receipt_chain(
        tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime, "_current_gen7_entries", lambda: {})
    ...
    completion = runtime.setup_m3_gen7(
        runtime.M2_REVIEWED_COMPLETION_SHA256,
        ...
        "7" * 64,
        "8" * 64)
    assert completion["schema_version"] == "msae_v3_gen7_m3_completion_v1"
