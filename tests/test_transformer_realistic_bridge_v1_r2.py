...
spec=importlib.util.spec_from_file_location('bridge',ROOT/'scripts/transformer_realistic_bridge_v1_r2.py')
b=importlib.util.module_from_spec(spec);sys.modules['bridge']=b;spec.loader.exec_module(b)
CFG=json.loads((ROOT/'configs/transformer_realistic_bridge_v1_r2/run.json').read_text())
MCFG=json.loads((ROOT/'configs/transformer_realistic_methods_v1_r2/run.json').read_text())
...
def test_candidate_preflight_does_not_need_scientific_payloads(monkeypatch,tmp_path):
    forbidden={str(ROOT/'data/transformer_realistic_bridge_v1_r2_prepared'/x) for x in ('development.jsonl','confirmation.jsonl')}
    forbidden|={str(ROOT/'data/transformer_realistic_methods_v1_r2_prepared'/x) for x in ('train.jsonl','development.jsonl','confirmation.jsonl')}
    attempted=[]
...
def test_launcher_defers_run_root_expansion_to_tmux_shell():
    launcher=(ROOT/'scripts/launch_transformer_realistic_bridge_v1_r2_tmux.sh').read_text()
    assert 'MSAE_ROOT="\\$RUN_ROOT" python "\\$RUN_ROOT/scripts/transformer_realistic_bridge_v1_r2.py"' in launcher
    assert '| tee "\\$RUN_ROOT/reports/provenance/transformer_realistic_bridge_v1_run_20260810r2/launcher.log"' in launcher
    body=launcher.split('cmd=$(cat <<EOF\n',1)[1].split('\nEOF\n)',1)[0]
    render='''set -u
lockdir=/tmp/never-used; launch_token=x; chosen_index=0; chosen_uuid=u; ROOT=/not-opened
cmd=$(cat <<EOF
'''+body+'''\nEOF
)
printf '%s' "$cmd"
'''
    out=subprocess.run(['bash','-c',render],check=True,text=True,capture_output=True).stdout
    assert 'RUN_ROOT=/proc/self/cwd' in out
    assert 'MSAE_ROOT="$RUN_ROOT" python "$RUN_ROOT/scripts/transformer_realistic_bridge_v1_r2.py"' in out
    assert not (ROOT/'results/transformer_realistic_bridge_v1_20260810r2').exists()
