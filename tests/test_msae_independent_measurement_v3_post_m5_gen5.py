...
for _local_module in (
    ...
    "run_msae_independent_calibration_v3", "run_msae_independent_calibration_v3_gen5",
):
    sys.modules.pop(_local_module, None)
_controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen5.py"
_controller = types.ModuleType("msae_independent_measurement_v3_post_m2_gen5")
...
SPEC = _controller.source_only_module_spec(
    ...
    ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen5_runtime.py")
...
def test_nfs_publisher_fault_matrix():
    ...
    try:
        ...
        raw = runtime.base.canonical_bytes({
            "schema_version": "msae_v3_gen5_publisher_baseline_v1"})
...
def _copy_source_only_cli_fixture(root: Path) -> None:
    ...
    for name in (
        ...
        "msae_independent_measurement_v3_post_m2_gen5.py",
        "msae_independent_measurement_v3_post_m2_gen5_runtime.py",
        "msae_measurement_remediation_v1.py",
        "msae_measurement_v2.py",
        "run_msae_independent_calibration_v3.py",
        "run_msae_independent_calibration_v3_gen5.py",
    ):
...
def test_controller_ignores_valid_timestamp_local_pyc_before_all_gates(tmp_path):
    ...
    result = subprocess.run([
        python, "-S", "-B", "-I",
        str(candidate / "scripts/msae_independent_measurement_v3_post_m2_gen5.py"),
        "not-a-command",
    ], text=True, capture_output=True)
...
def test_runner_ignores_valid_timestamp_controller_pyc_before_auth_gate(tmp_path):
    candidate = tmp_path / "candidate"
    _copy_source_only_cli_fixture(candidate)
    controller = candidate / "scripts/msae_independent_measurement_v3_post_m2_gen5.py"
    ...
    result = subprocess.run([
        ...
        str(candidate / "scripts/run_msae_independent_calibration_v3_gen5.py"),
...
def test_runner_removes_scripts_path_before_deferred_third_party_import(tmp_path):
    ...
    runner = candidate / "scripts/run_msae_independent_calibration_v3_gen5.py"
    result = subprocess.run([
        python, "-S", "-B", "-I", "-c",
        "import runpy,sys;"
...
def _minimal_config_candidate_root(root: Path) -> tuple[str, str]:
    ...
    return runtime.base.sha_file(
        root / "reports/adversarial/msae_independent_measurement_v3_post_m2_gen5_plan.md"), \
...
def test_real_protocol_config_is_byte_equal_across_two_copied_sparse_roots(tmp_path):
    ...
    for row in left_value["replay"]["strata"]:
        assert row["code_sha256"] == runtime.base.sha_file(
            left / "scripts/run_msae_independent_calibration_v3_gen5.py")
...
def test_frozen_plan_review_is_canonical_and_machine_bound():
    ...
    assert value["digests"] == {
        ...
        "GEN5_PLAN": runtime.FROZEN_PLAN_SHA256,
    }
...
def test_gen3_review_namespace_rejects_undeclared_prefixed_sibling(
    ...
    names = {
        "plan": "msae_independent_measurement_v3_post_m2_gen5_plan.md",
        "implementation":
            "msae_independent_measurement_v3_post_m2_gen5_implementation.md",
        "post": "msae_independent_measurement_v3_post_m2_gen5_post_m3.md",
        "prescore": "msae_independent_measurement_v3_post_m2_gen5_prescore.md",
    }
    ...
    (root / "msae_independent_measurement_v3_post_m2_gen5_backup.md").write_text(
        "unexpected\n")
...
def test_failed_gen2_is_wrapped_in_gen5_without_cross_generation_write():
...
def test_post_m3_review_binds_every_realized_directory_and_downstream_absence(
    ...
    assert result["review"]["scope"] == "gen5_post_m3"
    assert set(captured) == {
        "GEN5_IMPLEMENTATION_REVIEW", "POST_M3_SUBJECT", "GEN5_PROTOCOL",
        "GEN5_AUTHORIZATION_COMMITMENT", "GEN5_PUBLIC_KEY", "GEN5_CPU_TRACE",
        "GEN5_SETUP_MANIFEST", "GEN4_TERMINAL", "GEN3_TERMINAL",
        "GEN2_M4_FAILURE",
        "M2_COMPLETION",
    }
...
def test_setup_receipt_schema_is_exact_and_rejects_bad_digest():
    value = runtime._setup_receipt_value(
        1, "gen5_provenance_root", "mkdir", str(runtime.ACTIVE_PROV),
        "a" * 64, None, None)
    assert set(value) == {
...
def test_transition_cli_requires_both_review_gates_before_dispatch():
    ...
    setup = subprocess.run(
        [python, "-S", "-B", "-I", controller, "setup-m3-gen5",
         "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256],
        text=True, capture_output=True)
    assert setup.returncode != 0
    assert "noncanonical gen5 command shape" in setup.stderr
    m4 = subprocess.run(
        [python, "-S", "-B", "-I", controller, "build-m4-gen5"],
        text=True, capture_output=True)
    assert m4.returncode != 0
    assert "noncanonical gen5 command shape" in m4.stderr
...
def test_m3_rejects_every_nonreviewed_m2_digest_before_writing():
    with pytest.raises(ValueError, match="M2 pin drift"):
        runtime.setup_m3_gen5("0" * 64, "a" * 64, "b" * 64,
                              "c" * 64, "d" * 64, "e" * 64)
...
def test_review_verdict_parser_rejects_conflicts_and_duplicate_digest_lines(tmp_path, monkeypatch):
    ...
    good = "\n".join(["VERDICT: SHIP", "REVIEW_SCOPE: gen5_prescore",
                       "GEN5_IMPLEMENTATION_REVIEW_SHA256: " + runtime.base.sha_file(files["implementation"]),
                       "GEN5_POST_M3_REVIEW_SHA256: " + runtime.base.sha_file(files["post_m3"]),
                       "GEN5_PROTOCOL_SHA256: " + runtime.base.sha_file(active_config / "protocol.json"),
                       "GEN5_STAGE_A_SHA256: " + "a" * 64,
                       "GEN5_STATUS_SHA256: " + runtime.base.sha_file(active_prov / "status.json"),
                       "GEN5_CANDIDATE_MANIFEST_SHA256: " + "c" * 64,
                       "GEN5_DEPENDENCY_CLOSURE_SHA256: " + "b" * 64,
                       "GEN5_ENDPOINT_REGISTRY_SHA256: " + runtime.base.sha_file(active_data / "endpoint_registry.json"),
                       "GEN5_ENVIRONMENT_ALLOWLIST_SHA256: " + runtime.base.sha_file(active_data / "environment_allowlist.json"),
                       "GEN5_PRESCORE_CHECK_SHA256: " + runtime.base.sha_file(check),
                       "GEN5_PRESCORE_TRACE_SHA256: " + runtime.base.sha_file(trace),
                       "GEN5_PRESCORE_CHECKER_SHA256: " + runtime.base.sha_file(files["checker"]),
    ...
    with pytest.raises(ValueError):
        runtime._strict_review_verdict(good + "\nGEN5_STAGE_A_SHA256: " + "a" * 64, manifest, "c" * 64)
    with pytest.raises(ValueError):
        runtime._strict_review_verdict(good + "\nGEN5_STAGE_A_SHA256: " + "0" * 64, manifest, "c" * 64)
    with pytest.raises(ValueError):
...
def test_recursive_static_closure_rejects_planted_surface_escapes(tmp_path):
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    runner = tmp_path / "scripts/run_msae_independent_calibration_v3_gen5.py"
    planted = [
...
def test_static_dynamic_imports_have_exact_site_specific_targets(tmp_path):
    ...
    gen3_dynamic = [row for row in value["dynamic_import_mappings"]
                    if row["path"].endswith("post_m2_gen5_runtime.py")]
    ...
    for relative in runtime.LOCAL_PYTHON_ENTRYPOINTS:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / relative, target)
    module = tmp_path / "scripts/msae_independent_measurement_v3_post_m2_gen5_runtime.py"
    ...
    module.write_text(original.replace(
        ...
        '"scripts/run_msae_independent_calibration_v3_gen5.py")', 1))
...
def test_static_value_flow_ledger_closes_caller_argv_and_helper_return_bypasses(tmp_path):
    ...
    module.write_text(original.replace(
        ...
        '            str(continuation.CONTROLLER_PATH), "broker-gen5",',
        'argv = ["/bin/false", "-S", "-B", "-I",\n'
        '            str(continuation.CONTROLLER_PATH), "broker-gen5",', 1))
...
@pytest.mark.parametrize("boundary", ("new_session", "display"))
def test_initial_tmux_timeout_is_bounded_cleaned_and_releases_lease(
    ...
    monkeypatch.setattr(runtime, "_open_gpu_lock", lambda _uuid: (fd, directory_fd, path))
    terminal = []
    monkeypatch.setattr(runtime, "_technical_failure",
                        lambda *args, **kwargs: terminal.append((args, kwargs)))
    fake_server = []
    observed = []

    def fake_check_call(argv, **kwargs):
        observed.append(("new_session", kwargs.get("timeout")))
        socket_path = Path(argv[argv.index("-S") + 1])
        server = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        server.bind(str(socket_path)); server.listen(1); fake_server.append(server)
        if boundary == "new_session":
            raise subprocess.TimeoutExpired(argv, kwargs["timeout"])
        return 0

    def fake_check_output(argv, **kwargs):
        observed.append(("display", kwargs.get("timeout")))
        raise subprocess.TimeoutExpired(argv, kwargs["timeout"])

    def fake_extinction(socket_path, _session, _identities, expected):
        assert runtime._tmux_socket_identity(socket_path) == expected
        for server in fake_server:
            server.close()
        fake_server.clear()
        socket_path.unlink()

    monkeypatch.setattr(runtime.subprocess, "check_call", fake_check_call)
    monkeypatch.setattr(runtime.subprocess, "check_output", fake_check_output)
    monkeypatch.setattr(runtime, "_verify_failed_handoff_extinction", fake_extinction)
    with pytest.raises(subprocess.TimeoutExpired):
        runtime._launch_impl()
    assert observed[0] == ("new_session", 15)
    if boundary == "display":
        assert observed[1] == ("display", 15)
    assert terminal and not (run / "broker.sock").exists()
    code = ("import fcntl,os,sys; f=os.open(sys.argv[1],os.O_RDWR); "
            "fcntl.flock(f,fcntl.LOCK_EX|fcntl.LOCK_NB); print('acquired')")
    child = subprocess.run([sys.executable, "-c", code, str(path)], check=True,
                           text=True, capture_output=True)
...
def test_parent_death_watchdog_kills_worker_when_broker_exits(tmp_path):
    runtime_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen5_runtime.py"
    controller_path = ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen5.py"
    script = f'''\
import importlib.util, os, sys, time, types
controller_path={str(controller_path)!r}
controller=types.ModuleType("msae_independent_measurement_v3_post_m2_gen5")
...
FAKE_TMUX_BROKER_SOURCE = r'''
import ctypes,json,os,signal,socket,sys,time

ack_path, record_path = sys.argv[1:]
broker_pid = os.getpid()
fault = os.environ.get('MSAE_GEN5_TMUX_FAULT_MODE', '')
if fault not in {'','fork','partial_write','file_fsync','publish','identity',
                 'ack','barrier_release','resistance_arm','child_timeout'}:
    raise RuntimeError('unregistered fault mode')
libc = ctypes.CDLL(None, use_errno=True)
PR_SET_PDEATHSIG = 1
if libc.prctl(PR_SET_PDEATHSIG, signal.SIGKILL, 0, 0, 0) != 0:
    raise OSError(ctypes.get_errno(), "broker prctl")
parent_to_child_r, parent_to_child_w = os.pipe()
child_to_parent_r, child_to_parent_w = os.pipe()
descendant = os.fork()
if descendant == 0:
    os.close(parent_to_child_w); os.close(child_to_parent_r)
    os.setpgid(0, 0)
    expected_parent = broker_pid
    if libc.prctl(PR_SET_PDEATHSIG, signal.SIGKILL, 0, 0, 0) != 0:
        os._exit(120)
    if os.getppid() != expected_parent:
        os._exit(125)
    if os.read(parent_to_child_r, 1) != b'I':
        os._exit(121)
    signal.signal(signal.SIGTERM, signal.SIG_IGN)
    if libc.prctl(PR_SET_PDEATHSIG, 0, 0, 0, 0) != 0:
        os._exit(122)
    os.write(child_to_parent_w, b'A')
    while True: time.sleep(1)
os.close(parent_to_child_r); os.close(child_to_parent_w)
if fault == 'fork':
    time.sleep(0.25); os._exit(90)
...
broker_ppid,broker_pgid,broker_ticks=proc(broker_pid)
deadline=time.monotonic()+1
while True:
    desc_ppid,desc_pgid,desc_ticks=proc(descendant)
    if desc_pgid==descendant: break
    if time.monotonic()>=deadline: raise RuntimeError('descendant PGID setup timeout')
    time.sleep(0.005)
value={'schema_version':'msae_v3_gen5_tmux_test_process_v1',
       'pane_pid':broker_pid,'pane_start_ticks':broker_ticks,
       'broker_pid':broker_pid,'broker_start_ticks':broker_ticks,
       'broker_ppid':broker_ppid,'descendant_pid':descendant,
       'descendant_start_ticks':desc_ticks,'descendant_ppid':desc_ppid,
       'worker_pgid':desc_pgid}
...
while view:
    if fault == 'partial_write':
        os.write(fd,view[:max(1,len(view)//2)])
        time.sleep(0.25); os._exit(91)
    n=os.write(fd,view)
    if n<=0: raise OSError('short process record write')
    view=view[n:]
os.fsync(fd)
if fault == 'file_fsync':
    time.sleep(0.25); os._exit(92)
os.close(fd)
os.link(partial,record_path,follow_symlinks=False)
if fault == 'publish':
    time.sleep(0.25); os._exit(93)
...
client.sendall(raw)
if fault == 'identity':
    time.sleep(0.5); os._exit(94)
if client.recv(1)!=b'I': raise RuntimeError('missing identity ACK')
if fault == 'ack':
    client.sendall(b'1'); time.sleep(0.1); os._exit(95)
os.write(parent_to_child_w,b'I')
if fault == 'barrier_release':
    client.sendall(b'2'); time.sleep(0.1); os._exit(96)
if os.read(child_to_parent_r,1)!=b'A': raise RuntimeError('missing arm ACK')
if fault == 'resistance_arm':
    client.sendall(b'3'); time.sleep(0.1); os._exit(97)
client.sendall(b'A')
if client.recv(1)!=b'B': raise RuntimeError('missing bound ACK')
while True: time.sleep(1)
'''

FAKE_TMUX_BROKER_SOURCE_SHA256 = \
    "621483f01d5b9bc8dce7463c1d6b73a255e00aa62e81e4d9af91fdac95e0d13f"
assert hashlib.sha256(FAKE_TMUX_BROKER_SOURCE.encode("utf-8")).hexdigest() == \
    FAKE_TMUX_BROKER_SOURCE_SHA256


def test_tmux_supervisor_cleans_exact_crash_left_identity_pairs(tmp_path):
    module_name = "_msae_gen5_tmux_supervisor_pair_test"
    ...
    raw = supervisor._canonical_bytes({
        "schema_version": "msae_v3_gen5_tmux_test_process_v1",
        "pane_pid": 1, "pane_start_ticks": 1,
        "broker_pid": 1, "broker_start_ticks": 1, "broker_ppid": 1,
        "descendant_pid": 2, "descendant_start_ticks": 1,
        "descendant_ppid": 1, "worker_pgid": 2,
    })
...
def test_tmux_supervisor_cleanup_rejects_wrong_bytes_and_third_links(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_cleanup_poison", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    ...
    value = {"schema_version": "msae_v3_gen5_tmux_test_identity_bound_v1",
    ...
    process_raw = supervisor._canonical_bytes({
        "schema_version": "msae_v3_gen5_tmux_test_process_v1",
        "pane_pid": 10, "pane_start_ticks": 11,
        "broker_pid": 10, "broker_start_ticks": 11, "broker_ppid": 9,
        "descendant_pid": 12, "descendant_start_ticks": 13,
        "descendant_ppid": 10, "worker_pgid": 12,
    })
...
def test_tmux_supervisor_cleanup_rechecks_second_pair_member_before_unlink(
        tmp_path, monkeypatch):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_cleanup_substitution",
...
def test_tmux_supervisor_cleanup_accepts_valid_destination_only_record(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_cleanup_destination", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    final = tmp_path / "identity_bound.json"
    final.write_bytes(supervisor._canonical_bytes({
        "schema_version": "msae_v3_gen5_tmux_test_identity_bound_v1",
...
def test_tmux_supervisor_rejects_invocation_drift_before_namespace(drift):
    before = sorted(Path("/tmp").glob("msae-independent-v3-gen5-test-*"))
    argv = [str(ROOT / ".venv-atlas/bin/python"), "-S", "-B", "-I",
    ...
    assert sorted(Path("/tmp").glob("msae-independent-v3-gen5-test-*")) == before


def test_tmux_supervisor_child_timeout_terminates_real_resistant_group():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_child_timeout", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    source = (
...
def test_tmux_supervisor_extinguishes_group_after_pytest_leader_exit():
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_dead_leader", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
    source = (
        "import os,signal,time\n"
...
def test_tmux_supervisor_rejects_late_duplicate_identity_connection(tmp_path):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_duplicate_connection",
        runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_publication_fault_boundaries_cleanup(
        tmp_path, monkeypatch, boundary):
    module_name = f"_msae_gen5_tmux_supervisor_fault_{boundary}"
    spec = runtime.continuation.source_only_module_spec(
        module_name, runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_tmux_supervisor_publication_retries_short_writes(tmp_path, monkeypatch):
    spec = runtime.continuation.source_only_module_spec(
        "_msae_gen5_tmux_supervisor_short_write", runtime.TMUX_TEST_SUPERVISOR)
    assert spec is not None and spec.loader is not None
    supervisor = importlib.util.module_from_spec(spec); spec.loader.exec_module(supervisor)
...
def test_failed_handoff_extinction_kills_real_processes_and_socket(monkeypatch, request):
    if os.environ.get("MSAE_GEN5_ALLOW_REAL_TMUX_TEST") != "1":
        pytest.skip("the reviewed source-only supervisor is required")
    token = os.environ.get("MSAE_GEN5_TMUX_TEST_TOKEN", "")
    ...
    assert not any(name in os.environ for name in (
        ...
        "CUDA_MODULE_LOADING"))
    session = f"msae-independent-v3-gen5-test-{token}"
    ...
    for name in ("_gpu_rows", "setup_m3_gen5", "build_m4", "sign_authorization", "launch"):
        monkeypatch.setattr(runtime, name, forbidden)

    fake_argv = [str(Path(sys.executable).resolve()), "-S", "-B", "-I", "-c",
                 FAKE_TMUX_BROKER_SOURCE, str(ack_socket), str(record)]
    ...
    def validate_process(argv, kwargs):
        ...
        if vector[0:6] == (
                "/usr/bin/tmux", "-f", "/dev/null", "-S", str(tmux_socket),
                "new-session"):
            allowed_environments.extend([
                {**test_env, "MSAE_GEN5_TMUX_FAULT_MODE": mode}
    ...
    def child_cleanup():
        ...
        try:
            if ack_socket.exists() or ack_socket.is_symlink():
                if runtime._tmux_socket_identity(ack_socket) != ack_socket_identity:
    ...
    for fault_mode in runtime.TMUX_TEST_FAULT_MODES:
        assert not (tmux_socket.exists() or tmux_socket.is_symlink())
        assert not any((scratch / name).exists() for name in (
            ".process.json.partial", "process.json",
            ".identity_bound.json.partial", "identity_bound.json"))
        fault_environment = {
            **test_env, "MSAE_GEN5_TMUX_FAULT_MODE": fault_mode}
        started = subprocess.run([
            "/usr/bin/tmux", "-f", "/dev/null", "-S", str(tmux_socket),
            "new-session", "-d", "-s", session, shell_command,
        ], check=False, text=True, capture_output=True, env=fault_environment)
        ...
        while time.monotonic() < deadline:
            ...
            if (saw_full_namespace_absence and namespace_recreated
                    and not (tmux_socket.exists() or tmux_socket.is_symlink())
                    and records_absent):
                break
            time.sleep(0.02)
    ...
    assert set(identity) == {
        "schema_version", "pane_pid", "pane_start_ticks", "broker_pid",
        "broker_start_ticks", "broker_ppid", "descendant_pid",
        "descendant_start_ticks", "descendant_ppid", "worker_pgid"}
    assert identity["schema_version"] == "msae_v3_gen5_tmux_test_process_v1"
    ...
    descendant = (identity["descendant_pid"], identity["descendant_start_ticks"])
    cleanup_state["identities"] = [broker, descendant]
    ...
    cleanup_state["identities"] = [broker, descendant, server]
    ...
    runtime._verify_failed_handoff_extinction(
        tmux_socket, session, [broker, descendant, server], socket_identity)
    assert not runtime._pid_has_start_ticks(*broker)
    assert not runtime._pid_has_start_ticks(*descendant)
...
def test_candidate_set_is_unique_and_manifest_self_excluded():
    ...
    assert all("post_m2_gen5" in path for path in runtime.M4_NEW[3:])
...
def test_runner_uses_two_phase_model_gate_and_atomic_terminal_only():
    source = (ROOT / "scripts/run_msae_independent_calibration_v3_gen5.py").read_text()
...
def test_runner_rejects_alternate_failure_root_without_writing(tmp_path):
    ...
    result = subprocess.run([
        str(ROOT / ".venv-atlas/bin/python"), "-S", "-B", "-I",
        str(ROOT / "scripts/run_msae_independent_calibration_v3_gen5.py"),
...
def test_gen5_terminal_payload_preserves_two_failed_gen3_invocations_honestly():
...
def test_exact_gen5_cli_shape_rejects_reordering_duplicates_and_old_routes():
    setup = [
        "setup-m3-gen5",
        "--reviewed-m2-sha256", runtime.M2_REVIEWED_COMPLETION_SHA256,
        ...
        "--gen5-plan-review-sha256", runtime.FROZEN_PLAN_REVIEW_SHA256,
        "--gen5-implementation-review-sha256", "a" * 64,
    ]
    runtime._require_exact_command_shape(setup)
    for defective in (
        [*setup[:3], *setup[5:7], *setup[3:5], *setup[7:]],
        [*setup, "--gen5-implementation-review-sha256", "a" * 64],
        ["setup-m3", *setup[1:]],
        ["build-m4-gen5", "--post-m3-review-sha256", "A" * 64],
        ["sign-gen5", "extra"],
    ):
        with pytest.raises(ValueError):
...
def test_capability_reentry_rejects_undeclared_children_before_mutation(
    ...
    report = analysis / "gen4_publisher_capability_reproduction.json"
    ...
    with pytest.raises(ValueError, match=(
        ...
        runtime.probe_gen4_publisher_capability(report)
...
def test_capability_reauthorizes_report_and_outer_namespace_before_descriptor_delete(
    ...
    monkeypatch.setattr(runtime, "_plan_review_binding", lambda _value: {})
    monkeypatch.setattr(runtime, "_failed_gen4_entries", lambda: {})
    monkeypatch.setattr(runtime, "_current_gen5_entries", lambda: subject)
    monkeypatch.setattr(runtime, "_validate_capability_report", lambda _value: None)
...
def test_setup_prefix_validator_rejects_receipt_hole(tmp_path, monkeypatch):
    prov = tmp_path / "prov"; prov.mkdir(mode=0o700)
    journal = prov / ".setup_m3_gen5_transaction"; journal.mkdir(mode=0o700)
    ...
    monkeypatch.setattr(runtime, "SETUP_MANIFEST", prov / "setup_m3_gen5_manifest.json")
    monkeypatch.setattr(runtime, "M4_NEW", tuple())
...
def test_setup_m3_gen5_builds_and_cleans_exact_22_receipt_chain(
        tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime, "_current_gen5_entries", lambda: {})
    ...
    completion = runtime.setup_m3_gen5(
        runtime.M2_REVIEWED_COMPLETION_SHA256,
        ...
        "8" * 64)
    assert completion["schema_version"] == "msae_v3_gen5_m3_completion_v1"
    ...
    assert {entry.name for entry in os.scandir(prov)} == {
        "failed_gen4.json", "failed_gen3.json", "failed_gen2.json",
        "setup_manifest.json"}
