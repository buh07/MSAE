...
TEXT=(ROOT/"scripts/launch_trained_control_next_studies_v1_tmux.sh").read_text()
...
def test_one_gpu_failure_cleans_reserved_global_lock(tmp_path):
    ...
    proc=subprocess.run(['bash',str(ROOT/'scripts/launch_trained_control_next_studies_v1_tmux.sh')],env=env,capture_output=True,text=True)
...
case "$cmd" in
 ...
 new-session)
  ...
  if [[ "$count" == 2 && ( "$mode" == success || "$mode" == pid_remap ) ]]; then
   for spec in "trained_copy_sae_capacity_v1_run_20260813:$state/trained_copy_sae_capacity_v1_20260813.pid" "causal_manifold_bridge_v1_run_20260813:$state/causal_manifold_bridge_v1_20260813.pid"; do
 ...
 *query-compute-apps*)
  if [[ ( "$MOCK_MODE" == success || "$MOCK_MODE" == pid_remap ) && -f "$MOCK_STATE/trained_copy_sae_capacity_v1_20260813.pid" && -f "$MOCK_STATE/causal_manifold_bridge_v1_20260813.pid" ]]; then
   a=$(cat "$MOCK_STATE/trained_copy_sae_capacity_v1_20260813.pid");b=$(cat "$MOCK_STATE/causal_manifold_bridge_v1_20260813.pid")
...
def test_success_and_pid_remap_proof(tmp_path):
    _clear_mock_locks();root,state,env=_mock_runtime(tmp_path/'success','success');proc=subprocess.run(['bash',str(ROOT/'scripts/launch_trained_control_next_studies_v1_tmux.sh')],env=env,capture_output=True,text=True)
    assert proc.returncode==0 and 'SESSION=trained_copy_sae_capacity_v1_20260813' in proc.stdout
    for pf in state.glob('*.pid'):
        subprocess.run(['kill',pf.read_text().strip()],check=False)
    _clear_mock_locks()
    root,state,env=_mock_runtime(tmp_path/'remap','pid_remap');proc=subprocess.run(['bash',str(ROOT/'scripts/launch_trained_control_next_studies_v1_tmux.sh')],env=env,capture_output=True,text=True)
...
def test_signal_after_handoff_cleans_workers_and_locks(tmp_path):
    ...
    proc=subprocess.Popen(['bash',str(ROOT/'scripts/launch_trained_control_next_studies_v1_tmux.sh')],env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
