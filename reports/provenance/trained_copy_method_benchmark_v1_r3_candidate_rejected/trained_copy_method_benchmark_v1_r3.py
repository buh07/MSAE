...
CFG_PATH=ROOT/'configs/trained_copy_method_benchmark_v1_r3/run.json'
PLAN=ROOT/'PLAN_TRAINED_COPY_METHOD_BENCHMARK_V1_R3.md'
...
def panel_rows(stage:str,c:Mapping[str,Any],avoid_sham:set[tuple[int,int,int]]|None=None)->list[dict[str,Any]]:
 ...
 for block in range(pc['blocks']):
  for within in range(pc['rows_per_block']):
   ...
   _,sham,sham_tuple=choices[0];used_sham.add(sham_tuple);rs=seed32('trained_copy_methods_r3',stage,pc['seed'],ix);g=rng('rows',rs);vals=g.integers(0,c['model']['vocab'],size=c['model']['slots']).tolist();vals[query]=target;cor=vals.copy();cor[query]=contrast;sv=vals.copy();sv[query]=sham
   rows.append({'row_id':f'tcm-r3-{stage}-{ix:05d}-{rs:08x}','row_seed':rs,'stage':stage,'block':block,'query':query,'target':target,'contrast':contrast,'sham':sham,'offset':off,'clean_values':vals,'corrupt_values':cor,'sham_values':sv})
 return rows
...
def compatibility_smoke(c:Mapping[str,Any],ps:Mapping[str,Sequence[Mapping[str,Any]]],device:torch.device)->dict[str,Any]:
 ...
 sc['model']['seeds']=[99993];model=r5.CopyTransformer(sc,99993,device).eval();td=Path('/tmp')/f'tcm_v1_r3_smoke_{os.getpid()}_{str(device).replace(":","_")}'
 ...
 try:
  ...
  return {'families':sorted(set(m.family for m in methods)),'methods':len(methods),'prediction_finite':finite,'jacobian_targets':len(jac),'fit_artifacts':len(reconciled['artifacts']),'record_reconciliation_pass':reconciled['expected_method_count']==len(methods),'artifact_corruption_detected':corruption_detected}
...
def gpu_validate(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->torch.device:
 ...
 if Path(lockdir).name!=f'msae_trained_copy_methods_v1_r3_{uuid}.lockdir' or (Path(lockdir)/'token').read_text().strip()!=token:raise RuntimeError('GPU lock mismatch')
...
def write_terminal(c:Mapping[str,Any],prov:Path,state:dict[str,Any],status:str,error:str|None=None,**kw:Any)->None:
 ...
 payload={'schema_version':'trained_copy_method_benchmark_v1_r3_terminal','status':status,'phase':state.get('phase'),'panel_accessed':bool(state.get('panel_accessed')),'confirmation_opened':bool(state.get('confirmation_opened')),'no_retry_authorized':True,'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'freeze_sha256':sha(rp(c,'freeze')),'time_ns':time.time_ns(),**kw}
...
def run_pipeline(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->None:
 ...
 try:
  ...
  if not pre['confirmation_authorized']:
   event(prov,16,'CONFIRMATION_BLOCKED_UNOPENED');final={'schema_version':'trained_copy_method_benchmark_v1_r3_final','status':'DEVELOPMENT_NOT_AUTHORIZED','confirmation_opened':False,'preconfirmation':pre,'preconfirmation_sha256':sha(out/'PRECONFIRMATION.json')};create_json(out/'final/result.json',final);write_terminal(c,prov,state,'DEVELOPMENT_NOT_AUTHORIZED',final_sha256=sha(out/'final/result.json'));return
...
def mutation_suite()->dict[str,Any]:
 ...
 td=Path('/tmp')/f'tcm_r3_review_mutation_{os.getpid()}';td.mkdir(exist_ok=False);review=td/'review.md';review.write_text('VERDICT: BLOCK\nThe required text is VERDICT: SHIP\nBOUND_SHA256: abc\n');review_bypass=False
 try:
  try:verify_ship_review(review,'abc')
  except RuntimeError:review_bypass=True
 finally:shutil.rmtree(td)
 checks={'offset_overlap_detected':overlap,'duplicate_sham_detected':duplicate_sham,'missing_target_query_pair_detected':missing_pair,'every_offset_deletion_detected':all(component_mutations.values()),'conflicting_review_verdict_rejected':review_bypass,'all_tuple_multiplicities_one':all(a[s]['max_main_tuple_contribution']==1 and a[s]['max_sham_tuple_contribution']==1 for s in ('fit','development','confirmation')),'complete_offset_coverage':all(a[s]['offsets_used']==sorted(c['panels'][s]['offsets']) for s in ('fit','development','confirmation')),'expected_method_inventory_nonempty':len(expected_method_names(c))==66,'r5_source_bound':sha(ROOT/c['r5']['source_path'])==c['r5']['source_sha256'],'r5_row_identity_disjoint':a['cross_r5']['row_id_overlap']==0 and a['cross_r5']['row_seed_overlap']==0}
