...
If experiments continue, the representational object changes to a canonical attention-edge or
induction/copying circuit with an externally fixed token generator and a prespecified causal head.
Single-model circuit reproduction is treated only as technical pipeline validation; any general
method claim still requires prospective replication in at least two model families.
