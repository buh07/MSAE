#!/usr/bin/env python3
"""Label/text-only builder for relational-objects v2 development."""
...
from relational_objects_v2 import ROOT, atomic_json, atomic_jsonl, canonical_bytes, load_json, sha256_file, stable_hex
...
def build(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    output_root = ROOT / config["paths"]["prepared_root"]
    ...
    for source, specification in sorted(config["sources"].items()):
        paths = [ROOT / specification["root"] / name for name in specification["analysis_files"]]
        for path in paths:
            if sha256_file(path) != specification["files"][path.name]:
                raise RuntimeError(f"source drift: {path}")
        sentences, alignment = align_sentences(load_sentences(source, paths), tokenizer, config["model"]["max_sequence_length"])
        sentence_map = {sentence.key: sentence for sentence in sentences}
        pool = enumerate_by_document(sentences, config["matching"]["morph_keys"], config["matching"]["morph_value_vocabulary"], config["matching"]["candidate_cap_per_stratum"])
        components = match_components(source, pool, config["matching"]["max_pairs_per_orientation"], config["matching"]["maximum_components"], config["matching"]["target_pairs"])
        examples, pairs, units = flatten(source, components, sentence_map)
        source_root = output_root / source
        files = {}
        for name, rows in (("examples.jsonl", examples), ("pairs.jsonl", pairs), ("inference_units.jsonl", units), ("components.jsonl", components)):
            digest, count = atomic_jsonl(source_root / name, rows)
            files[name] = {"sha256": digest, "rows": count}
        orientations = Counter(pair["orientation"] for pair in pairs)
        support = {
            "source": source, "alignment": alignment, "documents": len({doc for component in components for doc in component["documents"]}),
            "components": len(components), "pairs": len(pairs), "examples": len(examples), "orientations": dict(orientations),
            "fold_components": dict(Counter(str(component["fold"]) for component in components)),
            "eligible": len(components) >= config["matching"]["minimum_components"] and len(pairs) >= config["matching"]["minimum_pairs"] and all(sum(pair["orientation"] == orientation for pair in pairs) >= 20 for orientation in ("later_query_is_head", "later_query_is_child")),
        }
        files["support.json"] = {"sha256": atomic_json(source_root / "support.json", support)}
        records[source] = {"support": support, "files": files}
    manifest = {
        "schema_version": "relational_objects_v2_prepared_v1", "namespace": config["namespace"],
        "config_sha256": sha256_file(ROOT / "configs/relational_objects_v2/run.json"),
        "parser_validation_sha256": config["parser_validation"]["sha256"], "sources": records,
        "tokenizer": {"name": config["model"]["name"], "revision": config["model"]["revision"]},
        "model_forward_run": False, "neural_training_run": False,
    }
    manifest["eligible"] = all(record["support"]["eligible"] for record in records.values())
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs/relational_objects_v2/run.json")
    args = parser.parse_args()
    print(json.dumps(build(load_json(args.config)), indent=2, sort_keys=True))
