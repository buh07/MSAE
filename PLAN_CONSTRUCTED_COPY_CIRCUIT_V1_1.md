# Constructed Copy Circuit v1.1 — Ground-Truth End-to-End Control

## Goal

**Successor rationale.** The v1 candidate was retired before scientific model inference because adversarial review found that its prefreeze preflight parsed the development panel. V1.1 uses newly generated panel seeds and treats both scientific panel paths as metadata-only until their registered post-freeze opening transitions. The v1 protocol lock, panels, and candidate artifacts remain unchanged and are not scientific results.
...
`m_i(z_cl)-m_i(z_co) > 6.0`. At creation time, before the lock exists, the generator writes a
non-model attestation that its analytic construction implies these values for all 256 rows in each
panel. No post-lock preopening consumer may parse or regenerate either panel; actual row-level
behavioral eligibility is computed only after the corresponding durable opening transition. Define
...
2. Prepare development/confirmation rows and calibration fixtures without running the model, then
   write a create-once `PROTOCOL_LOCK.json` binding this plan, config, thresholds, equations,
   creation-time panel sizes/digests and generator attestations, calibration expectations, and
   lifecycle schema. After creation, every preopening consumer—including preflight, freeze
   inventory, both reviews, review binding, and prelaunch verification—treats both panel paths only
   as opaque metadata from this lock and may not resolve, stat, hash, parse, or regenerate them.
   Symmetric access-trap tests cover all those consumers.
...
6. Before development access, write a create-once opening journal with `PRECHECK_JOURNALED`,
   `PHASE1_PASS_NO_ACCESS`, `ACCESS_MAY_HAVE_OCCURRED`, and either `CLOSED_AUTHORIZED` or
   `CLOSED_TECHNICAL_INVALID`. Only after the durable possible-access event may the runtime stat/hash
   development against frozen metadata. A crash at or after that event seals the namespace and
   prohibits retry.
7. Run two independent exact GPU passes for development, compare all registered tensors bitwise,
...
8. If development fails, write a permanent stop and a confirmation-block artifact without touching
   the payload. If it passes, authorize payload access by its frozen hash, run confirmation once with
   the same QA, and finalize.

Development opening has a two-phase create-once journal symmetric with confirmation, except that its
Phase 1 needs no scientific gate: it validates freeze, review binding, namespace, launch manifest,
and fresh-seed provenance without touching either panel, then durably records possible access before
checking development's frozen size/digest. Development-gate publication has a locked create-once journal with `INITIALIZED`,
`DEVELOPMENT_VALIDATED`, and `CLOSED_PASS`, `CLOSED_STOP`, or `CLOSED_TECHNICAL_INVALID`. Confirmation
authorization has its own locked create-once journal. It records `PRECHECK_JOURNALED`, performs
...
- Bind the retired v1 config hash and assert that both v1.1 scientific seeds differ from both retired
  v1 scientific seeds. This freshness check reads configuration metadata only, never either panel.
...
- `python -m pytest -q tests/test_constructed_copy_circuit_v1_1.py`
- `python -m py_compile scripts/constructed_copy_circuit_v1_1.py`
- `bash -n scripts/launch_constructed_copy_circuit_v1_1_tmux.sh`
- candidate preflight/calibration/smoke commands defined by the new runner
- `/adversarial PLAN_CONSTRUCTED_COPY_CIRCUIT_V1_1.md`
