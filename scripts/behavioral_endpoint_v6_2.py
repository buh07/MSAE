#!/usr/bin/env python3
"""Technical execution repair for endpoint-only behavioral study v6.2."""
...
ROOT=Path(__file__).resolve().parents[1];SCRIPT=Path(__file__).resolve();DEFAULT=ROOT/'configs/behavioral_endpoint_v6_2/run.json';PLAN=ROOT/'PLAN_BEHAVIORAL_ENDPOINT_V6_2.md';TEST=ROOT/'tests/test_behavioral_endpoint_v6_2.py';LAUNCHER=ROOT/'scripts/launch_behavioral_endpoint_v6_2_tmux.sh'
...
def verify_failure_tree_membership(failed:Mapping[str,Any])->None:
 roots=[ROOT/x for x in failed.get('tree_roots',[])]
 expected={x['path'] for x in failed.get('files',[]) if any(x['path']==r.relative_to(ROOT).as_posix() or x['path'].startswith(r.relative_to(ROOT).as_posix()+'/') for r in roots)}
 actual={p.relative_to(ROOT).as_posix() for r in roots for p in r.rglob('*') if p.is_file()}
 if not roots or actual!=expected:
  raise RuntimeError(f"v6.1 preserved tree membership drift added={sorted(actual-expected)} missing={sorted(expected-actual)}")
 counts={
  'development_worker_failures':sum('/development/' in p and p.endswith('_FAILED.json') for p in actual),
  'confirmation_blocks':sum('/confirmation/' in p and p.endswith('/BLOCKED.json') for p in actual),
  'development_gate_failures':sum('/development_gate/' in p and p.endswith('/FAILED.json') for p in actual),
  'final_failures':sum('/final/FAILED.json' in p for p in actual),
 }
 if counts!=failed.get('expected_terminal_counts'):raise RuntimeError(f'v6.1 terminal-count drift {counts}')
def recovery_verify(config:Path,cfg:Mapping[str,Any])->dict[str,Any]:
 r=cfg.get('recovery',{})
 if r.get('type')!='TECHNICAL_EXECUTION_REPAIR_NO_SCIENTIFIC_CHANGE' or r.get('new_model_forwards_after_v6_1_terminal') is not False or r.get('scientific_protocol_changed') is not False:raise RuntimeError('recovery contract drift')
 if set(r.get('permitted_code_repairs',[]))!={'condition_logits','merge_condition_batches','final_aggregate'}:raise RuntimeError('repair scope drift')
 old_freeze=ROOT/r['failed_v6_1_freeze'];old_review=ROOT/r['failed_v6_1_frozen_review'];binding=ROOT/r['failed_v6_1_review_binding'];failure=ROOT/r['failed_v6_1_preservation']
 checks=[(old_freeze,r['failed_v6_1_freeze_sha256']),(old_review,r['failed_v6_1_frozen_review_sha256']),(binding,r['failed_v6_1_review_binding_sha256']),(failure,r['failed_v6_1_preservation_sha256'])]
 if any(sha(p)!=d for p,d in checks):raise RuntimeError('failed-v6.1 provenance drift')
 if old_review.read_text().splitlines()[0]!='VERDICT: SHIP':raise RuntimeError('v6.1 frozen review drift')
 b=loadj(binding)
 if b.get('status')!='SHIP' or b.get('freeze_sha256')!=sha(old_freeze) or b.get('review_sha256')!=sha(old_review):raise RuntimeError('v6.1 review binding drift')
 prior=loadj(old_freeze)
 for rec in prior['candidate_inventory']:
  p=ROOT/rec['path']
  if not p.is_file() or p.stat().st_size!=rec['bytes'] or sha(p)!=rec['sha256']:raise RuntimeError(f"failed-v6.1 inventory drift: {rec['path']}")
 failed=loadj(failure)
 if failed.get('status')!='PRESERVED_TECHNICAL_FAILURE' or failed.get('formal_scientific_result') is not False or failed.get('failure')!='BATCH_ACCUMULATION_INDEX_ERROR_BEFORE_ANY_METRIC_ROW' or failed.get('file_count')!=len(failed.get('files',[])):raise RuntimeError('v6.1 failure classification drift')
 verify_failure_tree_membership(failed)
 for rec in failed['files']:
  p=ROOT/rec['path']
  if not p.is_file() or p.stat().st_size!=rec['bytes'] or sha(p)!=rec['sha256']:raise RuntimeError(f"v6.1 terminal drift: {rec['path']}")
 workers=[ROOT/x['path'] for x in failed['files'] if x['path'].endswith('_FAILED.json') and '/development/' in x['path']]
 if len(workers)!=6 or any(loadj(p).get('error_type')!='IndexError' or 'out of bounds for axis 0' not in loadj(p).get('error','') for p in workers):raise RuntimeError('v6.1 worker failure signature drift')
 if any('/metrics.jsonl' in x['path'] or '/COMPLETE.json' in x['path'] for x in failed['files']):raise RuntimeError('v6.1 produced scientific rows unexpectedly')
 # Scientific configuration is identical after removing only recovery and versioned paths.
 old_config=loadj(ROOT/'configs/behavioral_endpoint_v6_1/run.json');new_config=json.loads(json.dumps(cfg))
 ...
 old_config.pop('recovery',None);new_config.pop('recovery',None)
 ...
 if old_config!=new_config:raise RuntimeError('scientific config changed from failed v6.1')
 # All non-lifecycle functions except the two registered execution repairs remain identical.
 excluded={'verify_failure_tree_membership','recovery_verify','local_dependency_closure','candidate_files','inventory','preflight','freeze','verify_freeze','main','condition_logits','merge_condition_batches','final_aggregate'}
 ...
 if functions(ROOT/'scripts/behavioral_endpoint_v6_1.py')!=functions(SCRIPT):raise RuntimeError('unregistered implementation change from failed v6.1')
 return prior
...
def candidate_files(config:Path,cfg:Mapping[str,Any])->list[Path]:
 recovery_verify(config,cfg);p=ROOT/cfg['runtime']['prepared_root'];old=loadj(ROOT/cfg['recovery']['failed_v6_1_freeze']);failure=loadj(ROOT/cfg['recovery']['failed_v6_1_preservation'])
 paths=[ROOT/x['path'] for x in old['candidate_inventory']]
 paths += [ROOT/x['path'] for x in failure['files']]
 paths += [ROOT/cfg['recovery']['failed_v6_1_freeze'],ROOT/cfg['recovery']['failed_v6_1_frozen_review'],ROOT/cfg['recovery']['failed_v6_1_review_binding'],ROOT/cfg['recovery']['failed_v6_1_preservation'],config.resolve(),PLAN,SCRIPT,TEST,LAUNCHER,ROOT/cfg['runtime']['cache_attestation'],ROOT/'reports/adversarial/behavioral_endpoint_v6_2_candidate_review_block1.md']
 deps=local_dependency_closure([SCRIPT,ROOT/'scripts/behavioral_endpoint_v6_1.py',ROOT/'scripts/joint_controllability_benchmark_v3.py',ROOT/'scripts/joint_controllability_assay_v5.py'])
 paths += deps
 expected={ROOT/x for x in cfg['recovery']['required_dependency_closure']}
...
def freeze(config:Path)->None:
 cfg=loadj(config);preflight(config);inv=inventory(config,cfg);x={'schema_version':'behavioral_endpoint_v6_2_freeze','namespace':cfg['namespace'],'config_sha256':sha(config),'candidate_inventory':inv,'candidate_inventory_sha256':hashlib.sha256(canon(inv)).hexdigest(),'v5_preservation_sha256':sha(ROOT/cfg['preservation']['v5_manifest']),'failed_v6_1_freeze_sha256':cfg['recovery']['failed_v6_1_freeze_sha256'],'failed_v6_1_preservation_sha256':cfg['recovery']['failed_v6_1_preservation_sha256'],'technical_execution_repair':True,'scientific_protocol_changed':False,'endpoint_only':True,'training':False,'methods':False,'model_specific_confirmation':True,'two_family_barrier':True,'one_shot':True,'retry_authorized':False};exjson(ROOT/cfg['runtime']['freeze'],x);print(json.dumps(x,indent=2))
...
def merge_condition_batches(parts:Sequence[tuple[Sequence[int],np.ndarray]],total:int)->np.ndarray:
 order=[int(i) for indices,_ in parts for i in indices]
 if order!=list(range(total)):raise RuntimeError(f'condition logit order/coverage mismatch {order[:8]}.../{total}')
 if any(np.asarray(values).shape[0]!=len(indices) for indices,values in parts):raise RuntimeError('batch/logit row mismatch')
 merged=np.concatenate([np.asarray(values) for _,values in parts],axis=0)
 if merged.shape[0]!=total:raise RuntimeError(f'condition logit coverage mismatch {merged.shape[0]}/{total}')
 return merged
def condition_logits(model:Any,rows:list[dict[str,Any]],kind:str,batch:int,pad:int,device:torch.device)->np.ndarray:
 if not rows or batch<1:raise ValueError('nonempty rows and positive batch required')
 out=[]
 for st in range(0,len(rows),batch):
  ...
  selected=z[ar,bounds].cpu().numpy()
  out.append((range(st,st+len(rr)),selected))
 return merge_condition_batches(out,len(rows))
...
def final_aggregate(config:Path)->None:
 ...
 try:
  ...
  for e in cfg['endpoints']:
   gate=root/'development_gate'/e
   if (gate/'TIMEOUT.json').exists():raise TimeoutError(f'development gate timeout {e}')
   if (gate/'FAILED.json').exists():raise RuntimeError(f'development gate failed {e}')
   if not (gate/'result.json').is_file():raise RuntimeError(f'development gate terminal missing {e}')
