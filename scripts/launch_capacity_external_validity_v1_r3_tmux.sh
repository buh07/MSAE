...
SESSION="capacity_external_validity_v1_20260810r3"
CAP_OUT="$ROOT/results/capacity_controller_diagnostic_v1_20260810r3"
EXT_OUT="$ROOT/results/trained_copy_external_v1_20260810r3"
CAP_PROV="$ROOT/reports/provenance/capacity_controller_diagnostic_v1_r3_run_20260810r3"
EXT_PROV="$ROOT/reports/provenance/trained_copy_external_v1_r3_run_20260810r3"
SCRIPT="$ROOT/scripts/capacity_external_validity_v1_r3.py"
LOG="$ROOT/reports/provenance/capacity_external_validity_v1_r3_launcher_20260810r3.log"
render_cmd() {
...
set +e
timeout --signal=TERM --kill-after=30s 8h env MSAE_ROOT="\$RUN_ROOT" python "\$RUN_ROOT/scripts/capacity_external_validity_v1_r3.py" run-pipeline --physical-index "$chosen_index" --gpu-uuid "$chosen_uuid" --launch-token "$token" --pane-pid "\$\$" --gpu-lockdir "$lockdir" 2>&1 | tee -a "$LOG"
rc=\${PIPESTATUS[0]}
set -e
exit \$rc
EOF
...
if compgen -G '/tmp/msae_capacity_external_r3_*.lockdir' >/dev/null; then echo 'preexisting r3 GPU lock namespace' >&2; exit 1; fi
...
for row in "${GPUS[@]}"; do
 ...
 candidate="/tmp/msae_capacity_external_r3_${u}.lockdir"
