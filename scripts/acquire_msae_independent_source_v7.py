#!/usr/bin/env python3
"""Reviewed, one-shot upstream acquisition runner for MSAE v7."""
...
CONFIG=ROOT/"configs/msae_independent_source_v7/acquisition.json"
...
FILES=["fi_tdt-ud-train.conllu","fi_tdt-ud-dev.conllu","fi_tdt-ud-test.conllu","README.md","LICENSE.txt"]
...
V7_AUTHORITY={
    ...
    "scripts/prepare_msae_independent_source_v7.py","scripts/acquire_msae_independent_source_v7.py",
    "tests/test_prepare_msae_independent_source_v7.py","configs/msae_independent_source_v7/acquisition.json",
...
V7_ACQUISITION_STATE={
    ...
    "reports/provenance/msae_independent_source_v7/source_acquisition_rejection.json",
}
...
def expected_config()->dict[str,Any]:
    return {
        "schema_version":"msae_independent_source_v7_acquisition_v1",
...
def validate_entry(value:dict[str,Any],manifest_sha:str,review_sha:str,baseline_sha:str)->None:
    ...
    if (value.get("schema_version")!="msae_independent_source_v7_source_acquisition_entry_v1"
...
def validate_command_records(commands:Any,expected_names:list[str],config:dict[str,Any],
    ...
    if not isinstance(commands,list) or [item.get("name") if isinstance(item,dict) else None for item in commands]!=expected_names:
        raise AcquisitionFailure("invalid_preacquisition_state")
    expected_argv=[
        [*expand(config["ignore_argv"],clone_dir,empty_home)[:-1],"data/msae_independent_source_v7/raw/sentinel"],
        [*expand(config["ignore_argv"],clone_dir,empty_home)[:-1],"data/msae_independent_source_v7/private/sentinel"],
...
def validate_existing_raw(report:dict[str,Any])->None:
    fds=[]
    try:
        v7fd=open_root_dir(RAW.parent.parent);fds.append(v7fd)
        rawrootfd=os.open("raw",os.O_RDONLY|os.O_DIRECTORY|os.O_NOFOLLOW,dir_fd=v7fd);fds.append(rawrootfd)
        ...
        if sorted(os.listdir(v7fd))!=["raw"] or sorted(os.listdir(rawrootfd))!=[COMMIT] or sorted(os.listdir(commitfd))!=sorted(FILES):
            raise AcquisitionFailure("invalid_preacquisition_state")
...
def validate_success_report(report:dict[str,Any],entry_sha:str,manifest_sha:str,review_sha:str,
    ...
    if (not clone_path.is_absolute() or clone_path.name!="repo" or home_path.name!="home"
            or clone_path.parent!=home_path.parent or not clone_path.parent.name.startswith("msae-v7-acquire-")):
        raise AcquisitionFailure("invalid_preacquisition_state")
    ...
    if (report["schema_version"]!="msae_independent_source_v7_source_acquisition_v1"
    ...
    for index,(name,path) in enumerate((("ignore_raw","data/msae_independent_source_v7/raw/sentinel"),
                                        ("ignore_private","data/msae_independent_source_v7/private/sentinel"))):
...
def validate_rejection_report(rejection:dict[str,Any],entry_sha:str,manifest_sha:str,review_sha:str,
    ...
    if commands:
        ...
        if clone_record:
            ...
            clone_dir=str(Path("/tmp/msae-v7-acquire-placeholder/repo"));empty_home=str(Path(clone_dir).parent/"home")
        ...
        if clone_record:
            ...
            sentinels=("data/msae_independent_source_v7/raw/sentinel",
                       "data/msae_independent_source_v7/private/sentinel")
            for index,item in enumerate(commands):
                if item["argv"]!=[*config["ignore_argv"][:-1],sentinels[index]]:
                    raise AcquisitionFailure("invalid_preacquisition_state")
    if (rejection["schema_version"]!="msae_independent_source_v7_source_acquisition_rejection_v1"
...
def create_raw_namespace()->tuple[list[int],os.stat_result]:
    """Create and pin data/v7/raw/commit without path-following writes."""
    ...
    try:
        ...
        fds.append(datafd)
        current=datafd
        for name,mode in (("msae_independent_source_v7",0o700),("raw",0o700),(COMMIT,0o700)):
...
def walk_history_paths():
    for base,dirs,files in os.walk(ROOT,followlinks=False):
        ...
        for name in sorted(dirs):
            ...
            if name=="__pycache__" or any((rel+"/").startswith(item) for item in GENERATED_PREFIXES) or rel.startswith(V7_PREFIX.rstrip("/")):
                continue
...
def validate_baseline_history_binding()->str:
    ...
    if (registry.get("schema_version")!="msae_independent_source_v7_historical_source_registry_v1"
            or registry.get("status")!="frozen_preacquisition" or registry.get("domain_utf8")!="msae-v7/pedigree"
    ...
    dispositions={"text_scanned","archive_scanned","binary_unscanned","v7_authority","quarantine"}
    ...
    for item in entries:
        ...
        if (not required.issubset(item) or disposition not in dispositions
                ...
                or (item["path"] in V7_AUTHORITY)!=(disposition=="v7_authority")
    ...
    if (not isinstance(expected_absent,list) or expected_absent!=sorted(set(expected_absent))
            or set(expected_absent)!=(V7_AUTHORITY-baseline_paths)
            or not V7_POST_BASELINE.issubset(expected_absent)):
        raise AcquisitionFailure("history_baseline_schema")
    current={rel for _path,rel in walk_history_paths()}
    allowed=set(expected_absent)
    state_paths={rel for rel in V7_ACQUISITION_STATE if present(ROOT/rel)}
    if ("reports/provenance/msae_independent_source_v7/source_acquisition_entry.json" not in state_paths
            and state_paths):
        raise AcquisitionFailure("history_structural_drift")
    outcomes=state_paths-{"reports/provenance/msae_independent_source_v7/source_acquisition_entry.json"}
    if len(outcomes)>1:
        raise AcquisitionFailure("history_structural_drift")
...
def main()->None:
    ...
    if (len(args.authority_review_sha256)!=64 or args.authority_review_sha256!=review_sha
            ...
            or authority.get("artifact_sha256",{}).get("scripts/acquire_msae_independent_source_v7.py")!=sha_file(Path(__file__))
            or authority.get("artifact_sha256",{}).get("configs/msae_independent_source_v7/acquisition.json")!=sha_file(CONFIG)):
    ...
    scratch=Path(tempfile.mkdtemp(prefix="msae-v7-acquire-",dir="/tmp"));SCRATCH_CREATED=True;SCRATCH_CLEANUP_OK=False
    ...
    try:
        ...
        evidence={}
        ignore_outputs=[]
        for sentinel in ("data/msae_independent_source_v7/raw/sentinel","data/msae_independent_source_v7/private/sentinel"):
        ...
        report={"schema_version":"msae_independent_source_v7_source_acquisition_v1","source_repo":REPO,"source_commit":COMMIT,
...
def retain_failure(code:str)->None:
    ...
    value={"schema_version":"msae_independent_source_v7_source_acquisition_rejection_v1","status":"rejected",
