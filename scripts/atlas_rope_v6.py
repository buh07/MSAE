#!/usr/bin/env python3
"""Atlas v3.6 attempt-10 technical measurement primitives.
...
RUN_ROOT = ROOT / "pilot_runs/20260803_atlas_rope_technical_v6"
...
ATTEMPT8_RETIREMENT_SCHEMA = "atlas_rope_v6_attempt10_attempt8_retirement_v1"
PRESCORE_SCHEMA = "atlas_rope_v6_attempt10_prescore_v1"
PANEL_SCHEMA = "atlas_rope_v6_attempt10_panel_v1"
AUTH_SCHEMA = "atlas_rope_v6_attempt10_stage_authorization_v1"
BUNDLE_SCHEMA = "atlas_rope_v6_attempt10_technical_bundle_v1"
FREEZE_SCHEMA = "atlas_rope_v6_attempt10_validation_freeze_v1"
TERMINAL_SCHEMA = "atlas_rope_v6_attempt10_terminal_v1"
...
REL_L2_GRID: tuple[float, ...] = (2e-6, 5e-6, 1e-5, 2e-5)
...
def _write_signing_fault(path: Path, *, temporary: Path, reason: str) -> None:
    ...
    value = {
        "schema_version": "atlas_rope_v6_attempt10_signing_fault_v1",
...
def write_signed(path: Path, payload: Mapping[str, Any], signing_key: Path, *, create_once: bool = True) -> str:
    ...
    """
    if not create_once:
        raise RuntimeError("attempt-10 signed artifacts are always create-once")
    if path.exists():
...
def exclusive_lock(name: str) -> Iterator[None]:
    ...
    try:
        try:
            ...
            raise RuntimeError(f"another attempt-10 process holds lock {name}") from error
        yield
    finally:
...
def make_grid_units(source: str, bases: Sequence[Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for base in bases:
        ...
        base_id = f"atlas_rope_v6:{source}:base:{stable_utf8_sha256(source, base['document_id'], base['sent_id'], base['sequence_sha256'])[:24]}"
