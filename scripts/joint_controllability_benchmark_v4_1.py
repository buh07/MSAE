...
DEFAULT = ROOT / "configs/joint_controllability_benchmark_v4_1/run.json"
PLAN = ROOT / "PLAN_JOINT_CONTROLLABILITY_V4_1_LAUNCH_RECOVERY.md"
TEST = ROOT / "tests/test_joint_controllability_benchmark_v4_1.py"
LAUNCHER = ROOT / "scripts/launch_joint_controllability_benchmark_v4_1_tmux.sh"
CANDIDATE_REVIEW = ROOT / "reports/adversarial/joint_controllability_v4_1_candidate_review.md"
PREDECESSOR_TREE_SHA256 = "7fd172c0e169e9ae00cdc88d59b0b59419c831c501e71dece06f83c3dfe3e435"
...
def candidate_files(config: Path, cfg: Mapping[str, Any]) -> list[Path]:
    ...
    paths = [config.resolve(), PLAN, SCRIPT, TEST, LAUNCHER, ROOT / cfg["runtime"]["asset_manifest"], ROOT / cfg["runtime"]["cache_attestation"], ROOT / cfg["preservation"]["closure"], ROOT / cfg["preservation"]["v3_source_freeze"], ROOT / cfg["preservation"]["v3_recovery"], ROOT / cfg["preservation"]["v3_claim_review"], ROOT / cfg["preservation"]["predecessor_freeze"], ROOT / cfg["preservation"]["predecessor_failure"], ROOT / cfg["preservation"]["predecessor_aggregate"], ROOT / cfg["preservation"]["predecessor_launch_manifest"], ROOT / cfg["preservation"]["predecessor_error_log"], ROOT / cfg["preservation"]["predecessor_tree_manifest"], ROOT / "reports/provenance/joint_controllability_benchmark_v4/deterministic_prepare.json", prepared / "PRESCORE.json", prepared / "rows.jsonl"]
...
def exact_tree(root_path: Path) -> list[dict[str, Any]]:
    return [{"path": p.relative_to(ROOT).as_posix(), "bytes": p.stat().st_size, "sha256": sha(p)} for p in sorted(root_path.rglob("*")) if p.is_file()]


def validate_failure_semantics(freeze_sha: str, task_failures: Mapping[str, Mapping[str, Any]], gate: Mapping[str, Any], aggregate_record: Mapping[str, Any], synthetic_record: Mapping[str, Any], shard_failures: Sequence[Mapping[str, Any]]) -> None:
    cache_message = "couldn't find them in the cached files"
    if set(task_failures) != {"gpt2", "pythia160", "gemma2"} or any(r.get("status") != "FAIL" or r.get("error_type") != "OSError" or r.get("freeze_sha256") != freeze_sha or cache_message not in str(r.get("error", "")) for r in task_failures.values()):
        raise RuntimeError("predecessor task workers did not all fail at cache resolution")
    if gate.get("status") != "FAIL" or gate.get("reason") != "WORKER_FAILURE_OR_TIMEOUT" or gate.get("freeze_sha256") != freeze_sha or set(gate.get("failed_workers", [])) != {"gpt2_FAIL.json", "pythia160_FAIL.json", "gemma2_FAIL.json"}:
        raise RuntimeError("predecessor global task gate lineage invalid")
    if aggregate_record != {"status": "BLOCKED_BY_UPSTREAM_GATE", "training_authorized": False}:
        raise RuntimeError("predecessor aggregate was not cleanly blocked")
    if synthetic_record.get("status") != "PASS" or synthetic_record.get("freeze_sha256") != freeze_sha:
        raise RuntimeError("predecessor synthetic lineage invalid")
    if len(shard_failures) != 7 or any(r.get("status") != "FAIL" or r.get("error_type") != "RuntimeError" or r.get("error") != "blocked by a frozen upstream gate" or r.get("freeze_sha256") != freeze_sha for r in shard_failures):
        raise RuntimeError("predecessor fresh workers were not blocked before model load")


def verify_predecessor_failure(cfg: Mapping[str, Any]) -> None:
    manifest_path = ROOT / cfg["preservation"]["predecessor_tree_manifest"]
    if sha(manifest_path) != PREDECESSOR_TREE_SHA256:
        raise RuntimeError("predecessor tree manifest drift")
    manifest = loadj(manifest_path); actual = []
    for root_name in manifest["roots"]: actual.extend(exact_tree(ROOT / root_name))
    actual.sort(key=lambda row: row["path"]); expected = sorted(manifest["files"], key=lambda row: row["path"])
    if actual != expected:
        raise RuntimeError("predecessor result/provenance tree drift")
    predecessor_freeze_path = ROOT / cfg["preservation"]["predecessor_freeze"]; predecessor_freeze = loadj(predecessor_freeze_path); freeze_sha = sha(predecessor_freeze_path)
    predecessor_config = ROOT / "configs/joint_controllability_benchmark_v4/run.json"
    if predecessor_freeze["config_sha256"] != sha(predecessor_config):
        raise RuntimeError("predecessor config/freeze mismatch")
    for record in predecessor_freeze["candidate_inventory"]:
        path = ROOT / record["path"]
        if not path.is_file() or path.stat().st_size != record["bytes"] or sha(path) != record["sha256"]:
            raise RuntimeError(f"predecessor frozen candidate drift: {record['path']}")
    launch = loadj(ROOT / cfg["preservation"]["predecessor_launch_manifest"])
    if launch.get("freeze_sha256") != freeze_sha or launch.get("config_sha256") != sha(predecessor_config) or launch.get("new_training") is not False:
        raise RuntimeError("predecessor launch lineage mismatch")
    result_root = ROOT / "results/joint_controllability_benchmark_v4_20260808"
    forbidden = list(result_root.glob("task_gate/*/metrics.jsonl")) + list(result_root.glob("task_gate/*/COMPLETE.json")) + list(result_root.glob("shards/*/metrics.jsonl")) + list(result_root.glob("shards/*/HOOK_QA.json")) + list(result_root.glob("shards/*/COMPLETE.json"))
    if forbidden:
        raise RuntimeError("predecessor contains scientific forward artifacts")
    task_failures = {name: loadj(result_root / "task_gate" / f"{name}_FAIL.json") for name in ("gpt2", "pythia160", "gemma2")}
    shard_failures = [loadj(path) for path in sorted((result_root / "failures").glob("*_FAIL.json"))]
    validate_failure_semantics(freeze_sha, task_failures, loadj(result_root / "task_gate/FAIL.json"), loadj(result_root / "aggregate/BLOCKED.json"), loadj(result_root / "synthetic/PASS.json"), shard_failures)
    for name in ("gpt2", "pythia", "gemma"):
        text = (ROOT / f"reports/provenance/joint_controllability_benchmark_v4_run_20260808/task_{name}.log").read_text()
        if "from_pretrained" not in text or "LocalEntryNotFoundError" not in text:
            raise RuntimeError(f"predecessor traceback does not establish pre-forward cache failure: {name}")
...
def preflight(config: Path) -> None:
    ...
    old = loadj(ROOT / "configs/joint_controllability_benchmark_v4/run.json")
    for key in set(old) - {"schema_version", "namespace", "runtime", "preservation"}:
        if cfg.get(key) != old[key]: raise RuntimeError(f"launch recovery changed scientific field: {key}")
    authorized_runtime = {"freeze", "output_root", "provenance_root", "tmux_prefix", "model_cache_root", "cache_attestation"}
    for key in set(old["runtime"]) | set(cfg["runtime"]):
        if key not in authorized_runtime and cfg["runtime"].get(key) != old["runtime"].get(key): raise RuntimeError(f"launch recovery changed runtime field: {key}")
    if any(cfg["preservation"].get(key) != value for key, value in old["preservation"].items()) or not cfg["preservation"]["launch_recovery_only"] or cfg["preservation"]["scientific_protocol_changes"]:
        raise RuntimeError("launch recovery changed scientific protocol")
    verify_predecessor_failure(cfg)
...
def cached_weight_records(path: Path) -> list[dict[str, Any]]:
    index_paths = [path / "model.safetensors.index.json", path / "pytorch_model.bin.index.json"]
    index = next((p for p in index_paths if p.is_file()), None)
    if index is not None:
        required = sorted(set(loadj(index)["weight_map"].values()))
    else:
        required = [name for name in ("model.safetensors", "pytorch_model.bin") if (path / name).is_file()]
    if not required or any(not (path / name).is_file() or (path / name).stat().st_size <= 0 for name in required):
        raise RuntimeError("incomplete cached model weight set")
    return [{"name": name, "bytes": (path / name).stat().st_size, "resolved_blob": str((path / name).resolve())} for name in required]
...
def cache_preflight(config: Path) -> None:
    ...
    expected_cache = (Path(os.environ["HF_HOME"]) / "hub").resolve(); actual_cache = Path(os.environ.get("TRANSFORMERS_CACHE", "")).resolve()
    if actual_cache != expected_cache or str(cfg["runtime"]["model_cache_root"]) != "${HF_HOME}/hub":
        raise RuntimeError("model cache root does not resolve to $HF_HOME/hub")
    records = []
    for model in cfg["models"]:
        path = Path(snapshot_download(model["name"], revision=model["revision"], local_files_only=True))
        ...
        if not (path / "config.json").is_file():
            raise RuntimeError(f"incomplete cached model snapshot: {model['key']}")
        weights = cached_weight_records(path)
        records.append({"model": model["key"], "revision": model["revision"], "snapshot": str(path), "weight_files": weights})
    payload = {"status": "PASS", "model_cache_root": str(actual_cache), "models": records}
    attestation = ROOT / cfg["runtime"]["cache_attestation"]
    if attestation.exists():
        if loadj(attestation) != payload: raise RuntimeError("cache attestation drift")
    else: exjson(attestation, payload)
...
def freeze(config: Path) -> None:
    ...
    payload = {"schema_version": "joint_control_v4_1_freeze", "namespace": cfg["namespace"], "config_sha256": sha(config), "candidate_inventory": inv, "candidate_inventory_sha256": hashlib.sha256(json.dumps(inv, sort_keys=True, separators=(",", ":")).encode()).hexdigest(), "v3_preserved_freeze_sha256": sha(ROOT / cfg["preservation"]["v3_source_freeze"]), "predecessor_v4_freeze_sha256": sha(ROOT / cfg["preservation"]["predecessor_freeze"]), "launch_recovery_only": True, "scientific_protocol_changes": False, "new_k2_training": False, "new_sae_training": False, "task_gate_precedes_test_inference": True, "within_corpus_transfer": True, "retry_authorized": False}
...
def main() -> None:
    ...
    for name in ("prepare", "preflight", "cache-preflight", "freeze", "synthetic", "task-gate-aggregate", "aggregate"):
