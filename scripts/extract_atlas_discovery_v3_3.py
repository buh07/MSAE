#!/usr/bin/env python3
"""Trusted numerical QA and frozen activation extraction for Atlas v3.3 attempt 7.
...
from atlas_discovery_v3_3 import canonical_json_bytes, read_json, read_jsonl, sha256_file, stable_digest
...
QA_SCHEMA = "atlas_discovery_v3_3_attempt7_numerical_qa_v2"
CACHE_SCHEMA = "atlas_discovery_v3_3_attempt7_activation_cache_v2"
AUTH_SCHEMA = "atlas_discovery_v3_3_attempt7_full_extraction_authorization_v1"
...
def load_scoring_config(path: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if config.get("schema_version") != "atlas_discovery_v3_3_attempt7_scoring_v2":
    ...
    if any(preflight.get(key) is not False for key in ("representation_scoring_run", "neural_inference_run", "neural_training_run")):
        raise RuntimeError("prescore execution-role flags drift")
    if manifest.get("schema_version") != "atlas_discovery_v3_3_attempt7_prepared_v1":
    ...
    if (
        rebuild.get("schema_version") != "atlas_discovery_v3_3_attempt7_final_v3_rebuild_check_v1"
    ...
    if config.get("status") != "draft_no_inference_authorized":
        review_spec = config["implementation_adversarial_review"]
        review_path = (ROOT / _validate_relative_path(str(review_spec["path"]))).resolve(strict=True)
        if (
            sha256_file(review_path) != review_spec["sha256"]
            or review_spec.get("verdict") != "SHIP"
            ...
            raise RuntimeError("attempt7 implementation adversarial SHIP invalid")
    split_spec = config["population_split"]
    split_path = (ROOT / _validate_relative_path(str(split_spec["path"]))).resolve(strict=True)
    if sha256_file(split_path) != split_spec["sha256"]:
        raise RuntimeError("attempt7 population split drift")
    split = read_json(split_path)
    if split.get("schema_version") != "atlas_discovery_v3_3_attempt7_population_split_v3":
        raise RuntimeError("unknown attempt7 population split")
    for role in ("legacy", "fresh"):
        spec = config[f"{role}_qa_parent"]
        parent_config_path = (ROOT / _validate_relative_path(str(spec["config_path"]))).resolve(strict=True)
        parent_manifest_path = (ROOT / _validate_relative_path(str(spec["manifest_path"]))).resolve(strict=True)
        if sha256_file(parent_config_path) != spec["config_sha256"] or sha256_file(parent_manifest_path) != spec["manifest_sha256"]:
            raise RuntimeError(f"{role} QA parent lineage drift")
    terminal_spec = config["attempt5_terminal"]
    terminal_path = (ROOT / _validate_relative_path(str(terminal_spec["path"]))).resolve(strict=True)
    if sha256_file(terminal_path) != terminal_spec["sha256"] or read_json(terminal_path).get("payload", {}).get("status") != "TERMINAL_TECHNICALLY_INVALID":
        raise RuntimeError("attempt5 terminal lineage invalid")
    devices = config["extraction"]
    if len(set(devices["qa_devices"].values())) != 1 or len(set(devices["full_devices"].values())) != 1:
        raise RuntimeError("attempt7 sources must share one logical GPU")
    if len(set(devices["qa_gpu_uuids"].values())) != 1 or len(set(devices["full_gpu_uuids"].values())) != 1:
        raise RuntimeError("attempt7 sources must share one physical GPU")
...
def _load_model(prescore: Mapping[str, Any], device: torch.device) -> torch.nn.Module:
    ...
    if not floating or set(floating) != {torch.float32}:
        raise RuntimeError(f"attempt7 model is not entirely float32: {set(floating)}")
...
def _forward_units(model: torch.nn.Module, units: Sequence[Mapping[str, Any]], *, device: torch.device, hidden_index: int, batch_size: int) -> np.ndarray:
    ...
    for batch in _batches(units, batch_size):
        ...
        if hidden.dtype != torch.float32:
            raise RuntimeError(f"hidden state dtype drift: {hidden.dtype}")
        for index, row in enumerate(batch):
...
def _qa_panel_units(
    bundle: Mapping[str, Any], pair_ids: Sequence[str], prescore: Mapping[str, Any], *, panel: str
) -> tuple[list[Mapping[str, Any]], list[Mapping[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    by_id = {str(row["pair_id"]): row for row in bundle["pairs"]}
    if len(pair_ids) != len(set(pair_ids)) or any(pair_id not in by_id for pair_id in pair_ids):
        raise RuntimeError(f"{panel} QA pair identity drift")
    selected = [by_id[pair_id] for pair_id in pair_ids]
    expected = {"legacy": {"relative_gap": 16, "context_factorial": 16}, "fresh": {"relative_gap": 8, "context_factorial": 8}}[panel]
    counts = {key: sum(row["construct"] == key for row in selected) for key in expected}
    if counts != expected:
        raise RuntimeError(f"{panel} QA construct counts drift: {counts}")
    row_to_unit = bundle["row_to_unit"]
    references: list[Mapping[str, Any]] = []
    candidates: list[Mapping[str, Any]] = []
    qa_rows: list[dict[str, Any]] = []
    family_indices: dict[str, list[int]] = {"uniform_shift": [], "prefix_position_only": []}
    cursor = 0
    # Preserve the exact pair-ID order frozen in the population split.
    for pair in selected:
        if pair["construct"] == "relative_gap":
            if set(pair["rows"]) != ({"bare", "uniform_shift", "relative_gap"} if panel == "legacy" else {"bare", "uniform_shift"}):
                raise RuntimeError("relative-gap QA condition inventory drift")
            ...
            qa_rows.append({"panel": panel, "pair_id": pair["pair_id"], "family": "uniform_shift", "roles": ["pre", "post"]})
            family_indices["uniform_shift"].extend((cursor, cursor + 1)); cursor += 2
        elif pair["construct"] == "context_factorial":
            expected_conditions = {"bare", "prefix_position_only", "separator_only", "true_prefix", "unrelated_prefix"} if panel == "legacy" else {"bare", "prefix_position_only"}
            if set(pair["rows"]) != expected_conditions:
                raise RuntimeError("context QA condition inventory drift")
            ...
            references.append(reference); candidates.append(candidate)
            qa_rows.append({"panel": panel, "pair_id": pair["pair_id"], "family": "prefix_position_only", "roles": ["target"]})
            family_indices["prefix_position_only"].append(cursor); cursor += 1
        else:
            raise RuntimeError("unexpected QA construct")
    segments = [
        {
            "panel": panel,
            "family": family,
    ...
    if any(segment["indices"] != list(range(segment["start"], segment["stop"])) for segment in segments):
        raise RuntimeError("QA family rows are not contiguous in the frozen panel order")
    return references, candidates, qa_rows, segments

def _qa_stat(reference: np.ndarray, shifted: np.ndarray, *, atol: float, rtol: float) -> dict[str, Any]:
    left = reference.astype(np.float64)
    right = shifted.astype(np.float64)
    difference = np.abs(right - left)
    bound = atol + rtol * np.abs(left)
    finite = np.isfinite(left) & np.isfinite(right) & np.isfinite(difference) & np.isfinite(bound)
    failures = (~finite) | (difference > bound)
    ratios = np.divide(difference, bound, out=np.full_like(difference, np.inf), where=bound > 0)
    ratios[~finite] = np.inf
    finite_difference = difference[np.isfinite(difference)]
    finite_margin = (bound - difference)[finite]
    return {
        "status": "PASS" if not failures.any() else "FAIL",
        "rows": int(left.shape[0]),
        "width": int(left.shape[1]),
        "max_abs_error": float(finite_difference.max(initial=0.0)) if finite.all() else float("inf"),
        "max_bound_ratio": float(ratios.max(initial=0.0)),
        "failing_elements": int(failures.sum()),
        "failing_rows": int(np.any(failures, axis=1).sum()),
        "max_margin_to_bound": float(finite_margin.min(initial=float("inf"))) if finite.all() else float("-inf"),
    }


def _recompute_qa_evidence(
    arrays: Mapping[str, np.ndarray], expected_rows: Sequence[Mapping[str, Any]], qa_spec: Mapping[str, Any]
) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    required = ("reference", "repeat_1", "repeat_2", "candidate")
    if any(not np.isfinite(np.asarray(arrays[name])).all() for name in required):
        raise RuntimeError("QA arrays contain non-finite values")
    reference = np.asarray(arrays["reference"])
    repeat_error = max(
        float(np.max(np.abs(np.asarray(arrays[name], dtype=np.float64) - reference.astype(np.float64))))
        for name in ("repeat_1", "repeat_2")
    )
    tolerance = derive_numerical_tolerance(
        repeat_error,
        atol_floor=float(qa_spec["atol_floor"]),
        atol_hard_ceiling=float(qa_spec["atol_hard_ceiling"]),
        multiplier=float(qa_spec["atol_multiplier"]),
        rtol=float(qa_spec["rtol"]),
    )
    ...
    for row in expected_rows:
        width = len(row["roles"])
        key = (str(row["panel"]), str(row["family"]))
        indices.setdefault(key, []).extend(range(cursor, cursor + width))
        cursor += width
    if cursor != reference.shape[0]:
        raise RuntimeError("QA expected-row expansion does not match activation rows")
    for (panel, family), positions in indices.items():
        panels[panel][family] = _qa_stat(
            reference[np.asarray(positions, dtype=np.int64)],
            np.asarray(arrays["candidate"])[np.asarray(positions, dtype=np.int64)],
            atol=float(tolerance["atol"]),
            rtol=float(tolerance["rtol"]),
        ) if tolerance["status"] == "valid" else {"status": "FAIL"}
    return tolerance, panels


def _expected_qa_lineage_and_rows(
    config: Mapping[str, Any], source: str
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    prescore = read_json(ROOT / _validate_relative_path(str(config["prescore_config"]["path"])))
    split_path = ROOT / _validate_relative_path(str(config["population_split"]["path"]))
    split = read_json(split_path)
    expected_rows: list[dict[str, Any]] = []
    parents: dict[str, Any] = {}
    for panel in ("legacy", "fresh"):
        spec = config[f"{panel}_qa_parent"]
        parent_config = read_json(ROOT / _validate_relative_path(str(spec["config_path"])))
        parent_manifest = read_json(ROOT / _validate_relative_path(str(spec["manifest_path"])))
        bundle = _load_source_bundle(parent_config, parent_manifest, source)
        _, _, panel_rows, _ = _qa_panel_units(
            bundle,
            list(map(str, split["sources"][source][f"{panel}_pair_ids"])),
            prescore,
            panel=panel,
        )
        ...
        parents[panel] = {
            ...
            "inference_units_sha256": parent_manifest["sources"][source]["units_sha256"],
            "activation_rows_sha256": parent_manifest["sources"][source]["activation_rows_sha256"],
            "intervention_pairs_sha256": parent_manifest["sources"][source]["intervention_pairs_sha256"],
        }
    lineage = {
...
def _verify_qa(path: Path, config: Mapping[str, Any], source: str) -> dict[str, Any]:
    ...
    prescore = read_json(ROOT / _validate_relative_path(str(config["prescore_config"]["path"])))
    expected_lineage, expected_rows = _expected_qa_lineage_and_rows(config, source)
    expected_device = str(config["extraction"]["qa_devices"][source])
    environment = payload.get("environment", {})
    tolerance = payload.get("tolerance", {})
    qa_spec = prescore["numerical_qa"]
    repeat_error = float(tolerance.get("max_abs_repeat_error", float("inf")))
    expected_atol = max(float(qa_spec["atol_floor"]), 2.0 * repeat_error)
    expected_panel_rows = {
        "legacy": {"uniform_shift": 32, "prefix_position_only": 16},
        "fresh": {"uniform_shift": 16, "prefix_position_only": 8},
    }
    if (
        payload.get("qa_input_lineage") != expected_lineage
        ...
        or payload.get("layer") != int(prescore["model"]["layer"])
        or payload.get("hidden_state_index") != int(prescore["model"]["layer"]) + 1
        or payload.get("model") != prescore["model"]
        ...
        or tolerance.get("status") != "valid"
        or tolerance.get("atol") != expected_atol
        or tolerance.get("rtol") != float(qa_spec["rtol"])
        or 2.0 * repeat_error >= float(qa_spec["atol_hard_ceiling"])
        or set(payload.get("panels", {})) != {"legacy", "fresh"}
        or any(
            set(payload["panels"][panel]) != {"uniform_shift", "prefix_position_only"}
            or any(value.get("status") != "PASS" for value in payload["panels"][panel].values())
            or any(payload["panels"][panel][family].get("rows") != expected_panel_rows[panel][family] for family in expected_panel_rows[panel])
            for panel in ("legacy", "fresh")
        )
        or payload.get("dtype") != "float32_inference_float32_cache"
        or payload.get("parameter_dtype") != "float32"
        or payload.get("hidden_state_dtype") != "float32"
        or payload.get("cache_dtype") != "float32"
        or payload.get("autocast_enabled") is not False
        or environment.get("python") != platform.python_version()
        or environment.get("torch") != torch.__version__
        or environment.get("cuda") != torch.version.cuda
        or environment.get("cudnn") != torch.backends.cudnn.version()
        or environment.get("cublas_workspace_config") != EXPECTED_CUBLAS
    ...
    if {p.name for p in entries} != expected_files or any(not p.is_file() or p.is_symlink() for p in entries):
    ...
    observed_rows = read_json(root / "qa_rows.json")
    if observed_rows != expected_rows or payload.get("qa_underlying_units") != len(expected_rows):
        raise RuntimeError("QA exact panel order/row lineage drift")
    with np.load(root / "qa_arrays.float32.npz", allow_pickle=False) as arrays:
        if set(arrays.files) != {"reference", "repeat_1", "repeat_2", "candidate"}:
            raise RuntimeError("QA array inventory drift")
        shapes = {arrays[name].shape for name in arrays.files}
        if len(shapes) != 1 or any(arrays[name].dtype != np.float32 or arrays[name].ndim != 2 for name in arrays.files):
            raise RuntimeError("QA array dtype/shape drift")
        rows = next(iter(shapes))[0]
        recomputed_tolerance, recomputed_panels = _recompute_qa_evidence(arrays, expected_rows, qa_spec)
    if recomputed_tolerance != tolerance or recomputed_panels != payload.get("panels"):
        raise RuntimeError("QA signed numerical evidence does not recompute exactly")
    if not all(
        stat.get("status") == "PASS"
        for panel in recomputed_panels.values()
        for stat in panel.values()
    ):
        raise RuntimeError("QA recomputed decision is not PASS")
    if rows != payload.get("qa_activation_rows"):
        raise RuntimeError("QA activation-row count drift")
...
def run_qa(config: Mapping[str, Any], prescore: Mapping[str, Any], manifest: Mapping[str, Any], *, source: str, device: torch.device, batch_size: int, signing_key: Path) -> dict[str, Any]:
    ...
    with _exclusive_lock(run_root, "gpu-extraction-global"):
        ...
        split = read_json(ROOT / _validate_relative_path(str(config["population_split"]["path"])))
        references: list[Mapping[str, Any]] = []
        candidates: list[Mapping[str, Any]] = []
        qa_rows: list[dict[str, Any]] = []
        segments: list[dict[str, Any]] = []
        qa_input_lineage: dict[str, Any] = {}
        for panel in ("legacy", "fresh"):
            spec = config[f"{panel}_qa_parent"]
            parent_config = read_json(ROOT / _validate_relative_path(str(spec["config_path"])))
            parent_manifest = read_json(ROOT / _validate_relative_path(str(spec["manifest_path"])))
            bundle = _load_source_bundle(parent_config, parent_manifest, source)
            panel_refs, panel_candidates, panel_rows, panel_segments = _qa_panel_units(
                bundle, list(map(str, split["sources"][source][f"{panel}_pair_ids"])), prescore, panel=panel
            )
            offset = sum(len(unit["row_ids"]) for unit in references)
            for segment in panel_segments:
                segment = dict(segment); segment["indices"] = [offset + int(index) for index in segment["indices"]]; segments.append(segment)
            references.extend(panel_refs); candidates.extend(panel_candidates); qa_rows.extend(panel_rows)
            qa_input_lineage[panel] = {
                "config_sha256": spec["config_sha256"],
                "manifest_sha256": spec["manifest_sha256"],
                "inference_units_sha256": parent_manifest["sources"][source]["units_sha256"],
                "activation_rows_sha256": parent_manifest["sources"][source]["activation_rows_sha256"],
        ...
        qa_input_lineage = {
            "population_split_sha256": config["population_split"]["sha256"],
            "prescore_config_sha256": config["prescore_config"]["sha256"],
            "parents": qa_input_lineage,
        }
        ...
        panel_stats: dict[str, dict[str, Any]] = {"legacy": {}, "fresh": {}}
        for segment in segments:
            indices = np.asarray(segment["indices"], dtype=np.int64)
            stat = _qa_stat(reference[indices], shifted[indices], atol=float(tolerance["atol"]), rtol=float(tolerance["rtol"])) if valid else {"status": "FAIL"}
            panel_stats[str(segment["panel"])][str(segment["family"])] = stat
        passed = valid and all(stat["status"] == "PASS" for panel in panel_stats.values() for stat in panel.values())
        ...
        payload = {"schema_version": QA_SCHEMA, "source": source, "status": "PASS" if passed else "FAIL",
            "tolerance": tolerance, "panels": panel_stats, "qa_input_lineage": qa_input_lineage,
            ...
            "hidden_state_index": hidden_index, "layer": int(prescore["model"]["layer"]), "dtype": "float32_inference_float32_cache",
            "device": str(device), "gpu_uuid": _gpu_uuid(device), "batch_size": batch_size, "model": prescore["model"],
            "parameter_dtype": "float32", "hidden_state_dtype": "float32", "cache_dtype": "float32", "autocast_enabled": False,
            "optimizer_created": False, "checkpoint_created": False,
...
def _verify_complete_cache(final: Path, config: Mapping[str, Any], prescore: Mapping[str, Any], bundle: Mapping[str, Any], source: str) -> dict[str, Any]:
    ...
    if {p.name for p in entries} != expected_files or any(not p.is_file() or p.is_symlink() for p in entries):
        raise RuntimeError("activation bundle file set drift")
    ...
    if (
        ...
        or payload.get("inference_dtype") != "float32"
        or payload.get("parameter_dtype") != "float32"
        or payload.get("hidden_state_dtype") != "float32"
        or payload.get("cache_dtype") != "float32"
        or payload.get("autocast_enabled") is not False
    ...
    if (
        ...
        raise RuntimeError("activation deterministic environment drift")
...
def run_full(config: Mapping[str, Any], prescore: Mapping[str, Any], manifest: Mapping[str, Any], *, source: str, device: torch.device, batch_size: int, signing_key: Path) -> dict[str, Any]:
    ...
    with _exclusive_lock(run_root, "gpu-extraction-global"):
        ...
        payload = {"schema_version": CACHE_SCHEMA, "source": source, "status": "COMPLETE", "rows": len(rows), "units": len(units), "width": width,
            "layer": int(prescore["model"]["layer"]), "hidden_state_index": hidden_index, "dtype": "float32", "inference_dtype": "float32",
            "parameter_dtype": "float32", "hidden_state_dtype": "float32", "cache_dtype": "float32", "autocast_enabled": False,
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--config", default="configs/atlas_discovery_v3_3/scoring.json")
