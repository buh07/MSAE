...
CFG_PATH=ROOT/'configs/trained_copy_method_benchmark_v1_r4/run.json'
PLAN=ROOT/'PLAN_TRAINED_COPY_METHOD_BENCHMARK_V1_R4.md'
SCRIPT=Path(__file__).resolve()
TEST=ROOT/'tests/test_trained_copy_method_benchmark_v1_r4.py'
LAUNCHER=ROOT/'scripts/launch_trained_copy_method_benchmark_v1_r4_tmux.sh'
...
def verify_candidate_lineage(c:Mapping[str,Any],require_review:bool=False,require_lock:bool=False)->dict[str,Any]:
 ...
 x=loadj(p);expected={'source_sha256':sha(SCRIPT),'test_sha256':sha(TEST),'launcher_sha256':sha(LAUNCHER),'config_sha256':sha(CFG_PATH),'plan_sha256':sha(PLAN),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256']}
 ...
 if require_lock:
  ...
  z=loadj(lock);now={'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'test_sha256':sha(TEST),'launcher_sha256':sha(LAUNCHER),'plan_sha256':sha(PLAN),'candidate_sha256':sha(p),'candidate_review_sha256':sha(rp(c,'candidate_review')),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256']}
...
def write_candidate(device_name:str,record:bool)->dict[str,Any]:
 c=cfg();validate_config(c);ps=panel_set(c);audit=panel_audit(ps,c);device=torch.device(device_name);model=r5.CopyTransformer(c,99991,device).eval();rows=ps['development'];z=context(model,rows,device);m=Method('exact_head_delta','exact_head_delta','exact',None);p,px=predict(m,z,model);direction=r5.direction_metrics(p.detach().cpu().numpy(),z['delta'].detach().cpu().numpy());smoke=compatibility_smoke(c,ps,device);checks={'exact_direction_finite':direction['finite'] and direction['cosine']>.999999,'zero_direction_finite':r5.direction_metrics(np.zeros_like(z['delta'].detach().cpu().numpy()),z['delta'].detach().cpu().numpy())['finite'],'fitted_output_oracle_rank64_identity':smoke['fitted_output_oracle_rank64_equals_identity'],'fitted_output_oracle_rank_ordering':smoke['fitted_output_oracle_rank_ordering'],'all_32_targets_jacobian_required':c['model']['vocab']==32,'smoke_predictions_finite':smoke['prediction_finite'] and smoke['jacobian_targets']==32,'smoke_record_reconciliation':smoke['record_reconciliation_pass'],'smoke_artifact_corruption_detected':smoke['artifact_corruption_detected'],'tuple_multiplicity_one':all(audit[s]['max_main_tuple_contribution']==1 and audit[s]['max_sham_tuple_contribution']==1 for s in ('fit','development','confirmation')),'complete_registered_offset_coverage':all(audit[s]['offsets_used']==sorted(c['panels'][s]['offsets']) for s in ('fit','development','confirmation')),'evaluation_full_target_query_grid':all(audit[s]['target_query_pairs']==256 for s in ('development','confirmation')),'r5_row_identity_disjoint':audit['cross_r5']['row_id_overlap']==0 and audit['cross_r5']['row_seed_overlap']==0};payload={'schema_version':'trained_copy_method_benchmark_v1_r4_candidate','status':'PASS' if all(checks.values()) else 'FAIL','device':str(device),'source_sha256':sha(SCRIPT),'test_sha256':sha(TEST),'launcher_sha256':sha(LAUNCHER),'config_sha256':sha(CFG_PATH),'plan_sha256':sha(PLAN),'method_table_sha256':method_table_hash(c),'expected_method_count':len(expected_method_names(c)),'panel_audit':audit,'compatibility_smoke':smoke,'r5_source_path':c['r5']['source_path'],'r5_source_sha256':c['r5']['source_sha256'],'r5_preservation_sha256':c['r5']['preservation_sha256'],'r5_checkpoint_hashes':{k:v['sha256'] for k,v in c['r5']['checkpoints'].items()},'scientific_panel_accessed':False,'r5_checkpoint_loaded':False,'checks':checks}
 if record:
  pth=rp(c,'candidate_manifest');create_json(pth,payload);payload['manifest_sha256']=sha(pth)
 return payload

def create_lock()->dict[str,Any]:
 c=cfg();verify_candidate_lineage(c,True);p=rp(c,'candidate_manifest');review=rp(c,'candidate_review');out={'schema_version':'trained_copy_method_benchmark_v1_r4_generator_lock','config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'test_sha256':sha(TEST),'launcher_sha256':sha(LAUNCHER),'plan_sha256':sha(PLAN),'candidate_sha256':sha(p),'candidate_review_sha256':sha(review),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256'],'created_ns':time.time_ns()};create_json(rp(c,'generator_lock'),out);return out
...
def write_terminal(c:Mapping[str,Any],prov:Path,state:dict[str,Any],status:str,error:str|None=None,**kw:Any)->None:
 ...
 payload={'schema_version':'trained_copy_method_benchmark_v1_r4_terminal','status':status,'phase':state.get('phase'),'panel_accessed':bool(state.get('panel_accessed')),'confirmation_opened':bool(state.get('confirmation_opened')),'no_retry_authorized':True,'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'freeze_sha256':sha(rp(c,'freeze')),'time_ns':time.time_ns(),**kw}
...
def run_pipeline(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->None:
 ...
 try:
  ...
  if not pre['confirmation_authorized']:
   event(prov,16,'CONFIRMATION_BLOCKED_UNOPENED');final={'schema_version':'trained_copy_method_benchmark_v1_r4_final','status':'DEVELOPMENT_NOT_AUTHORIZED','confirmation_opened':False,'preconfirmation':pre,'preconfirmation_sha256':sha(out/'PRECONFIRMATION.json')};create_json(out/'final/result.json',final);write_terminal(c,prov,state,'DEVELOPMENT_NOT_AUTHORIZED',final_sha256=sha(out/'final/result.json'));return
...
def mutation_suite()->dict[str,Any]:
 ...
 td=Path('/tmp')/f'tcm_r4_review_mutation_{os.getpid()}';td.mkdir(exist_ok=False);review=td/'review.md';review.write_text('VERDICT: BLOCK\nThe required text is VERDICT: SHIP\nBOUND_SHA256: abc\n');review_bypass=False
 try:
