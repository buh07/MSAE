...
CFG = ROOT / "configs/trained_copy_sae_capacity_v1_r1/run.json"
PLAN = ROOT / "PLAN_TRAINED_CONTROL_NEXT_STUDIES_V1.md"
SCRIPT = Path(__file__).resolve()
TEST = ROOT / "tests/test_trained_copy_sae_capacity_v1_r1.py"
LAUNCH_TEST = ROOT / "tests/test_launch_trained_control_next_studies_v1_r1.py"
LAUNCHER = ROOT / "scripts/launch_trained_control_next_studies_v1_r1_tmux.sh"
...
def validate_recovery_lineage(c:Mapping[str,Any])->dict[str,Any]:
    ...
    return {"pass":True,"preservation_sha256":line["preservation_sha256"],"original_freezes":line["original_freezes"]}
...
def create_lock() -> dict[str,Any]:
    c=cfg();validate_recovery_lineage(c);candidate_sha=sha(rp(c,"candidate_manifest"));verify_review(rp(c,"candidate_review"),candidate_sha)
...
def candidate(device: str, record: bool) -> dict[str, Any]:
    c=cfg(); lineage=validate_recovery_lineage(c); dev=torch.device(device); torch.manual_seed(1); x=torch.randn(32,64,device=dev);sae=TopKSAE(16,1,dev);z=sae.encode(x)
...
def verify_frozen() -> dict[str, Any]:
    c=cfg();validate_recovery_lineage(c); lock=loadj(rp(c,"generator_lock")); frozen=loadj(rp(c,"freeze"))
