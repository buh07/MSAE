#!/usr/bin/env python3
"""Source-readiness builder for MSAE v8; standard-library and model-free."""
...
RAW = ROOT / "data/msae_independent_source_v8/raw" / COMMIT
PRIVATE = ROOT / "data/msae_independent_source_v8/private"
PROV = ROOT / "reports/provenance/msae_independent_source_v8"
QUARANTINES = {
...
V8_PREFIX = "data/msae_independent_source_v8/"
V8_AUTHORITY = {
...
V8_FROZEN_AUTHORITY_SHA256 = {
    "docs/plan-msae-independent-source-v8.md":
        "7aec70f8d6f384f2919f5f3086064b437dd34ff7731542aed2d7abe23ac5ae19",
    "reports/adversarial/msae_independent_source_v8_plan_review.md":
        "d159749125a6db4e1dd6a68f2207e4be6b643e2f0d7b8112695cfec98660204d",
    "reports/provenance/msae_independent_source_v8/preacquisition_alias_screen.json":
        "68a7536c95896087960dd4fc5882df85296baaa1a07aaf910e0e7c257b165e2f",
    "reports/provenance/msae_independent_source_v8/historical_source_registry.json":
        "ff4a36996f58531353dda9ed8fcbedb4c3a615f2d877e8b31f1462ddc8581a97",
}
...
def scientific_artifact_inventory() -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v8_scientific_artifact_inventory_v1",
            "entries":entries}
...
def strict_json(path: Path) -> Any:
    ...
    # presentation; every builder-published v8 final uses canonical_file_bytes.
...
def verify_process_snapshot_schema(value: Any, code: str) -> None:
    ...
    if (value["schema_version"] != "msae_independent_source_v8_process_snapshot_v1"
...
def validate_acquisition_config(path: Path) -> dict[str, Any]:
    ...
    if (value.get("schema_version") != "msae_independent_source_v8_acquisition_v1"
...
def group_split_key(group_id: str, source_commit: str = COMMIT) -> str:
    ...
    return sha_bytes(canonical_bytes(
        ["msae-independent-source-v8/C1C2-group", source_commit, group_id]))
...
def build_split_manifest(rows: list[dict[str, Any]], dedup: dict[str, Any],
    ...
    split={"schema_version":"msae_independent_source_v8_split_manifest_v1","source_commit":COMMIT,
...
def payload_record(row: dict[str, Any]) -> dict[str, Any]:
    return {"schema_version":"msae_independent_source_v8_payload_v1","source_repo":REPO,
...
def public_label(task: str, value: str) -> str:
    if task == "token_identity": return sha_bytes(("msae-v8/token\0" + value).encode())
    if task == "lemma_identity": return sha_bytes(("msae-v8/lemma\0" + value).encode())
    return value
...
def _walk_paths() -> Iterator[tuple[Path, str]]:
    for base, dirs, files in os.walk(ROOT):
        ...
        for d in sorted(dirs):
            ...
            if child_rel.startswith(V8_PREFIX.rstrip("/")):
                if any(child.iterdir()):
                    raise GateFailure("preexisting_v8_namespace")
                continue
...
def inventory_repository() -> dict[str, Any]:
    ...
    for key in sorted(by_inode, key=lambda z: min(x[1] for x in by_inode[z])):
        ...
        for _p, rel, s in group:
            ...
            elif rel.startswith(V8_PREFIX): item.update(disposition="v8_excluded", adapter="excluded", extracted_unit_count=0)
            elif rel in V8_AUTHORITY:
                item.update(disposition="v8_authority", adapter="authority", extracted_unit_count=0)
            entries.append(item)
    ...
    return {"schema_version": "msae_independent_source_v8_baseline_inventory_v1",
            ...
            "authority_expected_absent": sorted(V8_AUTHORITY - {x["path"] for x in entries}),
...
def validate_history_registry_binding(inventory: dict[str, Any],
    ...
    if (registry.get("schema_version") != "msae_independent_source_v8_historical_source_registry_v1"
            ...
            or registry.get("domain_utf8") != "msae-v8/pedigree"
            ...
            or screen.get("schema_version") != "msae_independent_source_v8_preacquisition_alias_screen_v2"
...
def deduplicate_roles(roles: dict[str, list[Sentence]]) -> tuple[dict[str, list[Sentence]], dict[str, Any]]:
    ...
    report = {"schema_version": "msae_independent_source_v8_dedup_v1", "roles": {},
...
def _support(rows: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v8_support_v1","floor_distinct_utterances":20,
...
def _license() -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v8_license_v1","license_id":"CC-BY-SA-4.0",
...
def document_group_census() -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v8_document_group_census_v1",
...
def _source_family(inventory: dict[str,Any], raw_hashes:set[str]) -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v8_source_family_v1","status":"eligible" if eligible else "ineligible",
            "disposition":"project_source_use_unseen_before_v8","content_occurrence_count":len(matches),
...
def pedigree_hash(value: str) -> str:
    return sha_bytes(b"msae-v8/pedigree\0" + value.encode("utf-8"))
...
def candidate_pedigree() -> dict[str, Any]:
    ...
    return {"schema_version": "msae_independent_source_v8_candidate_pedigree_v1",
...
def verify_frozen_v8_authority_bindings() -> dict[str, str]:
    if any(sha_file(ROOT/path)!=digest for path,digest in V8_FROZEN_AUTHORITY_SHA256.items()):
        raise GateFailure("predecessor_drift")
    return dict(V8_FROZEN_AUTHORITY_SHA256)


def _verify_predecessor_and_raw() -> dict[str, Any]:
    expected = {ROOT/path:digest for path,digest in verify_frozen_v8_authority_bindings().items()}
    ...
    required_public=(
        ...
        ROOT/"reports/adversarial/msae_independent_source_v8_preacquisition_authority_review.md",
    ...
    config=validate_acquisition_config(ROOT/"configs/msae_independent_source_v8/acquisition.json")
    ...
    if (entry.get("schema_version")!="msae_independent_source_v8_source_acquisition_entry_v1"
            ...
            or entry.get("config_sha256")!=sha_file(ROOT/"configs/msae_independent_source_v8/acquisition.json")
            or entry.get("runner_sha256")!=sha_file(ROOT/"scripts/acquire_msae_independent_source_v8.py")
    ...
    if (acquisition.get("schema_version")!="msae_independent_source_v8_source_acquisition_v1"
            ...
            or acquisition.get("config_sha256") != sha_file(ROOT/"configs/msae_independent_source_v8/acquisition.json")
            ...
            or acquisition.get("runner_sha256") != sha_file(ROOT/"scripts/acquire_msae_independent_source_v8.py")
    ...
    expected_command_argv=[
        [*config["ignore_argv"][:-1],"data/msae_independent_source_v8/raw/sentinel"],
        [*config["ignore_argv"][:-1],"data/msae_independent_source_v8/private/sentinel"],
    ...
    if ([item.get("path") for item in ignores] != ["data/msae_independent_source_v8/raw/sentinel","data/msae_independent_source_v8/private/sentinel"]
...
def _verify_history_snapshot(inventory: dict[str, Any], allowed_added: set[str] | None = None,
    ...
    current_paths={rel for _path,rel in _walk_paths_allow_v8(include_v8=True)}
    ...
    for item in inventory["entries"]:
        if item["disposition"] not in {"text_scanned", "archive_scanned", "binary_unscanned", "v8_authority", "quarantine"}:
...
def _raw_history_additions() -> set[str]:
    return {f"data/msae_independent_source_v8/raw/{COMMIT}/{name}"
            for name in [*SOURCE_FILES.values(),"LICENSE.txt"]}
...
def _control_history_additions(*, success: bool = False) -> set[str]:
    ...
    if success:additions.add("reports/provenance/msae_independent_source_v8/source_acquisition.json")
    else:
        for name in ("source_acquisition.json","source_acquisition_rejection.json"):
            if present_path(PROV/name):additions.add(f"reports/provenance/msae_independent_source_v8/{name}")
    return additions


def _success_history_additions() -> set[str]:
    return (_control_history_additions(success=True)|_raw_history_additions()|
            {f"reports/provenance/msae_independent_source_v8/{name}"
             for name in SCIENTIFIC_ARTIFACT_NAMES}|
            {"data/msae_independent_source_v8/private/blind_payload.jsonl"})
...
def verify_pre_payload_v8_namespace() -> None:
    fds=[]
    try:
        v8fd=open_repo_dir(RAW.parent.parent);fds.append(v8fd)
        rawrootfd=os.open("raw",os.O_RDONLY|getattr(os,"O_DIRECTORY",0)|getattr(os,"O_NOFOLLOW",0),dir_fd=v8fd);fds.append(rawrootfd)
        ...
        for fd,mode in ((v8fd,0o700),(rawrootfd,0o700),(commitfd,0o555)):
            if stat.S_IMODE(os.fstat(fd).st_mode)!=mode:raise GateFailure("v8_namespace_directory_drift")
        if sorted(os.listdir(v8fd)) != ["raw"]:raise GateFailure("v8_namespace_extra_path")
        if sorted(os.listdir(rawrootfd)) != [COMMIT]:raise GateFailure("v8_raw_parent_extra_path")
...
def process_snapshot() -> dict[str, Any]:
    ...
    for child in sorted(Path("/proc").iterdir(), key=lambda item: int(item.name) if item.name.isdigit() else -1):
        ...
        item={"pid":int(child.name),"start_ticks":stat_fields[19],"executable_basename":exe,
              "command_sha256":sha_bytes(b"msae-v8/process\0"+raw),"forbidden_codes":codes}
        entries.append(item)
        if codes:matches.append(item)
    return {"schema_version":"msae_independent_source_v8_process_snapshot_v1",
...
def verify_training_roots(inventory: dict[str, Any]) -> None:
    ...
    current=[]
    for path,rel in _walk_paths_allow_v8():
...
def _walk_paths_allow_v8(include_v8: bool = False) -> Iterator[tuple[Path,str]]:
    for base,dirs,files in os.walk(ROOT,followlinks=False):
        ...
        for d in sorted(dirs):
            ...
            if stat.S_ISLNK(st.st_mode) or not stat.S_ISDIR(st.st_mode):
                raise GateFailure("special_paths")
            if d=="__pycache__" or any((rel+"/").startswith(x) for x in GENERATED_PREFIXES) or (not include_v8 and rel.startswith(V8_PREFIX.rstrip("/"))):
                continue
            retained.append(d)
        dirs[:]=retained
        for name in sorted(files):
...
def implementation_review_sha() -> str:
    review=ROOT/"reports/adversarial/msae_independent_source_v8_preacquisition_implementation_review.md"
    required=[
        sha_file(ROOT/"docs/plan-msae-independent-source-v8.md"),
        sha_file(ROOT/"reports/adversarial/msae_independent_source_v8_plan_review.md"),
        sha_file(PROV/"preacquisition_alias_screen.json"),
        sha_file(PROV/"historical_source_registry.json"),
        sha_file(Path(__file__)),
        sha_file(ROOT/"scripts/acquire_msae_independent_source_v8.py"),
        sha_file(ROOT/"tests/test_prepare_msae_independent_source_v8.py"),
        sha_file(ROOT/"configs/msae_independent_source_v8/acquisition.json"),
        sha_file(ROOT/"reports/verification/msae_independent_source_v8_source_free_checks.log"),
    ]
...
def build_authority_manifest() -> None:
    ...
    authority_review=ROOT/"reports/adversarial/msae_independent_source_v8_preacquisition_authority_review.md"
    ...
    _verify_history_snapshot(
        baseline,
        allowed_added={"reports/provenance/msae_independent_source_v8/baseline_inventory.json"},
    )
    ...
    review_path=ROOT/"reports/adversarial/msae_independent_source_v8_preacquisition_implementation_review.md"
    _require_ship_review(review_path,[inputs["scripts/prepare_msae_independent_source_v8.py"],
                                      inputs["tests/test_prepare_msae_independent_source_v8.py"],
                                      inputs["configs/msae_independent_source_v8/acquisition.json"],
                                      inputs["reports/verification/msae_independent_source_v8_source_free_checks.log"]])
    ...
    value={"schema_version":"msae_independent_source_v8_preacquisition_authority_v1",
...
def verify_authority_chain() -> dict[str, Any]:
    ...
    if (manifest.get("schema_version")!="msae_independent_source_v8_preacquisition_authority_v1"
    ...
    review_path=ROOT/"reports/adversarial/msae_independent_source_v8_preacquisition_authority_review.md"
...
def build_baseline() -> None:
    ...
    validate_acquisition_config(ROOT/"configs/msae_independent_source_v8/acquisition.json")
...
def build_ready() -> None:
    ...
    verify_pre_payload_v8_namespace()
    ...
    source_manifest={"schema_version":"msae_independent_source_v8_source_manifest_v1","source_commit":COMMIT,
    ...
    cross=_role_overlap(rows);write_json(PROV/"cross_role_overlap.json",{"schema_version":"msae_independent_source_v8_cross_role_overlap_v1","blocking_collision_count":len(cross),"blocking_collisions":cross,"status":"eligible" if not cross else "ineligible"})
    ...
    role_manifest={"schema_version":"msae_independent_source_v8_role_manifest_v1","source_commit":COMMIT,"roles":{}}
    ...
    plan_sha=sha_file(ROOT/"docs/plan-msae-independent-source-v8.md");review_sha=sha_file(ROOT/"reports/adversarial/msae_independent_source_v8_plan_review.md")
    seal={"schema_version":"msae_independent_source_v8_seal_v1","status":"independent_source_ready_for_future_prescore_protocol","source_status":"project_source_use_unseen_before_v8","plan_sha256":plan_sha,"plan_review_sha256":review_sha,"program_sha256":sha_file(Path(__file__)),"acquisition_runner_sha256":sha_file(ROOT/'scripts/acquire_msae_independent_source_v8.py'),"test_sha256":sha_file(ROOT/'tests/test_prepare_msae_independent_source_v8.py'),"artifact_sha256":bindings,"payload":{"path":"data/msae_independent_source_v8/private/blind_payload.jsonl","sha256":sha_file(payload),"size":ps.st_size,"record_count":len(test_rows),"mode":stat.S_IMODE(ps.st_mode),"nlink":ps.st_nlink},"model_operations_initiated_by_builder":0,"gpu_queries_initiated_by_builder":0,"training_runs_initiated_by_builder":0,"external_observation_scope":"baseline_and_terminal_proc_snapshots_plus_training_root_diff","unsupported_tasks":["neutral_prefix_offset","entity_binary","entity_type","source_genre"],"opaque_binary_history_excluded":True,"document_speaker_cluster_inference_authorized":False}
    seal_sha=sha_bytes(canonical_file_bytes(seal))
    write_json(PROV/"no_training_gate.json",{"schema_version":"msae_independent_source_v8_no_training_gate_v1","status":"pending_terminal_seal","terminal_status_if_seal_matches":"independent_source_ready_for_future_prescore_protocol","model_scoring_authorized":False,"k2_or_branch_training_authorized":False,"stage_c_authorized":False,"seal_sha256":seal_sha})
...
def retain_rejection(code: str) -> None:
    ...
    value={"schema_version":"msae_independent_source_v8_rejection_v1","status":"rejected","failure_code":code,
...
def _validate_inventory_artifact(value: Any, code: str) -> None:
    ...
    for item in entries:
        ...
        if (keys!=expected or not isinstance(item.get("path"),str)
                ...
                or item.get("disposition") not in {"text_scanned","archive_scanned","binary_unscanned","v8_authority","quarantine"}
    ...
    if (value["schema_version"]!="msae_independent_source_v8_baseline_inventory_v1"
...
def validate_scientific_artifacts() -> None:
    ...
    if (census["schema_version"]!="msae_independent_source_v8_document_group_census_v1"
    ...
    if (source["schema_version"]!="msae_independent_source_v8_source_manifest_v1"
    ...
    for evidence in license_value["evidence"].values():
        ...
        if (evidence["eligible"] is not True
            ...
            raise GateFailure(code)
    if (license_value["schema_version"]!="msae_independent_source_v8_license_v1"
    ...
    if (family["schema_version"]!="msae_independent_source_v8_source_family_v1" or family["status"]!="eligible"
            or family["disposition"]!="project_source_use_unseen_before_v8"
    ...
    if (pedigree["schema_version"]!="msae_independent_source_v8_candidate_pedigree_v1"
    ...
    if (dedup["schema_version"]!="msae_independent_source_v8_dedup_v1"
    ...
    if (cross["schema_version"]!="msae_independent_source_v8_cross_role_overlap_v1"
    ...
    if (overlap["schema_version"]!="msae_independent_source_v8_history_overlap_v1"
    ...
    if (support["schema_version"]!="msae_independent_source_v8_support_v1" or support["floor_distinct_utterances"]!=20
...
def reconstruct_scientific_artifacts() -> dict[str, Any]:
    ...
    roles,source_counts=_all_source()
    source={"schema_version":"msae_independent_source_v8_source_manifest_v1","source_commit":COMMIT,
            "partitions":source_counts,"source_text_published":False}
    ...
    cross_value={"schema_version":"msae_independent_source_v8_cross_role_overlap_v1",
    ...
    overlap={"schema_version":"msae_independent_source_v8_history_overlap_v1",
        "baseline_inventory_sha256":sha_file(PROV/"baseline_inventory.json"),
...
def verify_terminal() -> None:
    ...
    if present_path(rejection_path):
        ...
        if (rejection.get("schema_version")!="msae_independent_source_v8_rejection_v1"
    ...
    if (seal.get("schema_version")!="msae_independent_source_v8_seal_v1"
            or seal.get("status")!="independent_source_ready_for_future_prescore_protocol"
            or seal.get("source_status")!="project_source_use_unseen_before_v8"
            or seal.get("plan_sha256")!=sha_file(ROOT/"docs/plan-msae-independent-source-v8.md")
            or seal.get("plan_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v8_plan_review.md")
            or seal.get("program_sha256")!=sha_file(Path(__file__))
            or seal.get("acquisition_runner_sha256")!=sha_file(ROOT/"scripts/acquire_msae_independent_source_v8.py")
            or seal.get("test_sha256")!=sha_file(ROOT/"tests/test_prepare_msae_independent_source_v8.py")
    ...
    expected_schemas={
        "baseline_inventory.json":"msae_independent_source_v8_baseline_inventory_v1",
        "source_acquisition_entry.json":"msae_independent_source_v8_source_acquisition_entry_v1",
        "source_acquisition.json":"msae_independent_source_v8_source_acquisition_v1",
        "source_family.json":"msae_independent_source_v8_source_family_v1",
        "candidate_pedigree.json":"msae_independent_source_v8_candidate_pedigree_v1",
        "license.json":"msae_independent_source_v8_license_v1",
        "document_group_census.json":"msae_independent_source_v8_document_group_census_v1",
        "source_manifest.json":"msae_independent_source_v8_source_manifest_v1",
        "dedup.json":"msae_independent_source_v8_dedup_v1",
        "history_manifest.json":"msae_independent_source_v8_baseline_inventory_v1",
        "history_overlap.json":"msae_independent_source_v8_history_overlap_v1",
        "cross_role_overlap.json":"msae_independent_source_v8_cross_role_overlap_v1",
        "support.json":"msae_independent_source_v8_support_v1",
        "role_manifest.json":"msae_independent_source_v8_role_manifest_v1",
        "split_manifest.json":"msae_independent_source_v8_split_manifest_v1",
        "post_process_snapshot.json":"msae_independent_source_v8_process_snapshot_v1",
        "preacquisition_alias_screen.json":"msae_independent_source_v8_preacquisition_alias_screen_v2",
        "historical_source_registry.json":"msae_independent_source_v8_historical_source_registry_v1",
        "preacquisition_authority_manifest.json":"msae_independent_source_v8_preacquisition_authority_v1",
    }
    ...
    if (any(not isinstance(item,dict) or set(item)!={"sent_id","panel","group_rank","within_group_rank",
            ...
            or split.get("schema_version")!="msae_independent_source_v8_split_manifest_v1"
    ...
    if (roles.get("schema_version")!="msae_independent_source_v8_role_manifest_v1"
    ...
    if (gate.get("schema_version")!="msae_independent_source_v8_no_training_gate_v1"
