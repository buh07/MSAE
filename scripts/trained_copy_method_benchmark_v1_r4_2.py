...
CFG_PATH=ROOT/'configs/trained_copy_method_benchmark_v1_r4_2/run.json'
PLAN=ROOT/'PLAN_TRAINED_COPY_METHOD_BENCHMARK_V1_R4_2.md'
SCRIPT=Path(__file__).resolve()
TEST=ROOT/'tests/test_trained_copy_method_benchmark_v1_r4_2.py'
LAUNCHER=ROOT/'scripts/launch_trained_copy_method_benchmark_v1_r4_2_tmux.sh'
...
def recovery_lineage(c:Mapping[str,Any])->dict[str,Any]:
 rec=c.get('recovery',{});pp=ROOT/rec.get('r4_failure_preservation_path','');ff=ROOT/rec.get('r4_freeze_path','');r41=ROOT/rec.get('r4_1_rejected_preservation_path','')
 ...
 if not r41.is_file() or sha(r41)!=rec.get('r4_1_rejected_preservation_sha256'):raise RuntimeError('R4.1 rejection preservation drift')
 y=loadj(r41)
 if y.get('status')!='PRESERVED_BLOCKED_BEFORE_LAUNCH' or y.get('launched') is not False or y.get('scientific_payload_opened') is not False:raise RuntimeError('R4.1 rejection semantics drift')
 ...
 return {'preservation_sha256':sha(pp),'r4_freeze_sha256':sha(ff),'r4_1_rejected_preservation_sha256':sha(r41),'r4_payloads':loadj(ff)['payloads']}
...
def gpu_validate(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->torch.device:
 ...
 if Path(lockdir).name!=f'msae_trained_copy_methods_v1_r4_2_{uuid}.lockdir' or (Path(lockdir)/'token').read_text().strip()!=token:raise RuntimeError('GPU lock mismatch')
...
def write_terminal(c:Mapping[str,Any],prov:Path,state:dict[str,Any],status:str,error:str|None=None,**kw:Any)->None:
 ...
 payload={'schema_version':'trained_copy_method_benchmark_v1_r4_2_terminal','status':status,'phase':state.get('phase'),'panel_accessed':bool(state.get('panel_accessed')),'confirmation_opened':bool(state.get('confirmation_opened')),'no_retry_authorized':True,'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'freeze_sha256':sha(rp(c,'freeze')),'time_ns':time.time_ns(),**kw}
...
def run_pipeline(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->None:
 ...
 try:
  ...
  if not pre['confirmation_authorized']:
   event(prov,16,'CONFIRMATION_BLOCKED_UNOPENED');final={'schema_version':'trained_copy_method_benchmark_v1_r4_2_final','status':'DEVELOPMENT_NOT_AUTHORIZED','confirmation_opened':False,'preconfirmation':pre,'preconfirmation_sha256':sha(out/'PRECONFIRMATION.json')};create_json(out/'final/result.json',final);write_terminal(c,prov,state,'DEVELOPMENT_NOT_AUTHORIZED',final_sha256=sha(out/'final/result.json'));return
...
def mutation_suite()->dict[str,Any]:
 ...
 td=Path('/tmp')/f'tcm_r4_2_review_mutation_{os.getpid()}';td.mkdir(exist_ok=False);review=td/'review.md';review.write_text('VERDICT: BLOCK\nThe required text is VERDICT: SHIP\nBOUND_SHA256: abc\n');review_bypass=False
 try:
