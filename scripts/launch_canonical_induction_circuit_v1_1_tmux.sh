...
PY="$ROOT/.venv-atlas/bin/python"; RUN="$ROOT/scripts/canonical_induction_circuit_v1_1.py"
CONFIG="configs/canonical_induction_circuit_v1_1/run.json"; FREEZE="configs/canonical_induction_circuit_v1_1/FREEZE.json"
PREFIX="msae_induction_v1_1"; OUT="results/canonical_induction_circuit_v1_1_20260809"; PROV="reports/provenance/canonical_induction_circuit_v1_1_run_20260809"
CANDIDATE="reports/adversarial/canonical_induction_circuit_v1_1_candidate_review.md"; FROZEN="reports/adversarial/canonical_induction_circuit_v1_1_frozen_review.md"; BINDING="reports/provenance/canonical_induction_circuit_v1_1_candidate/FROZEN_REVIEW_BINDING.json"
...
GPU_ROW="$(nvidia-smi --query-gpu=index,uuid,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits | awk -F, 'BEGIN{found=0} $3+0 < 4096 && !found {for(i=1;i<=5;i++)gsub(/^ +| +$/,"",$i); row=$1"|"$2"|"$3"|"$4"|"$5; found=1} END{if(found) print row}')"
