...
SAE_SESSION="trained_copy_sae_capacity_v1_r2_20260813"
BRIDGE_SESSION="causal_manifold_bridge_v1_r2_20260813"
SAE_LOG="$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_launcher_20260813.log"
BRIDGE_LOG="$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_launcher_20260813.log"
...
for p in \
  "$ROOT/results/trained_copy_sae_capacity_v1_r2_20260813" \
  "$ROOT/results/causal_manifold_bridge_v1_r2_20260813" \
  "$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_run_20260813" \
  "$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_run_20260813" \
  "$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_LAUNCH.json" \
  "$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_LAUNCH.json"; do
...
launch_one "$SAE_SESSION" trained_copy_sae_capacity_v1_r2.py "$SAE_LOG" 24 "${IDX[0]}" "${UUID[0]}" "${LOCK[0]}" "${TOKEN[0]}"
SAE_PID="$(tmux display-message -p -t "$SAE_SESSION:0.0" '#{pane_pid}')"
if ! launch_one "$BRIDGE_SESSION" causal_manifold_bridge_v1_r2.py "$BRIDGE_LOG" 36 "${IDX[1]}" "${UUID[1]}" "${LOCK[1]}" "${TOKEN[1]}"; then
...
write_manifest "$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_LAUNCH.json" "$SAE_SESSION" "${IDX[0]}" "${UUID[0]}" "$SAE_PID" "${TOKEN[0]}"
write_manifest "$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_LAUNCH.json" "$BRIDGE_SESSION" "${IDX[1]}" "${UUID[1]}" "$BRIDGE_PID" "${TOKEN[1]}"
...
for n in $(seq 1 "$HANDSHAKE_ITERATIONS"); do
  [[ -f "$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_run_20260813/events/010_FIT_ACCESS_MAY_HAVE_OCCURRED.json" && -f "$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_run_20260813/events/010_FIT_ACCESS_MAY_HAVE_OCCURRED.json" ]] && break
...
[[ -f "$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_run_20260813/events/010_FIT_ACCESS_MAY_HAVE_OCCURRED.json" && -f "$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_run_20260813/events/010_FIT_ACCESS_MAY_HAVE_OCCURRED.json" ]] || fail_all 'startup handshake timeout'
...
SAE_WORKER="$(python -c 'import json,sys;print(json.load(open(sys.argv[1]))["pid"])' "$ROOT/reports/provenance/trained_copy_sae_capacity_v1_r2_run_20260813/events/000_RUN_START_NO_PANEL_ACCESS.json")"
BRIDGE_WORKER="$(python -c 'import json,sys;print(json.load(open(sys.argv[1]))["pid"])' "$ROOT/reports/provenance/causal_manifold_bridge_v1_r2_run_20260813/events/000_RUN_START_NO_PANEL_ACCESS.json")"
