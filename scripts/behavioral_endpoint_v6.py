...
import argparse,hashlib,heapq,json,math,os,re,time,traceback
...
ROOT=Path(__file__).resolve().parents[1];SCRIPT=Path(__file__).resolve();DEFAULT=ROOT/'configs/behavioral_endpoint_v6/run.json';PLAN=ROOT/'PLAN_BEHAVIORAL_ENDPOINT_V6.md';TEST=ROOT/'tests/test_behavioral_endpoint_v6.py';LAUNCHER=ROOT/'scripts/launch_behavioral_endpoint_v6_tmux.sh'
...
def preflight(config:Path)->None:
 cfg=loadj(config);preservation_verify(cfg);verify_prepared(cfg);family_check(cfg)
...
def freeze(config:Path)->None:
 cfg=loadj(config);preflight(config);inv=inventory(config,cfg);x={'schema_version':'behavioral_endpoint_v6_freeze','namespace':cfg['namespace'],'config_sha256':sha(config),'candidate_inventory':inv,'candidate_inventory_sha256':hashlib.sha256(canon(inv)).hexdigest(),'v5_preservation_sha256':sha(ROOT/cfg['preservation']['v5_manifest']),'endpoint_only':True,'training':False,'methods':False,'model_specific_confirmation':True,'two_family_barrier':True,'one_shot':True,'retry_authorized':False};exjson(ROOT/cfg['runtime']['freeze'],x);print(json.dumps(x,indent=2))
