#!/usr/bin/env python3
"""Build Atlas v3.4 attempt-8 technical panels without loading model weights."""
...
PRESCORE_PATH = ROOT / "configs/atlas_rope_v4/prescore.json"
RETIREMENT_PATH = RUN_ROOT / "provenance/ATTEMPT7_RETIRED.json"
...
def _prior_project_exposure() -> dict[str, Any]:
    ...
    for path in sorted((ROOT / "data").rglob("inference_units.jsonl"), key=lambda p: str(p).encode("utf-8")):
        if "atlas_rope_v4_attempt8" in path.parts:
            continue
...
def _build_gum_sentinel(root: Path) -> dict[str, Any]:
    ...
    for pair_id in pair_ids:
        ...
        row_ids = [f"atlas_rope_v4:GUM_sentinel:{pair_id}:{role}" for role in roles]
    ...
    manifest = {
        "schema_version": "atlas_rope_v4_attempt8_fresh_gum_sentinel_v1",
...
def build(config_path: Path, output_root: Path) -> dict[str, Any]:
    ...
    if config.get("schema_version") != PRESCORE_SCHEMA or config.get("status") != "draft_no_model_calls_authorized":
        raise RuntimeError("attempt-8 prescore config is not the no-model-calls draft")
    ...
    if runtime_probe.get("schema_version") != "atlas_rope_v4_attempt8_runtime_probe_v1" or runtime_probe.get("status") != "PASS_NO_WEIGHT_NO_FORWARD":
    ...
    try:
        ...
        manifest = {
            "schema_version": "atlas_rope_v4_attempt8_prescore_manifest_v1",
...
def compare_rebuild(primary: Path, rebuilt: Path, report_path: Path) -> dict[str, Any]:
    ...
    report = {"schema_version": "atlas_rope_v4_attempt8_rebuild_check_v1", "status": status,
...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "reports/atlas_rope_v4/prescore_rebuild_check.json")
