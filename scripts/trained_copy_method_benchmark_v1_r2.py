...
CFG_PATH=ROOT/'configs/trained_copy_method_benchmark_v1_r2/run.json'
PLAN=ROOT/'PLAN_TRAINED_COPY_METHOD_BENCHMARK_V1_R2.md'
...
def panel_rows(stage:str,c:Mapping[str,Any],avoid_sham:set[tuple[int,int,int]]|None=None)->list[dict[str,Any]]:
 ...
 for block in range(pc['blocks']):
  for within in range(pc['rows_per_block']):
   ...
   off=pool[(query+replicate)%len(pool)] if stage=='fit' else pool[query%len(pool)];contrast=(target+off)%c['model']['vocab'];choices=[]
   ...
   _,sham,sham_tuple=choices[0];used_sham.add(sham_tuple);rs=seed32('trained_copy_methods_r2',stage,pc['seed'],ix);g=rng('rows',rs);vals=g.integers(0,c['model']['vocab'],size=c['model']['slots']).tolist();vals[query]=target;cor=vals.copy();cor[query]=contrast;sv=vals.copy();sv[query]=sham
   rows.append({'row_id':f'tcm-r2-{stage}-{ix:05d}-{rs:08x}','row_seed':rs,'stage':stage,'block':block,'query':query,'target':target,'contrast':contrast,'sham':sham,'offset':off,'clean_values':vals,'corrupt_values':cor,'sham_values':sv})
 return rows
...
def panel_audit(ps:Mapping[str,Sequence[Mapping[str,Any]]],c:Mapping[str,Any])->dict[str,Any]:
 ...
 for st,rows in ps.items():
  ...
  out[st]={'rows':len(rows),'unique_main_tuples':len(set(mt)),'unique_sham_tuples':len(set(sht)),'max_main_tuple_contribution':max(mt.count(x) for x in set(mt)),'max_sham_tuple_contribution':max(sht.count(x) for x in set(sht)),'target_query_pairs':len(set((r['target'],r['query']) for r in rows)),'main_tuple_sha256':hashlib.sha256(canon(sorted(mt))).hexdigest(),'sham_tuple_sha256':hashlib.sha256(canon(sorted(set(sht)))).hexdigest()}
...
def empirical_jacobian_diagnostics(model:r5.TargetMLP,x:torch.Tensor,targets:torch.Tensor,target_count:int,device:torch.device)->dict[str,Any]:
 ...
 return {'empirical_jacobian_ranks':ranks,'empirical_jacobian_union_rank':int(np.linalg.matrix_rank(np.concatenate(jacs,axis=0),tol=1e-5))}
...
def fit_methods(model:r5.CopyTransformer,rows:Sequence[Mapping[str,Any]],c:Mapping[str,Any],model_seed:int,device:torch.device,out:Path)->tuple[list[Method],dict[str,Any]]:
 ...
 for sd in sc['seeds']:
  ...
  with torch.no_grad():
   codes=sae.encode(sae_states);un=stable_top(codes.var(0,unbiased=False),sc['selected_features']);paired=stable_top(torch.mean(torch.abs(sae.encode(z['clean'])-sae.encode(z['corrupt'])),0),sc['selected_features']);full_fit_reconstruction_loss=float(F.mse_loss(sae(sae_states),sae_states).cpu());zc=sae.encode(z['corrupt']);base=sae.decode(zc);delta_norms={}
...
def compatibility_smoke(c:Mapping[str,Any],ps:Mapping[str,Sequence[Mapping[str,Any]]],device:torch.device)->dict[str,Any]:
 ...
 sc['model']['seeds']=[99993];model=r5.CopyTransformer(sc,99993,device).eval();td=Path('/tmp')/f'tcm_v1_r2_smoke_{os.getpid()}_{str(device).replace(":","_")}'
 ...
 try:
  ...
  jac=fit['fits']['target_nonlinear_rank4_seed8301']['empirical_jacobian_ranks'];dev=eval_methods(model,ps['development'],methods,sc,'development',device);reconciled=reconcile_records(sc,td,{'99993':fit},{'99993':dev})
  return {'families':sorted(set(m.family for m in methods)),'methods':len(methods),'prediction_finite':finite,'jacobian_targets':len(jac),'fit_artifacts':len(reconciled['artifacts']),'record_reconciliation_pass':reconciled['expected_method_count']==len(methods)}
...
def verify_candidate_lineage(c:Mapping[str,Any],require_review:bool=False,require_lock:bool=False)->dict[str,Any]:
 ...
 x=loadj(p);expected={'source_sha256':sha(SCRIPT),'config_sha256':sha(CFG_PATH),'plan_sha256':sha(PLAN),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256']}
 ...
 if require_review:
  review=rp(c,'candidate_review')
  if not review.is_file() or 'VERDICT: SHIP' not in review.read_text() or sha(p) not in review.read_text():raise RuntimeError('candidate review binding invalid')
 if require_lock:
  lock=rp(c,'generator_lock')
  if not lock.is_file():raise RuntimeError('missing generator lock')
  z=loadj(lock);now={'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'plan_sha256':sha(PLAN),'candidate_sha256':sha(p),'candidate_review_sha256':sha(rp(c,'candidate_review')),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256']}
...
def freeze()->dict[str,Any]:
 ...
 f={'schema_version':'trained_copy_method_benchmark_v1_r2_freeze','config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'plan_sha256':sha(PLAN),'generator_lock_sha256':sha(rp(c,'generator_lock')),'candidate_sha256':sha(rp(c,'candidate_manifest')),'candidate_review_sha256':sha(rp(c,'candidate_review')),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256'],'r5_preservation_sha256':c['r5']['preservation_sha256'],'r5_checkpoints':c['r5']['checkpoints'],'payloads':payloads,'payloads_opened':False};create_json(rp(c,'freeze'),f);return f
def verify_freeze(opaque:bool)->dict[str,Any]:
 c=cfg();verify_candidate_lineage(c,True,True);f=loadj(rp(c,'freeze'));expected={'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'plan_sha256':sha(PLAN),'generator_lock_sha256':sha(rp(c,'generator_lock')),'candidate_sha256':sha(rp(c,'candidate_manifest')),'candidate_review_sha256':sha(rp(c,'candidate_review')),'method_table_sha256':method_table_hash(c),'r5_source_sha256':c['r5']['source_sha256'],'r5_preservation_sha256':c['r5']['preservation_sha256'],'r5_checkpoints':c['r5']['checkpoints']}
...
def gpu_validate(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->torch.device:
 ...
 if Path(lockdir).name!=f'msae_trained_copy_methods_v1_r2_{uuid}.lockdir' or (Path(lockdir)/'token').read_text().strip()!=token:raise RuntimeError('GPU lock mismatch')
...
def write_terminal(c:Mapping[str,Any],prov:Path,state:dict[str,Any],status:str,error:str|None=None,**kw:Any)->None:
 ...
 payload={'schema_version':'trained_copy_method_benchmark_v1_r2_terminal','status':status,'phase':state.get('phase'),'panel_accessed':bool(state.get('panel_accessed')),'confirmation_opened':bool(state.get('confirmation_opened')),'no_retry_authorized':True,'config_sha256':sha(CFG_PATH),'source_sha256':sha(SCRIPT),'freeze_sha256':sha(rp(c,'freeze')),'time_ns':time.time_ns(),**kw}
...
def run_pipeline(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->None:
 ...
 def on_signal(signum: int,_frame:Any)->None:
  status='TECHNICAL_FAILURE_AFTER_ACCESS' if state['panel_accessed'] else 'TECHNICAL_FAILURE_BEFORE_ACCESS';write_terminal(c,prov,state,status,f'signal {signal.Signals(signum).name}');raise SystemExit(128+signum)
 ...
 try:
  ...
  if not pre['confirmation_authorized']:
   event(prov,16,'CONFIRMATION_BLOCKED_UNOPENED');final={'schema_version':'trained_copy_method_benchmark_v1_r2_final','status':'DEVELOPMENT_NOT_AUTHORIZED','confirmation_opened':False,'preconfirmation':pre,'preconfirmation_sha256':sha(out/'PRECONFIRMATION.json')};create_json(out/'final/result.json',final);write_terminal(c,prov,state,'DEVELOPMENT_NOT_AUTHORIZED',final_sha256=sha(out/'final/result.json'));return
...
def mutation_suite()->dict[str,Any]:
 ...
 checks={'offset_overlap_detected':overlap,'duplicate_sham_detected':duplicate_sham,'missing_target_query_pair_detected':missing_pair,'all_tuple_multiplicities_one':all(a[s]['max_main_tuple_contribution']==1 and a[s]['max_sham_tuple_contribution']==1 for s in ('fit','development','confirmation')),'expected_method_inventory_nonempty':len(expected_method_names(c))==66,'r5_source_bound':sha(ROOT/c['r5']['source_path'])==c['r5']['source_sha256'],'r5_row_identity_disjoint':a['cross_r5']['row_id_overlap']==0 and a['cross_r5']['row_seed_overlap']==0}
 return {'status':'PASS' if all(checks.values()) else 'FAIL','checks':checks,'panel_audit':a}
def main()->None:
