#!/usr/bin/env python3
"""Source-only supervisor for the single reviewed gen8 tmux extinction test.
...
SELF = ROOT / "scripts/run_msae_independent_measurement_v3_post_m2_gen8_tmux_test.py"
TERM_TIMEOUT = 3.0
...
def _validate_identity(value: dict[str, Any], pane_pid: int,
    ...
    if set(value) != expected_keys or value.get("schema_version") != \
            "msae_v3_gen8_tmux_test_process_v1":
...
def _write_test_terminal(scratch: Path, *, owner: str, reason: str,
    ...
    values = (
        ("terminal.claim.test", {
            "schema_version": "msae_v3_gen8_tmux_test_terminal_claim_v1",
            "owner": owner, "pane_pid": pane_pid}),
        ("technical_failure.test.json", {
            "schema_version": "msae_v3_gen8_tmux_test_technical_failure_v1",
...
def _cleanup_record_payload(raw: bytes, *, final_name: str,
    ...
    if final_name == "identity_bound.json":
        return (set(value) == {"schema_version", "process_sha256", "supervisor_pid"}
                and value.get("schema_version") ==
                    "msae_v3_gen8_tmux_test_identity_bound_v1"
    ...
    if final_name == "terminal.claim.test":
        return (set(value) == {"schema_version", "owner", "pane_pid"}
                and value.get("schema_version") ==
                    "msae_v3_gen8_tmux_test_terminal_claim_v1"
    ...
    if final_name == "technical_failure.test.json":
        return (set(value) == {"schema_version", "reason", "pane_pid"}
                and value.get("schema_version") ==
                    "msae_v3_gen8_tmux_test_technical_failure_v1"
...
# Gen8 registered 162-case outer supervisor.  The older pure helper functions
...
G7_PLAN = ROOT / "docs/plan-msae-independent-measurement-v3-post-m8-gen8.md"
G7_IMPLEMENTATION = (
    ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen8.py",
    ROOT / "scripts/msae_independent_measurement_v3_post_m2_gen8_runtime.py",
    ROOT / "scripts/run_msae_independent_calibration_v3_gen8.py",
    ROOT / "scripts/launch_msae_independent_calibration_v3_gen8.sh",
    SELF,
    ROOT / "docs/rfc-msae-independent-measurement-v3-post-m8-gen8.md",
    ROOT / "tests/test_msae_independent_measurement_v3_post_m8_gen8.py",
)
G7_CONTROL_SCHEMA = "msae_v3_gen8_test_control_frame_v1"
G7_TEST_NODE = ("tests/test_msae_independent_measurement_v3_post_m8_gen8.py::"
...
def _g7_validate_owner_ledger(
    ...
    if len(case) == 3:
        ...
        if (set(value) != expected_keys
                or value.get("schema_version") !=
                   "msae_v3_gen8_tmux_fault_case_v1"
        ...
        schema_checks = {
            ...
            "schema_version": value.get("schema_version") ==
                              "msae_v3_gen8_tmux_fault_scenario_v1",
    ...
    derivation = {
        "schema_version": "msae_v3_gen8_test_outer_final_derivation_v1",
        "test_token": token,
...
def _g7_test_token() -> str:
    value = {"schema_version": "msae_v3_gen8_test_token_preimage_v1",
...
def _g7_send(sock: socket.socket, frame: dict[str, Any], fd: int | None = None) -> str:
    ...
    if sock.sendmsg([raw], ancillary) != len(raw):
        raise RuntimeError("short gen8 control send")
...
def _g7_recv(sock: socket.socket, token: str, case_id: str, sequence: int,
    ...
    if not raw or ancillary or flags & (socket.MSG_TRUNC | socket.MSG_CTRUNC):
        raise RuntimeError("gen8 control frame/ancillary drift")
    ...
    if raw != _canonical_bytes(value) or set(value) != {
        ...
        raise RuntimeError("gen8 control canonical schema drift")
    if value != _g7_frame(token, case_id, sequence, prior, frame_type,
                          value["payload"]):
        raise RuntimeError("gen8 control frame ordering drift")
...
def _g7_fault_binding(read_fd: int, write_fd: int, *, supervisor_pid: int,
    ...
    return {"schema_version": "msae_v3_gen8_fault_event_binding_v1",
...
def _g7_terminal_hashes(case: tuple[Any, ...], terminal_owner: str) -> tuple[str, str]:
    ...
    claim = {"schema_version": "msae_v3_gen8_tmux_test_terminal_claim_v1",
...
def _g7_pre_exit_subject(
    ...
    tmux_path = Path(f"/tmp/m8x_{token}.sock")
    broker_path = Path(f"/tmp/m8a_{token}.sock")
    pane_path = Path(f"/tmp/m8c_{token}.sock")
    family_entries = _g7_family_entries()
    if (family_entries["m8s_"] != [scratch.name]
            or any(family_entries[prefix] for prefix in family_entries
                   if prefix != "m8s_")):
        raise RuntimeError("pre-exit short-family projection drift")
    return {
        "schema_version": "msae_v3_gen8_test_pre_exit_absence_v1",
        "test_token": token,
        "case_or_scenario_id": (case[1] if len(case) == 3 else case[0]),
        "captured_processes": processes, "process_groups": groups,
        "session": {"name": "msae-independent-v3-gen8-containment",
...
def _g7_scenario_action_projection(
    ...
    target: dict[str, Any] = {
        "schema_version": "msae_v3_gen8_scenario_process_v1",
    ...
    if scenario in {"S02_server_death", "S03_pane_death",
                    "S11_kill_escalation"}:
        target = {
            "schema_version": "msae_v3_gen8_scenario_process_group_v1",
        ...
        target = {
            "schema_version": "msae_v3_gen8_scenario_deadline_v1",
...
def _g7_validate_monitor_frame(value: Any, *, frame_type: str,
    ...
    if (not isinstance(value, dict)
            or set(value) != {"schema_version", "protocol_id", "generation",
                              ...
                              "payload"}
            or value.get("schema_version") != "msae_v3_gen8_monitor_frame_v1"
    ...
    if scenario in {"S06_duplicate_frame", "S07_late_frame"}:
        ...
        if (set(observation) != keys
                or observation.get("schema_version") !=
                   "msae_v3_gen8_monitor_frame_rejection_v1"
        ...
        if (set(observation) != keys
                or observation.get("schema_version") !=
                   "msae_v3_gen8_monitor_frame_receive_timeout_v1"
...
def _g7_fault_event(sock: socket.socket, token: str, case: tuple[Any, ...],
    ...
    if (value.get("schema_version") != "msae_v3_gen8_fault_event_v1"
...
def _g7_family_entries() -> dict[str, list[str]]:
    prefixes = ("m8t_", "m8b_", "m8p_", "m8x_", "m8a_", "m8c_", "m8s_")
...
def _g7_final_has_session_probe(tmux_path: Path) -> int:
    vector = ["/usr/bin/tmux", "-f", "/dev/null", "-S", str(tmux_path),
              "has-session", "-t", "msae-independent-v3-gen8-containment"]
...
def _g7_emergency_containment(
    ...
    # failing result.
    for path in (Path(f"/tmp/m8x_{token}.sock"),
                 Path(f"/tmp/m8a_{token}.sock"),
                 Path(f"/tmp/m8c_{token}.sock")):
        if not _path_present(path):
            continue
        st = path.lstat()
    ...
    try:
        for case in _g7_cases():
            ...
            if time.monotonic() >= global_deadline:
                raise TimeoutError("global gen8 tmux matrix deadline")
            case_id = str(case[1] if len(case) == 3 else case[0])
            ...
            capture_thread = threading.Thread(
                target=capture_loop, name="msae-gen8-outer-capture",
                daemon=True)
            ...
            final_subject = {
                "schema_version": "msae_v3_gen8_test_outer_final_absence_v1",
        ...
        success = False
        print(f"gen8 tmux matrix failed: case={active_case!r}: {error}",
              file=sys.stderr)
        traceback.print_exc()
        scratch = Path(f"/tmp/m8s_{token}")
        if _path_present(scratch):
...
if __name__ == "__main__":
    raise SystemExit(main_gen8_matrix())
