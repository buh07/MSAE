...
import argparse, ast, builtins, hashlib, json, math, os, random, secrets, shutil, signal, subprocess, sys, tempfile, time
...
CAP_CFG=ROOT/'configs/capacity_controller_diagnostic_v1_r4/run.json'
EXT_CFG=ROOT/'configs/trained_copy_external_v1_r4/run.json'
PLAN=ROOT/'PLAN_CAPACITY_EXTERNAL_VALIDITY_V1_R4.md'
PLAN_REVIEW=ROOT/'reports/adversarial/capacity_external_validity_v1_r4_plan_review.md'
...
SCRIPT=ROOT/'scripts/capacity_external_validity_v1_r4.py'
LAUNCHER=ROOT/'scripts/launch_capacity_external_validity_v1_r4_tmux.sh'
TEST=ROOT/'tests/test_capacity_external_validity_v1_r4.py'
SESSION_NAME='capacity_external_validity_v1_20260810r4'
GPU_LOCK_GLOB='msae_capacity_external_r4_*.lockdir'
...
def _is_jsonl_path(value:Any)->bool:
 try:return Path(os.fspath(value)).suffix.lower()=='.jsonl'
 except (TypeError,ValueError):return False
@contextmanager
def no_jsonl_access_guard():
 orig_open,orig_os_open,orig_stat,orig_lstat,orig_scandir,orig_listdir,orig_popen=builtins.open,os.open,os.stat,os.lstat,os.scandir,os.listdir,subprocess.Popen
 ...
 with mock.patch.object(builtins,'open',guarded_open),mock.patch.object(os,'open',guarded_os_open),mock.patch.object(os,'stat',guarded_stat),mock.patch.object(os,'lstat',guarded_lstat),mock.patch.object(os,'scandir',guarded_scandir),mock.patch.object(os,'listdir',guarded_listdir),mock.patch.object(subprocess,'Popen',guarded_popen),mock.patch.object(Path,'open',path_guard(orig_path_open,'Path.open')),mock.patch.object(Path,'read_text',path_guard(orig_read_text,'read_text')),mock.patch.object(Path,'read_bytes',path_guard(orig_read_bytes,'read_bytes')),mock.patch.object(Path,'stat',path_guard(orig_pstat,'Path.stat')),mock.patch.object(Path,'glob',guarded_glob),mock.patch.object(Path,'rglob',guarded_rglob):
  yield
...
def verify_parity()->dict[str,Any]:
 ...
 if forbidden_restore:raise RuntimeError(f'checkpoint/state restore path forbidden in r4: {forbidden_restore}')
 ...
 return {'status':'PASS','config_pairs':pairs,'scientific_symbols_verified':scientific,'exact_fit_ast_sha256':{'v1':EXPECTED_V1_FIT_AST_SHA256,'derived_r4':_ast_sha(expected_fit)},'restore_paths_forbidden':True,'source_allowlist':{'compatibility':['first_true_index','empirical_jacobian_ranks','deterministic v1 fit_methods rewrite'],'recovery_lifecycle':sorted(allowed_common_drift-{'fit_methods'})},'v1_source_sha256':sha(V1_SOURCE),'r4_source_sha256':sha(SCRIPT)}
...
def candidate_files()->list[Path]:return [PLAN,PLAN_REVIEW,PRESERVE,FAILURE_PRESERVE,INVALID_R2_PRESERVE,INVALID_R3_PRESERVE,ROOT/'scripts/preserve_capacity_external_validity_v1_failure.py',ROOT/'PAPER.md',ROOT/'reports/paper_claim_ledger_v1.json',CAP_CFG,EXT_CFG,SCRIPT,LAUNCHER,TEST]
...
def compatibility_smoke(device_name:str)->dict[str,Any]:
 ...
 rows=make_capacity_rows(97991,'smoke',c,16,16,97992);dev=make_capacity_rows(97991,'smoke',c,24,8,97993);td=Path('/tmp')/f'msae_capacity_external_r4_compat_{os.getpid()}_{device_name.replace(":","_")}';ckpt=td/'checkpoints'
...
def recovery_environment_absence()->dict[str,Any]:
 ...
 if tmux_exists or locks:raise RuntimeError(f'r4 runtime namespace preexists: tmux={tmux_exists} locks={locks}')
...
def postcandidate_absence_paths(include_locks:bool=True)->list[Path]:
 ...
 paths.extend([ROOT/'reports/provenance/capacity_external_validity_v1_r4_REVIEW_BINDING.json',ROOT/'reports/provenance/capacity_external_validity_v1_r4_launcher_20260810r4.log'])
 return paths
...
def _candidate_preflight_impl(write:bool=True,device_name:str='cpu')->dict[str,Any]:
 ...
 extras=[ROOT/'reports/provenance/capacity_external_validity_v1_r4_REVIEW_BINDING.json',ROOT/'reports/provenance/capacity_external_validity_v1_r4_launcher_20260810r4.log']
 ...
 if not all(x['absent'] for x in absent):raise RuntimeError(f'r4 namespace preexists {[x for x in absent if not x["absent"]]}')
 result={'status':'PASS','preservation':pv,'failure_preservation':fp,'parity':parity,'paper':pap,'basis_checks':checks,'nonpanel_ols_max_abs':err,'timed_smoke':smoke,'compatibility_smoke':compatibility,'r4_absence_checks':absent,'environment_absence':env_absence,'scientific_jsonl_accessed':False,'files':inventory(candidate_files())}
 if write:
  for kind in ('capacity','external'):
...
def launcher_render(uuid:str)->str:
 return f'export CUDA_VISIBLE_DEVICES="$GPU_UUID"\n[[ "$GPU_UUID" == "{uuid}" ]]\ncd "$ROOT"\nRUN_ROOT=$(readlink /proc/self/cwd)\nMSAE_ROOT="$RUN_ROOT" python "$RUN_ROOT/scripts/capacity_external_validity_v1_r4.py" run-pipeline --physical-index "$PHYSICAL_INDEX" --gpu-uuid "$GPU_UUID" --launch-token "$TOKEN" --pane-pid "$$" --gpu-lockdir "$LOCKDIR"'
...
def create_locks()->dict[str,Any]:
 ...
 failure=failure_preservation_verify();shared={'created_ns':time.time_ns(),'candidate_reviews':[cr,er],'candidate_manifests':[cm,em],'payloads_preexisting':True,'recovery_binding':True,'both_bindings_before_r4_access':True,'failure_preservation_sha256':failure['manifest_sha256'],'v1_lineage':current_v1_lineage(),'invalid_r2_candidate_preservation_sha256':sha(INVALID_R2_PRESERVE),'invalid_r3_candidate_preservation_sha256':sha(INVALID_R3_PRESERVE),'v1_partial_checkpoints_reused':False}
 atomic_json(rp(cc,'generator_lock'),{'schema_version':'capacity_recovery_binding_v1_r4',**shared,'config_sha256':sha(CAP_CFG)});atomic_json(rp(ec,'generator_lock'),{'schema_version':'external_recovery_binding_v1_r4',**shared,'config_sha256':sha(EXT_CFG)})
...
def locks_valid()->None:
 ...
 for kind,c,p in [('capacity',cc,CAP_CFG),('external',ec,EXT_CFG)]:
  ...
  if x.get('schema_version')!=f'{kind}_recovery_binding_v1_r4' or x.get('config_sha256')!=sha(p) or not x.get('payloads_preexisting') or not x.get('recovery_binding') or not x.get('both_bindings_before_r4_access') or x.get('v1_partial_checkpoints_reused') is not False:raise RuntimeError('invalid recovery binding')
  if x.get('failure_preservation_sha256')!=failure['manifest_sha256'] or x.get('invalid_r2_candidate_preservation_sha256')!=sha(INVALID_R2_PRESERVE) or x.get('invalid_r3_candidate_preservation_sha256')!=sha(INVALID_R3_PRESERVE) or x.get('v1_lineage')!=current_v1_lineage():raise RuntimeError('preserved lineage drift')
...
def freeze_one(kind:str)->dict[str,Any]:
 c=cfg(kind);p=CAP_CFG if kind=='capacity' else EXT_CFG;prep=rp(c,'prepared_root');metadata=loadj(prep/'PANEL_METADATA.json');payload=payload_records_from_metadata(metadata);base=[PLAN,PLAN_REVIEW,PRESERVE,FAILURE_PRESERVE,INVALID_R2_PRESERVE,INVALID_R3_PRESERVE,ROOT/'scripts/preserve_capacity_external_validity_v1_failure.py',p,SCRIPT,LAUNCHER,TEST,rp(c,'generator_lock'),prep/'PANEL_METADATA.json',prep/'PREPARED.json']
...
def bind_reviews()->dict[str,Any]:
 recovery_environment_absence();require_absent('review binding phase',[rp(cfg(k),'output_root') for k in ('capacity','external')]+[rp(cfg(k),'provenance_root') for k in ('capacity','external')]+[binding_path(),ROOT/'reports/provenance/capacity_external_validity_v1_r4_launcher_20260810r4.log'])
 ...
 out=rp(cfg('capacity'),'provenance_root').parent/'capacity_external_validity_v1_r4_REVIEW_BINDING.json';atomic_json(out,{'schema_version':'capacity_external_review_binding_v1_r4','reviews':rows,'created_ns':time.time_ns()});return {'status':'PASS','path':out.relative_to(ROOT).as_posix(),'sha256':sha(out)}
def binding_path()->Path:return rp(cfg('capacity'),'provenance_root').parent/'capacity_external_validity_v1_r4_REVIEW_BINDING.json'
@jsonl_guarded
...
def gpu_validate(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->tuple[torch.device,dict[str,Any]]:
 ...
 expected_lock=Path('/tmp')/f'msae_capacity_external_r4_{uuid}.lockdir';actual_lock=Path(lockdir)
