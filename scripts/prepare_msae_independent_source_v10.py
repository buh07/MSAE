#!/usr/bin/env python3
"""Source-readiness builder for MSAE v10; standard-library and model-free."""
...
RAW = ROOT / "data/msae_independent_source_v10/raw" / COMMIT
PRIVATE = ROOT / "data/msae_independent_source_v10/private"
PROV = ROOT / "reports/provenance/msae_independent_source_v10"
QUARANTINES = {
...
V10_PREFIX = "data/msae_independent_source_v10/"
V9_DATA_PREFIX = "data/msae_independent_source_v9/"
V8_DATA_PREFIX = "data/msae_independent_source_v8/"
CARRYOVER_PATH = "reports/provenance/msae_independent_source_v10/v9_carryover_authority.json"
CARRYOVER_SHA256 = "da13ae40e64c9828b2d08c29b4ff280bac5c71eeb8578a977f953f2169f7585d"
V9_CONTROL_PATHS = {
    ...
    "reports/adversarial/msae_independent_source_v9_terminal_failure_review.md",
    ...
    "reports/provenance/msae_independent_source_v9/document_group_census.json",
...
V10_AUTHORITY = {
    ...
    "reports/adversarial/msae_independent_source_v10_preacquisition_implementation_review.md",
    "reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md",
}
V10_FROZEN_AUTHORITY_SHA256 = {
    "docs/plan-msae-independent-source-v10.md":
...
TERMINAL_B2_RECONSTRUCTION_AUTHORIZED = False
...
def open_repo_file(path: Path) -> int:
    ...
    is_v10_raw=path.is_relative_to(REAL_V10_RAW)
    if is_v10_raw and not RAW_ACCESS_AUTHORIZED:
        raise GateFailure("v10_raw_open_before_scientific_entry")
    try:rel=path.relative_to(ROOT)
    except ValueError:
        fd=os.open(path,os.O_RDONLY|getattr(os,"O_NOFOLLOW",0))
        opened=os.fstat(fd)
        if not stat.S_ISREG(opened.st_mode):os.close(fd);raise GateFailure("nonregular_open")
        if is_v10_raw:RAW_FILE_OPEN_COUNT+=1
        return fd
...
def scientific_artifact_inventory() -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v10_scientific_artifact_inventory_v1",
            "entries":entries}
...
def classify_private_terminal_state(payload: list[dict[str, Any]]) -> str:
    """Return the exact classifier-proven private state without reading payload content."""
    if len(payload) != 2:
        raise GateFailure("terminal_state_cardinality")
    final,temporary=payload
    if temporary.get("state") != "absent":
        raise GateFailure("terminal_state_cardinality")
    if not present_path(PRIVATE):
        if final.get("state") != "absent":
            raise GateFailure("terminal_state_cardinality")
        return "absent"
    st,names=directory_snapshot_nofollow(PRIVATE)
    if stat.S_IMODE(st.st_mode) != 0o700:
        raise GateFailure("terminal_state_cardinality")
    if final.get("state") == "absent" and names == []:
        return "empty"
    if (final.get("state") == "present" and final.get("type") == "regular"
            and final.get("mode") == 0o600 and final.get("nlink") == 1
            and _is_sha(final.get("sha256")) and names == ["blind_payload.jsonl"]):
        return "payload"
    raise GateFailure("terminal_state_cardinality")
...
def strict_json(path: Path) -> Any:
    ...
    # presentation; every builder-published v10 final uses canonical_file_bytes.
...
def verify_process_snapshot_schema(value: Any, code: str) -> None:
    ...
    if (value["schema_version"] != "msae_independent_source_v10_process_snapshot_v1"
...
def validate_acquisition_config(path: Path) -> dict[str, Any]:
    ...
    if (value.get("schema_version") != "msae_independent_source_v10_acquisition_v1"
...
def verify_v9_carryover(*, hash_raw: bool = False) -> dict[str, Any]:
    """Verify the reviewed V9 terminal carryover without opening predecessor raw bytes."""
    path = ROOT / CARRYOVER_PATH
    regular_state_nofollow(path, mode=0o644, nlink=1)
    if sha_file(path) != CARRYOVER_SHA256:
        raise GateFailure("v9_carryover_authority_drift")
    value = strict_json(path)
    exact_keys(value, {
        "schema_version", "status", "v9_source_commit", "v9_control_files",
        "v9_control_map_sha256", "v9_data_directories", "v9_raw_files",
        "v9_raw_file_map_sha256", "v9_acquisition_sha256", "v9_scientific_entry_sha256",
        "v9_partial_scientific_prefix", "v9_rejection_sha256", "v9_rejection_failure_code",
        "v9_terminal_verify_status", "v9_terminal_review_sha256",
        "v9_source_content_semantically_read_by_v9_builder",
        "v9_source_content_semantically_read_for_this_record", "v9_source_content_printed",
        "opaque_raw_hash_reads_for_this_record", "quarantine_content_reads",
        "model_operations", "gpu_queries", "training_runs", "model_scoring_authorized",
        "k2_or_branch_training_authorized", "stage_c_authorized",
    }, "v9_carryover_authority_schema")
    if (value["schema_version"] != "msae_independent_source_v10_v9_carryover_authority_v1"
            or value["status"] != "retained_v9_scientific_rejection_and_terminal_verifier_defect"
            or value["v9_source_commit"] != COMMIT
            or value["v9_control_map_sha256"] != "d0035594710b62cd9b998367136f2bf02110bf59095d359b0f2dbdd75e1aec3c"
            or value["v9_raw_file_map_sha256"] != "881ac46eab4a9e4ecf50a2afb088d37ac98f060b84a45ede90d79007e216f363"
            or value["v9_acquisition_sha256"] != "d353a5a04d8c853ea255526c20742a5e4c3578f672bc5d8cdd889cb193cff837"
            or value["v9_scientific_entry_sha256"] != "0ccb9df33f63e06dfd610308793bc926aa2be2c4fd71b30d4cf83493d5ef2f05"
            or value["v9_partial_scientific_prefix"] != ["document_group_census.json"]
            or value["v9_rejection_sha256"] != "91c1e28f8f6f1e6efebef44875e4460848a5007360287787f715d8dea8a8896b"
            or value["v9_rejection_failure_code"] != "missing_or_unknown_field"
            or value["v9_terminal_verify_status"] != "failed_post_baseline_inventory_path_drift"
            or value["v9_terminal_review_sha256"] != "bafa2fbcc1c4ba4e6f3ffe9c696e63f4aa846d5b12a8922bbaa3ac36a770af28"
            or value["v9_source_content_semantically_read_by_v9_builder"] is not True
            or value["v9_source_content_semantically_read_for_this_record"] is not False
            or value["v9_source_content_printed"] is not False
            or value["opaque_raw_hash_reads_for_this_record"] != 0
            or value["quarantine_content_reads"] != 0
        ...
        raise GateFailure("v9_carryover_authority_drift")
    controls = value["v9_control_files"]
    if (not isinstance(controls, list) or len(controls) != 20
            or [item.get("path") for item in controls] != sorted(V9_CONTROL_PATHS)):
        raise GateFailure("v9_control_universe_drift")
    control_map: dict[str, str] = {}
    for item in controls:
        exact_keys(item, {"path", "device", "inode", "mode", "nlink", "size", "sha256"},
                   "v9_control_record_schema")
        ...
        if (not stat.S_ISREG(current.st_mode) or stat.S_ISLNK(current.st_mode)
            ...
            raise GateFailure("v9_control_file_drift")
        control_map[item["path"]] = item["sha256"]
    if sha_bytes(_canonical_ascii_bytes(control_map)) != value["v9_control_map_sha256"]:
        raise GateFailure("v9_control_map_drift")
    directories = value["v9_data_directories"]
    expected_dirs = ["data/msae_independent_source_v9", "data/msae_independent_source_v9/raw",
                     f"data/msae_independent_source_v9/raw/{COMMIT}"]
    if [item.get("path") for item in directories] != expected_dirs:
        raise GateFailure("v9_data_directory_universe_drift")
    for item in directories:
        exact_keys(item, {"path", "device", "inode", "mode", "nlink", "size"},
                   "v9_data_directory_schema")
        current = (ROOT / item["path"]).lstat()
        if (not stat.S_ISDIR(current.st_mode) or stat.S_ISLNK(current.st_mode)
                or current.st_dev != item["device"] or current.st_ino != item["inode"]
                or stat.S_IMODE(current.st_mode) != item["mode"] or current.st_nlink != item["nlink"]
                or current.st_size != item["size"]):
            raise GateFailure("v9_data_directory_drift")
    raw = value["v9_raw_files"]
    expected_names = sorted((*SOURCE_FILES.values(), "LICENSE.txt"))
    raw_dir = ROOT / f"data/msae_independent_source_v9/raw/{COMMIT}"
    if (not isinstance(raw,list) or len(raw)!=4
            or [item.get("path", "").rsplit("/", 1)[-1] for item in raw] != expected_names):
        raise GateFailure("v9_raw_universe_drift")
    _raw_stat,current_raw_names=directory_snapshot_nofollow(raw_dir)
    if current_raw_names != expected_names:
        raise GateFailure("v9_raw_extra_path")
    raw_map: dict[str, dict[str, Any]] = {}
    for item in raw:
        exact_keys(item, {"path", "device", "inode", "mode", "nlink", "size", "sha256", "git_blob_sha1"},
                   "v9_raw_record_schema")
        current = (ROOT / item["path"]).lstat()
        if (not stat.S_ISREG(current.st_mode) or stat.S_ISLNK(current.st_mode)
                or current.st_dev != item["device"] or current.st_ino != item["inode"]
                or stat.S_IMODE(current.st_mode) != item["mode"] or current.st_nlink != item["nlink"]
                or current.st_size != item["size"]):
            raise GateFailure("v9_raw_metadata_drift")
    ...
    if (_directory_fingerprint(_raw_stat)!=_directory_fingerprint(raw_after)
        ...
        raise GateFailure("v9_raw_metadata_drift")
    if sha_bytes(_canonical_ascii_bytes(raw_map)) != value["v9_raw_file_map_sha256"]:
        raise GateFailure("v9_raw_map_drift")
    return value


def _predecessor_hashes(*, hash_raw: bool = False) -> dict[str, str]:
    carryover = verify_v9_carryover(hash_raw=hash_raw)
    return dict(sorted((item["path"],item["sha256"]) for item in carryover["v9_control_files"]))
...
def sentence_labels(sentence: Sentence) -> list[dict[str, str | None]]:
    ...
    for tok in sentence.tokens:
        ...
        out.append({
            ...
            "token_identity": tok.form.casefold(),
            "lemma_identity": None if tok.lemma == "_" else tok.lemma.casefold(),
...
def group_split_key(group_id: str, source_commit: str = COMMIT) -> str:
    ...
    return sha_bytes(canonical_bytes(
        ["msae-independent-source-v10/C1C2-group", source_commit, group_id]))
...
def build_split_manifest(rows: list[dict[str, Any]], dedup: dict[str, Any],
    ...
    split={"schema_version":"msae_independent_source_v10_split_manifest_v1","source_commit":COMMIT,
...
def payload_record(row: dict[str, Any]) -> dict[str, Any]:
    return {"schema_version":"msae_independent_source_v10_payload_v1","source_repo":REPO,
...
def public_label(task: str, value: str) -> str:
    if task == "token_identity": return sha_bytes(("msae-v10/token\0" + value).encode())
    if task == "lemma_identity": return sha_bytes(("msae-v10/lemma\0" + value).encode())
    return value
...
def _walk_descriptor_paths(*, include_v10: bool, reject_v10_namespace: bool
            ...
            for name in names:
                ...
                if stat.S_ISDIR(entry.st_mode):
                    if (name=="__pycache__" or any((rel+"/").startswith(x) for x in GENERATED_PREFIXES)
                            or rel in {V8_DATA_PREFIX.rstrip("/"), V9_DATA_PREFIX.rstrip("/")}
                            or (not include_v10 and rel==V10_PREFIX.rstrip("/"))):
                        if reject_v10_namespace and rel==V10_PREFIX.rstrip("/"):
                            raise GateFailure("preexisting_v10_namespace")
                        continue
                ...
                if reject_v10_namespace and (
                        rel == V10_PREFIX.rstrip("/") or rel.startswith(V10_PREFIX)):
                    raise GateFailure("preexisting_v10_namespace")
                yield ROOT/rel,rel
...
def _walk_paths() -> Iterator[tuple[Path, str]]:
    yield from _walk_descriptor_paths(include_v10=False,reject_v10_namespace=True)
...
def inventory_repository() -> dict[str, Any]:
    ...
    for key in sorted(by_inode, key=lambda z: min(x[1] for x in by_inode[z])):
        ...
        for _p, rel, s in group:
            ...
            elif rel.startswith(V10_PREFIX): item.update(disposition="v10_excluded", adapter="excluded", extracted_unit_count=0)
            ...
            elif rel in V9_CONTROL_PATHS:
                item.update(disposition="v9_candidate_control_carryover", adapter="carryover",
                            extracted_unit_count=0)
            elif rel in V10_AUTHORITY:
                item.update(disposition="v10_authority", adapter="authority", extracted_unit_count=0)
            entries.append(item)
    ...
    return {"schema_version": "msae_independent_source_v10_baseline_inventory_v1",
            ...
            "v9_carryover_authority_sha256": CARRYOVER_SHA256,
            "v9_carryover_status": "opaque_exact_universe_verified",
            "authority_expected_absent": sorted(V10_AUTHORITY - {x["path"] for x in entries}),
...
def deduplicate_roles(roles: dict[str, list[Sentence]]) -> tuple[dict[str, list[Sentence]], dict[str, Any]]:
    ...
    report = {"schema_version": "msae_independent_source_v10_dedup_v1", "roles": {},
...
def _support(rows: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v10_support_v1","floor_distinct_utterances":20,
...
def _license() -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v10_license_v1","license_id":"CC-BY-SA-4.0",
...
def document_group_census() -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v10_document_group_census_v1",
...
def _source_family(inventory: dict[str,Any], raw_hashes:set[str]) -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v10_source_family_v1","status":"eligible" if eligible else "ineligible",
            "disposition":"project_source_use_unseen_before_v10","content_occurrence_count":len(matches),
...
def candidate_pedigree() -> dict[str, Any]:
    ...
    return {"schema_version": "msae_independent_source_v10_candidate_pedigree_v1",
...
def build_registry() -> None:
    """Verify the byte-identical V9 registry/screen copied into the V10 namespace."""
    registry_path = PROV / "historical_source_registry.json"
    screen_path = PROV / "preacquisition_alias_screen.json"
    verify_v9_carryover(hash_raw=False)
    verify_frozen_v10_authority_bindings()
    v9_registry_path = ROOT / "reports/provenance/msae_independent_source_v9/historical_source_registry.json"
    v9_screen_path = ROOT / "reports/provenance/msae_independent_source_v9/preacquisition_alias_screen.json"
    for current, predecessor, expected in (
        (registry_path, v9_registry_path, V10_FROZEN_AUTHORITY_SHA256[registry_path.relative_to(ROOT).as_posix()]),
        (screen_path, v9_screen_path, V10_FROZEN_AUTHORITY_SHA256[screen_path.relative_to(ROOT).as_posix()]),
    ):
        regular_state_nofollow(current, mode=0o644, nlink=1)
        regular_state_nofollow(predecessor, mode=0o644, nlink=1)
        if sha_file(current) != expected or sha_file(predecessor) != expected:
            raise GateFailure("copied_history_authority_drift")
    registry = strict_json(registry_path)
    screen = strict_json(screen_path)
    inputs = registry.get("inputs")
    if (registry.get("schema_version") != "msae_independent_source_v9_historical_source_registry_v1"
            or registry.get("status") != "frozen_preacquisition"
            or registry.get("domain_utf8") != "msae-v9/pedigree"
            or not isinstance(inputs, list) or registry.get("input_count") != 16133
            or len(inputs) != 16133
            or registry.get("inputs_sha256") != "8070a7cdd536a0dbd6104ff1dfb279ef01fb0acf98e969227625324155227564"
            or registry.get("inputs_sha256") != sha_bytes(_canonical_ascii_bytes(inputs))
            or registry.get("overbound_json_source_key_construct_count") != 0
            or registry.get("quarantine_content_reads") != 0
            or screen.get("schema_version") != "msae_independent_source_v9_preacquisition_alias_screen_v2"
            or screen.get("history_input_count") != 16133
            or screen.get("history_inputs_sha256") != registry.get("inputs_sha256")
            or screen.get("history_registry_sha256") != sha_file(registry_path)
            or any(screen.get(key) != 0 for key in ("content_occurrence_count",
                "pathname_occurrence_count", "source_use_evidence_count", "quarantine_content_reads",
                "model_operations", "gpu_queries", "training_runs"))
            or screen.get("status") != "no_project_source_use_evidence"):
        raise GateFailure("copied_history_authority_drift")


def verify_frozen_v10_authority_bindings() -> dict[str, str]:
    if any(sha_file(ROOT/path)!=digest for path,digest in V10_FROZEN_AUTHORITY_SHA256.items()):
        raise GateFailure("predecessor_drift")
    return dict(V10_FROZEN_AUTHORITY_SHA256)
...
def _verify_predecessor_and_raw(*, read_raw: bool = True) -> dict[str, Any]:
    expected = {ROOT/path:digest for path,digest in verify_frozen_v10_authority_bindings().items()}
    ...
    required_public=(
        ...
        ROOT/"reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md",
    ...
    config=validate_acquisition_config(ROOT/"configs/msae_independent_source_v10/acquisition.json")
    ...
    if (entry.get("schema_version")!="msae_independent_source_v10_source_acquisition_entry_v1"
            ...
            or entry.get("config_sha256")!=sha_file(ROOT/"configs/msae_independent_source_v10/acquisition.json")
            or entry.get("runner_sha256")!=sha_file(ROOT/"scripts/acquire_msae_independent_source_v10.py")
    ...
    if (acquisition.get("schema_version")!="msae_independent_source_v10_source_acquisition_v1"
            ...
            or acquisition.get("config_sha256") != sha_file(ROOT/"configs/msae_independent_source_v10/acquisition.json")
            ...
            or acquisition.get("runner_sha256") != sha_file(ROOT/"scripts/acquire_msae_independent_source_v10.py")
    ...
    expected_command_argv=[
        [*config["ignore_argv"][:-1],"data/msae_independent_source_v10/raw/sentinel"],
    ...
    if ([item.get("path") for item in ignores] != ["data/msae_independent_source_v10/raw/sentinel","data/msae_independent_source_v10/private/sentinel"]
...
def _verify_history_snapshot(inventory: dict[str, Any], allowed_added: set[str] | None = None,
    ...
    current_paths={rel for _path,rel in _walk_paths_allow_v10(include_v10=True)}
    ...
    for item in inventory["entries"]:
        if item["disposition"] not in {"text_scanned", "archive_scanned", "binary_unscanned", "v10_authority",
                                      "v8_candidate_control_carryover",
                                      "v9_candidate_control_carryover", "quarantine"}:
...
def _raw_history_additions() -> set[str]:
    return {f"data/msae_independent_source_v10/raw/{COMMIT}/{name}"
            for name in [*SOURCE_FILES.values(),"LICENSE.txt"]}
...
def _control_history_additions(*, success: bool = False) -> set[str]:
    additions={
        ...
        "reports/provenance/msae_independent_source_v10/preacquisition_authority_manifest.json",
        "reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md",
        "reports/provenance/msae_independent_source_v10/source_acquisition_entry.json",
    }
    if success:additions.add("reports/provenance/msae_independent_source_v10/source_acquisition.json")
    else:
        for name in ("source_acquisition.json","source_acquisition_rejection.json"):
            if present_path(PROV/name):additions.add(f"reports/provenance/msae_independent_source_v10/{name}")
    return additions


def _success_history_additions() -> set[str]:
    return (_control_history_additions(success=True)|_raw_history_additions()|
            {f"reports/provenance/msae_independent_source_v10/{name}"
...
def verify_pre_payload_v10_namespace() -> None:
    fds=[]
    try:
        v10fd=open_repo_dir(RAW.parent.parent);fds.append(v10fd)
        rawrootfd=os.open("raw",os.O_RDONLY|getattr(os,"O_DIRECTORY",0)|getattr(os,"O_NOFOLLOW",0),dir_fd=v10fd);fds.append(rawrootfd)
        ...
        for fd,mode in ((v10fd,0o700),(rawrootfd,0o700),(commitfd,0o555)):
            if stat.S_IMODE(os.fstat(fd).st_mode)!=mode:raise GateFailure("v10_namespace_directory_drift")
        if sorted(os.listdir(v10fd)) != ["raw"]:raise GateFailure("v10_namespace_extra_path")
        if sorted(os.listdir(rawrootfd)) != [COMMIT]:raise GateFailure("v10_raw_parent_extra_path")
...
def process_snapshot() -> dict[str, Any]:
    ...
    for child in sorted(Path("/proc").iterdir(), key=lambda item: int(item.name) if item.name.isdigit() else -1):
        ...
        item={"pid":int(child.name),"start_ticks":stat_fields[19],"executable_basename":exe,
              "command_sha256":sha_bytes(b"msae-v10/process\0"+raw),"forbidden_codes":codes}
        entries.append(item)
        if codes:matches.append(item)
    return {"schema_version":"msae_independent_source_v10_process_snapshot_v1",
...
def _walk_paths_allow_v10(include_v10: bool = False) -> Iterator[tuple[Path,str]]:
    yield from _walk_descriptor_paths(include_v10=include_v10,reject_v10_namespace=False)
...
def implementation_review_sha() -> str:
    review=ROOT/"reports/adversarial/msae_independent_source_v10_preacquisition_implementation_review.md"
    required=[
        sha_file(ROOT/"docs/plan-msae-independent-source-v10.md"),
        sha_file(ROOT/"reports/adversarial/msae_independent_source_v10_plan_review.md"),
        sha_file(ROOT/CARRYOVER_PATH),
        sha_file(PROV/"preacquisition_alias_screen.json"),
        sha_file(PROV/"historical_source_registry.json"),
        sha_file(Path(__file__)),
        sha_file(ROOT/"scripts/acquire_msae_independent_source_v10.py"),
        sha_file(ROOT/"tests/test_prepare_msae_independent_source_v10.py"),
        sha_file(ROOT/"configs/msae_independent_source_v10/acquisition.json"),
        sha_file(ROOT/"reports/verification/msae_independent_source_v10_source_free_checks.log"),
    ]
...
def build_authority_manifest() -> None:
    ...
    _verify_history_snapshot(
        baseline,
        allowed_added={"reports/provenance/msae_independent_source_v10/baseline_inventory.json"},
    )
    ...
    review_path=ROOT/"reports/adversarial/msae_independent_source_v10_preacquisition_implementation_review.md"
    _require_ship_review(review_path,[inputs["scripts/prepare_msae_independent_source_v10.py"],
                                      inputs["tests/test_prepare_msae_independent_source_v10.py"],
                                      inputs["configs/msae_independent_source_v10/acquisition.json"],
                                      inputs["reports/verification/msae_independent_source_v10_source_free_checks.log"]])
    ...
    value={"schema_version":"msae_independent_source_v10_preacquisition_authority_v1",
...
def verify_authority_chain() -> dict[str, Any]:
    ...
    if (manifest.get("schema_version")!="msae_independent_source_v10_preacquisition_authority_v1"
    ...
    review_path=ROOT/"reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md"
...
def build_baseline() -> None:
    ...
    validate_acquisition_config(ROOT/"configs/msae_independent_source_v10/acquisition.json")
...
def _eligible_history_recensus(inventory: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "msae_independent_source_v10_history_recensus_v1",
...
def _eligible_training_recensus(inventory: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "msae_independent_source_v10_training_recensus_v1",
        "status": "eligible", "failure_code": None,
...
def _current_training_entries(inventory: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for path, rel in _walk_paths_allow_v10():
        folded = rel.casefold()
...
def _post_baseline_inventory(inventory: dict[str, Any], state_name: str,
                             ...
                             allowed_later_files: set[str] | None = None,
                             allowed_later_private: bool = False) -> dict[str, Any]:
    if state_name not in {"scientific_entry", "B0", "B1", "B2"}:
        raise GateFailure("post_baseline_inventory_state")
    later_files = set() if allowed_later_files is None else set(allowed_later_files)
    payload_rel = "data/msae_independent_source_v10/private/blind_payload.jsonl"
    allowed_later_universe = {
        "reports/provenance/msae_independent_source_v10/rejection.json",
        *{f"reports/provenance/msae_independent_source_v10/{name}"
          for name in SCIENTIFIC_ARTIFACT_NAMES},
        payload_rel,
    }
    if ((state_name != "scientific_entry" and (later_files or allowed_later_private))
            or not later_files <= allowed_later_universe):
        raise GateFailure("post_baseline_inventory_path_drift")
    later_private = allowed_later_private
    if (payload_rel in later_files) != (later_private and present_path(PRIVATE / "blind_payload.jsonl")):
    ...
    common_files = {
        ...
        "reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md",
        "reports/provenance/msae_independent_source_v10/source_acquisition_entry.json",
        "reports/provenance/msae_independent_source_v10/source_acquisition.json",
        *{f"data/msae_independent_source_v10/raw/{COMMIT}/{name}" for name in raw_names},
    }
    ...
    if state_name == "scientific_entry":
        expected_files.add("reports/provenance/msae_independent_source_v10/scientific_preparation_entry.json")
    elif state_name == "B0":
        expected_files.add("reports/provenance/msae_independent_source_v10/preflight_rejection.json")
    else:
        expected_files.add("reports/provenance/msae_independent_source_v10/scientific_preparation_entry.json")
        expected_files.update(
            f"reports/provenance/msae_independent_source_v10/{name}" for name in scientific_prefix)
        if state_name == "B1":
            expected_files.add("reports/provenance/msae_independent_source_v10/rejection.json")
        ...
        if expected_payload:
            expected_files.add("data/msae_independent_source_v10/private/blind_payload.jsonl")

    # History/training recensuses separately bind every baseline path and every
    # unrelated repository addition.  This inventory owns the generated v10
    # protocol universe only; otherwise a history-addition failure could not be
    # represented by its required B0 terminal.
    managed_prefixes = ("reports/provenance/msae_independent_source_v10/",
                        "data/msae_independent_source_v10/")
    managed_singletons = {
        "reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md",
    }
    current_files = {
        rel for _path, rel in _walk_paths_allow_v10(include_v10=True)
        if rel not in baseline_paths
        and (rel.startswith(managed_prefixes) or rel in managed_singletons)
    }
    if not later_files <= current_files:
        raise GateFailure("post_baseline_inventory_path_drift")
    ...
    # record while still requiring every other addition exactly.  B2's gate
    # binds the seal hash while the seal binds this inventory, so its exact
    # gate bytes are separately reconstructed rather than recursively hashed.
    nonrecursive_paths={self_path} if self_path is not None else set()
    dependent_path="reports/provenance/msae_independent_source_v10/no_training_gate.json"
    if state_name=="B2" and self_path=="reports/provenance/msae_independent_source_v10/seal.json":
        nonrecursive_paths.add(dependent_path)
    current_for_compare = set(current_files) - later_files
    current_for_compare.update(nonrecursive_paths-current_for_compare)
    if current_for_compare != expected_files:
        raise GateFailure("post_baseline_inventory_path_drift")

    raw_hashes_verified = state_name in {"B1", "B2"}
    data_root = ROOT / "data/msae_independent_source_v10"
    raw_root = data_root / "raw"
    raw_commit = raw_root / COMMIT
    private_exists = present_path(PRIVATE)
    if later_private and not private_exists:
        raise GateFailure("post_baseline_private_directory_drift")
    expected_root_names = ["raw"] + (["private"] if private_exists else [])
    directory_states: dict[Path,tuple[os.stat_result,list[str]]]={}
    for directory, expected_mode, expected_names in (
        ...
        st,names = directory_snapshot_nofollow(directory);directory_states[directory]=(st,names)
    ...
    if private_exists:
        ...
        st,names=directory_snapshot_nofollow(PRIVATE);directory_states[PRIVATE]=(st,names)
        if (not stat.S_ISDIR(st.st_mode) or stat.S_ISLNK(st.st_mode)
    ...
    directory_paths = ((data_root, raw_root, raw_commit)
                       + ((PRIVATE,) if private_exists and not later_private else ()))
    for directory in directory_paths:
        st,names = directory_states[directory]
        child_names = ["raw"] if directory == data_root and later_private else names
        entries.append({"path": directory.relative_to(ROOT).as_posix(), "state": "present",
                        "disposition": "v10_generated_protocol", "type": "directory",
                        "mode": stat.S_IMODE(st.st_mode), "child_names": child_names})
    for rel in sorted(expected_files):
        if rel == self_path:
            entries.append({"path": rel, "state": "self_canonical_final"})
            continue
        if rel == dependent_path and rel in nonrecursive_paths:
            entries.append({"path": rel, "state": "dependent_canonical_final"})
            continue
        path = ROOT / rel
        is_raw = rel.startswith(f"data/msae_independent_source_v10/raw/{COMMIT}/")
        ...
        item: dict[str, Any] = {
            "path": rel, "state": "present", "disposition": "v10_generated_protocol",
        ...
        if is_raw:
        ...
        elif rel == "data/msae_independent_source_v10/private/blind_payload.jsonl":
            ...
            item["sha256"] = sha_file(path)
        entries.append(item)
    for directory,(before,_names) in directory_states.items():
        after,_names=directory_snapshot_nofollow(directory)
        if _directory_fingerprint(before)!=_directory_fingerprint(after):
            raise GateFailure("path_identity_changed")
    return {"schema_version": "msae_independent_source_v10_post_baseline_inventory_v1",
...
def publish_scientific_entry(inventory: dict[str, Any], acquisition: dict[str, Any],
    ...
    files = [{"path": name, **{key: acquisition["files"][name][key]
             ...
             for name in [*SOURCE_FILES.values(), "LICENSE.txt"]]
    value = {
        "schema_version": "msae_independent_source_v10_scientific_preparation_entry_v1",
        "status": "entered_before_first_v10_raw_open",
        "plan_sha256": sha_file(ROOT / "docs/plan-msae-independent-source-v10.md"),
        "plan_review_sha256": sha_file(ROOT / "reports/adversarial/msae_independent_source_v10_plan_review.md"),
        ...
        "authority_review_sha256": sha_file(ROOT / "reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md"),
        ...
        "post_baseline_inventory": _post_baseline_inventory(
            ...
            "reports/provenance/msae_independent_source_v10/scientific_preparation_entry.json"),
...
def _seal_value(post_inventory: dict[str, Any], bindings: dict[str, str],
                payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "msae_independent_source_v10_seal_v1",
        ...
        "v9_candidate_control_carryover_authority_sha256": CARRYOVER_SHA256,
        ...
        "plan_sha256": sha_file(ROOT / "docs/plan-msae-independent-source-v10.md"),
        "plan_review_sha256": sha_file(ROOT / "reports/adversarial/msae_independent_source_v10_plan_review.md"),
        "program_sha256": sha_file(Path(__file__)),
        "acquisition_runner_sha256": sha_file(ROOT / "scripts/acquire_msae_independent_source_v10.py"),
        "test_sha256": sha_file(ROOT / "tests/test_prepare_msae_independent_source_v10.py"),
        "artifact_sha256": bindings,
        "payload": payload,
        "model_operations_initiated_by_builder": 0,
        "gpu_queries_initiated_by_builder": 0,
        "training_runs_initiated_by_builder": 0,
        "source_content_reported": False,
        "model_scoring_authorized": False,
        "k2_or_branch_training_authorized": False,
        "stage_c_authorized": False,
        ...
        "opaque_binary_history_excluded": True,
        "document_speaker_cluster_inference_authorized": False,
    }


def _no_training_gate_value(seal: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "msae_independent_source_v10_no_training_gate_v1",
        "status": "pending_terminal_seal",
        "terminal_status_if_seal_matches": seal["status"],
        "scientific_preparation_entry_sha256": seal["scientific_preparation_entry_sha256"],
        "model_scoring_authorized": False,
        "k2_or_branch_training_authorized": False,
        "stage_c_authorized": False,
        "seal_sha256": sha_bytes(canonical_file_bytes(seal)),
    }
...
def build_ready() -> None:
    ...
    if (any(item["state"]!="absent" for item in initial_public["entries"])
        ...
        verify_terminal()
        return
    verify_pre_payload_v10_namespace()
    ...
    source_manifest={"schema_version":"msae_independent_source_v10_source_manifest_v1","source_commit":COMMIT,
    ...
    cross=_role_overlap(rows);write_json(PROV/"cross_role_overlap.json",{"schema_version":"msae_independent_source_v10_cross_role_overlap_v1","blocking_collision_count":len(cross),"blocking_collisions":cross,"status":"eligible" if not cross else "ineligible"})
    ...
    role_manifest={"schema_version":"msae_independent_source_v10_role_manifest_v1","source_commit":COMMIT,"roles":{}}
    ...
    post_inventory=_post_baseline_inventory(
        inv,"B2","reports/provenance/msae_independent_source_v10/seal.json")
    seal=_seal_value(post_inventory,bindings,{
    ...
    write_json(PROV/"no_training_gate.json",_no_training_gate_value(seal))
...
def _failed_history_recensus(inventory: dict[str, Any], code: str) -> dict[str, Any]:
    return {"schema_version":"msae_independent_source_v10_history_recensus_v1",
...
def _failed_training_recensus(inventory: dict[str, Any], code: str) -> dict[str, Any]:
    return {"schema_version":"msae_independent_source_v10_training_recensus_v1",
            "status":"boundary_failure","failure_code":code,
            "expected_entries_sha256":inventory["training_root_entries_sha256"],
...
def retain_preflight_rejection(code: str, history: dict[str, Any],
    ...
    public=_self_inventory(scientific_artifact_inventory(),
                           "reports/provenance/msae_independent_source_v10/preflight_rejection.json")
    value={"schema_version":"msae_independent_source_v10_preflight_rejection_v1",
           ...
           "post_baseline_inventory":_post_baseline_inventory(
               inv,"B0","reports/provenance/msae_independent_source_v10/preflight_rejection.json"),
...
def retain_rejection(code: str) -> None:
    ...
    public=_self_inventory(scientific_artifact_inventory(),
                           "reports/provenance/msae_independent_source_v10/rejection.json")
    value={"schema_version":"msae_independent_source_v10_rejection_v1",
           ...
           "post_baseline_inventory":_post_baseline_inventory(
               inv,"B1","reports/provenance/msae_independent_source_v10/rejection.json"),
           "scientific_artifact_inventory":public,"payload_state":payload_state(),
...
def _validate_inventory_artifact(value: Any, code: str) -> None:
    exact_keys(value,{"schema_version","baseline_head","entry_count","counts","quarantine_content_reads",
        ...
        "v9_carryover_authority_sha256","v9_carryover_status"},code)
    entries=value["entries"]
    if not isinstance(entries,list) or any(not isinstance(item,dict) for item in entries):raise GateFailure(code)
    ...
    for item in entries:
        ...
        if (keys!=expected or not isinstance(item.get("path"),str)
                ...
                or item.get("disposition") not in {"text_scanned","archive_scanned","binary_unscanned",
                    "v10_authority","v8_candidate_control_carryover",
                    "v9_candidate_control_carryover","quarantine"}
    ...
    if (value["schema_version"]!="msae_independent_source_v10_baseline_inventory_v1"
            ...
            or value["v9_carryover_authority_sha256"]!=CARRYOVER_SHA256
            or value["v9_carryover_status"]!="opaque_exact_universe_verified"
...
def validate_scientific_artifacts(*, verify_raw_hashes: bool = True) -> None:
    ...
    if (census["schema_version"]!="msae_independent_source_v10_document_group_census_v1"
    ...
    if (source["schema_version"]!="msae_independent_source_v10_source_manifest_v1"
    ...
    if (license_value["schema_version"]!="msae_independent_source_v10_license_v1"
    ...
    if (family["schema_version"]!="msae_independent_source_v10_source_family_v1" or family["status"]!="eligible"
            or family["disposition"]!="project_source_use_unseen_before_v10"
    ...
    if (pedigree["schema_version"]!="msae_independent_source_v10_candidate_pedigree_v1"
    ...
    if (dedup["schema_version"]!="msae_independent_source_v10_dedup_v1"
    ...
    if (cross["schema_version"]!="msae_independent_source_v10_cross_role_overlap_v1"
    ...
    if (overlap["schema_version"]!="msae_independent_source_v10_history_overlap_v1"
    ...
    if (support["schema_version"]!="msae_independent_source_v10_support_v1" or support["floor_distinct_utterances"]!=20
...
def reconstruct_scientific_artifacts() -> dict[str, Any]:
    ...
    if not TERMINAL_B2_RECONSTRUCTION_AUTHORIZED:
        raise GateFailure("classified_b2_reconstruction_required")
    ...
    roles,source_counts=_all_source()
    source={"schema_version":"msae_independent_source_v10_source_manifest_v1","source_commit":COMMIT,
            "partitions":source_counts,"source_text_published":False}
    ...
    cross_value={"schema_version":"msae_independent_source_v10_cross_role_overlap_v1",
    ...
    overlap={"schema_version":"msae_independent_source_v10_history_overlap_v1",
        "baseline_inventory_sha256":sha_file(PROV/"baseline_inventory.json"),
...
def _validate_recensus(value: Any, kind: str, code: str) -> None:
    if kind == "history":
        ...
        if (value.get("schema_version")!="msae_independent_source_v10_history_recensus_v1"
        ...
        if (value.get("schema_version")!="msae_independent_source_v10_training_recensus_v1"
                or value.get("status") not in {"eligible","drift","boundary_failure"}
                or not _is_sha(value.get("expected_entries_sha256"))
...
def _terminal_common(value: dict[str,Any], baseline: dict[str,Any], code: str) -> None:
    if (not isinstance(value.get("failure_code"),str) or not value["failure_code"]
            or value.get("plan_sha256")!=sha_file(ROOT/"docs/plan-msae-independent-source-v10.md")
            or value.get("plan_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v10_plan_review.md")
            ...
            or value.get("authority_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md")
...
def _verify_terminal_entry(baseline: dict[str,Any], later_files: set[str], *,
                           later_private: bool) -> dict[str,Any]:
    ...
    if (entry.get("schema_version")!="msae_independent_source_v10_scientific_preparation_entry_v1"
            or entry.get("status")!="entered_before_first_v10_raw_open"
            or entry.get("plan_sha256")!=sha_file(ROOT/"docs/plan-msae-independent-source-v10.md")
            or entry.get("plan_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v10_plan_review.md")
            or entry.get("builder_sha256")!=sha_file(Path(__file__))
            or entry.get("carryover_authority_sha256")!=CARRYOVER_SHA256
            or entry.get("baseline_inventory_sha256")!=sha_file(PROV/"baseline_inventory.json")
            or entry.get("authority_manifest_sha256")!=sha_file(PROV/"preacquisition_authority_manifest.json")
            or entry.get("authority_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v10_preacquisition_authority_review.md")
    ...
    expected_inventory=_post_baseline_inventory(
        ...
        "reports/provenance/msae_independent_source_v10/scientific_preparation_entry.json",
        verify_raw_hashes=False, allowed_later_files=later_files,
        allowed_later_private=later_private)
...
def _regular_additions(post_inventory: dict[str,Any]) -> set[str]:
    return {item["path"] for item in post_inventory.get("entries",[])
            ...
            or item.get("state") in {"self_canonical_final","dependent_canonical_final"}}
...
def _verify_fresh_terminal_process() -> None:
    snapshot=process_snapshot()
    verify_process_snapshot_schema(snapshot,"terminal_process_snapshot")
    if snapshot["status"]!="eligible" or snapshot["forbidden_identity_count"]!=0:
        raise GateFailure("terminal_process_snapshot")


def verify_terminal() -> None:
    global RAW_ACCESS_AUTHORIZED,TERMINAL_B2_RECONSTRUCTION_AUTHORIZED
    RAW_ACCESS_AUTHORIZED=False
    TERMINAL_B2_RECONSTRUCTION_AUTHORIZED=False
    ...
    sci_paths=[f"reports/provenance/msae_independent_source_v10/{name}" for name in SCIENTIFIC_ARTIFACT_NAMES]
    entry_rel="reports/provenance/msae_independent_source_v10/scientific_preparation_entry.json"
    preflight_rel="reports/provenance/msae_independent_source_v10/preflight_rejection.json"
    rejection_rel="reports/provenance/msae_independent_source_v10/rejection.json"
    payload_now=payload_state()
    payload_rel="data/msae_independent_source_v10/private/blind_payload.jsonl"
    private_state=classify_private_terminal_state(payload_now)
    payload_absent=private_state in {"absent","empty"}
    payload_valid=private_state=="payload"

    is_b0=present_finals=={preflight_rel} and private_state=="absent"
    ...
    is_b1=(entry_rel in present_finals and rejection_rel in present_finals
           ...
           and "reports/provenance/msae_independent_source_v10/seal.json" not in prefix
           and (payload_absent or payload_valid))
    ...
    if is_b0:
        ...
        if (value.get("schema_version")!="msae_independent_source_v10_preflight_rejection_v1"
        ...
        _verify_fresh_terminal_process()
        return

    if is_b1:
        expected_post=_post_baseline_inventory(baseline,"B1",rejection_rel,verify_raw_hashes=False)
        later_files={*prefix,rejection_rel}
        if payload_valid:later_files.add(payload_rel)
        entry=_verify_terminal_entry(
            baseline,later_files,later_private=private_state!="absent")
        ...
        exact_keys(value,{"schema_version","status","failure_code","plan_sha256","plan_review_sha256",
            ...
            "k2_or_branch_training_authorized","stage_c_authorized","next_action"},code)
        _terminal_common(value,baseline,code)
        if (value.get("schema_version")!="msae_independent_source_v10_rejection_v1"
    ...
    entry=_verify_terminal_entry(
        baseline,{*sci_paths,payload_rel},later_private=True)
    seal=strict_terminal_json(seal_path)
    exact_keys(seal,{"schema_version","status","source_status","source_maintenance_relation",
        "source_family_relation","project_history_boundary","history_screen_temporal_scope",
        "same_candidate_predecessor_attempts","v9_semantic_parse_predecessor",
        "v9_candidate_control_carryover_authority_sha256",
    ...
    if (seal.get("schema_version")!="msae_independent_source_v10_seal_v1"
            ...
            or seal.get("history_screen_temporal_scope")!="project_accessible_history_frozen_before_first_v8_candidate_acquisition"
            or seal.get("same_candidate_predecessor_attempts")!=["v8","v9"]
            or seal.get("v9_semantic_parse_predecessor") is not True
            or seal.get("v9_candidate_control_carryover_authority_sha256")!=CARRYOVER_SHA256
            ...
            or seal.get("plan_sha256")!=sha_file(ROOT/"docs/plan-msae-independent-source-v10.md")
            or seal.get("plan_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v10_plan_review.md")
            or seal.get("program_sha256")!=sha_file(Path(__file__))
            or seal.get("acquisition_runner_sha256")!=sha_file(ROOT/"scripts/acquire_msae_independent_source_v10.py")
            or seal.get("test_sha256")!=sha_file(ROOT/"tests/test_prepare_msae_independent_source_v10.py")
    ...
    expected_schemas={
        "baseline_inventory.json":"msae_independent_source_v10_baseline_inventory_v1",
        "source_acquisition_entry.json":"msae_independent_source_v10_source_acquisition_entry_v1",
        "source_acquisition.json":"msae_independent_source_v10_source_acquisition_v1",
        "scientific_preparation_entry.json":"msae_independent_source_v10_scientific_preparation_entry_v1",
        "source_family.json":"msae_independent_source_v10_source_family_v1",
        "candidate_pedigree.json":"msae_independent_source_v10_candidate_pedigree_v1",
        "license.json":"msae_independent_source_v10_license_v1",
        "document_group_census.json":"msae_independent_source_v10_document_group_census_v1",
        "source_manifest.json":"msae_independent_source_v10_source_manifest_v1",
        "dedup.json":"msae_independent_source_v10_dedup_v1",
        "history_manifest.json":"msae_independent_source_v10_baseline_inventory_v1",
        "history_overlap.json":"msae_independent_source_v10_history_overlap_v1",
        "cross_role_overlap.json":"msae_independent_source_v10_cross_role_overlap_v1",
        "support.json":"msae_independent_source_v10_support_v1",
        "role_manifest.json":"msae_independent_source_v10_role_manifest_v1",
        "split_manifest.json":"msae_independent_source_v10_split_manifest_v1",
        "post_process_snapshot.json":"msae_independent_source_v10_process_snapshot_v1",
        ...
        "preacquisition_authority_manifest.json":"msae_independent_source_v10_preacquisition_authority_v1",
    }
    ...
    try:
        RAW_ACCESS_AUTHORIZED=True
        TERMINAL_B2_RECONSTRUCTION_AUTHORIZED=True
    ...
    if (any(not isinstance(item,dict) or set(item)!={"sent_id","panel","group_rank","within_group_rank",
            ...
            or split.get("schema_version")!="msae_independent_source_v10_split_manifest_v1"
    ...
    if (roles.get("schema_version")!="msae_independent_source_v10_role_manifest_v1"
    ...
    expected_payload={"path":"data/msae_independent_source_v10/private/blind_payload.jsonl",
        **reconstructed_payload,"mode":0o600,"nlink":1}
    expected_seal=_seal_value(expected_post,bindings,expected_payload)
    if seal!=expected_seal:raise GateFailure("seal_exact_reconstruction")
    ...
    expected_gate=_no_training_gate_value(expected_seal)
    if gate!=expected_gate or gate.get("seal_sha256")!=sha_file(seal_path):
