...
import argparse, ast, builtins, glob as globlib, hashlib, json, math, os, random, secrets, shutil, signal, subprocess, sys, tempfile, time
...
CAP_CFG=ROOT/'configs/capacity_controller_diagnostic_v1_r5/run.json'
EXT_CFG=ROOT/'configs/trained_copy_external_v1_r5/run.json'
PLAN=ROOT/'PLAN_CAPACITY_EXTERNAL_VALIDITY_V1_R5.md'
PLAN_REVIEW=ROOT/'reports/adversarial/capacity_external_validity_v1_r5_plan_review.md'
...
INVALID_R2_PRESERVE=ROOT/'reports/provenance/capacity_external_validity_v1_r2_invalid_candidate/PRESERVATION.json'
INVALID_R3_PRESERVE=ROOT/'reports/provenance/capacity_external_validity_v1_r3_invalid_candidate/PRESERVATION.json'
INVALID_R4_PRESERVE=ROOT/'reports/provenance/capacity_external_validity_v1_r4_invalid_candidate/PRESERVATION.json'
...
SCRIPT=ROOT/'scripts/capacity_external_validity_v1_r5.py'
LAUNCHER=ROOT/'scripts/launch_capacity_external_validity_v1_r5_tmux.sh'
TEST=ROOT/'tests/test_capacity_external_validity_v1_r5.py'
SESSION_NAME='capacity_external_validity_v1_20260810r5'
GPU_LOCK_GLOB='msae_capacity_external_r5_*.lockdir'
EXPECTED_BRIDGE_PRESERVATION_SHA256='d112fdc7aba9c153eb8ffc4b62750333631b5f0c8e6617a444286b7cbaf53bd2'
EXPECTED_V1_FIT_AST_SHA256='50c4765b3eea25ed2f6dd12607a90c6f58fcbae25640c73b5eac12cbff976250'
_ACTIVE_RUN:dict[str,Path]={}

def _is_jsonl_path(value:Any)->bool:
 try:
  raw=os.fspath(value);raw=os.fsdecode(raw) if isinstance(raw,bytes) else raw;return Path(raw).suffix.lower()=='.jsonl'
 except (TypeError,ValueError):return False
@contextmanager
def no_jsonl_access_guard():
 orig_open,orig_os_open,orig_stat,orig_lstat,orig_scandir,orig_listdir,orig_popen,orig_globlib,orig_igloblib=builtins.open,os.open,os.stat,os.lstat,os.scandir,os.listdir,subprocess.Popen,globlib.glob,globlib.iglob
 orig_path_open,orig_read_text,orig_read_bytes,orig_pstat,orig_glob,orig_rglob=Path.open,Path.read_text,Path.read_bytes,Path.stat,Path.glob,Path.rglob
 def reject(value:Any,operation:str)->None:
  if _is_jsonl_path(value):raise AssertionError(f'pre-access {operation} attempted forbidden JSONL: {value}')
 def guarded_open(file,*args,**kwargs):reject(file,'open');return orig_open(file,*args,**kwargs)
 def guarded_os_open(path,*args,**kwargs):reject(path,'os.open');return orig_os_open(path,*args,**kwargs)
 def guarded_stat(path,*args,**kwargs):reject(path,'stat');return orig_stat(path,*args,**kwargs)
 def guarded_lstat(path,*args,**kwargs):reject(path,'lstat');return orig_lstat(path,*args,**kwargs)
 def guarded_scandir(path='.'):
  reject(path,'scandir')
  if Path(os.fspath(path))!=Path('/tmp'):raise AssertionError(f'pre-access scandir not allowlisted: {path}')
  return orig_scandir(path)
 def guarded_listdir(path='.'):
  reject(path,'listdir');q=Path(os.fspath(path))
  try:inside_workspace=q.resolve().is_relative_to(ROOT.resolve())
  except (FileNotFoundError,RuntimeError):inside_workspace=False
  if inside_workspace or q==Path('/tmp'):raise AssertionError(f'pre-access workspace/temp listdir forbidden: {path}')
  rows=orig_listdir(path)
  if any(str(x).lower().endswith('.jsonl') for x in rows):raise AssertionError(f'pre-access listdir encountered JSONL outside workspace: {path}')
  return rows
 def guarded_popen(args,*pargs,**kwargs):
  seq=[args] if isinstance(args,(str,bytes,os.PathLike)) else args
  if any('.jsonl' in os.fspath(x).lower() for x in seq if isinstance(x,(str,bytes,os.PathLike))):raise AssertionError('pre-access subprocess referenced JSONL')
  return orig_popen(args,*pargs,**kwargs)
 def path_guard(method,operation):
  def wrapped(self,*args,**kwargs):reject(self,operation);return method(self,*args,**kwargs)
  return wrapped
 def guarded_glob(self,pattern):
  if self!=Path('/tmp') or pattern!=GPU_LOCK_GLOB:raise AssertionError(f'pre-access glob not allowlisted: {self}/{pattern}')
  return orig_glob(self,pattern)
 def guarded_rglob(self,pattern):raise AssertionError(f'pre-access rglob forbidden: {self}/{pattern}')
 def guarded_globlib(*args,**kwargs):raise AssertionError(f'pre-access stdlib glob forbidden: {args[:1]}')
 def guarded_igloblib(*args,**kwargs):raise AssertionError(f'pre-access stdlib iglob forbidden: {args[:1]}')
 with mock.patch.object(builtins,'open',guarded_open),mock.patch.object(os,'open',guarded_os_open),mock.patch.object(os,'stat',guarded_stat),mock.patch.object(os,'lstat',guarded_lstat),mock.patch.object(os,'scandir',guarded_scandir),mock.patch.object(os,'listdir',guarded_listdir),mock.patch.object(subprocess,'Popen',guarded_popen),mock.patch.object(globlib,'glob',guarded_globlib),mock.patch.object(globlib,'iglob',guarded_igloblib),mock.patch.object(Path,'open',path_guard(orig_path_open,'Path.open')),mock.patch.object(Path,'read_text',path_guard(orig_read_text,'read_text')),mock.patch.object(Path,'read_bytes',path_guard(orig_read_bytes,'read_bytes')),mock.patch.object(Path,'stat',path_guard(orig_pstat,'Path.stat')),mock.patch.object(Path,'glob',guarded_glob),mock.patch.object(Path,'rglob',guarded_rglob):
  yield
def jsonl_guarded(fn):
 def wrapped(*args,**kwargs):
  with no_jsonl_access_guard():return fn(*args,**kwargs)
 wrapped.__name__=fn.__name__;wrapped.__doc__=fn.__doc__;return wrapped
...
def write_external_failure_terminal(reason:str)->None:
 ep=_ACTIVE_RUN.get('external_prov');eo=_ACTIVE_RUN.get('external_out')
 if ep is None or eo is None:return
 accessed=(ep/'events'/'021_ACCESS_MAY_HAVE_OCCURRED.json').exists()
 if not accessed:write_external_failure_if_unopened(reason);return
 final=eo/'final/result.json'
 if not final.exists():atomic_json(final,{'status':'TECHNICAL_FAILURE_AFTER_ACCESS','capacity_authorized':True,'external_payload_accessed':'MAY_HAVE_OCCURRED','training_performed':(ep/'events'/'022_EXTERNAL_TRAIN_OPENED.json').exists(),'reason':reason})
 safe_event(ep,992,'EXTERNAL_TECHNICAL_FAILURE_AFTER_ACCESS',reason=reason)
def signal_terminal(signum:int,_frame:Any)->None:
 ...
 write_external_failure_terminal(reason)
 raise SystemExit(128+signum)

def preservation_verify()->dict[str,Any]:
 # The signed inventory includes scientific JSONL. Bind its manifest without
 # traversing the payload inventory during recovery setup.
 manifest_sha=sha(PRESERVE)
 if manifest_sha!=EXPECTED_BRIDGE_PRESERVATION_SHA256:raise RuntimeError('bridge preservation manifest drift')
 x=loadj(PRESERVE)
 return {'status':'PASS_NONPAYLOAD_MANIFEST_ONLY','files_declared':len(x['inventory']),'manifest_sha256':manifest_sha,'inventory_traversed':False}
...
def _ast_sha(dump:str)->str:return hashlib.sha256(dump.encode()).hexdigest()
def _expected_fit_ast_from_v1()->str:
 source=V1_SOURCE.read_text();tree=ast.parse(source);node=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='fit_methods');fit=ast.get_source_segment(source,node)
 old="""     jr=[]
     for tv in range(c['targets']):
      ix=int(torch.flatnonzero(tt==tv)[0]);sample=xt[ix].detach().requires_grad_(True);target=torch.tensor([tv],device=device)
      jac=torch.autograd.functional.jacobian(lambda z:model(z[None,:],target)[0],sample,vectorize=True).detach().cpu().numpy();jr.append(int(np.linalg.matrix_rank(jac,tol=1e-5)))"""
 new="""     jr=empirical_jacobian_ranks(model,xt,tt,c['targets'],device)"""
 if fit.count(old)!=1:raise RuntimeError('frozen v1 fit_methods rewrite anchor missing or ambiguous')
 rewritten=fit.replace(old,new)
 return ast.dump(ast.parse(rewritten).body[0],include_attributes=False)
def _expected_pipeline_ast_from_v1(name:str,replacements:Sequence[tuple[str,str]])->str:
 source=V1_SOURCE.read_text();tree=ast.parse(source);node=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name==name);text=ast.get_source_segment(source,node)
 for old,new in replacements:
  if text.count(old)!=1:raise RuntimeError(f'frozen v1 {name} rewrite anchor missing or ambiguous')
...
def verify_parity()->dict[str,Any]:
 ...
 oldf,newf=_functions(V1_SOURCE),_functions(SCRIPT)
 allowed_common_drift={'fit_methods','candidate_files','candidate_preflight','capacity_pipeline','create_locks','locks_valid','freeze_both','freeze_one','bind_reviews','binding_path','external_pipeline','gpu_validate','launch_preflight','launcher_render','main','preservation_verify','run_pipeline','signal_terminal','verify_binding','verify_freeze'}
 actual_common_drift={name for name in set(oldf)&set(newf) if oldf[name]!=newf[name]}
 if actual_common_drift!=allowed_common_drift:raise RuntimeError(f'top-level source drift differs from frozen allowlist: {sorted(actual_common_drift^allowed_common_drift)}')
 expected_fit=_expected_fit_ast_from_v1()
 if _ast_sha(oldf['fit_methods'])!=EXPECTED_V1_FIT_AST_SHA256 or newf['fit_methods']!=expected_fit:raise RuntimeError('fit_methods is not the deterministic one-block compatibility rewrite')
 expected_capacity=_expected_pipeline_ast_from_v1('capacity_pipeline',[
  (" c=cfg('capacity');event(prov,10,'CAPACITY_TRAIN_OPENED');train={i:open_rows('capacity','train',i) for i in c['instances']};event(prov,11,'CAPACITY_DEVELOPMENT_OPENED');dev={i:open_rows('capacity','development',i) for i in c['instances']}"," c=cfg('capacity');train=open_capacity_stage(c,'train',prov,10,11);dev=open_capacity_stage(c,'development',prov,12,13)"),
  (" event(prov,12,'CAPACITY_DEVELOPMENT_COMPLETE');event(prov,13,'CAPACITY_CONFIRMATION_OPENED');conf={i:open_rows('capacity','confirmation',i) for i in c['instances']}"," event(prov,14,'CAPACITY_DEVELOPMENT_COMPLETE');conf=open_capacity_stage(c,'confirmation',prov,15,16)"),
  ("event(prov,14,'CAPACITY_COMPLETE'","event(prov,17,'CAPACITY_COMPLETE'")])
 expected_external=_expected_pipeline_ast_from_v1('external_pipeline',[
  (" event(prov,20,'PRECHECK_NO_ACCESS',capacity_final_sha256=sha(rp(cfg('capacity'),'output_root')/'final/result.json'))\n if not cap['external_authorized']:external_blocked(prov,out,cap);return"," cap_path=rp(cfg('capacity'),'output_root')/'final/result.json';disk_cap=loadj(cap_path);cap_sha=sha(cap_path);event(prov,20,'PRECHECK_NO_ACCESS',capacity_final_sha256=cap_sha,capacity_external_authorized=bool(disk_cap.get('external_authorized')))\n if canon(disk_cap)!=canon(cap):raise RuntimeError('in-memory capacity decision differs from durable final')\n if not disk_cap['external_authorized']:external_blocked(prov,out,disk_cap);return"),
  (" event(prov,23,'EXTERNAL_DEVELOPMENT_OPENED');dev=open_rows('external','development');development={str(sd):score_copy_model(models[sd],dev,c,sd,'development',device) for sd in c['model']['seeds']};event(prov,24,'EXTERNAL_DEVELOPMENT_COMPLETE')"," event(prov,23,'EXTERNAL_DEVELOPMENT_ACCESS_MAY_HAVE_OCCURRED');dev=open_rows('external','development');event(prov,24,'EXTERNAL_DEVELOPMENT_OPENED');development={str(sd):score_copy_model(models[sd],dev,c,sd,'development',device) for sd in c['model']['seeds']};event(prov,25,'EXTERNAL_DEVELOPMENT_COMPLETE')"),
  (" event(prov,25,'EXTERNAL_CONFIRMATION_OPENED');conf=open_rows('external','confirmation');confirmation={str(sd):score_copy_model(models[sd],conf,c,sd,'confirmation',device) for sd in c['model']['seeds']}"," event(prov,26,'EXTERNAL_CONFIRMATION_ACCESS_MAY_HAVE_OCCURRED');conf=open_rows('external','confirmation');event(prov,27,'EXTERNAL_CONFIRMATION_OPENED');confirmation={str(sd):score_copy_model(models[sd],conf,c,sd,'confirmation',device) for sd in c['model']['seeds']}"),
  ("event(prov,26,'EXTERNAL_TERMINAL'","event(prov,28,'EXTERNAL_TERMINAL'")])
 if newf['capacity_pipeline']!=expected_capacity or newf['external_pipeline']!=expected_external:raise RuntimeError('scientific pipeline differs beyond deterministic lifecycle rewrites')
 new_tree=ast.parse(SCRIPT.read_text())
 forbidden_restore=[]
 for n in ast.walk(new_tree):
  if isinstance(n,ast.Call) and ((isinstance(n.func,ast.Attribute) and n.func.attr in {'load','load_state_dict'}) or (isinstance(n.func,ast.Name) and n.func.id in {'load','loads'})):forbidden_restore.append(ast.unparse(n.func))
 if forbidden_restore:raise RuntimeError(f'checkpoint/state restore path forbidden in r5: {forbidden_restore}')
 scientific=sorted((set(oldf)&set(newf))-allowed_common_drift)
 old_tree=ast.parse(V1_SOURCE.read_text());is_torch_flat=lambda n:isinstance(n,ast.Attribute) and n.attr=='flatnonzero' and isinstance(n.value,ast.Name) and n.value.id=='torch';old_flat=any(is_torch_flat(n) for n in ast.walk(old_tree));new_flat=any(is_torch_flat(n) for n in ast.walk(new_tree));new_jac=any(isinstance(n,ast.Call) and isinstance(n.func,ast.Name) and n.func.id=='empirical_jacobian_ranks' for n in ast.walk(new_tree))
 if not old_flat or new_flat or not new_jac:raise RuntimeError('compatibility diff contract failed')
 return {'status':'PASS','config_pairs':pairs,'scientific_symbols_verified':scientific,'exact_derived_ast_sha256':{'fit_methods':_ast_sha(expected_fit),'capacity_pipeline':_ast_sha(expected_capacity),'external_pipeline':_ast_sha(expected_external),'v1_fit_methods':EXPECTED_V1_FIT_AST_SHA256},'restore_paths_forbidden':True,'source_allowlist':{'compatibility':['first_true_index','empirical_jacobian_ranks','deterministic v1 fit_methods rewrite'],'recovery_lifecycle':sorted(allowed_common_drift-{'fit_methods','capacity_pipeline','external_pipeline'}),'exact_lifecycle_rewrites':['capacity_pipeline','external_pipeline']},'v1_source_sha256':sha(V1_SOURCE),'r5_source_sha256':sha(SCRIPT)}
...
def candidate_files()->list[Path]:return [PLAN,PLAN_REVIEW,PRESERVE,FAILURE_PRESERVE,INVALID_R2_PRESERVE,INVALID_R3_PRESERVE,INVALID_R4_PRESERVE,ROOT/'scripts/preserve_capacity_external_validity_v1_failure.py',ROOT/'PAPER.md',ROOT/'reports/paper_claim_ledger_v1.json',CAP_CFG,EXT_CFG,SCRIPT,LAUNCHER,TEST]
...
def compatibility_smoke(device_name:str)->dict[str,Any]:
 ...
 rows=make_capacity_rows(97991,'smoke',c,16,16,97992);dev=make_capacity_rows(97991,'smoke',c,24,8,97993);td=Path('/tmp')/f'msae_capacity_external_r5_compat_{os.getpid()}_{device_name.replace(":","_")}';ckpt=td/'checkpoints'
 if td.exists():raise RuntimeError(f'compatibility temp namespace exists: {td}')
 td.mkdir()
 try:models,fit=fit_methods(rows,97991,c,device,ckpt);scored=eval_methods(dev,97991,c,models,'development',device)
 finally:
  for family in ('paired_linear','target_linear','target_nonlinear'):
   seed=c['methods'][family]['seeds'][0];(ckpt/f'{family}_rank4_seed{seed}.pt').unlink(missing_ok=True)
  if ckpt.exists():ckpt.rmdir()
  td.rmdir()
 nonlinear=next(v for k,v in fit['fits'].items() if k.startswith('target_nonlinear_'));jr=nonlinear['empirical_jacobian_ranks']
 if jr!=[4]*c['targets']:raise RuntimeError(f'all-target jacobian compatibility smoke failed: {jr}')
...
def recovery_environment_absence()->dict[str,Any]:
 tmux_exists=subprocess.run(['tmux','has-session','-t',SESSION_NAME],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode==0
 locks=sorted(str(p) for p in Path('/tmp').glob(GPU_LOCK_GLOB))
 if tmux_exists or locks:raise RuntimeError(f'r5 runtime namespace preexists: tmux={tmux_exists} locks={locks}')
 return {'tmux_session':SESSION_NAME,'tmux_absent':True,'gpu_lock_glob':GPU_LOCK_GLOB,'gpu_locks_absent':True}
def require_absent(label:str,paths:Sequence[Path])->None:
 present=[str(p.relative_to(ROOT)) if p.is_relative_to(ROOT) else str(p) for p in paths if p.exists()]
 if present:raise RuntimeError(f'{label} requires absent paths: {present}')
def postcandidate_absence_paths(include_locks:bool=True)->list[Path]:
 paths=[]
 for kind in ('capacity','external'):
  c=cfg(kind);paths.extend(rp(c,k) for k in ('freeze','frozen_review','output_root','provenance_root'))
  if include_locks:paths.append(rp(c,'generator_lock'))
 paths.extend([ROOT/'reports/provenance/capacity_external_validity_v1_r5_REVIEW_BINDING.json',ROOT/'reports/provenance/capacity_external_validity_v1_r5_launcher_20260810r5.log'])
 return paths

def _candidate_preflight_impl(write:bool=True,device_name:str='cpu')->dict[str,Any]:
 ...
 for kind in ('capacity','external'):
  q=cfg(kind)
  for key in ('candidate_root','candidate_review','generator_lock','freeze','frozen_review','output_root','provenance_root'):
   p=rp(q,key);absent.append({'path':p.relative_to(ROOT).as_posix(),'absent':not p.exists()})
 extras=[ROOT/'reports/provenance/capacity_external_validity_v1_r5_REVIEW_BINDING.json',ROOT/'reports/provenance/capacity_external_validity_v1_r5_launcher_20260810r5.log']
 absent.extend({'path':p.relative_to(ROOT).as_posix(),'absent':not p.exists()} for p in extras)
 env_absence=recovery_environment_absence()
 if not all(x['absent'] for x in absent):raise RuntimeError(f'r5 namespace preexists {[x for x in absent if not x["absent"]]}')
 result={'status':'PASS','preservation':pv,'failure_preservation':fp,'parity':parity,'paper':pap,'basis_checks':checks,'nonpanel_ols_max_abs':err,'timed_smoke':smoke,'compatibility_smoke':compatibility,'r5_absence_checks':absent,'environment_absence':env_absence,'scientific_jsonl_accessed':False,'files':inventory(candidate_files())}
 if write:
  for kind in ('capacity','external'):
   out=rp(cfg(kind),'candidate_root')
   out.mkdir(parents=True,exist_ok=False);atomic_json(out/'CANDIDATE_PREFLIGHT.json',{**result,'kind':kind})
 return result

@jsonl_guarded
def candidate_preflight(write:bool=True,device_name:str='cpu')->dict[str,Any]:return _candidate_preflight_impl(write,device_name)
...
def launcher_render(uuid:str)->str:
 return f'export CUDA_VISIBLE_DEVICES="$GPU_UUID"\n[[ "$GPU_UUID" == "{uuid}" ]]\ncd "$ROOT"\nRUN_ROOT=$(readlink /proc/self/cwd)\nMSAE_ROOT="$RUN_ROOT" python "$RUN_ROOT/scripts/capacity_external_validity_v1_r5.py" run-pipeline --physical-index "$PHYSICAL_INDEX" --gpu-uuid "$GPU_UUID" --launch-token "$TOKEN" --pane-pid "$$" --gpu-lockdir "$LOCKDIR"'
...
def create_locks()->dict[str,Any]:
 ...
 failure=failure_preservation_verify();shared={'created_ns':time.time_ns(),'candidate_reviews':[cr,er],'candidate_manifests':[cm,em],'payloads_preexisting':True,'recovery_binding':True,'both_bindings_before_r5_access':True,'failure_preservation_sha256':failure['manifest_sha256'],'v1_lineage':current_v1_lineage(),'invalid_r2_candidate_preservation_sha256':sha(INVALID_R2_PRESERVE),'invalid_r3_candidate_preservation_sha256':sha(INVALID_R3_PRESERVE),'invalid_r4_candidate_preservation_sha256':sha(INVALID_R4_PRESERVE),'v1_partial_checkpoints_reused':False}
 atomic_json(rp(cc,'generator_lock'),{'schema_version':'capacity_recovery_binding_v1_r5',**shared,'config_sha256':sha(CAP_CFG)});atomic_json(rp(ec,'generator_lock'),{'schema_version':'external_recovery_binding_v1_r5',**shared,'config_sha256':sha(EXT_CFG)})
...
def locks_valid()->None:
 ...
 failure=failure_preservation_verify();records=[]
 expected_manifests={str((rp(cc,'candidate_root')/'CANDIDATE_PREFLIGHT.json').relative_to(ROOT)),str((rp(ec,'candidate_root')/'CANDIDATE_PREFLIGHT.json').relative_to(ROOT))}
 expected_reviews={str(rp(cc,'candidate_review').relative_to(ROOT)),str(rp(ec,'candidate_review').relative_to(ROOT))}
 for kind,c,p in [('capacity',cc,CAP_CFG),('external',ec,EXT_CFG)]:
  x=loadj(rp(c,'generator_lock'))
  if x.get('schema_version')!=f'{kind}_recovery_binding_v1_r5' or x.get('config_sha256')!=sha(p) or not x.get('payloads_preexisting') or not x.get('recovery_binding') or not x.get('both_bindings_before_r5_access') or x.get('v1_partial_checkpoints_reused') is not False:raise RuntimeError('invalid recovery binding')
  if x.get('failure_preservation_sha256')!=failure['manifest_sha256'] or x.get('invalid_r2_candidate_preservation_sha256')!=sha(INVALID_R2_PRESERVE) or x.get('invalid_r3_candidate_preservation_sha256')!=sha(INVALID_R3_PRESERVE) or x.get('invalid_r4_candidate_preservation_sha256')!=sha(INVALID_R4_PRESERVE) or x.get('v1_lineage')!=current_v1_lineage():raise RuntimeError('preserved lineage drift')
  manifests=x.get('candidate_manifests');reviews=x.get('candidate_reviews')
  if not isinstance(manifests,list) or len(manifests)!=2 or {rec.get('path') for rec in manifests}!=expected_manifests:raise RuntimeError('candidate manifest bindings incomplete')
  if not isinstance(reviews,list) or len(reviews)!=2 or {rec.get('path') for rec in reviews}!=expected_reviews:raise RuntimeError('candidate review bindings incomplete')
  ...
  records.append({k:v for k,v in x.items() if k not in {'schema_version','config_sha256'}})
 if records[0]!=records[1]:raise RuntimeError('recovery bindings disagree')
...
def freeze_one(kind:str)->dict[str,Any]:
 c=cfg(kind);p=CAP_CFG if kind=='capacity' else EXT_CFG;prep=rp(c,'prepared_root');metadata=loadj(prep/'PANEL_METADATA.json');payload=payload_records_from_metadata(metadata);base=[PLAN,PLAN_REVIEW,PRESERVE,FAILURE_PRESERVE,INVALID_R2_PRESERVE,INVALID_R3_PRESERVE,INVALID_R4_PRESERVE,ROOT/'scripts/preserve_capacity_external_validity_v1_failure.py',p,SCRIPT,LAUNCHER,TEST,rp(c,'generator_lock'),prep/'PANEL_METADATA.json',prep/'PREPARED.json']
 ...
 f=rp(c,'freeze');atomic_json(f,{'schema_version':f'{kind}_recovery_freeze_v1_r5','status':'FROZEN_PANEL_SPECIFIC_RECOVERY','kind':kind,'panel_access_state':access,'payloads_preexisting':True,'recovery_binding_sha256':sha(rp(c,'generator_lock')),'failure_preservation_sha256':sha(FAILURE_PRESERVE),'invalid_r2_candidate_preservation_sha256':sha(INVALID_R2_PRESERVE),'invalid_r3_candidate_preservation_sha256':sha(INVALID_R3_PRESERVE),'invalid_r4_candidate_preservation_sha256':sha(INVALID_R4_PRESERVE),'v1_partial_checkpoints_reused':False,'inventory':inventory(base),'payloads':payload,'payload_hash_source':'existing v1 PANEL_METADATA.json; r5 freeze did not stat/glob/hash/read payloads','created_ns':time.time_ns()});return {'kind':kind,'sha256':sha(f),'files':len(base)}
@jsonl_guarded
def freeze_both()->dict[str,Any]:
 recovery_environment_absence()
 require_absent('freeze phase',postcandidate_absence_paths(include_locks=False))
 locks_valid()
...
def bind_reviews()->dict[str,Any]:
 recovery_environment_absence();require_absent('review binding phase',[rp(cfg(k),'output_root') for k in ('capacity','external')]+[rp(cfg(k),'provenance_root') for k in ('capacity','external')]+[binding_path(),ROOT/'reports/provenance/capacity_external_validity_v1_r5_launcher_20260810r5.log'])
 locks_valid();verify_freeze('capacity');verify_freeze('external')
 rows=[]
 for kind in ('capacity','external'):
 ...
 out=rp(cfg('capacity'),'provenance_root').parent/'capacity_external_validity_v1_r5_REVIEW_BINDING.json';atomic_json(out,{'schema_version':'capacity_external_review_binding_v1_r5','reviews':rows,'created_ns':time.time_ns()});return {'status':'PASS','path':out.relative_to(ROOT).as_posix(),'sha256':sha(out)}
def binding_path()->Path:return rp(cfg('capacity'),'provenance_root').parent/'capacity_external_validity_v1_r5_REVIEW_BINDING.json'
@jsonl_guarded
...
def ancestor_chain(pid:int)->list[int]:
 out=[];seen=set();cur=pid
 while cur>1 and cur not in seen:
  seen.add(cur);out.append(cur)
  try:
   line=next(x for x in (Path('/proc')/str(cur)/'status').read_text().splitlines() if x.startswith('PPid:'));cur=int(line.split(':',1)[1])
  except (FileNotFoundError,StopIteration,ValueError):break
 return out
def gpu_validate(index:int,uuid:str,pane_pid:int,lockdir:str,token:str)->tuple[torch.device,dict[str,Any]]:
 ...
 if os.environ.get('CUDA_VISIBLE_DEVICES')!=uuid:raise RuntimeError('CUDA_VISIBLE_DEVICES is not UUID-bound')
 expected_lock=Path('/tmp')/f'msae_capacity_external_r5_{uuid}.lockdir';actual_lock=Path(lockdir)
 if actual_lock!=expected_lock or not actual_lock.is_dir() or (actual_lock/'token').read_text().strip()!=token:raise RuntimeError('GPU UUID lock ownership mismatch')
 chain=ancestor_chain(os.getpid())
 if pane_pid not in chain:raise RuntimeError(f'worker is not a descendant of tmux pane {pane_pid}: {chain}')
 if not torch.cuda.is_available() or torch.cuda.device_count()!=1:raise RuntimeError('CUDA visibility not exactly one')
 torch.empty(1,device='cuda:0');torch.cuda.synchronize()
 rows=subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,gpu_uuid','--format=csv,noheader,nounits'],text=True).splitlines()
 visible={u.strip() for row in rows if ',' in row for pid,u in [row.split(',',1)] if pid.strip()==str(os.getpid())}
 if visible!={uuid}:raise RuntimeError(f'Torch process UUID mismatch: expected {uuid}, observed {sorted(visible)}')
 return torch.device('cuda:0'),{'worker_pid':os.getpid(),'pane_pid':pane_pid,'ancestor_chain':chain,'locked_gpu_uuid':uuid,'gpu_lockdir':lockdir}
@jsonl_guarded
...
def open_rows(kind:str,stage:str,instance:int|None=None)->list[dict[str,Any]]:
 ...
 return read_jsonl(p)
def open_capacity_stage(c:Mapping[str,Any],stage:str,prov:Path,access_index:int,opened_index:int)->dict[int,list[dict[str,Any]]]:
 event(prov,access_index,f'CAPACITY_{stage.upper()}_ACCESS_MAY_HAVE_OCCURRED');rows={i:open_rows('capacity',stage,i) for i in c['instances']};event(prov,opened_index,f'CAPACITY_{stage.upper()}_OPENED');return rows
def capacity_pipeline(device:torch.device,prov:Path,out:Path)->dict[str,Any]:
 c=cfg('capacity');train=open_capacity_stage(c,'train',prov,10,11);dev=open_capacity_stage(c,'development',prov,12,13)
 ...
 event(prov,14,'CAPACITY_DEVELOPMENT_COMPLETE');conf=open_capacity_stage(c,'confirmation',prov,15,16)
 ...
 final={'schema_version':'capacity_controller_diagnostic_v1_final','status':'CAPACITY_DIAGNOSTIC_COMPLETE','external_authorized':bool(auth),'instances':summaries,'family_pass_all_instances_both_panels':family_all,'fits':fits,'lineage':lineage,'scope':c['scope']};atomic_json(out/'final/result.json',final);event(prov,17,'CAPACITY_COMPLETE',external_authorized=bool(auth),final_sha256=sha(out/'final/result.json'));return final
...
def external_pipeline(device:torch.device,prov:Path,out:Path,cap:Mapping[str,Any])->None:
 cap_path=rp(cfg('capacity'),'output_root')/'final/result.json';disk_cap=loadj(cap_path);cap_sha=sha(cap_path);event(prov,20,'PRECHECK_NO_ACCESS',capacity_final_sha256=cap_sha,capacity_external_authorized=bool(disk_cap.get('external_authorized')))
 if canon(disk_cap)!=canon(cap):raise RuntimeError('in-memory capacity decision differs from durable final')
 if not disk_cap['external_authorized']:external_blocked(prov,out,disk_cap);return
 ...
 event(prov,23,'EXTERNAL_DEVELOPMENT_ACCESS_MAY_HAVE_OCCURRED');dev=open_rows('external','development');event(prov,24,'EXTERNAL_DEVELOPMENT_OPENED');development={str(sd):score_copy_model(models[sd],dev,c,sd,'development',device) for sd in c['model']['seeds']};event(prov,25,'EXTERNAL_DEVELOPMENT_COMPLETE')
 event(prov,26,'EXTERNAL_CONFIRMATION_ACCESS_MAY_HAVE_OCCURRED');conf=open_rows('external','confirmation');event(prov,27,'EXTERNAL_CONFIRMATION_OPENED');confirmation={str(sd):score_copy_model(models[sd],conf,c,sd,'confirmation',device) for sd in c['model']['seeds']}
 qualified=all(development[str(sd)]['all_gates_pass'] and confirmation[str(sd)]['all_gates_pass'] for sd in c['model']['seeds'])
 result={'schema_version':'trained_copy_external_v1_final','status':'TRAINED_COPY_EXTERNAL_QUALIFIED' if qualified else 'TRAINED_COPY_EXTERNAL_NOT_QUALIFIED','training_performed':True,'payload_accessed':True,'fits':fits,'development':development,'confirmation':confirmation,'all_seeds_both_panels_pass':qualified,'scope':c['scope']};atomic_json(out/'final/result.json',result);event(prov,28,'EXTERNAL_TERMINAL',status=result['status'],final_sha256=sha(out/'final/result.json'))
def run_pipeline(index:int,uuid:str,token:str,pane_pid:int,lockdir:str)->None:
 cc,ec=cfg('capacity'),cfg('external');cp,ep=rp(cc,'provenance_root'),rp(ec,'provenance_root');co,eo=rp(cc,'output_root'),rp(ec,'output_root')
 ...
 try:
  ...
  launch_preflight(index,uuid);device,ownership=gpu_validate(index,uuid,pane_pid,lockdir,token)
  for p in (cp,ep,co,eo):p.mkdir(parents=True,exist_ok=False)
  atomic_json(cp/'LAUNCH.json',{'physical_index':index,'gpu_uuid':uuid,'cuda_visible_devices':os.environ.get('CUDA_VISIBLE_DEVICES'),'ownership':ownership,'launch_token_sha256':hashlib.sha256(token.encode()).hexdigest(),'time_ns':time.time_ns(),'freeze_sha256':sha(rp(cc,'freeze'))});atomic_json(ep/'LAUNCH.json',{'physical_index':index,'gpu_uuid':uuid,'cuda_visible_devices':os.environ.get('CUDA_VISIBLE_DEVICES'),'ownership':ownership,'launch_token_sha256':hashlib.sha256(token.encode()).hexdigest(),'time_ns':time.time_ns(),'freeze_sha256':sha(rp(ec,'freeze'))})
  torch.use_deterministic_algorithms(True);torch.manual_seed(cc['seed']);np.random.seed(cc['seed']);random.seed(cc['seed'])
  cap=capacity_pipeline(device,cp,co);external_pipeline(device,ep,eo,cap)
 except BaseException as e:
  write_external_failure_terminal(f'{type(e).__name__}: {e}')
  for p in (cp,ep):
...
def main()->None:
 ...
 elif a.cmd=='run-pipeline':run_pipeline(a.physical_index,a.gpu_uuid,a.launch_token,a.pane_pid,a.gpu_lockdir)
