...
DEFAULT = ROOT / "configs/constructed_copy_circuit_v1_1/run.json"
...
def config(path: Path = DEFAULT) -> dict[str, Any]:
    cfg = loadj(path)
    if cfg.get("schema_version") != "constructed_copy_circuit_v1_1_config":
...
def verify_fresh_seed_provenance(cfg:Mapping[str,Any])->dict[str,Any]:
    predecessor_path=ROOT/"configs/constructed_copy_circuit_v1/run.json"; predecessor=loadj(predecessor_path)
    old={int(predecessor["panels"]["development_seed"]),int(predecessor["panels"]["confirmation_seed"])}
    new={int(cfg["panels"]["development_seed"]),int(cfg["panels"]["confirmation_seed"])}
    if len(new)!=2 or old&new: raise RuntimeError("scientific panel seed reuse")
    return {"predecessor_config_path":rel(predecessor_path),"predecessor_config_sha256":sha(predecessor_path),
            "retired_scientific_seeds":sorted(old),"successor_scientific_seeds":sorted(new),"seed_sets_disjoint":True}
...
def create_candidate_attestations(config_path:Path)->None:
    cfg=config(config_path); verify_protocol(cfg); verify_fresh_seed_provenance(cfg); prepared=loadj(ROOT/cfg["runtime"]["prepared_root"]/"PREPARED.json")
    ...
    generator=loadj(ROOT/cfg["runtime"]["prepared_root"]/"GENERATOR_ATTESTATION.json")
    if generator.get("status")!="PASS" or generator.get("development_rows_implied_eligible")!=256 or generator.get("confirmation_rows_implied_eligible")!=256:
        raise RuntimeError("creation-time analytic eligibility attestation")
    atomic_json(candidate/"ANALYTIC_ELIGIBILITY.json",{"schema_version":"constructed_copy_circuit_v1_1_analytic_eligibility","status":"PASS","derivation":"creation_time_generator_attestation",
        "development_rows_passing":prepared["development"]["rows"],"confirmation_rows_passing":prepared["confirmation"]["rows"],"generator_attestation_sha256":sha(ROOT/cfg["runtime"]["prepared_root"]/"GENERATOR_ATTESTATION.json"),**SCOPE})
...
def panel_rows(cfg: Mapping[str, Any], stage: str, *, smoke: bool = False) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        ...
        rows.append({
            "schema_version": "constructed_copy_circuit_v1_1_row", "stage": stage,
...
def prepare(config_path: Path) -> None:
    cfg=config(config_path); freshness=verify_fresh_seed_provenance(cfg); root=ROOT/cfg["runtime"]["prepared_root"]
    ...
    fixture={"schema_version":"constructed_copy_circuit_v1_1_calibration_fixture","scenarios":{
    ...
    v=cfg["model"]["vocab_size"]; gain=float(cfg["model"]["readout_gain"]); clean=gain; corrupt=-gain/(v-1); effect=clean-corrupt; e=cfg["eligibility"]
    implied=clean>e["clean_margin_strict"] and corrupt<e["corrupt_margin_strict_upper"] and effect>e["effect_strict"]
    if not implied: raise RuntimeError("generator analytic eligibility")
    atomic_json(root/"GENERATOR_ATTESTATION.json",{"schema_version":"constructed_copy_circuit_v1_1_generator_attestation","status":"PASS","created_before_protocol_lock":True,
        "derivation":"12_times_unique_one_hot_transport","clean_margin":clean,"corrupt_margin":corrupt,"effect":effect,
        "development_rows_implied_eligible":len(panels["development"]),"confirmation_rows_implied_eligible":len(panels["confirmation"]),"fresh_seed_provenance":freshness,**SCOPE})
    atomic_json(root/"PREPARED.json",{
        "schema_version":"constructed_copy_circuit_v1_1_prepared","status":"PASS",
        ...
        "calibration_fixture_sha256":sha(root/"calibration_fixture.json"),"generator_attestation_sha256":sha(root/"GENERATOR_ATTESTATION.json"),"prompt_overlap":0,
        **SCOPE})
...
def verify_prepared(cfg: Mapping[str, Any], *, include_development:bool, include_confirmation: bool) -> dict[str,Any]:
    ...
    if rec["smoke"]["sha256"]!=sha(smoke) or rec["calibration_rows"]["sha256"]!=sha(calibration_rows) or rec["calibration_fixture_sha256"]!=sha(root/"calibration_fixture.json") or rec["generator_attestation_sha256"]!=sha(root/"GENERATOR_ATTESTATION.json"):
...
def protocol_lock(config_path: Path) -> None:
    cfg=config(config_path); verify_v2_preservation(cfg); freshness=verify_fresh_seed_provenance(cfg); verify_prepared(cfg,include_development=True,include_confirmation=True)
    root=ROOT/cfg["runtime"]["prepared_root"]
    paths=[Path(config_path),ROOT/"PLAN_CONSTRUCTED_COPY_CIRCUIT_V1_1.md",root/"development.jsonl",root/"confirmation.jsonl",root/"smoke_fixture.jsonl",root/"calibration_rows.jsonl",root/"calibration_fixture.json",root/"GENERATOR_ATTESTATION.json",root/"PREPARED.json"]
    items=[{"path":rel(p),"bytes":p.stat().st_size,"sha256":sha(p)} for p in paths]
    atomic_json(ROOT/cfg["runtime"]["protocol_lock"],{
        "schema_version":"constructed_copy_circuit_v1_1_protocol_lock","status":"LOCKED_BEFORE_MODEL_SMOKE",
        "items":items,"items_sha256":hashlib.sha256(canon(items)).hexdigest(),"equations":"PLAN_CONSTRUCTED_COPY_CIRCUIT_V1_1.md",
        "development_payload":next(x for x in items if x["path"].endswith("development.jsonl")),"confirmation_payload":next(x for x in items if x["path"].endswith("confirmation.jsonl")),"fresh_seed_provenance":freshness,**SCOPE})
...
def verify_protocol(cfg: Mapping[str, Any], *, include_development:bool=False, include_confirmation: bool=False) -> dict[str,Any]:
    ...
    if rec.get("fresh_seed_provenance")!=verify_fresh_seed_provenance(cfg): raise RuntimeError("fresh seed provenance drift")
...
def evaluate(arr: Mapping[str,np.ndarray], rows:Sequence[Mapping[str,Any]],cfg:Mapping[str,Any],stage:str)->tuple[list[dict[str,Any]],dict[str,Any],dict[str,Any]]:
    ...
    summary={"schema_version":"constructed_copy_circuit_v1_1_summary","stage":stage,"technical_valid":True,"support_pass":support,
    ...
    qa={"schema_version":"constructed_copy_circuit_v1_1_qa","pre_softmax_mask_contract":True,"attention_exact_one_hot":True,"all_intervention_routes_recomputed":True,
...
def calibration(config_path:Path)->None:
    ...
    atomic_json(target,{"schema_version":"constructed_copy_circuit_v1_1_calibration","status":"PASS","outcomes":outcomes,"comparators":comparators,
...
def smoke(config_path:Path,device_name:str)->None:
    ...
    atomic_json(target,{"schema_version":"constructed_copy_circuit_v1_1_smoke","status":"PASS","device":device.type,"exact_oracle":comp["pass"],
...
def candidate_inventory(config_path:Path,cfg:Mapping[str,Any])->list[dict[str,Any]]:
    ...
    root=ROOT/cfg["runtime"]["prepared_root"]
    paths += [root/x for x in ("development.jsonl","confirmation.jsonl","smoke_fixture.jsonl","calibration_rows.jsonl","calibration_fixture.json","GENERATOR_ATTESTATION.json","PREPARED.json")]
...
def candidate_preflight(config_path:Path)->None:
    cfg=config(config_path); verify_v2_preservation(cfg); verify_fresh_seed_provenance(cfg); verify_protocol(cfg); verify_prepared(cfg,include_development=False,include_confirmation=False); verify_calibration_smokes(cfg)
...
def freeze(config_path:Path)->None:
    ...
    inv=candidate_inventory(config_path,cfg); develop=next(x for x in inv if x["path"].endswith("development.jsonl")); confirm=next(x for x in inv if x["path"].endswith("confirmation.jsonl"))
    atomic_json(ROOT/cfg["runtime"]["freeze"],{"schema_version":"constructed_copy_circuit_v1_1_freeze","status":"FROZEN",
        "config_sha256":sha(config_path),"candidate_inventory":inv,"candidate_inventory_sha256":hashlib.sha256(canon(inv)).hexdigest(),
        "development_payload":develop,"confirmation_payload":confirm,"protocol_lock_sha256":sha(ROOT/cfg["runtime"]["protocol_lock"]),"v2_preservation_sha256":sha(ROOT/cfg["runtime"]["v2_preservation"]),**SCOPE})
...
def bind_review(config_path:Path)->None:
    ...
    atomic_json(ROOT/cfg["runtime"]["review_binding"],{"schema_version":"constructed_copy_circuit_v1_1_review_binding","status":"SHIP","freeze_sha256":sha(fr),"candidate_review_sha256":sha(cr),"frozen_review_sha256":sha(rr)})
...
def verify_review_binding(config_path:Path,*,include_development:bool=False,include_confirmation:bool=False)->dict[str,Any]:
    ...
    b=loadj(ROOT/cfg["runtime"]["review_binding"]); expected={"schema_version":"constructed_copy_circuit_v1_1_review_binding","status":"SHIP","freeze_sha256":sha(ROOT/cfg["runtime"]["freeze"]),"candidate_review_sha256":sha(ROOT/cfg["runtime"]["candidate_review"]),"frozen_review_sha256":sha(ROOT/cfg["runtime"]["frozen_review"])}
...
def validate_launch(config_path:Path,token:str)->dict[str,Any]:
    ...
    if set(r)!=required or r["schema_version"]!="constructed_copy_circuit_v1_1_launch" or r["launch_token"]!=token: raise RuntimeError("launch schema/token")
...
def new_attempt(cfg:Mapping[str,Any],stage:str,launch_hash:str)->str:
    ...
    atomic_json(d/"00_INITIALIZED.json",{"schema_version":"constructed_copy_circuit_v1_1_attempt_event","stage":stage,"state":"INITIALIZED","attempt_token":token,"launch_manifest_sha256":launch_hash,"time_ns":time.time_ns()})
    return token
...
def transition(cfg:Mapping[str,Any],stage:str,token:str,expected:str,state:str)->None:
    ...
    atomic_json(d/f"{idx:02d}_{state}.json",{"schema_version":"constructed_copy_circuit_v1_1_attempt_event","stage":stage,"state":state,"attempt_token":token,"launch_manifest_sha256":sha(launch_manifest(cfg)),"time_ns":time.time_ns()})
...
def reference(config_path:Path,stage:str,replicate:int,launch_token:str,attempt_token:str)->None:
    ...
    if stage=="development": validate_development_authorization(config_path,launch_token)
    else: validate_authorization(config_path,launch_token)
    ...
    atomic_json(q/f"reference_{replicate}.json",{"schema_version":"constructed_copy_circuit_v1_1_reference","stage":stage,"replicate":replicate,"arrays_sha256":sha(target),"oracle_exact":comp["pass"],"fields":len(comp["fields"]),"gpu_uuid":launch["gpu"]["uuid"],"rows_sha256":sha(path),**SCOPE})
...
def compare_references(config_path:Path,stage:str,launch_token:str,attempt_token:str)->None:
    ...
    atomic_json(q/"REFERENCE_COMPARISON.json",{"schema_version":"constructed_copy_circuit_v1_1_reference_comparison","stage":stage,"status":"PASS","bitwise":comp["pass"],"fields":len(comp["fields"]),"reference_1_sha256":sha(q/"reference_1.npz"),"reference_2_sha256":sha(q/"reference_2.npz"),**SCOPE})


def validate_reference_bundle(q:Path,row_path:Path,stage:str)->dict[str,Any]:
    comp=loadj(q/"REFERENCE_COMPARISON.json")
    if comp.get("schema_version")!="constructed_copy_circuit_v1_1_reference_comparison" or comp.get("status")!="PASS" or not comp.get("bitwise"): raise RuntimeError("reference comparison")
...
def score_stage(config_path:Path,stage:str,launch_token:str,attempt_token:str)->None:
    ...
    atomic_json(out/"STARTED.json",{"schema_version":"constructed_copy_circuit_v1_1_stage_runtime","stage":stage,"gpu_uuid":launch["gpu"]["uuid"],"torch":torch.__version__,"cuda":torch.version.cuda,"device":torch.cuda.get_device_name(0),"inference_mode":True,**SCOPE})
    atomic_json(out/"COMPLETE.json",{"schema_version":"constructed_copy_circuit_v1_1_complete","stage":stage,"status":summary["status"],"metrics_sha256":sha(out/"metrics.jsonl"),"summary_sha256":sha(out/"SUMMARY.json"),"qa_sha256":sha(out/"QA.json"),"freeze_sha256":sha(ROOT/cfg["runtime"]["freeze"]),"launch_manifest_sha256":sha(launch_manifest(cfg)),"attempt_token":attempt_token,**SCOPE})
...
def technical_terminal(cfg:Mapping[str,Any],stage:str,reason:str,token:str)->None:
    ...
    if not p.exists(): atomic_json(p,{"schema_version":"constructed_copy_circuit_v1_1_technical_invalid","stage":stage,"status":"TECHNICAL_INVALID_STOP","reason":reason,"attempt_token":token,"launch_manifest_sha256":sha(launch_manifest(cfg)),**SCOPE})


def development_authorization_paths(cfg:Mapping[str,Any])->tuple[Path,Path]:
    return ROOT/cfg["runtime"]["output_root"]/"development_authorization",ROOT/cfg["runtime"]["provenance_root"]/"development_authorization_events"


def development_auth_event(events:Path,index:int,state:str,**extra:Any)->None:
    atomic_json(events/f"{index:02d}_{state}.json",{"schema_version":"constructed_copy_circuit_v1_1_development_authorization_event","state":state,"time_ns":time.time_ns(),**extra})


def authorize_development(config_path:Path,launch_token:str)->None:
    cfg=config(config_path); validate_launch(config_path,launch_token); root,events=development_authorization_paths(cfg)
    if root.exists() or events.exists(): raise RuntimeError("development authorization exists")
    events.mkdir(parents=True); root.mkdir(parents=True); development_auth_event(events,0,"PRECHECK_JOURNALED",payload_access_may_have_occurred=False)
    try:
        verify_review_binding(config_path); freshness=verify_fresh_seed_provenance(cfg)
        development_auth_event(events,1,"PHASE1_PASS_NO_ACCESS",payload_access_may_have_occurred=False,review_binding_sha256=sha(ROOT/cfg["runtime"]["review_binding"]),fresh_seed_provenance=freshness)
        development_auth_event(events,2,"ACCESS_MAY_HAVE_OCCURRED",payload_access_may_have_occurred=True)
        payload=ROOT/cfg["runtime"]["prepared_root"]/"development.jsonl"; size=payload.stat().st_size; digest=sha(payload); fr=loadj(ROOT/cfg["runtime"]["freeze"])
        if size!=fr["development_payload"]["bytes"] or digest!=fr["development_payload"]["sha256"]: raise RuntimeError("development payload drift")
        authorization={"schema_version":"constructed_copy_circuit_v1_1_development_authorization","status":"AUTHORIZED","payload_accessed":True,"payload_bytes":size,"payload_sha256":digest,
            "review_binding_sha256":sha(ROOT/cfg["runtime"]["review_binding"]),"fresh_seed_provenance":freshness,**SCOPE}
        atomic_json(root/"DEVELOPMENT_AUTHORIZATION.json",authorization)
        development_auth_event(events,3,"CLOSED_AUTHORIZED",payload_access_may_have_occurred=True,authorization_sha256=sha(root/"DEVELOPMENT_AUTHORIZATION.json"))
    except Exception as e:
        may=any("ACCESS_MAY_HAVE_OCCURRED" in p.name for p in events.glob("*.json")); development_auth_event(events,len(list(events.glob('*.json'))),"CLOSED_TECHNICAL_INVALID",payload_access_may_have_occurred=may)
        atomic_json(root/"DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP.json",{"status":"DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP","reason":f"{type(e).__name__}: {e}","payload_access_may_have_occurred":may,**SCOPE}); raise


def validate_development_authorization(config_path:Path,launch_token:str)->dict[str,Any]:
    cfg=config(config_path); validate_launch(config_path,launch_token); root,events=development_authorization_paths(cfg); paths=sorted(events.glob("*.json")); records=[loadj(p) for p in paths]
    if [r.get("state") for r in records] != ["PRECHECK_JOURNALED","PHASE1_PASS_NO_ACCESS","ACCESS_MAY_HAVE_OCCURRED","CLOSED_AUTHORIZED"]: raise RuntimeError("development authorization journal")
    rec=loadj(root/"DEVELOPMENT_AUTHORIZATION.json"); payload=ROOT/cfg["runtime"]["prepared_root"]/"development.jsonl"
    if records[-1].get("authorization_sha256")!=sha(root/"DEVELOPMENT_AUTHORIZATION.json") or rec.get("status")!="AUTHORIZED" or rec.get("payload_sha256")!=sha(payload) or rec.get("fresh_seed_provenance")!=verify_fresh_seed_provenance(cfg):
        raise RuntimeError("development authorization validation")
    return rec
...
def stage_controller(config_path:Path,stage:str,launch_token:str)->None:
    ...
    try:
        validate_launch(config_path,launch_token)
        if stage=="development": validate_development_authorization(config_path,launch_token)
        else: validate_authorization(config_path,launch_token)
        verify_review_binding(config_path,include_development=True,include_confirmation=stage=="confirmation")
...
def gate_event(events:Path,index:int,state:str,**extra:Any)->None:
    payload={"schema_version":"constructed_copy_circuit_v1_1_gate_event","state":state,"time_ns":time.time_ns(),**extra}; atomic_json(events/f"{index:02d}_{state}.json",payload)
...
def development_gate(config_path:Path,launch_token:str)->None:
    ...
    try:
        ...
        atomic_json(root/"result.json",{"schema_version":"constructed_copy_circuit_v1_1_development_gate","status":status,"confirmation_authorized":status=="PASS","development_summary_sha256":sha(ROOT/cfg["runtime"]["output_root"]/"development/SUMMARY.json"),"metrics":summary["metrics"],"gate_decisions":summary["gate_decisions"],**SCOPE})
...
def auth_event(events:Path,index:int,state:str,**extra:Any)->None:
    atomic_json(events/f"{index:02d}_{state}.json",{"schema_version":"constructed_copy_circuit_v1_1_authorization_event","state":state,"time_ns":time.time_ns(),**extra})
...
def authorize_confirmation(config_path:Path,launch_token:str)->None:
    ...
    try:
        ...
        if gate["status"]!="PASS":
            ...
            atomic_json(root/"CONFIRMATION_BLOCKED.json",{"schema_version":"constructed_copy_circuit_v1_1_confirmation_blocked","status":"BLOCKED","reason":"DEVELOPMENT_GATE_STOP","payload_touched":False,"natural_prompt_assay":False})
            return
        ...
        authorization={"schema_version":"constructed_copy_circuit_v1_1_confirmation_authorization","status":"AUTHORIZED","phase1_pass":True,"phase2_pass":True,"payload_accessed":True,"payload_bytes":size,"payload_sha256":digest,"gate_sha256":sha(ROOT/cfg["runtime"]["output_root"]/"development_gate/result.json"),**SCOPE}
...
def final(config_path:Path,launch_token:str)->None:
    ...
    devauthtech=out/"development_authorization/DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP.json"; devtech=out/"development/TECHNICAL_INVALID_STOP.json"; gatetech=out/"development_gate/DEVELOPMENT_GATE_TECHNICAL_INVALID_STOP.json"; authtech=out/"confirmation_authorization/CONFIRMATION_AUTHORIZATION_TECHNICAL_INVALID_STOP.json"; contech=out/"confirmation/TECHNICAL_INVALID_STOP.json"
    ...
    try:
        if any(p.is_file() for p in (devauthtech,devtech,gatetech,authtech,contech)): status="TECHNICAL_INVALID_STOP"
        elif dev is None: status="TECHNICAL_INVALID_STOP"
        elif not dev["all_gates_pass"]:
            validate_development_authorization(config_path,launch_token); validate_development_stop_bundle(cfg); status="DEVELOPMENT_GROUND_TRUTH_STOP"
        else:
            validate_development_authorization(config_path,launch_token); validate_development_gate_bundle(cfg); validate_authorization(config_path,launch_token)
    ...
    atomic_json(target,{"schema_version":"constructed_copy_circuit_v1_1_final","status":status,"development":dev,"confirmation":conf,
...
def reconcile(config_path:Path,launch_token:str)->None:
    ...
    dev_auth_root,dev_auth_events=development_authorization_paths(cfg); dev_auth_files=sorted(dev_auth_events.glob("*.json")) if dev_auth_events.exists() else []
    if dev_auth_files:
        try: dev_auth_records=[loadj(p) for p in dev_auth_files]; dev_auth_state=dev_auth_records[-1]["state"]
        except Exception: dev_auth_records=[]; dev_auth_state="MALFORMED"
        dev_auth_valid=False
        if dev_auth_state=="CLOSED_AUTHORIZED":
            try: validate_development_authorization(config_path,launch_token); dev_auth_valid=True
            except Exception: dev_auth_valid=False
        decision,may=authorization_reconcile_decision(dev_auth_records,dev_auth_valid)
        if dev_auth_state=="MALFORMED": decision,may="SEAL_TECHNICAL_INVALID",True
        if decision=="SEAL_TECHNICAL_INVALID":
            if dev_auth_state!="MALFORMED": development_auth_event(dev_auth_events,len(dev_auth_files),"CLOSED_TECHNICAL_INVALID",payload_access_may_have_occurred=may)
            dev_auth_root.mkdir(parents=True,exist_ok=True); p=dev_auth_root/"DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP.json"
            if not p.exists(): atomic_json(p,{"status":"DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP","reason":"NO_FORWARD_RECONCILER_SEALED_DEVELOPMENT_AUTHORIZATION","payload_access_may_have_occurred":may,**SCOPE})
    elif dev_auth_root.exists() or dev_auth_events.exists():
        dev_auth_events.mkdir(parents=True,exist_ok=True); development_auth_event(dev_auth_events,0,"CLOSED_TECHNICAL_INVALID",payload_access_may_have_occurred=False)
        dev_auth_root.mkdir(parents=True,exist_ok=True); atomic_json(dev_auth_root/"DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP.json",{"status":"DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP","reason":"NO_FORWARD_RECONCILER_SEALED_EMPTY_DEVELOPMENT_AUTHORIZATION","payload_access_may_have_occurred":False,**SCOPE})
    ...
    upstream_terminal=any((out/p).is_file() for p in ("development_authorization/DEVELOPMENT_AUTHORIZATION_TECHNICAL_INVALID_STOP.json","development/TECHNICAL_INVALID_STOP.json","development_gate/DEVELOPMENT_GATE_TECHNICAL_INVALID_STOP.json"))
    if upstream_terminal and not auth_files and not (auth_root.exists() or auth_events.exists()):
        ...
        if not p.exists(): atomic_json(p,{"schema_version":"constructed_copy_circuit_v1_1_confirmation_blocked","status":"BLOCKED","reason":"UPSTREAM_TECHNICAL_STOP","payload_touched":False,"natural_prompt_assay":False})
...
def main()->None:
    ...
    for name in ("authorize-development","development-gate","authorize-confirmation","final","reconcile"):
    ...
    elif c=="authorize-development": authorize_development(a.config,a.launch_token)
