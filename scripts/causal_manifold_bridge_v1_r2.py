...
import preserve_trained_control_r1_failures as preserve_r1

CFG=ROOT/"configs/causal_manifold_bridge_v1_r2/run.json";PLAN=ROOT/"PLAN_TRAINED_CONTROL_STUDIES_R2_TECHNICAL_RECOVERY.md";SCRIPT=Path(__file__).resolve();TEST=ROOT/"tests/test_causal_manifold_bridge_v1_r2.py";LAUNCH_TEST=ROOT/"tests/test_launch_trained_control_next_studies_v1_r2.py";LAUNCHER=ROOT/"scripts/launch_trained_control_next_studies_v1_r2_tmux.sh";PARITY=ROOT/"reports/provenance/trained_control_studies_v1_r2_PROTOCOL_PARITY.json"
...
def validate_recovery_lineage(c:Mapping[str,Any])->dict[str,Any]:
    line=c["recovery_lineage"];rec=verify_preservation_record(ROOT/line["preservation_path"],line["preservation_sha256"])
    for item in line["original_freezes"].values():
        if sha(ROOT/item["path"])!=item["sha256"]:raise RuntimeError("original freeze continuity drift")
    if rec["original_freezes"]!=line["original_freezes"]:raise RuntimeError("preservation/config freeze lineage mismatch")
    r1=line["r1_freezes"]
    if sha(ROOT/r1["preservation_path"])!=r1["preservation_sha256"]:raise RuntimeError("R1 failure preservation drift")
    verified=preserve_r1.verify()
    if verified["preservation_sha256"]!=r1["preservation_sha256"]:raise RuntimeError("R1 preservation verifier mismatch")
    for item in r1["items"].values():
        if sha(ROOT/item["path"])!=item["sha256"]:raise RuntimeError("R1 freeze continuity drift")
    return {"pass":True,"preservation_sha256":line["preservation_sha256"],"original_freezes":line["original_freezes"],"r1_freezes":r1}

def verify_protocol_parity()->dict[str,Any]:
 rec=loadj(PARITY)
 if rec.get("status")!="PASS":raise RuntimeError("protocol parity did not pass")
 for item in rec["bound_artifacts"]:
  if sha(ROOT/item["path"])!=item["sha256"]:raise RuntimeError(f"protocol parity artifact drift: {item['path']}")
 return rec
...
def donor_basis_qa(delta:np.ndarray,states:np.ndarray,c:Mapping[str,Any])->dict[str,Any]:
 delta64=delta.astype(np.float64);states64=states.astype(np.float64);_,sd,vd=np.linalg.svd(delta64,full_matrices=False);_,sa,va=np.linalg.svd(states64,full_matrices=False);dq=matrix_qa(delta,max(c["methods"]["ranks"]),c);aq=matrix_qa(states,max(c["methods"]["ranks"]),c)
 return {"delta_basis":vd.astype(np.float32),"ambient_basis":va.astype(np.float32),"delta_singular":sd,"ambient_singular":sa,"donor_matrix_qa":dq,"ambient_matrix_qa":aq}
def write_donor_basis(ck:Path,basis:Mapping[str,Any])->dict[str,Any]:
 path=ck/"bases.npz";np.savez_compressed(path,delta_basis=basis["delta_basis"],ambient_basis=basis["ambient_basis"],delta_singular=basis["delta_singular"],ambient_singular=basis["ambient_singular"]);dq=basis["donor_matrix_qa"];aq=basis["ambient_matrix_qa"]
 return {"sha256":sha(path),"delta_rank":int(dq["rank"]),"ambient_rank":int(aq["rank"]),"delta_condition":float(dq["condition"]),"ambient_condition":float(aq["condition"]),"donor_matrix_qa":dq,"ambient_matrix_qa":aq}
def fit_registry(rows:Sequence[Mapping[str,Any]],c:Mapping[str,Any],gen:int,device:torch.device,ck:Path)->dict[str,Any]:
 ...
 basis=donor_basis_qa(delta,states,c);delta_qa=basis["donor_matrix_qa"];ambient_qa=basis["ambient_matrix_qa"]
 if not delta_qa["pass"] or not ambient_qa["pass"]:raise RuntimeError(f"registered donor fit rank/conditioning failed: donor={delta_qa}, ambient={ambient_qa}")
 records["bases"]=write_donor_basis(ck,basis)
...
def candidate(device_name:str,record:bool)->dict[str,Any]:
 c=cfg();validate_config(c);lineage=validate_recovery_lineage(c);verify_protocol_parity();device=torch.device(device_name);checks=[];incomplete_mutations=True;design_qa=[]
 for gen in c["design"]["generator_seeds"]:
 ...
 result={"schema_version":"causal_manifold_bridge_v1_candidate","status":"PASS" if all(x["pass"] for x in checks) and incomplete_mutations and patch_mutations_rejected and all(x["pass"] for x in design_qa) and all(x["pass"] for x in view_qas) else "FAIL","scientific_panel_accessed":False,"recovery_lineage":lineage,"smoke_seed_namespace":"bridge-smoke","factorial_cells":128,"generators":4,"all_cell_exact_checks":len(checks),"incomplete_mutations_detected":bool(incomplete_mutations),"patch_mutations_rejected":patch_mutations_rejected,"realized_design_qa_pass":all(x["pass"] for x in design_qa),"view_firewall_qa_pass":all(x["pass"] for x in view_qas),"method_registry_sha256":hashlib.sha256(canon(c["methods"])).hexdigest(),"exact_qa_max_abs":max(x["torch_numpy_max_abs"] for x in checks),"exact_qa_max_relative_l2":max(x["identity_relative_l2"] for x in checks),"source_sha256":sha(SCRIPT),"config_sha256":sha(CFG),"plan_sha256":sha(PLAN),"tests_sha256":{"study":sha(TEST),"launcher":sha(LAUNCH_TEST)},"launcher_sha256":sha(LAUNCHER),"protocol_parity_sha256":sha(PARITY)}
 if record:atomic_json(rp(c,"candidate_manifest"),result)
 return result

def create_lock()->dict[str,Any]:
 c=cfg();validate_config(c);validate_recovery_lineage(c);verify_protocol_parity();candidate_sha=sha(rp(c,"candidate_manifest"));verify_review(rp(c,"candidate_review"),candidate_sha);lock={"schema_version":"causal_manifold_bridge_v1_generator_lock","source_sha256":sha(SCRIPT),"config_sha256":sha(CFG),"plan_sha256":sha(PLAN),"tests_sha256":{"study":sha(TEST),"launcher":sha(LAUNCH_TEST)},"launcher_sha256":sha(LAUNCHER),"protocol_parity_sha256":sha(PARITY),"candidate_sha256":candidate_sha,"candidate_review_sha256":sha(rp(c,"candidate_review")),"method_table_sha256":hashlib.sha256(canon(c["methods"])).hexdigest(),"factorial_sha256":hashlib.sha256(canon(c["design"]["cells"])).hexdigest()};atomic_json(rp(c,"generator_lock"),lock);return lock
...
def freeze()->dict[str,Any]:
 c=cfg();validate_recovery_lineage(c);verify_protocol_parity();prep=loadj(rp(c,"prepared_root")/"PREPARED.json");out={"schema_version":"causal_manifold_bridge_v1_freeze","generator_lock_sha256":sha(rp(c,"generator_lock")),"payloads":prep["payloads"],"audit":prep["audit"]};r1=loadj(ROOT/c["recovery_lineage"]["r1_freezes"]["items"]["causal_manifold_bridge_v1_r1"]["path"])
 if out["payloads"]!=r1["payloads"] or out["audit"]!=r1["audit"]:raise RuntimeError("R2 freeze payload/audit differs from R1")
 atomic_json(rp(c,"freeze"),out);return out
...
def verify_frozen()->dict[str,Any]:
 c=cfg();validate_recovery_lineage(c);verify_protocol_parity();lock=loadj(rp(c,"generator_lock"));fr=loadj(rp(c,"freeze"));binding=loadj(rp(c,"review_binding"))
 if lock["source_sha256"]!=sha(SCRIPT) or lock["config_sha256"]!=sha(CFG) or lock["protocol_parity_sha256"]!=sha(PARITY) or lock["candidate_sha256"]!=sha(rp(c,"candidate_manifest")) or lock["candidate_review_sha256"]!=sha(rp(c,"candidate_review")) or fr["generator_lock_sha256"]!=sha(rp(c,"generator_lock")):raise RuntimeError("freeze drift")
