...
from run_atlas_rope_v5 import (
    ...
    _verify_attempt8_history,
    _retained_attempt7_pair,
    verify_grid_bundle,
    verify_sentinel_bundle,
)

ADAPTER_CONFIG = ROOT / "configs/atlas_rope_v5/science_adapter.json"
SCIENCE_ROOT = RUN_ROOT / "science"
RESULT_ROOT = ROOT / "results/atlas_rope_v5_attempt9_science"
...
def verify_frozen_contract() -> dict[str, Any]:
    ...
    if (config.get("schema_version") != "atlas_rope_v5_attempt9_science_adapter_v1" or
...
def _verify_science_authorization() -> dict[str, Any]:
    ...
    authorization = _verify_authorization(SCIENCE_AUTHORIZATION, stage="UNCHANGED_SCIENTIFIC_ATLAS")
    if (authorization.get("implementation_candidate_sha256") != sha256_file(IMPLEMENTATION_CANDIDATE) or
            authorization.get("science_adapter") != implementation["science_adapter"] or
    ...
    if authorization.get("science_adapter") != science_adapter_binding():
        raise RuntimeError("science adapter binding drift")
...
def verify_science_cache(source: str) -> dict[str, Any]:
    ...
    if (payload.get("schema_version") != "atlas_rope_v5_attempt9_science_activation_v1" or
...
def extract_source(source: str, signing_key: Path) -> dict[str, Any]:
    ...
    with exclusive_lock("attempt-wide-gpu"):
        ...
        payload = {
            "schema_version": "atlas_rope_v5_attempt9_science_activation_v1",
...
def _qa_bridge_payload(source: str) -> dict[str, Any]:
    ...
    caps = {**cap["caps"], "rtol": cap["rtol"]}
    if source == "EWT":
        old_ref, old_candidate, old_lineage = _retained_attempt7_pair()
        new_ref, new_candidate, history = _verify_attempt8_history()
        ...
        sentinel_root = RUN_ROOT / "validation/GUM_sentinel"
        grid_root = RUN_ROOT / "validation/GUM_grid"
        if not gum_pass.is_file():
            raise RuntimeError("GUM bridge requires signed GUM PASS")
        sentinel_payload = verify_sentinel_bundle(sentinel_root, cap["caps"])
        grid_payload = verify_grid_bundle(grid_root, source="GUM_fresh_validation", validation_caps=cap["caps"])
        pairs = read_json(sentinel_root / "pair_scores.json")["pairs"]
        panels = {"fresh_sentinel": _sentinel_bridge_stats(pairs),
                  "five_bin_grid": {"uniform_shift": _grid_bridge_stat(grid_payload["score"])}}
        lineage = {"gum_pass_sha256": sha256_file(gum_pass),
                   "sentinel_complete_sha256": sha256_file(sentinel_root / "COMPLETE.json"),
                   "grid_complete_sha256": sha256_file(grid_root / "COMPLETE.json"),
                   "sentinel_status": sentinel_payload["status"], "grid_status": grid_payload["status"]}
    else:
    ...
    return {
        "schema_version": "atlas_rope_v5_attempt9_science_qa_bridge_v1",
...
def run_analysis(signing_key: Path) -> dict[str, Any]:
    ...
    artifacts = {path.name: {"bytes": path.stat().st_size, "sha256": sha256_file(path)}
                 for path in stage.iterdir() if path.is_file()}
    payload = {
        "schema_version": "atlas_rope_v5_attempt9_science_complete_v1",
