#!/usr/bin/env python3
"""Source-readiness builder for MSAE v5; standard-library and model-free."""
...
V5_AUTHORITY = {
    ...
    "reports/provenance/msae_independent_source_v5/preacquisition_authority_manifest.json",
    "reports/provenance/msae_independent_source_v5/baseline_inventory.json",
    ...
    "reports/adversarial/msae_independent_source_v5_preacquisition_implementation_review.md",
    "reports/adversarial/msae_independent_source_v5_preacquisition_authority_review.md",
}
...
def scientific_artifact_inventory() -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v5_scientific_artifact_inventory_v1",
            "entries":entries}
...
def strict_json(path: Path) -> Any:
    ...
    # one escaped-slash normalization; every create-once v5 final is canonical.
...
def validate_acquisition_config(path: Path) -> dict[str, Any]:
    ...
    if (value.get("schema_version") != "msae_independent_source_v5_acquisition_v1"
...
def split_key(sent_id: str, normalized: str) -> str:
    ...
    return sha_bytes(canonical_bytes(["msae-independent-source-v5/C1C2", sent_id, normalized]))
...
def public_label(task: str, value: str) -> str:
    if task == "token_identity": return sha_bytes(("msae-v5/token\0" + value).encode())
    if task == "lemma_identity": return sha_bytes(("msae-v5/lemma\0" + value).encode())
    return value
...
def _walk_paths() -> Iterator[tuple[Path, str]]:
    for base, dirs, files in os.walk(ROOT):
        ...
        for d in sorted(dirs):
            ...
            if child_rel.startswith(V5_PREFIX.rstrip("/")):
                if any(child.iterdir()):
                    raise GateFailure("preexisting_v5_namespace")
                continue
...
def inventory_repository() -> dict[str, Any]:
    ...
    for key in sorted(by_inode, key=lambda z: min(x[1] for x in by_inode[z])):
        ...
        for _p, rel, s in group:
            ...
            elif rel.startswith(V5_PREFIX): item.update(disposition="v5_excluded", adapter="excluded", extracted_unit_count=0)
            elif rel in V5_AUTHORITY:
                item.update(disposition="v5_authority", adapter="authority", extracted_unit_count=0)
            entries.append(item)
    ...
    return {"schema_version": "msae_independent_source_v5_baseline_inventory_v1",
            ...
            "authority_expected_absent": sorted(V5_AUTHORITY - {x["path"] for x in entries}),
...
def deduplicate_roles(roles: dict[str, list[Sentence]]) -> tuple[dict[str, list[Sentence]], dict[str, Any]]:
    ...
    report = {"schema_version": "msae_independent_source_v5_dedup_v1", "roles": {},
...
def _support(rows: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v5_support_v1","floor_distinct_utterances":20,
...
def _license() -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v5_license_v1","license_id":"CC-BY-SA-4.0",
...
def document_group_census() -> dict[str, Any]:
    ...
    for name in SOURCE_FILES.values():
        ...
        files[name] = local
        count += local
    return {"schema_version":"msae_independent_source_v5_document_group_census_v1",
...
def _source_family(inventory: dict[str,Any], raw_hashes:set[str]) -> dict[str,Any]:
    ...
    unexpected_paths=[path for path in pathname_matches if path not in V5_AUTHORITY]
    eligible=not unexpected and not unexpected_paths and not whole and bound_files_ok
    return {"schema_version":"msae_independent_source_v5_source_family_v1","status":"eligible" if eligible else "ineligible",
...
def candidate_pedigree() -> dict[str, Any]:
    ...
    return {"schema_version": "msae_independent_source_v5_candidate_pedigree_v1",
...
def _verify_predecessor_and_raw() -> dict[str, Any]:
    expected = {
        ROOT/"docs/plan-msae-independent-source-v5.md": "1c450895bf836b142bc46dced85afd4f109edd68a96f6e4e91dc5719f0b3fc6b",
        ROOT/"reports/adversarial/msae_independent_source_v5_plan_review.md": "8137a0e7e2dffbf7c4c052e27dc60f5de3f1d1e5568723f5834de5c404aa3abc",
        ROOT/"reports/provenance/msae_independent_source_v5/preacquisition_alias_screen.json": "c0c17ed89e19b3af611bae43e73095ca4acb4d0a61a01f7c3ef8979cbe2fbd7d",
        ROOT/"reports/provenance/msae_independent_source_v5/historical_source_registry.json": "484a2b8c13798924c159d0e1bf7fbc90d19010111f89b08e7ddb7ceb6780b83f",
    ...
    required_public=(
        ...
        ROOT/"reports/adversarial/msae_independent_source_v5_preacquisition_authority_review.md",
    ...
    config=validate_acquisition_config(ROOT/"configs/msae_independent_source_v5/acquisition.json")
    ...
    if (entry.get("schema_version")!="msae_independent_source_v5_source_acquisition_entry_v1"
            ...
            or entry.get("config_sha256")!=sha_file(ROOT/"configs/msae_independent_source_v5/acquisition.json")
            or entry.get("runner_sha256")!=sha_file(ROOT/"scripts/acquire_msae_independent_source_v5.py")
    ...
    if (acquisition.get("schema_version")!="msae_independent_source_v5_source_acquisition_v1"
            ...
            or acquisition.get("config_sha256") != sha_file(ROOT/"configs/msae_independent_source_v5/acquisition.json")
            ...
            or acquisition.get("runner_sha256") != sha_file(ROOT/"scripts/acquire_msae_independent_source_v5.py")
    ...
    expected_command_argv=[
        [*config["ignore_argv"][:-1],"data/msae_independent_source_v5/raw/sentinel"],
    ...
    if ([item.get("path") for item in ignores] != ["data/msae_independent_source_v5/raw/sentinel","data/msae_independent_source_v5/private/sentinel"]
...
def _verify_history_snapshot(inventory: dict[str, Any], allowed_added: set[str] | None = None,
    ...
    current_paths={rel for _path,rel in _walk_paths_allow_v5(include_v5=True)}
    ...
    for item in inventory["entries"]:
        if item["disposition"] not in {"text_scanned", "archive_scanned", "binary_unscanned", "v5_authority", "quarantine"}:
            continue
        path=ROOT/item["path"]
        s=path.lstat()
...
def verify_pre_payload_v5_namespace() -> None:
    fds=[]
    try:
        v5fd=open_repo_dir(RAW.parent.parent);fds.append(v5fd)
        rawrootfd=os.open("raw",os.O_RDONLY|getattr(os,"O_DIRECTORY",0)|getattr(os,"O_NOFOLLOW",0),dir_fd=v5fd);fds.append(rawrootfd)
        ...
        for fd,mode in ((v5fd,0o700),(rawrootfd,0o700),(commitfd,0o555)):
            if stat.S_IMODE(os.fstat(fd).st_mode)!=mode:raise GateFailure("v5_namespace_directory_drift")
        if sorted(os.listdir(v5fd)) != ["raw"]:raise GateFailure("v5_namespace_extra_path")
        if sorted(os.listdir(rawrootfd)) != [COMMIT]:raise GateFailure("v5_raw_parent_extra_path")
...
def verify_training_roots(inventory: dict[str, Any]) -> None:
    ...
    current=[]
    for path,rel in _walk_paths_allow_v5():
...
def _walk_paths_allow_v5(include_v5: bool = False) -> Iterator[tuple[Path,str]]:
    for base,dirs,files in os.walk(ROOT,followlinks=False):
        ...
        for d in sorted(dirs):
            ...
            if stat.S_ISLNK(st.st_mode) or not stat.S_ISDIR(st.st_mode):
                raise GateFailure("special_paths")
            if d=="__pycache__" or any((rel+"/").startswith(x) for x in GENERATED_PREFIXES) or (not include_v5 and rel.startswith(V5_PREFIX.rstrip("/"))):
                continue
            retained.append(d)
        dirs[:]=retained
        for name in sorted(files):
...
def implementation_review_sha() -> str:
    review=ROOT/"reports/adversarial/msae_independent_source_v5_preacquisition_implementation_review.md"
    required=[
        sha_file(ROOT/"docs/plan-msae-independent-source-v5.md"),
        sha_file(ROOT/"reports/adversarial/msae_independent_source_v5_plan_review.md"),
        sha_file(PROV/"preacquisition_alias_screen.json"),
        sha_file(PROV/"historical_source_registry.json"),
        sha_file(Path(__file__)),
        sha_file(ROOT/"scripts/acquire_msae_independent_source_v5.py"),
        sha_file(ROOT/"tests/test_prepare_msae_independent_source_v5.py"),
        sha_file(ROOT/"configs/msae_independent_source_v5/acquisition.json"),
        sha_file(ROOT/"reports/verification/msae_independent_source_v5_source_free_checks.log"),
    ]
...
def build_authority_manifest() -> None:
    ...
    _verify_history_snapshot(
        baseline,
        allowed_added={"reports/provenance/msae_independent_source_v5/baseline_inventory.json"},
    )
    verify_training_roots(baseline)
    implementation_review_sha();inputs=_authority_inputs()
    review_path=ROOT/"reports/adversarial/msae_independent_source_v5_preacquisition_implementation_review.md"
    _require_ship_review(review_path,[inputs["scripts/prepare_msae_independent_source_v5.py"],
                                      inputs["tests/test_prepare_msae_independent_source_v5.py"],
                                      inputs["configs/msae_independent_source_v5/acquisition.json"],
                                      inputs["reports/verification/msae_independent_source_v5_source_free_checks.log"]])
    ...
    value={"schema_version":"msae_independent_source_v5_preacquisition_authority_v1",
...
def verify_authority_chain() -> dict[str, Any]:
    ...
    if (manifest.get("schema_version")!="msae_independent_source_v5_preacquisition_authority_v1"
    ...
    review_path=ROOT/"reports/adversarial/msae_independent_source_v5_preacquisition_authority_review.md"
...
def build_baseline() -> None:
    ...
    validate_acquisition_config(ROOT/"configs/msae_independent_source_v5/acquisition.json")
...
def build_ready() -> None:
    ...
    verify_pre_payload_v5_namespace()
    _verify_predecessor_and_raw()
    inv=strict_json(PROV/"baseline_inventory.json")
    raw_additions={f"data/msae_independent_source_v5/raw/{COMMIT}/{name}"
    ...
    _verify_history_snapshot(
        ...
        allowed_added=raw_additions|{
            ...
            "reports/provenance/msae_independent_source_v5/preacquisition_authority_manifest.json",
            "reports/adversarial/msae_independent_source_v5_preacquisition_authority_review.md",
            "reports/provenance/msae_independent_source_v5/source_acquisition_entry.json",
            "reports/provenance/msae_independent_source_v5/source_acquisition.json",
        },
    )
    ...
    source_manifest={"schema_version":"msae_independent_source_v5_source_manifest_v1","source_commit":COMMIT,
    ...
    cross=_role_overlap(rows);write_json(PROV/"cross_role_overlap.json",{"schema_version":"msae_independent_source_v5_cross_role_overlap_v1","blocking_collision_count":len(cross),"blocking_collisions":cross,"status":"eligible" if not cross else "ineligible"})
    ...
    role_manifest={"schema_version":"msae_independent_source_v5_role_manifest_v1","source_commit":COMMIT,"roles":{}}
    ...
    plan_sha=sha_file(ROOT/"docs/plan-msae-independent-source-v5.md");review_sha=sha_file(ROOT/"reports/adversarial/msae_independent_source_v5_plan_review.md")
    seal={"schema_version":"msae_independent_source_v5_seal_v1","status":"independent_source_ready_for_future_prescore_protocol","source_status":"previously_considered_but_project_source_use_unseen","plan_sha256":plan_sha,"plan_review_sha256":review_sha,"program_sha256":sha_file(Path(__file__)),"acquisition_runner_sha256":sha_file(ROOT/'scripts/acquire_msae_independent_source_v5.py'),"test_sha256":sha_file(ROOT/'tests/test_prepare_msae_independent_source_v5.py'),"artifact_sha256":bindings,"payload":{"path":"data/msae_independent_source_v5/private/blind_payload.jsonl","sha256":sha_file(payload),"size":ps.st_size,"record_count":len(test_rows),"mode":stat.S_IMODE(ps.st_mode),"nlink":ps.st_nlink},"model_operations_initiated_by_builder":0,"gpu_queries_initiated_by_builder":0,"training_runs_initiated_by_builder":0,"external_observation_scope":"baseline_and_terminal_proc_snapshots_plus_training_root_diff","unsupported_tasks":["neutral_prefix_offset","entity_binary","entity_type","source_genre"],"opaque_binary_history_excluded":True,"document_speaker_cluster_inference_authorized":False}
    seal_sha=sha_bytes(canonical_file_bytes(seal))
    write_json(PROV/"no_training_gate.json",{"schema_version":"msae_independent_source_v5_no_training_gate_v1","status":"pending_terminal_seal","terminal_status_if_seal_matches":"independent_source_ready_for_future_prescore_protocol","model_scoring_authorized":False,"k2_or_branch_training_authorized":False,"stage_c_authorized":False,"seal_sha256":seal_sha})
...
def retain_rejection(code: str) -> None:
    ...
    value={"schema_version":"msae_independent_source_v5_rejection_v1","status":"rejected","failure_code":code,
...
def _validate_inventory_artifact(value: Any, code: str) -> None:
    ...
    for item in entries:
        ...
        if (keys!=expected or not isinstance(item.get("path"),str)
                ...
                or item.get("disposition") not in {"text_scanned","archive_scanned","binary_unscanned","v5_authority","quarantine"}
    ...
    if (value["schema_version"]!="msae_independent_source_v5_baseline_inventory_v1"
...
def validate_scientific_artifacts() -> None:
    ...
    if (census["schema_version"]!="msae_independent_source_v5_document_group_census_v1"
    ...
    if (source["schema_version"]!="msae_independent_source_v5_source_manifest_v1"
    ...
    if (license_value["schema_version"]!="msae_independent_source_v5_license_v1"
    ...
    if (family["schema_version"]!="msae_independent_source_v5_source_family_v1" or family["status"]!="eligible"
    ...
    if (pedigree["schema_version"]!="msae_independent_source_v5_candidate_pedigree_v1"
    ...
    if (dedup["schema_version"]!="msae_independent_source_v5_dedup_v1"
    ...
    if (cross["schema_version"]!="msae_independent_source_v5_cross_role_overlap_v1"
    ...
    if (overlap["schema_version"]!="msae_independent_source_v5_history_overlap_v1"
    ...
    if (support["schema_version"]!="msae_independent_source_v5_support_v1" or support["floor_distinct_utterances"]!=20
...
def reconstruct_scientific_artifacts() -> dict[str, Any]:
    ...
    roles,source_counts=_all_source()
    source={"schema_version":"msae_independent_source_v5_source_manifest_v1","source_commit":COMMIT,
            "partitions":source_counts,"source_text_published":False}
    ...
    cross_value={"schema_version":"msae_independent_source_v5_cross_role_overlap_v1",
    ...
    overlap={"schema_version":"msae_independent_source_v5_history_overlap_v1",
        "baseline_inventory_sha256":sha_file(PROV/"baseline_inventory.json"),
    ...
    split={"schema_version":"msae_independent_source_v5_split_manifest_v1","source_commit":COMMIT,
    ...
    payload_bytes=b"".join(canonical_file_bytes({"schema_version":"msae_independent_source_v5_payload_v1",
...
def verify_terminal() -> None:
    ...
    if present_path(rejection_path):
        ...
        expected_payload=({key:final_state[key] for key in ("path","sha256","size","mode","nlink")}
                          if payload_created else None)
        if (rejection.get("schema_version")!="msae_independent_source_v5_rejection_v1"
    ...
    if (seal.get("schema_version")!="msae_independent_source_v5_seal_v1"
            ...
            or seal.get("plan_sha256")!=sha_file(ROOT/"docs/plan-msae-independent-source-v5.md")
            or seal.get("plan_review_sha256")!=sha_file(ROOT/"reports/adversarial/msae_independent_source_v5_plan_review.md")
            or seal.get("program_sha256")!=sha_file(Path(__file__))
            or seal.get("acquisition_runner_sha256")!=sha_file(ROOT/"scripts/acquire_msae_independent_source_v5.py")
            or seal.get("test_sha256")!=sha_file(ROOT/"tests/test_prepare_msae_independent_source_v5.py")
    ...
    expected_schemas={
        "baseline_inventory.json":"msae_independent_source_v5_baseline_inventory_v1",
        "source_acquisition_entry.json":"msae_independent_source_v5_source_acquisition_entry_v1",
        "source_acquisition.json":"msae_independent_source_v5_source_acquisition_v1",
        "source_family.json":"msae_independent_source_v5_source_family_v1",
        "candidate_pedigree.json":"msae_independent_source_v5_candidate_pedigree_v1",
        "license.json":"msae_independent_source_v5_license_v1",
        "document_group_census.json":"msae_independent_source_v5_document_group_census_v1",
        "source_manifest.json":"msae_independent_source_v5_source_manifest_v1",
        "dedup.json":"msae_independent_source_v5_dedup_v1",
        "history_manifest.json":"msae_independent_source_v5_baseline_inventory_v1",
        "history_overlap.json":"msae_independent_source_v5_history_overlap_v1",
        "cross_role_overlap.json":"msae_independent_source_v5_cross_role_overlap_v1",
        "support.json":"msae_independent_source_v5_support_v1",
        "role_manifest.json":"msae_independent_source_v5_role_manifest_v1",
        "split_manifest.json":"msae_independent_source_v5_split_manifest_v1",
        "post_process_snapshot.json":"msae_independent_source_v5_process_snapshot_v1",
        ...
        "preacquisition_authority_manifest.json":"msae_independent_source_v5_preacquisition_authority_v1",
    }
    ...
    if (any(not isinstance(item,dict) or set(item)!={"sent_id","panel","zero_based_rank","split_key_sha256","source_record_sha256"}
            for item in entries)
            or split.get("schema_version")!="msae_independent_source_v5_split_manifest_v1"
    ...
    if (roles.get("schema_version")!="msae_independent_source_v5_role_manifest_v1"
    ...
    if (gate.get("schema_version")!="msae_independent_source_v5_no_training_gate_v1"
