...
CFG=ROOT/"configs/causal_manifold_bridge_v1_r1/run.json";PLAN=ROOT/"PLAN_TRAINED_CONTROL_NEXT_STUDIES_V1.md";SCRIPT=Path(__file__).resolve();TEST=ROOT/"tests/test_causal_manifold_bridge_v1_r1.py";LAUNCH_TEST=ROOT/"tests/test_launch_trained_control_next_studies_v1_r1.py";LAUNCHER=ROOT/"scripts/launch_trained_control_next_studies_v1_r1_tmux.sh"
...
def candidate(device_name:str,record:bool)->dict[str,Any]:
 c=cfg();validate_config(c);lineage=validate_recovery_lineage(c);device=torch.device(device_name);checks=[];incomplete_mutations=True;design_qa=[]
 for gen in c["design"]["generator_seeds"]:
...
def verify_frozen()->dict[str,Any]:
 c=cfg();validate_recovery_lineage(c);lock=loadj(rp(c,"generator_lock"));fr=loadj(rp(c,"freeze"));binding=loadj(rp(c,"review_binding"))
