#!/usr/bin/env python3
"""Source-only supervisor for the single reviewed gen5 tmux extinction test.
...
SELF = ROOT / "scripts/run_msae_independent_measurement_v3_post_m2_gen5_tmux_test.py"
PROTOCOL_PREFIX = "msae-independent-v3-gen5-test"
ACK_TIMEOUT = 5.0
CHILD_TIMEOUT = 30.0
TERM_TIMEOUT = 3.0
FAULT_MODES = (
...
def _validate_identity(value: dict[str, Any], pane_pid: int,
    ...
    expected_keys = {"schema_version", "pane_pid", "pane_start_ticks",
                     "broker_pid", "broker_start_ticks", "broker_ppid",
                     "descendant_pid", "descendant_start_ticks",
                     "descendant_ppid", "worker_pgid"}
    if set(value) != expected_keys or value.get("schema_version") != \
            "msae_v3_gen5_tmux_test_process_v1":
    ...
    if peer_uid != os.getuid() or peer_gid != os.getgid() or peer_pid != value["broker_pid"]:
        raise RuntimeError("tmux test peer credential drift")
    if value["pane_pid"] != pane_pid or value["broker_pid"] != pane_pid:
        raise RuntimeError("tmux pane/broker identity drift")
    for prefix in ("pane", "broker", "descendant"):
    ...
    if not _is_equal_or_descended(value["broker_pid"], pane_pid):
        raise RuntimeError("tmux broker is outside pane lineage")
    if value["descendant_ppid"] != value["broker_pid"]:
        raise RuntimeError("tmux descendant parent drift")
    if value["worker_pgid"] != value["descendant_pid"]:
        raise RuntimeError("tmux descendant PGID drift")
    return value
...
def _cleanup_record_payload(raw: bytes, *, final_name: str,
    ...
    if final_name == "process.json":
        expected = {"schema_version", "pane_pid", "pane_start_ticks",
                    "broker_pid", "broker_start_ticks", "broker_ppid",
                    "descendant_pid", "descendant_start_ticks",
                    "descendant_ppid", "worker_pgid"}
    ...
    if final_name == "identity_bound.json":
        return (set(value) == {"schema_version", "process_sha256", "supervisor_pid"}
                and value.get("schema_version") ==
                    "msae_v3_gen5_tmux_test_identity_bound_v1"
...
def _cleanup_fault_case(
    *, socket_path: Path, session: str, scratch: Path,
    ...
    if identity is not None:
        trusted.extend([
            (int(identity["descendant_pid"]),
             int(identity["descendant_start_ticks"])),
...
def _supervise_fault_case(
    ...
    try:
        ...
        if socket_identity is None or captured_tmux is None:
            raise RuntimeError(f"tmux identity not captured for {mode}")
...
def _recycle_fault_namespace(
    ...
    try:
        if (not _path_present(ack_socket)
                or _socket_identity(ack_socket) != ack_identity):
    ...
    return replacement, _socket_identity(ack_socket)
...
def main() -> int:
    ...
    try:
        ...
        ack_socket_identity = _socket_identity(ack_socket)
        listener.setblocking(False)
        ...
        child_env = {
            ...
            "MSAE_GEN5_ALLOW_REAL_TMUX_TEST": "1",
            "MSAE_GEN5_TMUX_TEST_TOKEN": token,
            "CUDA_VISIBLE_DEVICES": "",
        }
        child_argv = [str(PYTHON), "-B", "-I", "-m", "pytest", "-q",
                      "-p", "no:cacheprovider",
                      "tests/test_msae_independent_measurement_v3_post_m5_gen5.py::test_failed_handoff_extinction_kills_real_processes_and_socket"]
        ...
        if candidate is not None:
            ...
            for prefix in ("descendant", "broker", "pane"):
        ...
        if _path_present(ack_socket):
            try:
                if (ack_socket_identity is None
                        or _socket_identity(ack_socket) != ack_socket_identity):
                    success = False
                else:
                    ack_socket.unlink()
