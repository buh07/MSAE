...
RUN="$ROOT/scripts/joint_controllability_benchmark_v4_1.py"
CFG="configs/joint_controllability_benchmark_v4_1/run.json"
PREFIX="msae_joint_control_v4_1"
LOG="$ROOT/reports/provenance/joint_controllability_benchmark_v4_1_run_20260808"
FREEZE="$ROOT/configs/joint_controllability_benchmark_v4_1/FREEZE.json"
REVIEW="$ROOT/reports/adversarial/joint_controllability_v4_1_candidate_review.md"
...
TRANSFORMERS_CACHE="$HF_HOME/hub" HF_DATASETS_OFFLINE=1 TRANSFORMERS_OFFLINE=1 HF_HUB_OFFLINE=1 PYTHONPATH="$ROOT/scripts" "$PY" "$RUN" cache-preflight --config "$CFG"
...
record={'schema_version':'joint_control_v4_1_launch_recovery','freeze_sha256':freeze,'gpu_uuids':gpus,'sessions':sorted(s for s in sessions if s.startswith('msae_joint_control_v4_1_')),'config_sha256':hashlib.sha256(Path('configs/joint_controllability_benchmark_v4_1/run.json').read_bytes()).hexdigest(),'launched_unix':time.time(),'launcher_pid':os.getpid(),'development_gate_precedes_fresh_test_model_load':True,'new_training':False}
