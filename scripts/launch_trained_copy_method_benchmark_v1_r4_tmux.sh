...
SESSION="trained_copy_method_benchmark_v1_r4_20260810"
LOG="$ROOT/reports/provenance/trained_copy_method_benchmark_v1_r4_launcher_20260810.log"
PY="$ROOT/scripts/trained_copy_method_benchmark_v1_r4.py"
HOURS="$(python -c 'import json,sys; x=json.load(open(sys.argv[1]))["hard_timeout_hours"]; assert isinstance(x,int) and x>0; print(x)' "$ROOT/configs/trained_copy_method_benchmark_v1_r4/run.json")"
[[ ! -e "$LOG" ]] || { echo "launcher log exists" >&2; exit 1; }
! tmux has-session -t "$SESSION" 2>/dev/null || { echo "tmux session exists" >&2; exit 1; }
if compgen -G '/tmp/msae_trained_copy_methods_v1_r4_*.lockdir' >/dev/null; then echo "stale benchmark GPU lock" >&2; exit 1; fi
...
for row in "${GPUS[@]}"; do
  ...
  candidate="/tmp/msae_trained_copy_methods_v1_r4_${u}.lockdir"
...
set +e
timeout --signal=TERM --kill-after=30s "${HOURS}h" env MSAE_ROOT=\"\$RUN_ROOT\" python \"\$RUN_ROOT/scripts/trained_copy_method_benchmark_v1_r4.py\" run-pipeline --physical-index \"$INDEX\" --gpu-uuid \"$UUID\" --pane-pid \"\$\$\" --gpu-lockdir \"$LOCK\" --launch-token \"$TOKEN\" 2>&1 | tee -a \"$LOG\"
rc=\${PIPESTATUS[0]}
set -e
exit \$rc"
