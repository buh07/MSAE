...
NAMESPACE = "atlas_discovery_v3_3_attempt7"
DROP = "__DROP__"
...
def build_prepared_data(config: Mapping[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    if str(config.get("data_root")) != "data/atlas_discovery_v3_3_attempt7_final_v3_rebuild1":
        raise RuntimeError("attempt7 requires its exact v2 external-QA science namespace")
    ...
    process_snapshot = {
        "schema_version": "atlas_discovery_v3_3_attempt7_process_snapshot_v1",
        "argv": sys.argv,
        "cwd": str(Path.cwd()),
    ...
    split_path = _canonical_root(repository_root / str(split_spec["path"]))
    if not split_path.is_relative_to(repository_root) or sha256_file(split_path) != str(split_spec["sha256"]):
        raise RuntimeError("attempt7 population split drift")
    split_artifact = read_json(split_path)
    if split_artifact.get("schema_version") != "atlas_discovery_v3_3_attempt7_population_split_v3":
    ...
    opened_input_ledger = {
        "schema_version": "atlas_discovery_v3_3_attempt7_opened_input_ledger_v1",
        ...
        "sources": {
            name: {
                ...
                "role": "reused_public_discovery_source",
            }
        ...
        "common_joint_cells": {
            ...
            "role": "attempt5_label_only_source_common_nuisance_support_imported_by_attempt7",
        },
        "population_split": {
            "path": str(split_path.relative_to(repository_root)),
            "sha256": sha256_file(split_path),
            "role": "attempt7_qa_science_document_firewall",
        },
    ...
    for name in ("EWT", "GUM"):
        excluded = set(map(str, split_artifact["sources"][name]["science_excluded_documents"]))
        observed = {f"{name}:{row.document_id}" for row in parsed[name]}
        if not excluded.issubset(observed):
            raise RuntimeError(f"attempt7 excluded document absent from source: {name}")
        parsed[name] = [row for row in parsed[name] if f"{name}:{row.document_id}" not in excluded]
        parse_exclusions[name]["attempt7_qa_reserved_documents"] = len(excluded)
        parse_exclusions[name]["attempt7_qa_reserved_sentences"] = raw_sentence_counts[name] - len(parsed[name])
    ...
    manifest: dict[str, Any] = {
        "schema_version": "atlas_discovery_v3_3_attempt7_prepared_v1",
        ...
        "science_excluded_documents": {name: split_artifact["sources"][name]["science_excluded_documents"] for name in ("EWT", "GUM")},
        "sources": {},
    }
    ...
    preflight = {
        "schema_version": "atlas_discovery_v3_3_attempt7_preflight_v1",
