#!/usr/bin/env python3
"""Reviewed, one-shot upstream acquisition runner for MSAE v10."""
...
CONFIG=ROOT/"configs/msae_independent_source_v10/acquisition.json"
...
FILES=["sv_talbanken-ud-train.conllu","sv_talbanken-ud-dev.conllu","sv_talbanken-ud-test.conllu","LICENSE.txt"]
...
CARRYOVER="reports/provenance/msae_independent_source_v10/v9_carryover_authority.json"
...
V9_CONTROL_PATHS={
    ...
    "scripts/prepare_msae_independent_source_v9.py","scripts/acquire_msae_independent_source_v9.py",
    "tests/test_prepare_msae_independent_source_v9.py","configs/msae_independent_source_v9/acquisition.json",
...
V8_CONTROL_PATHS={
    ...
    "scripts/prepare_msae_independent_source_v8.py","scripts/acquire_msae_independent_source_v8.py",
    "tests/test_prepare_msae_independent_source_v8.py","configs/msae_independent_source_v8/acquisition.json",
...
V10_AUTHORITY={
    ...
    "scripts/prepare_msae_independent_source_v10.py","scripts/acquire_msae_independent_source_v10.py",
    "tests/test_prepare_msae_independent_source_v10.py","configs/msae_independent_source_v10/acquisition.json",
...
V10_ACQUISITION_STATE={
    ...
    "reports/provenance/msae_independent_source_v10/source_acquisition_rejection.json",
}
QUARANTINES={"data/atlas_v1/private/final.jsonl","data/atlas_v1/private/final.records.jsonl",
...
def relaxed_json(path:Path)->Any:
    return strict_json_bytes(read_bytes_nofollow(path))

def canonical_ascii(value:Any)->bytes:
...
def directory_fingerprint(value:os.stat_result)->tuple[int,int,int,int,int,int]:
...
def directory_snapshot_nofollow(path:Path)->tuple[os.stat_result,list[str]]:
    ...
    if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
        raise AcquisitionFailure("path_identity_changed")
    fd=open_root_dir(path)
    try:
        opened=os.fstat(fd)
        if directory_fingerprint(before)!=directory_fingerprint(opened):
            raise AcquisitionFailure("path_identity_changed")
        names=sorted(os.listdir(fd));after=os.fstat(fd);current=path.lstat()
        if (directory_fingerprint(opened)!=directory_fingerprint(after)
                or directory_fingerprint(opened)!=directory_fingerprint(current)):
            raise AcquisitionFailure("path_identity_changed")
        return opened,names
    except OSError as error:
        raise AcquisitionFailure("path_identity_changed") from error
    finally:os.close(fd)

def verify_v9_carryover()->dict[str,Any]:
    path=ROOT/CARRYOVER
    value=strict_json(path)
    exact_keys(value,{"schema_version","status","v9_source_commit","v9_control_files",
        "v9_control_map_sha256","v9_data_directories","v9_raw_files","v9_raw_file_map_sha256",
        "v9_acquisition_sha256","v9_scientific_entry_sha256","v9_partial_scientific_prefix",
        "v9_rejection_sha256","v9_rejection_failure_code","v9_terminal_verify_status",
        "v9_terminal_review_sha256","v9_source_content_semantically_read_by_v9_builder",
        "v9_source_content_semantically_read_for_this_record","v9_source_content_printed",
        "opaque_raw_hash_reads_for_this_record","quarantine_content_reads","model_operations",
        "gpu_queries","training_runs","model_scoring_authorized",
        "k2_or_branch_training_authorized","stage_c_authorized"},"v9_carryover_authority_drift")
    if (sha_file(path)!=CARRYOVER_SHA256
            or value.get("schema_version")!="msae_independent_source_v10_v9_carryover_authority_v1"
            or value.get("status")!="retained_v9_scientific_rejection_and_terminal_verifier_defect"
            or value.get("v9_source_commit")!=COMMIT
            or value.get("v9_control_map_sha256")!="d0035594710b62cd9b998367136f2bf02110bf59095d359b0f2dbdd75e1aec3c"
            or value.get("v9_raw_file_map_sha256")!="881ac46eab4a9e4ecf50a2afb088d37ac98f060b84a45ede90d79007e216f363"
            or value.get("v9_acquisition_sha256")!="d353a5a04d8c853ea255526c20742a5e4c3578f672bc5d8cdd889cb193cff837"
            or value.get("v9_scientific_entry_sha256")!="0ccb9df33f63e06dfd610308793bc926aa2be2c4fd71b30d4cf83493d5ef2f05"
            or value.get("v9_partial_scientific_prefix")!=["document_group_census.json"]
            or value.get("v9_rejection_sha256")!="91c1e28f8f6f1e6efebef44875e4460848a5007360287787f715d8dea8a8896b"
            or value.get("v9_rejection_failure_code")!="missing_or_unknown_field"
            or value.get("v9_terminal_verify_status")!="failed_post_baseline_inventory_path_drift"
            or value.get("v9_terminal_review_sha256")!="bafa2fbcc1c4ba4e6f3ffe9c696e63f4aa846d5b12a8922bbaa3ac36a770af28"
            or value.get("v9_source_content_semantically_read_by_v9_builder") is not True
            or value.get("v9_source_content_semantically_read_for_this_record") is not False
            or value.get("v9_source_content_printed") is not False
            or value.get("opaque_raw_hash_reads_for_this_record")!=0
            or value.get("quarantine_content_reads")!=0 or value.get("model_operations")!=0
            or value.get("gpu_queries")!=0 or value.get("training_runs")!=0
        ...
        raise AcquisitionFailure("v9_carryover_authority_drift")
    controls=value.get("v9_control_files")
    if (not isinstance(controls,list) or len(controls)!=20
            or [item.get("path") for item in controls]!=sorted(V9_CONTROL_PATHS)):
        raise AcquisitionFailure("v9_control_universe_drift")
    control_map={}
    for item in controls:
        exact_keys(item,{"path","device","inode","mode","nlink","size","sha256"},"v9_control_record_schema")
        ...
        if (not stat.S_ISREG(current.st_mode) or stat.S_ISLNK(current.st_mode)
            ...
            raise AcquisitionFailure("v9_control_file_drift")
        control_map[item["path"]]=item["sha256"]
    if sha_bytes(canonical_ascii(control_map))!=value["v9_control_map_sha256"]:
        raise AcquisitionFailure("v9_control_map_drift")
    directories=value.get("v9_data_directories")
    ...
    if not isinstance(directories,list) or [item.get("path") for item in directories]!=expected_dirs:
        raise AcquisitionFailure("v9_data_directory_universe_drift")
    for item in directories:
        exact_keys(item,{"path","device","inode","mode","nlink","size"},"v9_data_directory_schema")
        current,names=directory_snapshot_nofollow(ROOT/item["path"])
        if (current.st_dev!=item["device"] or current.st_ino!=item["inode"]
                or stat.S_IMODE(current.st_mode)!=item["mode"] or current.st_nlink!=item["nlink"]
                or current.st_size!=item["size"]):
            raise AcquisitionFailure("v9_data_directory_drift")
        if item["path"]==expected_dirs[-1] and names!=sorted(FILES):
            raise AcquisitionFailure("v9_raw_extra_path")
    raw=value.get("v9_raw_files");raw_map={}
    if (not isinstance(raw,list) or len(raw)!=4
            or [item.get("path","").rsplit("/",1)[-1] for item in raw]!=sorted(FILES)):
        raise AcquisitionFailure("v9_raw_universe_drift")
    for item in raw:
        ...
        if (not stat.S_ISREG(current.st_mode) or stat.S_ISLNK(current.st_mode)
                ...
                or stat.S_IMODE(current.st_mode)!=item["mode"] or current.st_nlink!=item["nlink"]
                or current.st_size!=item["size"]):
            raise AcquisitionFailure("v9_raw_metadata_drift")
        raw_map[item["path"]]={key:item[key] for key in ("sha256","size","git_blob_sha1","mode","nlink")}
    if sha_bytes(canonical_ascii(raw_map))!=value["v9_raw_file_map_sha256"]:
        raise AcquisitionFailure("v9_raw_map_drift")
    return value
...
def expected_config()->dict[str,Any]:
    return {
        "schema_version":"msae_independent_source_v10_acquisition_v1",
...
def validate_entry(value:dict[str,Any],manifest_sha:str,review_sha:str,baseline_sha:str)->None:
    ...
    if (value.get("schema_version")!="msae_independent_source_v10_source_acquisition_entry_v1"
...
def validate_command_records(commands:Any,expected_names:list[str],config:dict[str,Any],
    ...
    if not isinstance(commands,list) or [item.get("name") if isinstance(item,dict) else None for item in commands]!=expected_names:
        raise AcquisitionFailure("invalid_preacquisition_state")
    expected_argv=[
        [*expand(config["ignore_argv"],clone_dir,empty_home)[:-1],"data/msae_independent_source_v10/raw/sentinel"],
        [*expand(config["ignore_argv"],clone_dir,empty_home)[:-1],"data/msae_independent_source_v10/private/sentinel"],
...
def validate_existing_raw(report:dict[str,Any])->None:
    fds=[]
    try:
        v10fd=open_root_dir(RAW.parent.parent);fds.append(v10fd)
        rawrootfd=os.open("raw",os.O_RDONLY|os.O_DIRECTORY|os.O_NOFOLLOW,dir_fd=v10fd);fds.append(rawrootfd)
        ...
        if sorted(os.listdir(v10fd))!=["raw"] or sorted(os.listdir(rawrootfd))!=[COMMIT] or sorted(os.listdir(commitfd))!=sorted(FILES):
            raise AcquisitionFailure("invalid_preacquisition_state")
...
def partial_raw_metadata()->list[dict[str,Any]]:
    ...
    if present(namespace):
        ...
        if stat.S_ISDIR(root_stat.st_mode) and not stat.S_ISLNK(root_stat.st_mode):
            rootfd=open_root_dir(namespace)
            try:
                opened_root=os.fstat(rootfd)
                if directory_fingerprint(root_stat)!=directory_fingerprint(opened_root):
                    raise AcquisitionFailure("path_identity_changed")
                root_rel=namespace.relative_to(ROOT).as_posix()
                def visit(dfd:int,relbase:str)->None:
                    before=os.fstat(dfd)
                    try:names=sorted(os.listdir(dfd))
                    except OSError as error:raise AcquisitionFailure("path_identity_changed") from error
                    for name in names:
                        try:st=os.stat(name,dir_fd=dfd,follow_symlinks=False)
                        except OSError as error:raise AcquisitionFailure("path_identity_changed") from error
                        rel=relbase+"/"+name
                        partial.append({"path":rel,
                            "type":"directory" if stat.S_ISDIR(st.st_mode) and not stat.S_ISLNK(st.st_mode)
                                   else "regular" if stat.S_ISREG(st.st_mode) and not stat.S_ISLNK(st.st_mode)
                                   else "special",
                            "size":st.st_size,"mode":stat.S_IMODE(st.st_mode),"device":st.st_dev,
                            "inode":st.st_ino,"nlink":st.st_nlink})
                        if stat.S_ISDIR(st.st_mode) and not stat.S_ISLNK(st.st_mode):
                            childfd=os.open(name,os.O_RDONLY|os.O_DIRECTORY|os.O_NOFOLLOW,dir_fd=dfd)
                            try:
                                child_open=os.fstat(childfd)
                                if directory_fingerprint(st)!=directory_fingerprint(child_open):
                                    raise AcquisitionFailure("path_identity_changed")
                                visit(childfd,rel);child_after=os.fstat(childfd)
                            finally:os.close(childfd)
                            current=os.stat(name,dir_fd=dfd,follow_symlinks=False)
                            if (directory_fingerprint(st)!=directory_fingerprint(child_after)
                                    or directory_fingerprint(st)!=directory_fingerprint(current)):
                                raise AcquisitionFailure("path_identity_changed")
                    if directory_fingerprint(before)!=directory_fingerprint(os.fstat(dfd)):
                        raise AcquisitionFailure("path_identity_changed")
                visit(rootfd,root_rel)
                after_root=os.fstat(rootfd);current_root=namespace.lstat()
                if (directory_fingerprint(opened_root)!=directory_fingerprint(after_root)
                        or directory_fingerprint(opened_root)!=directory_fingerprint(current_root)):
                    raise AcquisitionFailure("path_identity_changed")
            finally:os.close(rootfd)
    return sorted(partial,key=lambda item:item["path"])
...
def validate_success_report(report:dict[str,Any],entry_sha:str,manifest_sha:str,review_sha:str,
    ...
    if (not clone_path.is_absolute() or clone_path.name!="repo" or home_path.name!="home"
            or clone_path.parent!=home_path.parent or not clone_path.parent.name.startswith("msae-v10-acquire-")):
        raise AcquisitionFailure("invalid_preacquisition_state")
    ...
    if (report["schema_version"]!="msae_independent_source_v10_source_acquisition_v1"
    ...
    for index,(name,path) in enumerate((("ignore_raw","data/msae_independent_source_v10/raw/sentinel"),
                                        ("ignore_private","data/msae_independent_source_v10/private/sentinel"))):
...
def validate_rejection_report(rejection:dict[str,Any],entry_sha:str,manifest_sha:str,review_sha:str,
    ...
    if commands:
        ...
        if clone_record:
            ...
            clone_dir=str(Path("/tmp/msae-v10-acquire-placeholder/repo"));empty_home=str(Path(clone_dir).parent/"home")
        ...
        if clone_record:
            ...
            sentinels=("data/msae_independent_source_v10/raw/sentinel",
                       "data/msae_independent_source_v10/private/sentinel")
            for index,item in enumerate(commands):
                if item["argv"]!=[*config["ignore_argv"][:-1],sentinels[index]]:
                    raise AcquisitionFailure("invalid_preacquisition_state")
    if (rejection["schema_version"]!="msae_independent_source_v10_source_acquisition_rejection_v1"
...
def create_raw_namespace()->tuple[list[int],os.stat_result]:
    """Create and pin data/v10/raw/commit without path-following writes."""
    ...
    try:
        ...
        fds.append(datafd)
        current=datafd
        for name,mode in (("msae_independent_source_v10",0o700),("raw",0o700),(COMMIT,0o700)):
...
def walk_history_paths():
    root_before=ROOT.lstat();rootfd=open_root_dir(ROOT)
    try:
        root_open=os.fstat(rootfd)
        if directory_fingerprint(root_before)!=directory_fingerprint(root_open):
            raise AcquisitionFailure("path_identity_changed")
        def visit(dfd:int,relbase:str):
            before=os.fstat(dfd)
            try:names=sorted(os.listdir(dfd))
            except OSError as error:raise AcquisitionFailure("path_identity_changed") from error
            for name in names:
                ...
                if stat.S_ISLNK(entry.st_mode):raise AcquisitionFailure("training_root_special_path")
                if stat.S_ISDIR(entry.st_mode):
                    if (name=="__pycache__" or any((rel+"/").startswith(item) for item in GENERATED_PREFIXES)
                    ...
                    try:
                        child_open=os.fstat(childfd)
                        if directory_fingerprint(entry)!=directory_fingerprint(child_open):
                            raise AcquisitionFailure("path_identity_changed")
                        yield from visit(childfd,rel);child_after=os.fstat(childfd)
                    finally:os.close(childfd)
                    current=os.stat(name,dir_fd=dfd,follow_symlinks=False)
                    if (directory_fingerprint(entry)!=directory_fingerprint(child_after)
                            or directory_fingerprint(entry)!=directory_fingerprint(current)):
                        raise AcquisitionFailure("path_identity_changed")
                    continue
                if not rel.startswith(".git/"):yield ROOT/rel,rel
            if directory_fingerprint(before)!=directory_fingerprint(os.fstat(dfd)):
                raise AcquisitionFailure("path_identity_changed")
        yield from visit(rootfd,"")
        root_after=os.fstat(rootfd);root_current=ROOT.lstat()
        if (directory_fingerprint(root_open)!=directory_fingerprint(root_after)
                or directory_fingerprint(root_open)!=directory_fingerprint(root_current)):
            raise AcquisitionFailure("path_identity_changed")
    finally:os.close(rootfd)

def validate_baseline_history_binding()->str:
    baseline=strict_json(BASELINE)
    registry=relaxed_json(PROV/"historical_source_registry.json")
    screen=relaxed_json(PROV/"preacquisition_alias_screen.json")
    records=sorted(({"path":item["path"],"sha256":item["sha256"],"size":item["size"]}
                    for item in baseline.get("entries",[]) if item.get("disposition")=="text_scanned"),
                   key=lambda item:item["path"])
    history_digest=sha_bytes(canonical_ascii(records))
    if (registry.get("schema_version")!="msae_independent_source_v9_historical_source_registry_v1"
            or registry.get("status")!="frozen_preacquisition" or registry.get("domain_utf8")!="msae-v9/pedigree"
            or registry.get("inputs")!=records or registry.get("input_count")!=len(records)
            or registry.get("inputs_sha256")!=history_digest or registry.get("overbound_json_source_key_construct_count")!=0
            or registry.get("quarantine_content_reads")!=0 or baseline.get("history_inputs_sha256")!=history_digest
            or screen.get("schema_version")!="msae_independent_source_v9_preacquisition_alias_screen_v2"
            or screen.get("history_input_count")!=len(records) or screen.get("history_inputs_sha256")!=history_digest
            or screen.get("history_registry_sha256")!=sha_file(PROV/"historical_source_registry.json")
            or screen.get("content_occurrence_count")!=0 or screen.get("pathname_occurrence_count")!=0
            or screen.get("source_use_evidence_count")!=0 or screen.get("quarantine_content_reads")!=0
            or screen.get("model_operations")!=0 or screen.get("gpu_queries")!=0
            or screen.get("training_runs")!=0 or screen.get("status")!="no_project_source_use_evidence"):
        raise AcquisitionFailure("history_registry_input_drift")
    entries=baseline.get("entries",[])
    dispositions={"text_scanned","archive_scanned","binary_unscanned","v10_authority",
                  "v8_candidate_control_carryover","v9_candidate_control_carryover","quarantine"}
    required={"path","size","mode","device","inode","nlink","mtime_ns","sha256","disposition",
              "adapter","content_reads","extracted_unit_count"}
    if (not isinstance(entries,list) or any(not isinstance(item,dict) for item in entries)
            or len({item.get("path") for item in entries})!=len(entries)):
        raise AcquisitionFailure("history_baseline_schema")
    for item in entries:
        disposition=item.get("disposition")
        if (not required.issubset(item) or disposition not in dispositions
                or not isinstance(item.get("path"),str) or not item["path"]
                or any(not is_count(item.get(key)) for key in
                       ("size","mode","device","inode","nlink","mtime_ns","content_reads","extracted_unit_count"))
                or (disposition=="quarantine")!=(item.get("sha256") is None)
                or (item.get("sha256") is not None and not is_hex(item["sha256"],64))
                or (item["path"] in V10_AUTHORITY)!=(disposition=="v10_authority")
                or (item["path"] in V8_CONTROL_PATHS)!=(disposition=="v8_candidate_control_carryover")
                or (item["path"] in V9_CONTROL_PATHS)!=(disposition=="v9_candidate_control_carryover")
                or (disposition=="quarantine")!=(item["path"] in QUARANTINES)):
            raise AcquisitionFailure("history_baseline_schema")
    baseline_paths={item["path"] for item in entries}
    expected_absent=baseline.get("authority_expected_absent")
    if (not isinstance(expected_absent,list) or expected_absent!=sorted(set(expected_absent))
            or set(expected_absent)!=(V10_AUTHORITY-baseline_paths)
            or not V10_POST_BASELINE.issubset(expected_absent)):
        raise AcquisitionFailure("history_baseline_schema")
    current={rel for _path,rel in walk_history_paths()}
    allowed=set(expected_absent)
    state_paths={rel for rel in V10_ACQUISITION_STATE if present(ROOT/rel)}
    if ("reports/provenance/msae_independent_source_v10/source_acquisition_entry.json" not in state_paths
            and state_paths):
        raise AcquisitionFailure("history_structural_drift")
    outcomes=state_paths-{"reports/provenance/msae_independent_source_v10/source_acquisition_entry.json"}
    if len(outcomes)>1:
        raise AcquisitionFailure("history_structural_drift")
    allowed.update(state_paths)
    if not baseline_paths.issubset(current) or current-baseline_paths-allowed:
        raise AcquisitionFailure("history_structural_drift")
    for item in entries:
        path=ROOT/item["path"]
        if item["disposition"]=="quarantine":
            st=path.lstat();current_digest=None
        else:
            fd=open_root_file(path)
            try:
                st=os.fstat(fd);current_digest=hash_fd(fd);after=os.fstat(fd)
                current=path.lstat()
            finally:os.close(fd)
            if ((st.st_dev,st.st_ino,st.st_mode,st.st_nlink,st.st_size,st.st_mtime_ns)!=
                    (after.st_dev,after.st_ino,after.st_mode,after.st_nlink,after.st_size,after.st_mtime_ns)
                    or (st.st_dev,st.st_ino,st.st_mode,st.st_nlink,st.st_size,st.st_mtime_ns)!=
                    (current.st_dev,current.st_ino,current.st_mode,current.st_nlink,current.st_size,current.st_mtime_ns)):
                raise AcquisitionFailure("history_snapshot_drift")
        if (not stat.S_ISREG(st.st_mode) or stat.S_ISLNK(st.st_mode) or st.st_size!=item["size"]
                or st.st_dev!=item["device"] or st.st_ino!=item["inode"] or st.st_mtime_ns!=item["mtime_ns"]
                or stat.S_IMODE(st.st_mode)!=item["mode"] or st.st_nlink!=item["nlink"]):
            raise AcquisitionFailure("history_snapshot_drift")
        if current_digest!=item["sha256"]:
            raise AcquisitionFailure("history_snapshot_hash_drift")
...
def main()->None:
    ...
    if (len(args.authority_review_sha256)!=64 or args.authority_review_sha256!=review_sha
            ...
            or carryover.get("status")!="retained_v9_scientific_rejection_and_terminal_verifier_defect"
            or authority.get("artifact_sha256",{}).get(CARRYOVER)!=CARRYOVER_SHA256
            or authority.get("artifact_sha256",{}).get("scripts/acquire_msae_independent_source_v10.py")!=sha_file(Path(__file__))
            or authority.get("artifact_sha256",{}).get("configs/msae_independent_source_v10/acquisition.json")!=sha_file(CONFIG)):
    ...
    history_digest=validate_baseline_history_binding()
    if authority.get("history_inputs_sha256")!=history_digest:
        raise AcquisitionFailure("authority_history_binding")
    ...
    scratch=Path(tempfile.mkdtemp(prefix="msae-v10-acquire-",dir="/tmp"));SCRATCH_CREATED=True;SCRATCH_CLEANUP_OK=False
    ...
    try:
        ...
        evidence={}
        ignore_outputs=[]
        for sentinel in ("data/msae_independent_source_v10/raw/sentinel","data/msae_independent_source_v10/private/sentinel"):
        ...
        report={"schema_version":"msae_independent_source_v10_source_acquisition_v1","source_repo":REPO,"source_commit":COMMIT,
...
def retain_failure(code:str)->None:
    ...
    value={"schema_version":"msae_independent_source_v10_source_acquisition_rejection_v1","status":"rejected",
