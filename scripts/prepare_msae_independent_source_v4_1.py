...
PRIVATE = ROOT / "data/msae_independent_source_v4_1/private"
PROV = ROOT / "reports/provenance/msae_independent_source_v4_1"
QUARANTINES = {
...
V4_AUTHORITY = {
    ...
    "reports/adversarial/msae_independent_source_v4_1_plan_review.md",
...
def inventory_repository() -> dict[str, Any]:
    ...
    for key in sorted(by_inode, key=lambda z: min(x[1] for x in by_inode[z])):
        ...
        for _p, rel, s in group:
            ...
            elif rel in V4_AUTHORITY or rel.startswith("reports/adversarial/msae_independent_source_v4_1_"):
    ...
    return {"schema_version": "msae_independent_source_v4_1_baseline_inventory_v1",
...
def _support(rows: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v4_1_support_v1","floor_distinct_utterances":20,
...
def _license() -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v4_1_license_v1","license_id":"CC-BY-SA-4.0",
...
def _source_family(inventory: dict[str,Any], raw_hashes:set[str]) -> dict[str,Any]:
    ...
    unexpected=[x for x in matches if x["path"] not in allowed_paths and not x["path"].startswith("reports/adversarial/msae_independent_source_v4_1_")]
    return {"schema_version":"msae_independent_source_v4_1_source_family_v1","status":"eligible" if not unexpected and not whole else "ineligible",
...
def build_ready() -> None:
    ...
    cross=_role_overlap(rows);replace_json(PROV/"cross_role_overlap.json",{"schema_version":"msae_independent_source_v4_1_cross_role_overlap_v1","blocking_collision_count":len(cross),"blocking_collisions":cross,"status":"eligible" if not cross else "ineligible"})
    ...
    overlap={"schema_version":"msae_independent_source_v4_1_history_overlap_v1","history_unit_count":unit_count,"candidate_utterance_count":sum(map(len,rows.values())),"blocking_collision_count":len(hits),"blocking_collisions":hits,"quarantine_content_reads":0,"status":"eligible" if not hits else "ineligible"}
    ...
    role_manifest={"schema_version":"msae_independent_source_v4_1_role_manifest_v1","source_commit":COMMIT,"roles":{}}
    ...
    split={"schema_version":"msae_independent_source_v4_1_split_manifest_v1","source_commit":COMMIT,"split_algorithm":"sha256-canonical-json-array-v1-even-C1-odd-C2","record_count":len(test_rows),"C1_count":sum(x["panel"]=="C1" for x in test_rows),"C2_count":sum(x["panel"]=="C2" for x in test_rows),"entries":[{"sent_id":x["sent_id"],"panel":x["panel"],"zero_based_rank":x["zero_based_rank"],"split_key_sha256":x["split_key_sha256"]} for x in test_rows]}
    replace_json(PROV/"split_manifest.json",split)
    payload_records=({"schema_version":"msae_independent_source_v4_1_payload_v1","source_repo":REPO,"source_commit":COMMIT,"upstream_partition":"test","panel":x["panel"],"split_key_sha256":x["split_key_sha256"],"sent_id":x["sent_id"],"tokens":_token_objects(x["sentence"])} for x in test_rows)
    payload=publish_private_jsonl(PRIVATE,"blind_payload.jsonl",payload_records)
    ps=payload.lstat()
    acquisition={"schema_version":"msae_independent_source_v4_1_acquisition_v1","source_repo":REPO,"source_commit":COMMIT,"predecessor_acquisition_sha256":sha_file(ROOT/'reports/provenance/msae_independent_source_v4/source_acquisition.json'),"files":source_counts|{"README.md":{"sha256":sha_file(RAW/'README.md')},"LICENSE.txt":{"sha256":sha_file(RAW/'LICENSE.txt')}},"source_content_printed":False,"status":"eligible"}
    ...
    plan_sha=sha_file(ROOT/"docs/plan-msae-independent-source-v4-1.md");review_sha=sha_file(ROOT/"reports/adversarial/msae_independent_source_v4_1_plan_review.md")
    seal={"schema_version":"msae_independent_source_v4_1_seal_v1","status":"independent_source_ready_for_future_prescore_protocol","plan_sha256":plan_sha,"plan_review_sha256":review_sha,"program_sha256":sha_file(Path(__file__)),"test_sha256":sha_file(ROOT/'tests/test_prepare_msae_independent_source_v4_1.py'),"artifact_sha256":bindings,"payload":{"path":"data/msae_independent_source_v4_1/private/blind_payload.jsonl","sha256":sha_file(payload),"size":ps.st_size,"record_count":len(test_rows),"mode":stat.S_IMODE(ps.st_mode),"nlink":ps.st_nlink},"model_operations":0,"gpu_queries":0,"training_runs":0,"unsupported_tasks":["neutral_prefix_offset","entity_binary","entity_type","source_genre"],"opaque_binary_history_excluded":True}
    replace_json(PROV/"seal.json",seal)
    replace_json(PROV/"no_training_gate.json",{"schema_version":"msae_independent_source_v4_1_no_training_gate_v1","status":"independent_source_ready_for_future_prescore_protocol","model_scoring_authorized":False,"k2_or_branch_training_authorized":False,"stage_c_authorized":False,"seal_sha256":sha_file(PROV/'seal.json')})
