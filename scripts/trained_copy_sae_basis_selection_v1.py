#!/usr/bin/env python3
"""Fresh basis-versus-selection successor for the trained synthetic copy model."""
...
CFG = ROOT / "configs/trained_copy_sae_basis_selection_v1/run.json"
PLAN = ROOT / "PLAN_SAE_GEOMETRY_SELECTION_AND_BRIDGE_V2.md"
SCRIPT = Path(__file__).resolve()
TEST = ROOT / "tests/test_trained_copy_sae_basis_selection_v1.py"
LAUNCH_TEST = ROOT / "tests/test_launch_sae_selection_bridge_v2_tmux.py"
LAUNCHER = ROOT / "scripts/launch_sae_selection_bridge_v2_tmux.sh"
PARITY = ROOT / "PLAN_SAE_GEOMETRY_SELECTION_AND_BRIDGE_V2.md"
BASE_SCRIPT = ROOT / "scripts/trained_copy_method_benchmark_v1_r4_2.py"
R5_SCRIPT = ROOT / "scripts/capacity_external_validity_v1_r5.py"
PRESERVATION_SCRIPT = ROOT / "scripts/preserve_trained_control_r2_outcomes.py"
...
def expected_method_count(c:Mapping[str,Any])->int:
    partial=len([b for b in c["sae"]["selector_budgets"] if b<256]);selectors=len(c["sae"]["selectors"]);budgets=len(c["sae"]["selector_budgets"])
    # Full-dictionary span projections are selector-invariant and occur once per
    # checkpoint, TopK and SAE seed, rather than six duplicated aliases.
    sae=len(c["sae"]["topks"])*len(c["sae"]["seeds"])*(2+selectors*partial+selectors*partial+budgets)
...
def expected_method_names(c:Mapping[str,Any])->set[str]:
    ...
    for k in c["sae"]["topks"]:
      for s in c["sae"]["seeds"]:
        names.add(f"full_code_sae_topk{k}_seed{s}")
        names.add(f"decoder_full_span_oracle_topk{k}_seed{s}")
        names|={f"sae_topk{k}_{sel}_budget{b}_seed{s}" for sel in c["sae"]["selectors"] for b in c["sae"]["selector_budgets"] if b<256}
        names|={f"selected_span_oracle_topk{k}_{sel}_budget{b}_seed{s}" for sel in c["sae"]["selectors"] for b in c["sae"]["selector_budgets"] if b<256}
        names|={f"fit_supervised_omp_beta_topk{k}_budget{b}_seed{s}" for b in c["sae"]["selector_budgets"]}
...
def verify_preservation_record(path:Path,expected_sha:str)->dict[str,Any]:
    if sha(path)!=expected_sha:raise RuntimeError("attempt1 preservation drift")
    rec=loadj(path)
    for artifact in rec["artifacts"]:
        p=ROOT/artifact["path"]
        if not p.is_file() or p.stat().st_size!=artifact["bytes"] or sha(p)!=artifact["sha256"]:raise RuntimeError(f"attempt1 artifact drift: {p}")
    for root in rec["root_inventories"]:
        p=ROOT/root["path"];count=sum(x.is_file() for x in p.rglob("*")) if p.exists() else 0
        if p.exists()!=root["exists"] or count!=root["files"]:raise RuntimeError(f"attempt1 root inventory drift: {p}")
    return rec
def validate_recovery_lineage(c:Mapping[str,Any])->dict[str,Any]:
    rec=c["preservation"];p=ROOT/rec["path"]
    if sha(p)!=rec["sha256"]:raise RuntimeError("R2 preservation drift")
    subprocess.run([sys.executable,str(ROOT/"scripts/preserve_trained_control_r2_outcomes.py"),"--verify"],check=True)
    return {"pass":True,"r2_preservation_sha256":rec["sha256"]}

def verify_protocol_parity()->dict[str,Any]:
    return {"status":"PASS","plan_sha256":sha(PARITY)}

def transitive_runtime_hashes(c:Mapping[str,Any])->dict[str,str]:
    declared=c["r5"]
    paths={"base_source":BASE_SCRIPT,"r5_source":R5_SCRIPT,"r5_config":ROOT/declared["config_path"],"r5_freeze":ROOT/declared["freeze_path"],"r2_preservation_source":PRESERVATION_SCRIPT}
    got={k:sha(v) for k,v in paths.items()}
    expected={"r5_source":declared["source_sha256"],"r5_config":declared["config_sha256"],"r5_freeze":declared["freeze_sha256"]}
    if any(got[k]!=v for k,v in expected.items()):raise RuntimeError(f"R5 transitive drift: {got} != {expected}")
    return got
...
def panel_rows(stage: str, c: Mapping[str, Any], avoid_sham: set[tuple[int, int, int]] | None = None) -> list[dict[str, Any]]:
    ...
    for block in range(pc["blocks"]):
        for within in range(pc["rows_per_block"]):
            ...
            row_seed = seed32("trained_copy_sae_basis_selection_v1", stage, pc["seed"], ix)
            ...
            rows.append({"row_id": f"tc-sae-select-v1-{stage}-{ix:05d}-{row_seed:08x}", "row_seed": row_seed, "stage": stage,
...
def panel_set(c: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
    out, avoid = {}, set()
    for stage in ("fit", "selector_fit", "development", "confirmation"):
...
def freshness_audit(panels: Mapping[str, Sequence[Mapping[str, Any]]], c: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len({r["row_id"] for r in fresh})!=len(fresh) or len({r["row_seed"] for r in fresh})!=len(fresh):
        raise RuntimeError("new split row ID/seed collision")
    if len({canon((r["clean_values"],r["corrupt_values"],r["sham_values"])) for r in fresh})!=len(fresh):
        raise RuntimeError("new split complete-state collision")
    if len({canon((r["query"],r["target"],r["contrast"],r["sham"],r["clean_values"])) for r in fresh})!=len(fresh):
        raise RuntimeError("new split full-condition collision")
    result = {
...
def pca_coverage_order(decoder: torch.Tensor, target_basis: np.ndarray) -> list[int]:
    """Exact incremental maximization of target-subspace coverage."""
    d=decoder.detach().cpu().double().numpy();p=np.asarray(target_basis,dtype=np.float64)
    _,_,vh=np.linalg.svd(p,full_matrices=False);p=vh[:int(np.linalg.matrix_rank(p))]
    q=np.zeros((0,d.shape[1]),dtype=np.float64);chosen=[];remain=set(range(len(d)))
    for _ in range(len(d)):
        best_gain,best=-1.0,-1
        for j in sorted(remain):
            r=d[j]-(d[j]@q.T)@q if len(q) else d[j].copy();n=np.linalg.norm(r)
            gain=0.0 if n<=1e-12 else float(np.sum((p@(r/n))**2)/max(len(p),1))
            if gain>best_gain+1e-12:best_gain,best=gain,j
        chosen.append(best);remain.remove(best)
        r=d[best]-(d[best]@q.T)@q if len(q) else d[best].copy();n=np.linalg.norm(r)
        if n>1e-12:q=np.vstack([q,r/n])
    return chosen


def omp_order_and_coefficients(clean_code:torch.Tensor,corrupt_code:torch.Tensor,decoder:torch.Tensor,truth:torch.Tensor,coefficient_budgets:Sequence[int]|None=None)->tuple[list[int],dict[str,list[float]]]:
    """Deterministic OMP solved on the direct float64 design matrix.

    Solving the normal equations squares the condition number and previously
    produced wrong coefficients for ill-conditioned decoder features.  The
    registered solver therefore performs an SVD least-squares solve of A_S at
    every step, using exactly NumPy's matrix-rank tolerance.
    """
    dz=(clean_code-corrupt_code).detach().cpu().double().numpy();d=decoder.detach().cpu().double().numpy();y=truth.detach().cpu().double().numpy()
    a=(dz[:,:,None]*d[None,:,:]).transpose(0,2,1).reshape(-1,len(d));yv=y.reshape(-1)
    norm=np.linalg.norm(a,axis=0);nonzero=set(np.where(norm>1e-12)[0].tolist());remain=set(range(len(d)));order=[];paths={};residual=yv.copy();budgets=set(coefficient_budgets or range(1,len(d)+1))
    for _ in range(len(d)):
        if nonzero:
            residual_corr=a.T@residual
            best=min(nonzero,key=lambda j:(-abs(float(residual_corr[j]/norm[j])),j))
        else:best=min(remain)
        order.append(best);remain.remove(best);nonzero.discard(best)
        # Exact direct-design SVD least-squares refit at *every* prefix.  This
        # intentionally avoids both normal equations and approximate QR rank
        # updates so the registered ordering is defined by np.linalg.lstsq.
        design=a[:,order];beta=np.linalg.lstsq(design,yv,rcond=None)[0];residual=yv-design@beta
        if len(order) in budgets:
            full=np.zeros(len(d),dtype=np.float64);full[order]=beta;paths[str(len(order))]=full.tolist()
    return order,paths
...
def greedy_behavior_order(sae: TopKSAE, model: Any, z: Mapping[str, torch.Tensor]) -> list[int]:
    ...
    for _ in range(224):
        ...
        for start in range(0,len(candidates),32):
         js=candidates[start:start+32];A=current[None]+ac[:,js].permute(1,0,2);S=current_s[None]+sh[:,js].permute(1,0,2)
         with torch.no_grad():
...
def fit_sae(model: Any, rows: Sequence[Mapping[str, Any]], c: Mapping[str, Any], topk: int, seed: int, device: torch.device) -> tuple[TopKSAE, dict[str, Any]]:
    ...
    with torch.no_grad():
        ...
        _,ambient_basis=base.fit_unpaired_ambient_basis(states)
        omp_order,omp_coefficients=omp_order_and_coefficients(cc,rc,sae.decoder,z["delta"],c["sae"]["selector_budgets"])
        orders={"variance":stable_order(ambient.var(0,unbiased=False)),"paired_change":stable_order(torch.mean(torch.abs(cc-rc),0)),
                ...
                "pca_coverage_greedy":pca_coverage_order(sae.decoder,ambient_basis[:32]),"fit_supervised_omp":omp_order}
    with torch.no_grad():
    ...
    return sae,{"initial_loss":losses[0],"final_loss":losses[-1],"ambient_reconstruction_relative_l2":ambient_rel,"full_code_delta_relative_l2":delta_rel,"trainable_parameters":sum(p.numel() for p in sae.parameters()),"selectors":orders,"omp_coefficients":omp_coefficients}
...
def create_lock() -> dict[str,Any]:
    c=cfg();validate_recovery_lineage(c);verify_protocol_parity();candidate_sha=sha(rp(c,"candidate_manifest"));verify_review(rp(c,"candidate_review"),candidate_sha)
    payload={"schema_version":"trained_copy_sae_capacity_v1_generator_lock","source_sha256":sha(SCRIPT),"transitive_runtime_hashes":transitive_runtime_hashes(c),"config_sha256":sha(CFG),"plan_sha256":sha(PLAN),"tests_sha256":{"study":sha(TEST),"launcher":sha(LAUNCH_TEST)},"launcher_sha256":sha(LAUNCHER),"protocol_parity_sha256":sha(PARITY),"candidate_sha256":candidate_sha,"candidate_review_sha256":sha(rp(c,"candidate_review")),"method_table_sha256":hashlib.sha256(canon({"sae":c["sae"],"linear":c["linear"]})).hexdigest()};atomic_json(rp(c,"generator_lock"),payload);return payload

def prepare() -> dict[str, Any]:
    c=cfg(); root=rp(c,"prepared_root")
    if root.exists(): raise FileExistsError(root)
    lock=loadj(rp(c,"generator_lock"))
    if lock["source_sha256"]!=sha(SCRIPT) or lock["transitive_runtime_hashes"]!=transitive_runtime_hashes(c) or lock["config_sha256"]!=sha(CFG) or lock["candidate_sha256"]!=sha(rp(c,"candidate_manifest")) or lock["candidate_review_sha256"]!=sha(rp(c,"candidate_review")):raise RuntimeError("generator lock drift")
...
def candidate(device: str, record: bool) -> dict[str, Any]:
    c=cfg(); lineage=validate_recovery_lineage(c);verify_protocol_parity();transitive_runtime_hashes(c); dev=torch.device(device); torch.manual_seed(1); x=torch.randn(32,64,device=dev);sae=TopKSAE(16,1,dev);z=sae.encode(x)
    ...
    # No scientific rows: time the exact registered 64-row x 256-feature OMP
    # kernel on seeded synthetic matrices and conservatively scale over 27 SAEs.
    rg=np.random.default_rng(4401);n=c["panels"]["fit"]["blocks"]*c["panels"]["fit"]["rows_per_block"]
    cc_b=torch.tensor(rg.normal(size=(n,256)),dtype=torch.float64);rc_b=torch.tensor(rg.normal(size=(n,256)),dtype=torch.float64);d_b=torch.tensor(rg.normal(size=(256,64)),dtype=torch.float64);y_b=torch.tensor(rg.normal(size=(n,64)),dtype=torch.float64)
    tic=time.perf_counter();bo,bp=omp_order_and_coefficients(cc_b,rc_b,d_b,y_b,c["sae"]["selector_budgets"]);omp_seconds=time.perf_counter()-tic;estimated_omp_seconds=omp_seconds*len(c["model"]["seeds"])*len(c["sae"]["topks"])*len(c["sae"]["seeds"])
    runtime_ok=sorted(bo)==list(range(256)) and set(bp)=={str(x) for x in c["sae"]["selector_budgets"]} and estimated_omp_seconds<c["hard_timeout_hours"]*3600*.5
    names=expected_method_names(c);result={"schema_version":"trained_copy_sae_basis_selection_v1_candidate","status":"PASS" if runtime_ok else "FAIL","scientific_panel_accessed":False,"recovery_lineage":lineage,"expected_method_count_per_checkpoint":len(names),"method_inventory_sha256":hashlib.sha256(canon(sorted(names))).hexdigest(),
            ...
            "runtime_profile":{"fit_rows":n,"omp_features":256,"representative_omp_seconds":omp_seconds,"estimated_all_27_omp_seconds":estimated_omp_seconds,"below_half_timeout_budget":runtime_ok,"method_records_per_checkpoint":len(names),"cached_sae_codes":4*len(c["model"]["seeds"])*len(c["sae"]["topks"])*len(c["sae"]["seeds"])},
            "source_sha256":sha(SCRIPT),"transitive_runtime_hashes":transitive_runtime_hashes(c),"config_sha256":sha(CFG),"plan_sha256":sha(PLAN),"tests_sha256":{"study":sha(TEST),"launcher":sha(LAUNCH_TEST)},"launcher_sha256":sha(LAUNCHER),"protocol_parity_sha256":sha(PARITY)}
    if record: atomic_json(rp(c,"candidate_manifest"),result)
    return result


def freeze() -> dict[str, Any]:
    c=cfg();validate_recovery_lineage(c);verify_protocol_parity(); prepared=loadj(rp(c,"prepared_root")/"PREPARED.json")
    frozen={"schema_version":"trained_copy_sae_basis_selection_v1_freeze","generator_lock_sha256":sha(rp(c,"generator_lock")),"payloads":prepared["payloads"],"freshness":prepared["freshness"],"method_inventory_sha256":hashlib.sha256(canon(sorted(expected_method_names(c)))).hexdigest()}
...
def verify_frozen() -> dict[str, Any]:
    c=cfg();validate_recovery_lineage(c);verify_protocol_parity(); lock=loadj(rp(c,"generator_lock")); frozen=loadj(rp(c,"freeze"))
    if lock["source_sha256"]!=sha(SCRIPT) or lock["transitive_runtime_hashes"]!=transitive_runtime_hashes(c) or lock["config_sha256"]!=sha(CFG) or lock["plan_sha256"]!=sha(PLAN) or lock["tests_sha256"]!={"study":sha(TEST),"launcher":sha(LAUNCH_TEST)} or lock["launcher_sha256"]!=sha(LAUNCHER) or lock["protocol_parity_sha256"]!=sha(PARITY) or lock["candidate_sha256"]!=sha(rp(c,"candidate_manifest")) or lock["candidate_review_sha256"]!=sha(rp(c,"candidate_review")) or frozen["generator_lock_sha256"]!=sha(rp(c,"generator_lock")):raise RuntimeError("freeze drift")
...
def load_model(c: Mapping[str, Any], seed: int, device: torch.device) -> Any:
    rec=c["r5"]["checkpoints"][str(seed)];path=ROOT/rec["path"]
    if sha(path)!=rec["sha256"]:raise RuntimeError("checkpoint drift")
    model=base.r5.CopyTransformer(c,seed,device);ck=torch.load(path,map_location=device,weights_only=False);model.load_state_dict(ck["state_dict"],strict=True);return model.eval()
...
def active_feature_ids(code:torch.Tensor,k:int)->torch.Tensor:
    ids=torch.topk(code,k,dim=1).indices;values=torch.gather(code,1,ids)
    return torch.where(values!=0,ids,torch.full_like(ids,-1))

def cache_context(out:Path,stage:str,model_seed:int,z:Mapping[str,torch.Tensor],saes:Sequence[tuple[str,TopKSAE]])->dict[str,Any]:
    root=out/"caches"/stage/str(model_seed);root.mkdir(parents=True,exist_ok=True)
    arrays={k:z[k].detach().cpu().float().numpy() for k in ("clean","corrupt","sham","delta","sham_delta")}
    p=root/"activations.npz";np.savez(p,**arrays);items=[{"path":p.relative_to(out).as_posix(),"sha256":sha(p)}]
    with torch.no_grad():
      for name,sae in saes:
        vals={}
        for role in ("clean","corrupt","sham"):
          code=sae.encode(z[role]).detach().cpu().float();active=active_feature_ids(code,sae.k);vals[f"{role}_code"]=code.numpy();vals[f"{role}_active_ids"]=active.numpy().astype(np.int16)
        q=root/f"{name}_codes.npz";np.savez(q,**vals);items.append({"path":q.relative_to(out).as_posix(),"sha256":sha(q)})
    return {"stage":stage,"model_seed":model_seed,"items":items}

def decoder_span_patch(decoder:torch.Tensor,order:Sequence[int],budget:int,delta:torch.Tensor)->tuple[torch.Tensor,int]:
    a=decoder[torch.tensor(order[:budget],device=decoder.device)];_,s,vh=torch.linalg.svd(a.double(),full_matrices=False);tol=max(a.shape)*torch.finfo(torch.float64).eps*(s[0] if len(s) else 0);rank=int(torch.sum(s>tol));q=vh[:rank].float();return (delta@q.T@q if rank else torch.zeros_like(delta)),rank

def method_family(name:str)->str:
    """Registered conjunction family, explicitly invariant to SAE TopK/seed."""
    import re
    if name.startswith("full_code_sae_topk"): return "full_code_sae"
    if name.startswith("decoder_full_span_oracle_topk"): return "decoder_full_span_oracle"
    m=re.match(r"sae_topk\d+_(.+)_budget(\d+)_seed\d+$",name)
    if m:return f"sae_{m.group(1)}_budget{m.group(2)}"
    m=re.match(r"selected_span_oracle_topk\d+_(.+)_budget(\d+)_seed\d+$",name)
    if m:return f"selected_span_oracle_{m.group(1)}_budget{m.group(2)}"
    m=re.match(r"fit_supervised_omp_beta_topk\d+_budget(\d+)_seed\d+$",name)
    if m:return f"fit_supervised_omp_beta_budget{m.group(1)}"
...
def write_failure_terminal(c:Mapping[str,Any],exc:BaseException)->None:
    """Best-effort, one-shot signed forensic terminal for errors/timeouts."""
    prov=rp(c,"provenance_root");terminal=prov/"TERMINAL.json"
    if terminal.exists():return
    try:
        atomic_json(terminal,{"status":"TECHNICAL_FAILURE","no_retry_authorized":True,
            "error_type":type(exc).__name__,"error":str(exc),
            "partial_artifacts":"FORENSIC_ONLY_NOT_SCIENTIFIC","time_ns":time.time_ns()})
...
def omp_beta_patch(sae:TopKSAE,order:Sequence[int],coefficient_path:Sequence[Sequence[float]],budget:int,z:Mapping[str,torch.Tensor])->tuple[torch.Tensor,torch.Tensor]:
    beta=torch.tensor(coefficient_path[str(budget)],dtype=torch.float32,device=z["clean"].device);sel=torch.tensor(order[:budget],device=beta.device);dc=sae.encode(z["clean"])-sae.encode(z["corrupt"]);ds=sae.encode(z["sham"])-sae.encode(z["corrupt"]);w=beta[sel]
    return (dc[:,sel]*w)@sae.decoder[sel],(ds[:,sel]*w)@sae.decoder[sel]
...
def run(physical_index: int, gpu_uuid: str, pane_pid:int, lockdir: str, token: str) -> None:
    ...
    for model_seed in c["model"]["seeds"]:
        model=load_model(c,model_seed,device);models[model_seed]=model;zfit=base.context(model,fit,device);entries={};sae_items=[]
        ...
        for topk in c["sae"]["topks"]:
            for method_seed in c["sae"]["seeds"]:
                ...
                torch.save({"state_dict":sae.state_dict(),"topk":topk,"seed":method_seed,"selectors":rec["selectors"],"omp_coefficients":rec["omp_coefficients"]},path)
                entries[f"sae_topk{topk}_seed{method_seed}"]={k:v for k,v in rec.items() if k!="omp_coefficients"};entries[f"sae_topk{topk}_seed{method_seed}"]["sha256"]=sha(path);sae_items.append((f"sae_topk{topk}_seed{method_seed}",sae))
        cache_context(out,"fit",model_seed,zfit,sae_items)
        fit_records[str(model_seed)]=entries;atomic_json(out/"fit"/f"model_{model_seed}.json",entries)
    atomic_json(out/"FIT_SELECTOR_SEAL.json",{"schema_version":"trained_copy_sae_basis_selection_v1_fit_seal","checkpoints":tree_inventory(out/"checkpoints"),"fit":tree_inventory(out/"fit"),"cache":tree_inventory(out/"caches"/"fit")});event(prov,15,"FIT_SELECTOR_SEALED")
    event(prov,16,"SELECTOR_FIT_ACCESS_MAY_HAVE_OCCURRED");selector_rows=base.read_jsonl(payloads["selector_fit"]);event(prov,17,"SELECTOR_FIT_OPENED",rows=len(selector_rows))
    for model_seed,model in models.items():
        zsel=base.context(model,selector_rows,device);sae_items=[]
        for topk in c["sae"]["topks"]:
          for method_seed in c["sae"]["seeds"]:
            cp_path=out/"checkpoints"/str(model_seed)/f"sae_topk{topk}_seed{method_seed}.pt";saved=torch.load(cp_path,map_location=device,weights_only=False);sae=TopKSAE(topk,method_seed,device);sae.load_state_dict(saved["state_dict"]);sae.eval();order=greedy_behavior_order(sae,model,zsel)
            atomic_json(out/"privileged_selectors"/str(model_seed)/f"sae_topk{topk}_seed{method_seed}.json",{"selector":"selector_fit_behavior_greedy","order":order,"source_stage":"selector_fit"});sae_items.append((f"sae_topk{topk}_seed{method_seed}",sae))
        cache_context(out,"selector_fit",model_seed,zsel,sae_items)
    atomic_json(out/"PRIVILEGED_SELECTOR_SEAL.json",{"schema_version":"trained_copy_sae_basis_selection_v1_privileged_selector_seal","selectors":tree_inventory(out/"privileged_selectors"),"cache":tree_inventory(out/"caches"/"selector_fit")});event(prov,18,"PRIVILEGED_SELECTOR_SEALED")
    ...
    def evaluate(stage:str,rows:Sequence[Mapping[str,Any]])->dict[str,Any]:
        all_models={}
        for model_seed,model in models.items():
            z=base.context(model,rows,device);methods={};cache_saes=[]
            ...
            for topk in c["sae"]["topks"]:
                for method_seed in c["sae"]["seeds"]:
                    saved=torch.load(out/"checkpoints"/str(model_seed)/f"sae_topk{topk}_seed{method_seed}.pt",map_location=device,weights_only=False);sae=TopKSAE(topk,method_seed,device);sae.load_state_dict(saved["state_dict"]);sae.eval();cache_saes.append((f"sae_topk{topk}_seed{method_seed}",sae));selectors=dict(saved["selectors"]);selectors["selector_fit_behavior_greedy"]=loadj(out/"privileged_selectors"/str(model_seed)/f"sae_topk{topk}_seed{method_seed}.json")["order"]
                    for selector,order in selectors.items():
                        for budget in c["sae"]["selector_budgets"]:
                            if budget<256:
                                p,ps=sae_prediction(sae,order[:budget],z);add_scored(f"sae_topk{topk}_{selector}_budget{budget}_seed{method_seed}",p,ps,None)
                                span,span_rank=decoder_span_patch(sae.decoder,order,budget,z["delta"]);span_sham,span_sham_rank=decoder_span_patch(sae.decoder,order,budget,z["sham_delta"])
                                if span_rank!=span_sham_rank:raise RuntimeError("decoder span rank instability")
                                add_scored(f"selected_span_oracle_topk{topk}_{selector}_budget{budget}_seed{method_seed}",span,span_sham,span_rank)
                    full_span,full_rank=decoder_span_patch(sae.decoder,list(range(256)),256,z["delta"]);full_span_sham,full_sham_rank=decoder_span_patch(sae.decoder,list(range(256)),256,z["sham_delta"])
                    if full_rank!=full_sham_rank:raise RuntimeError("full decoder span rank instability")
                    add_scored(f"decoder_full_span_oracle_topk{topk}_seed{method_seed}",full_span,full_span_sham,full_rank)
                    p,ps=sae_prediction(sae,list(range(256)),z);add_scored(f"full_code_sae_topk{topk}_seed{method_seed}",p,ps,64)
                    omp_order=saved["selectors"]["fit_supervised_omp"]
                    for budget in c["sae"]["selector_budgets"]:
                        p,ps=omp_beta_patch(sae,omp_order,saved["omp_coefficients"],budget,z);add_scored(f"fit_supervised_omp_beta_topk{topk}_budget{budget}_seed{method_seed}",p,ps,int(np.linalg.matrix_rank(p.detach().cpu().numpy(),tol=1e-6)))
            cache_context(out,stage,model_seed,z,cache_saes)
    ...
    atomic_json(out/"DEVELOPMENT_SEAL.json",{"schema_version":"trained_copy_sae_basis_selection_v1_development_seal","results":tree_inventory(out/"development"),"cache":tree_inventory(out/"caches"/"development")});dev_seal=loadj(out/"DEVELOPMENT_SEAL.json");event(prov,22,"DEVELOPMENT_SEALED")
    exact_ok=all(v["methods"][name]["all_gates_pass"] for v in dev.values() for name in ("exact_head_delta","identity_full64"));fit_seal=loadj(out/"FIT_SELECTOR_SEAL.json");priv_seal=loadj(out/"PRIVILEGED_SELECTOR_SEAL.json");reconcile={"checkpoints_unchanged":tree_inventory(out/"checkpoints")==fit_seal["checkpoints"],"fit_records_unchanged":tree_inventory(out/"fit")==fit_seal["fit"],"fit_cache_unchanged":tree_inventory(out/"caches"/"fit")==fit_seal["cache"],"privileged_selectors_unchanged":tree_inventory(out/"privileged_selectors")==priv_seal["selectors"],"selector_fit_cache_unchanged":tree_inventory(out/"caches"/"selector_fit")==priv_seal["cache"],"development_results_unchanged":tree_inventory(out/"development")==dev_seal["results"],"development_cache_unchanged":tree_inventory(out/"caches"/"development")==dev_seal["cache"],"frozen_inputs_verified":verify_frozen()["status"]=="PASS"};reconcile["pass"]=all(reconcile.values());pre={"exact_identity_authorized":exact_ok,"artifact_reconciliation":reconcile,"freshness_pass":freeze_rec["freshness"]["pass"],"estimated_method_outcomes_used_for_authorization":False,"confirmation_authorized":confirmation_authorization(exact_ok,reconcile["pass"] and freeze_rec["freshness"]["pass"],None)}
    atomic_json(out/"PRECONFIRMATION.json",pre);event(prov,30,"PRECONFIRMATION_SEALED",confirmation_authorized=pre["confirmation_authorized"])
    if not pre["confirmation_authorized"]:atomic_json(prov/"TERMINAL.json",{"status":"DEVELOPMENT_TECHNICAL_STOP","no_retry_authorized":True});return
    ...
    family_pass={fam:all(panel[str(ms)]["methods"][n]["all_gates_pass"] for panel in (dev,conf) for ms in c["model"]["seeds"] for n in scientific_names if method_family(n)==fam) for fam in sorted({method_family(n) for n in scientific_names})}
    final={"schema_version":"trained_copy_sae_basis_selection_v1_final","status":"SAE_BASIS_SELECTION_COMPLETE","confirmation_opened":True,"fits":fit_records,"development":dev,"confirmation":conf,"family_pass_all_panels_checkpoints_method_seeds_estimands":family_pass,"random_rank64_classification":"identity_equivalent_qa_not_random_baseline","scope":c["scope"]}
    atomic_json(out/"final"/"result.json",final);event(prov,50,"TERMINAL_COMPLETE",final_sha256=sha(out/"final"/"result.json"));atomic_json(prov/"TERMINAL.json",{"status":"SAE_BASIS_SELECTION_COMPLETE","no_retry_authorized":True,"final_sha256":sha(out/"final"/"result.json")})
