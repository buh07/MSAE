...
def build_prepared_data(config: Mapping[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    if str(config.get("data_root")) != "data/atlas_discovery_v3_3_attempt7_final_rebuild1":
        raise RuntimeError("attempt7 requires its exact external-QA science namespace")
    ...
    if not split_path.is_relative_to(repository_root) or sha256_file(split_path) != str(split_spec["sha256"]):
        raise RuntimeError("attempt6 population split drift")
    split_artifact = read_json(split_path)
    if split_artifact.get("schema_version") != "atlas_discovery_v3_3_attempt7_population_split_v1":
        raise RuntimeError("unknown attempt6 population split")
    ...
    opened_input_ledger = {
        ...
        "common_joint_cells": {
            ...
            "role": "attempt5_label_only_source_common_nuisance_support_imported_by_attempt6",
        },
        "population_split": {
            "path": str(split_path.relative_to(repository_root)),
            "sha256": sha256_file(split_path),
            "role": "attempt6_qa_science_document_firewall",
        },
    ...
    for name in ("EWT", "GUM"):
        ...
        if not excluded.issubset(observed):
            raise RuntimeError(f"attempt6 excluded document absent from source: {name}")
        parsed[name] = [row for row in parsed[name] if f"{name}:{row.document_id}" not in excluded]
        parse_exclusions[name]["attempt6_qa_reserved_documents"] = len(excluded)
        parse_exclusions[name]["attempt6_qa_reserved_sentences"] = raw_sentence_counts[name] - len(parsed[name])
