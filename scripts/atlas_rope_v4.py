#!/usr/bin/env python3
"""Atlas v3.4 attempt-8 technical measurement primitives.
...
RUN_ROOT = ROOT / "pilot_runs/20260803_atlas_rope_technical_v4"
...
PRESCORE_SCHEMA = "atlas_rope_v4_attempt8_prescore_v1"
PANEL_SCHEMA = "atlas_rope_v4_attempt8_panel_v1"
AUTH_SCHEMA = "atlas_rope_v4_attempt8_stage_authorization_v1"
BUNDLE_SCHEMA = "atlas_rope_v4_attempt8_technical_bundle_v1"
FREEZE_SCHEMA = "atlas_rope_v4_attempt8_validation_freeze_v1"
TERMINAL_SCHEMA = "atlas_rope_v4_attempt8_terminal_v1"
...
WIDTH = 768
RTOL = 5e-6
ATOL_GRID: tuple[float, ...] = (5e-7, 1e-6, 2e-6, 4e-6, 8e-6)
...
def write_signed(path: Path, payload: Mapping[str, Any], signing_key: Path, *, create_once: bool = True) -> str:
    ...
    envelope = sign_payload(payload, load_private_key(signing_key))
    digest = atomic_json(path, envelope)
    if verify_envelope(read_json(path)) != dict(payload):
        raise RuntimeError("signed artifact did not verify after write")
    return digest
...
def make_grid_units(source: str, bases: Sequence[Mapping[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for base in bases:
        ...
        base_id = f"atlas_rope_v4:{source}:base:{stable_utf8_sha256(source, base['document_id'], base['sent_id'], base['sequence_sha256'])[:24]}"
...
def verify_library_attestation(required: Sequence[Mapping[str, Any]], observed: Sequence[Mapping[str, Any]], *,
    ...
    conflicts = {name: sorted(values) for name, values in by_soname.items() if len(values) > 1}
    ...
    return {"status": "PASS" if passed else "FAIL", "missing_required": missing,
            ...
            "extra_observed": sorted(actual - expected)}
