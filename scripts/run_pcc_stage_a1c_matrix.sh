...
SCRIPT="${REPO_ROOT}/scripts/pcc_stage_a1c_confirmation.py"
RUN_TS="$(date +%Y%m%d_%H%M%S)"
RUN_ROOT="${REPO_ROOT}/pilot_runs/${RUN_TS}_pcc_stage_a1c"
...
launch_job() {
  ...
  local cache_dir="${RUN_ROOT}/cache/${job_id}"
  local session="pcc-a1c-${job_id}-${RUN_TS}"
