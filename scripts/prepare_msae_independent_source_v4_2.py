...
PRIVATE = ROOT / "data/msae_independent_source_v4_2/private"
PROV = ROOT / "reports/provenance/msae_independent_source_v4_2"
QUARANTINES = {
...
V4_AUTHORITY = {
    ...
    "reports/adversarial/msae_independent_source_v4_2_plan_review.md",
...
def inventory_repository() -> dict[str, Any]:
    ...
    for key in sorted(by_inode, key=lambda z: min(x[1] for x in by_inode[z])):
        ...
        for _p, rel, s in group:
            ...
            elif rel in V4_AUTHORITY or rel.startswith("reports/adversarial/msae_independent_source_v4_2_"):
    ...
    return {"schema_version": "msae_independent_source_v4_2_baseline_inventory_v1",
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        while True:
            ...
            if len(pending) > 4 * 1024 * 1024:
                tokens = lexical_tokens(pending[:3 * 1024 * 1024])
                if tokens:
                    yield tokens
                pending = pending[-4096:]
        tokens = lexical_tokens(pending)
        if tokens:
            yield tokens
...
def _support(rows: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {"schema_version":"msae_independent_source_v4_2_support_v1","floor_distinct_utterances":20,
...
def _license() -> dict[str,Any]:
    ...
    return {"schema_version":"msae_independent_source_v4_2_license_v1","license_id":"CC-BY-SA-4.0",
...
def _source_family(inventory: dict[str,Any], raw_hashes:set[str]) -> dict[str,Any]:
    ...
    for item in inventory["entries"]:
        ...
        with p.open("r",encoding="utf-8") as f:
            ...
            while True:
                chunk=f.read(1024*1024)
                if not chunk:break
                probe=(tail+chunk).casefold(); found=[a for a in ALIASES if a in probe]
                if re.search(r"\batis\b",probe):found.append("ATIS")
                if found:matches.append({"path":rel,"line":line_base,"aliases":sorted(set(found))})
    ...
    unexpected=[x for x in matches if x["path"] not in allowed_paths and not x["path"].startswith("reports/adversarial/msae_independent_source_v4_2_")]
    return {"schema_version":"msae_independent_source_v4_2_source_family_v1","status":"eligible" if not unexpected and not whole else "ineligible",
...
def _verify_predecessor_and_raw() -> dict[str, Any]:
    expected = {
        ROOT/"docs/plan-msae-independent-source-v4-2.md": "ff93c8b2dc5de9e0920bb910126d3ef79d5ab941bbf9c61a47803fccf9d83ae4",
        ROOT/"reports/adversarial/msae_independent_source_v4_2_plan_review.md": "9d065acea90371ffe51ba85d05923f2eb1a1fedfad719a0977fa8c610298f357",
        ROOT/"docs/plan-msae-independent-source-v4-1.md": "aad3fb92acf588d18d9fd77a040b47717d07f3e0a97ebcfcdce9e1e650b9f749",
        ROOT/"reports/adversarial/msae_independent_source_v4_1_plan_review.md": "c683e6daf4fcb8a112175390ab9902fe7c66189b1e4a295dcf5d6dae38324d55",
        ROOT/"scripts/prepare_msae_independent_source_v4_1.py": "49b2d12feb61061032839bde092dd49cf743090f3861cc8aabdf4a4bb1909564",
        ROOT/"tests/test_prepare_msae_independent_source_v4_1.py": "4055cfabca0f13c68afa6570d64d1e33c33d5cdf36ff406938bc614519785feb",
        ROOT/"reports/provenance/msae_independent_source_v4_1/rejection.json": "758527095279226cc918a5faa98f6ea634d4fc084d6786434ecc169bc0188236",
        ROOT/"docs/plan-msae-independent-source-v4.md": "79cfa0bed9140e80dd7fa97f5e6f7133c506d034e01bb0ff8822615cf4889cc2",
        ROOT/"reports/adversarial/msae_independent_source_v4_plan_review.md": "f082ff9485964057cca7f13ae6688a48ba3e628e015dd9042c895c96de03db4e",
        ROOT/"scripts/prepare_msae_independent_source_v4.py": "4cae36aef9b910426226da5d4bf649f5df2ef0ddb791e64c8d60c2454ac38238",
        ROOT/"tests/test_prepare_msae_independent_source_v4.py": "c8b661a9635cb79fe81f469209fe64fd9a204de373875bb5727e69801a146c12",
        ROOT/"reports/provenance/msae_independent_source_v4/baseline_inventory.json": "4467e76ce0ef016f32f58538df6440257b5765dad688317cf0f43e1592c5128b",
        ROOT/"reports/provenance/msae_independent_source_v4/source_acquisition.json": "26766a492aab9044aa93bddf93e4e719372522dc1d63561a6684ae259d93ea2d",
        ROOT/"reports/provenance/msae_independent_source_v4/rejection.json": "f9470c92cf659c85eed8f06aa6c85fec4ed357b228ddb12cd4b23378cc2f0fa6",
    }
    if any(sha_file(path) != digest for path,digest in expected.items()):
        raise GateFailure("predecessor_drift")
    acquisition=strict_json(ROOT/"reports/provenance/msae_independent_source_v4/source_acquisition.json")
    for name,item in acquisition["files"].items():
        path=RAW/name; s=path.lstat()
        if (not stat.S_ISREG(s.st_mode) or stat.S_IMODE(s.st_mode)!=0o444
                or s.st_nlink!=1 or sha_file(path)!=item["sha256"]):
            raise GateFailure("raw_binding_drift")
    return acquisition


def _verify_history_snapshot(inventory: dict[str, Any]) -> None:
    for item in inventory["entries"]:
        if item["disposition"] not in {"text_scanned", "v4_authority", "quarantine"}:
            continue
        path=ROOT/item["path"]
        s=path.lstat()
        if (not stat.S_ISREG(s.st_mode) or s.st_size!=item["size"]
...
def build_ready() -> None:
    ...
    predecessor_acquisition=_verify_predecessor_and_raw()
    inv=strict_json(ROOT/"reports/provenance/msae_independent_source_v4/baseline_inventory.json")
    _verify_history_snapshot(inv)
    ...
    cross=_role_overlap(rows);replace_json(PROV/"cross_role_overlap.json",{"schema_version":"msae_independent_source_v4_2_cross_role_overlap_v1","blocking_collision_count":len(cross),"blocking_collisions":cross,"status":"eligible" if not cross else "ineligible"})
    ...
    overlap={"schema_version":"msae_independent_source_v4_2_history_overlap_v1","history_unit_count":unit_count,"candidate_utterance_count":sum(map(len,rows.values())),"blocking_collision_count":len(hits),"blocking_collisions":hits,"quarantine_content_reads":0,"status":"eligible" if not hits else "ineligible"}
    ...
    role_manifest={"schema_version":"msae_independent_source_v4_2_role_manifest_v1","source_commit":COMMIT,"roles":{}}
    ...
    split={"schema_version":"msae_independent_source_v4_2_split_manifest_v1","source_commit":COMMIT,"split_algorithm":"sha256-canonical-json-array-v1-even-C1-odd-C2","record_count":len(test_rows),"C1_count":sum(x["panel"]=="C1" for x in test_rows),"C2_count":sum(x["panel"]=="C2" for x in test_rows),"entries":[{"sent_id":x["sent_id"],"panel":x["panel"],"zero_based_rank":x["zero_based_rank"],"split_key_sha256":x["split_key_sha256"]} for x in test_rows]}
    replace_json(PROV/"split_manifest.json",split)
    payload_records=({"schema_version":"msae_independent_source_v4_2_payload_v1","source_repo":REPO,"source_commit":COMMIT,"upstream_partition":"test","panel":x["panel"],"split_key_sha256":x["split_key_sha256"],"sent_id":x["sent_id"],"tokens":_token_objects(x["sentence"])} for x in test_rows)
    payload=publish_private_jsonl(PRIVATE,"blind_payload.jsonl",payload_records)
    ps=payload.lstat()
    acquisition={"schema_version":"msae_independent_source_v4_2_acquisition_v1","source_repo":REPO,"source_commit":COMMIT,"predecessor_acquisition_sha256":sha_file(ROOT/'reports/provenance/msae_independent_source_v4/source_acquisition.json'),"files":source_counts|{"README.md":{"sha256":sha_file(RAW/'README.md')},"LICENSE.txt":{"sha256":sha_file(RAW/'LICENSE.txt')}},"source_content_printed":False,"status":"eligible"}
    ...
    plan_sha=sha_file(ROOT/"docs/plan-msae-independent-source-v4-2.md");review_sha=sha_file(ROOT/"reports/adversarial/msae_independent_source_v4_2_plan_review.md")
    seal={"schema_version":"msae_independent_source_v4_2_seal_v1","status":"independent_source_ready_for_future_prescore_protocol","plan_sha256":plan_sha,"plan_review_sha256":review_sha,"program_sha256":sha_file(Path(__file__)),"test_sha256":sha_file(ROOT/'tests/test_prepare_msae_independent_source_v4_2.py'),"artifact_sha256":bindings,"payload":{"path":"data/msae_independent_source_v4_2/private/blind_payload.jsonl","sha256":sha_file(payload),"size":ps.st_size,"record_count":len(test_rows),"mode":stat.S_IMODE(ps.st_mode),"nlink":ps.st_nlink},"model_operations":0,"gpu_queries":0,"training_runs":0,"unsupported_tasks":["neutral_prefix_offset","entity_binary","entity_type","source_genre"],"opaque_binary_history_excluded":True}
    replace_json(PROV/"seal.json",seal)
    replace_json(PROV/"no_training_gate.json",{"schema_version":"msae_independent_source_v4_2_no_training_gate_v1","status":"independent_source_ready_for_future_prescore_protocol","model_scoring_authorized":False,"k2_or_branch_training_authorized":False,"stage_c_authorized":False,"seal_sha256":sha_file(PROV/'seal.json')})
