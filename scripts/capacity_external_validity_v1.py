...
import argparse, hashlib, json, math, os, random, secrets, shutil, signal, subprocess, sys, tempfile, time
...
CAP_CFG=ROOT/'configs/capacity_controller_diagnostic_v1/run.json'
EXT_CFG=ROOT/'configs/trained_copy_external_v1/run.json'
PLAN=ROOT/'PLAN_CAPACITY_EXTERNAL_VALIDITY_V1.md'
PLAN_REVIEW=ROOT/'reports/adversarial/capacity_external_validity_v1_plan_review.md'
PRESERVE=ROOT/'reports/provenance/transformer_realistic_bridge_v1_r2_postresult/PRESERVATION.json'
SCRIPT=ROOT/'scripts/capacity_external_validity_v1.py'
LAUNCHER=ROOT/'scripts/launch_capacity_external_validity_v1_tmux.sh'
TEST=ROOT/'tests/test_capacity_external_validity_v1.py'
...
def candidate_files()->list[Path]:return [PLAN,PLAN_REVIEW,PRESERVE,ROOT/'PAPER.md',ROOT/'reports/paper_claim_ledger_v1.json',CAP_CFG,EXT_CFG,SCRIPT,LAUNCHER,TEST]
...
def candidate_preflight(write:bool=True,device_name:str='cpu')->dict[str,Any]:
 ...
 pv=preservation_verify();pap=paper_verify();c=cfg('capacity');checks=[capacity_basis_check(x,c) for x in c['instances']]
 ...
 smoke=timed_smoke(device_name)
 result={'status':'PASS','preservation':pv,'paper':pap,'basis_checks':checks,'nonpanel_ols_max_abs':err,'timed_smoke':smoke,'scientific_payload_accessed':False,'files':inventory(candidate_files())}
 if write:
...
def launcher_render(uuid:str)->str:
 return f'export CUDA_VISIBLE_DEVICES="$PHYSICAL_INDEX"\n[[ "$GPU_UUID" == "{uuid}" ]]\ncd "$ROOT"\nRUN_ROOT=$(readlink /proc/self/cwd)\nMSAE_ROOT="$RUN_ROOT" python "$RUN_ROOT/scripts/capacity_external_validity_v1.py" run-pipeline --physical-index "$PHYSICAL_INDEX" --gpu-uuid "$GPU_UUID" --launch-token "$TOKEN"'
...
def create_locks()->dict[str,Any]:
 ...
 shared={'created_ns':time.time_ns(),'candidate_reviews':[cr,er],'candidate_manifests':[cm,em],'both_locks_before_payload':True}
 atomic_json(rp(cc,'generator_lock'),{'schema_version':'capacity_generator_lock',**shared,'config_sha256':sha(CAP_CFG)});atomic_json(rp(ec,'generator_lock'),{'schema_version':'external_generator_lock',**shared,'config_sha256':sha(EXT_CFG)})
...
def locks_valid()->None:
 ...
 for c,p in [(cc,CAP_CFG),(ec,EXT_CFG)]:
  ...
  if x['config_sha256']!=sha(p) or not x['both_locks_before_payload']:raise RuntimeError('invalid lock')
  for rec in x['candidate_manifests']:
   q=ROOT/rec['path']
   if sha(q)!=rec['sha256']:raise RuntimeError('candidate manifest drift after lock')
   verify_candidate_manifest(q)
  for rec in x['candidate_reviews']:
...
def freeze_one(kind:str)->dict[str,Any]:
 c=cfg(kind);p=CAP_CFG if kind=='capacity' else EXT_CFG;prep=rp(c,'prepared_root');metadata=loadj(prep/'PANEL_METADATA.json');payload=payload_records_from_metadata(metadata);base=[PLAN,PLAN_REVIEW,PRESERVE,p,SCRIPT,LAUNCHER,TEST,rp(c,'generator_lock'),prep/'PANEL_METADATA.json',prep/'PREPARED.json']
 f=rp(c,'freeze');atomic_json(f,{'schema_version':f'{kind}_freeze','status':'FROZEN_UNOPENED','kind':kind,'inventory':inventory(base),'payloads':payload,'payload_hash_source':'PANEL_METADATA.json written during generation; freeze did not stat/glob/read payloads','created_ns':time.time_ns()});return {'kind':kind,'sha256':sha(f),'files':len(base)}
...
def bind_reviews()->dict[str,Any]:
 ...
 out=rp(cfg('capacity'),'provenance_root').parent/'capacity_external_validity_v1_REVIEW_BINDING.json';atomic_json(out,{'schema_version':'capacity_external_review_binding','reviews':rows,'created_ns':time.time_ns()});return {'status':'PASS','path':out.relative_to(ROOT).as_posix(),'sha256':sha(out)}
def binding_path()->Path:return rp(cfg('capacity'),'provenance_root').parent/'capacity_external_validity_v1_REVIEW_BINDING.json'
...
def main()->None:
 ...
 q=sp.add_parser('candidate-preflight');q.add_argument('--device',default='cpu');sp.add_parser('mutation-suite');q=sp.add_parser('launcher-render');q.add_argument('--gpu-uuid',required=True);sp.add_parser('create-locks');sp.add_parser('prepare');sp.add_parser('freeze');q=sp.add_parser('verify-freezes');q.add_argument('--opaque',action='store_true');sp.add_parser('bind-reviews');sp.add_parser('verify-review-binding');q=sp.add_parser('launch-preflight');q.add_argument('--physical-index',type=int,required=True);q.add_argument('--gpu-uuid',required=True);q=sp.add_parser('run-pipeline');q.add_argument('--physical-index',type=int,required=True);q.add_argument('--gpu-uuid',required=True);q.add_argument('--launch-token',required=True)
 a=ap.parse_args()
 ...
 elif a.cmd=='create-locks':print(json.dumps(create_locks(),sort_keys=True))
