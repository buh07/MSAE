...
CFG = ROOT / "configs/trained_copy_sae_capacity_v1_r2/run.json"
PLAN = ROOT / "PLAN_TRAINED_CONTROL_STUDIES_R2_TECHNICAL_RECOVERY.md"
SCRIPT = Path(__file__).resolve()
TEST = ROOT / "tests/test_trained_copy_sae_capacity_v1_r2.py"
LAUNCH_TEST = ROOT / "tests/test_launch_trained_control_next_studies_v1_r2.py"
LAUNCHER = ROOT / "scripts/launch_trained_control_next_studies_v1_r2_tmux.sh"
PARITY = ROOT / "reports/provenance/trained_control_studies_v1_r2_PROTOCOL_PARITY.json"
...
def candidate(device: str, record: bool) -> dict[str, Any]:
    c=cfg(); lineage=validate_recovery_lineage(c);verify_protocol_parity(); dev=torch.device(device); torch.manual_seed(1); x=torch.randn(32,64,device=dev);sae=TopKSAE(16,1,dev);z=sae.encode(x)
    ...
    names=expected_method_names(c);result={"schema_version":"trained_copy_sae_capacity_v1_candidate","status":"PASS","scientific_panel_accessed":False,"recovery_lineage":lineage,"expected_method_count_per_checkpoint":len(names),"method_inventory_sha256":hashlib.sha256(canon(sorted(names))).hexdigest(),
            ...
            "source_sha256":sha(SCRIPT),"config_sha256":sha(CFG),"plan_sha256":sha(PLAN),"tests_sha256":{"study":sha(TEST),"launcher":sha(LAUNCH_TEST)},"launcher_sha256":sha(LAUNCHER),"protocol_parity_sha256":sha(PARITY)}
...
def freeze() -> dict[str, Any]:
    ...
    r1=loadj(ROOT/c["recovery_lineage"]["r1_freezes"]["items"]["trained_copy_sae_capacity_v1_r1"]["path"])
    if frozen["payloads"]!=r1["payloads"] or frozen["freshness"]!=r1["freshness"]:raise RuntimeError("R2 freeze payload/audit differs from R1")
...
def verify_frozen() -> dict[str, Any]:
    ...
    if lock["source_sha256"]!=sha(SCRIPT) or lock["config_sha256"]!=sha(CFG) or lock["protocol_parity_sha256"]!=sha(PARITY) or lock["candidate_sha256"]!=sha(rp(c,"candidate_manifest")) or lock["candidate_review_sha256"]!=sha(rp(c,"candidate_review")) or frozen["generator_lock_sha256"]!=sha(rp(c,"generator_lock")):raise RuntimeError("freeze drift")
                ...
                for method_seed in c["sae"]["seeds"]:
                    ...
                    for selector,order in saved["selectors"].items():
                        for budget in c["sae"]["selector_budgets"]:
                            ...
                            methods[name]={"topk":topk,"budget":budget,"selector":selector,"seed":method_seed,"primary":budget>=topk,"trainable_parameters":sum(x.numel() for x in sae.parameters()),"selector_decoder_storage_coordinates":budget*64,"decoded_empirical_rank":int(np.linalg.matrix_rank(p.detach().cpu().numpy(),tol=1e-6)),"mean_patch_norm":float(torch.mean(pn).cpu()),"direction_cosine":float(torch.mean(torch.sum(p*z["delta"],1)/torch.clamp(pn*tn,min=1e-12)).cpu()),"relative_l2":float(torch.mean(torch.linalg.norm(p-z["delta"],dim=1)/torch.clamp(tn,min=1e-12)).cpu()),"native":native,"matched":matched,"all_gates_pass":native["all_gates_pass"] and matched["all_gates_pass"]}
