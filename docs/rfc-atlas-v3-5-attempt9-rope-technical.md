# RFC — Atlas v3.5 attempt 9: RoPE technical measurement and unchanged atlas
...
- A label-only audit showed ESLSpok is unusable as held-out validation because every eligible
  ...
  not opened by attempt 9. Official GUMReddit was also rejected because all token forms are redacted
...
Attempt 9 is governed by `PLAN_ATTEMPT9.md`. It imports the signed attempt-8 EWT staging bundle only as `VERIFIED_OPENED_CALIBRATION_HISTORY`, appends the calibration-selected validation cap `2e-05`, freezes caps before diagnosis, uses preverified atomic no-clobber signing, and freezes the unchanged-science adapter before held-out validation. Attempt 8 remains terminal and unpromoted.
