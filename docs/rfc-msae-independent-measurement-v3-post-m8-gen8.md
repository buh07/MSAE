# MSAE v3 post-M7 gen8 execution protocol

Status: implementation candidate. This document implements the immutable gen8
...
- plan: `docs/plan-msae-independent-measurement-v3-post-m8-gen8.md`, SHA-256
...
Gen8 preserves the reviewed M1/M2 science, unchanged AMALGUM split, calibration
...
2. add the mode-0700, 78-byte `/tmp/m8p_<stage-a-sha>.sock` monitor-to-pane
...
gen8 plan and plan review, the current seven-file implementation subject, and
...
No gen8 operation opens, hashes, or content-reads these objects. Every guarded
...
and filesystem identity. Every gate rejects any unregistered `m8t_*`, `m8b_*`,
`m8p_*`, `m8x_*`, `m8a_*`, `m8c_*`, or `m8s_*` entry. Creators set and verify
...
- tmux control: `/tmp/m8t_${D}.sock`;
- launcher-to-broker gate: `/tmp/m8b_${D}.sock`; and
- monitor-to-pane lease: `/tmp/m8p_${D}.sock`.
...
`/tmp/m8x_<test-token>.sock`, `/tmp/m8a_<test-token>.sock`,
`/tmp/m8c_<test-token>.sock`, and scratch root `/tmp/m8s_<test-token>`, plus
session `msae-independent-v3-gen8-containment`. `test-token` is the SHA-256 of
...
```text
.venv-atlas/bin/python -S -B -I scripts/msae_independent_measurement_v3_post_m2_gen8.py probe-gen8-capability --pre-capability-review-sha256 <64-lowerhex> --output reports/analysis/msae_independent_measurement_v3_post_m2_gen8/gen8_capability_reproduction.json
```
...
permits implementation review; false is permanent failure for gen8.
...
3. grouped bind/listen/connect/close/unlink round trips for all three 78-byte
   gen8 production sockets, including exact live socket and `/tmp` parent identities.
...
`msae_v3_gen8_tmux_monitor_v2` and the exact plan-registered keys/nested
...
`msae_v3_gen8_pane_lease_frame_v1` protocol carries monitor-to-pane
...
`m8b_*` family empty, then sends frame 6. The monitor independently reopens the
...
- complete gen8 CPU/static/source-only/publisher/mount/recovery tests;
...
`predecessor_remediation_v1`, `predecessor_gen4`, `gen8_cpu`,
`gen8_publisher`, and `gen8_real_tmux`.  Each object has schema
`msae_v3_gen8_pre_capability_check_v1` and exact keys `schema_version`, `id`,
...
`msae_v3_gen8_check_subject_v1` digest over the registered ordered path/SHA-256
...
Only `launch-gen8`, after signed authorization, may invoke `nvidia-smi`. It
...
handoff/session information without waiting for calibration results. No gen8
