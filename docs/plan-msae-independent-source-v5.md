# PLAN — MSAE independent source v5 and replacement blind payload

Status: prospective source contract; no ParTUT source blob or v5 payload has been acquired.
...
No ATIS payload exists. V5 does not repair, rescore, or reinterpret ATIS.

## Frozen source and pre-acquisition evidence

The sole v5 candidate is the official Universal Dependencies repository
...
model use. The corrected screen is bound at
`reports/provenance/msae_independent_source_v5/preacquisition_alias_screen.json`, SHA-256
...
After this plan and its independent SHIP review, but still before source acquisition, the v5
...
`reports/provenance/msae_independent_source_v5/{preacquisition_alias_screen,historical_source_registry,preacquisition_authority_manifest,baseline_inventory}.json`;
...
`reports/verification/msae_independent_source_v5_source_free_checks.log`; and
`reports/adversarial/msae_independent_source_v5_preacquisition_implementation_review.md`; and
`reports/adversarial/msae_independent_source_v5_preacquisition_authority_review.md`.
Each receives exactly `v5_authority`, while only the unpopulated
`data/msae_independent_source_v5/` namespace receives `v5_excluded`; no prefix-based exclusion of
other v5-named paths is allowed. The baseline inventory output path is absent during its own walk and
...
The v5 namespace itself must contain exactly a non-symlink `0700` namespace, non-symlink `0700`
...
`data/msae_independent_source_v5/raw/<commit>/`, with files `0444` and the directory `0555`.
...
The only network/process-capable artifact is `scripts/acquire_msae_independent_source_v5.py`; its
...
`reports/provenance/msae_independent_source_v5/source_acquisition.json` is canonical create-once
JSON with schema `msae_independent_source_v5_source_acquisition_v1`. It records authority manifest,
...
The ignored v5 namespace, raw parents, commit directory, files, acquisition entry marker,
...
does not enter or resume a v5 acquisition attempt and requires a new reviewed protocol without
cleanup or mutation under v5. Every named temporary, whether apparently complete, partial, or
malformed, is case (c) and is never finished, removed, or promoted by this runner. This explicit
non-entry state is not claimed as a retained v5 outcome because no
...
`reports/provenance/msae_independent_source_v5/source_acquisition_entry.json` (schema
`msae_independent_source_v5_source_acquisition_entry_v1`) binding the authority manifest/review,
...
may match. The exact enumerated `v5_authority` paths are non-history and audited separately. Any unexpected alias, pathname, or whole-file digest match
is terminal. The source-free historical pedigree registry is already frozen at
`reports/provenance/msae_independent_source_v5/historical_source_registry.json`, SHA-256
...
v5 namespaces, candidate paths, predecessor bindings, generated-directory exclusion, the explicit
...
unchanged from v4.2, with lexical domains changed exactly to `msae-v5/token\0` and
`msae-v5/lemma\0`. `neutral_prefix_offset`, `entity_binary`, `entity_type`, and `source_genre` are
...
V5 inherits the v4.2 split, manifest, payload, and custody byte contract with only these enumerated
changes: domain `msae-independent-source-v5/C1C2`; schema literals
`msae_independent_source_v5_{split_manifest,role_manifest,payload}_v1`; ParTUT repo/commit,
...
`é`/C1/rank0/zero-key and `x!`/C2/rank1/f-key has v5 SHA-256
`1d131b624f72da60b576c173b16d91dba8d265eed701e27e8db0546ba2a1e513`. The public split and role
...
`data/msae_independent_source_v5/private/blind_payload.jsonl`. Each row has exactly
...
`msae_independent_source_v5_scientific_artifact_inventory_v1`. Its ordered universe is every final
...
### V5-M1 — prospective plan and source-free implementation

- [ ] Independent plan review returns SHIP while all v5 source/payload paths remain absent.
- [ ] Implement and test every source-free parser, adapter, archive, dedup, split, custody, pedigree,
      capability, inventory, and separately reviewed acquisition-runner/validation path; bind the exact v4.2 predecessor
      builder/test and v4/v4.1/v4.2 rejections.
...
### V5-M2 — source and scientific gates
...
- [ ] Parser, source-family, prospective dedup, four-role separation, full-history overlap, and
      support gates pass in order, or the first failure is retained and ends v5.

### V5-M3 — payload, review, and stop
...
- [ ] The reviewed v5 builder/runner initiated no model, tokenizer, GPU, scoring, K2/branch training,
...
- [ ] All v4/v4.1/v4.2/v5 tests pass with plugin autoload disabled; `py_compile` and AST closure pass.
