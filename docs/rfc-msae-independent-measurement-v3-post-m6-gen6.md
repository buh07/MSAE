# MSAE v3 post-M6 gen6 execution protocol

Status: implementation candidate. This document implements the immutable gen6
...
- plan: `docs/plan-msae-independent-measurement-v3-post-m6-gen6.md`, SHA-256
  `dc5e72ebd015acdeb1bbe472ca0d1a275e976fb07f3b5af51251bec66ca0c6ac`;
- plan review:
  `reports/adversarial/msae_independent_measurement_v3_post_m2_gen6_plan.md`,
  SHA-256
  `852d6acd0b6ead8df5b7b4aa316dfa766cee2be799513781bedb9f6d85fcaba5`.
...
Gen6 preserves the reviewed M1/M2 science, unchanged AMALGUM split, calibration
...
1. preserve gen5 as failed before capability/protocol state;
2. replace its impossible 119-byte and 129-byte Linux AF_UNIX names with
   distinct 78-byte `/tmp/m6t_<stage-a-sha>.sock` and
   `/tmp/m6b_<stage-a-sha>.sock` names; and
3. run tmux as a captured foreground server owned by a persistent outside-tmux
   monitor, with a complementary in-pane supervisor and bounded clients.
...
The disclosed gen5 real-tmux invocations were non-scientific containment tests;
they were not capability runs and do not establish that gen5 terminal bytes
passed. No gen5 capability, implementation review, M3, Stage A, signature,
GPU/model job, or calibration session exists.

## 2. Immutable authority and review sequence

Every gate rehashes the frozen M1/M2 evidence, failed-gen3/gen4/gen5 subjects,
gen6 plan and plan review, the current seven-file implementation subject, and
...
1. reviewed gen6 plan;
2. all implementation/static/CPU/publisher/source-only/real-tmux tests;
3. external `gen6_pre_capability` SHIP review;
4. exactly one capability reproduction and publication;
5. external `gen6_implementation` SHIP review;
6. 23-receipt M3 setup;
7. external `gen6_post_m3` SHIP review;
8. two-tree M4 and traced prescore evidence;
9. external `gen6_prescore` SHIP review;
...
No gen6 operation opens, hashes, or content-reads these objects. Every guarded
...
Both short `/tmp` families are independently closed. A held no-follow `/tmp`
directory FD binds its device, inode, UID/GID, mode 01777, mountinfo digest, and
filesystem identity. Every gate rejects any unregistered `m6t_*` or `m6b_*`
entry. Creators set and verify umask 0077. Live sockets are current-UID/GID,
mode-0700, nlink-1 sockets whose device/inode identity remains stable around
every command and unlink. Foreign/substituted paths are never removed.

For Stage-A digest `D` (exactly 64 lowercase hexadecimal characters):

- tmux control: `/tmp/m6t_${D}.sock`;
- broker handoff: `/tmp/m6b_${D}.sock`.

Each encoded name is exactly 78 bytes. Both are absent before launch. The broker
socket exists only from launcher bind through final gate confirmation and is
removed before takeover ACK/launcher return. The tmux socket remains bound for
the live run and is removed only after terminal outcome or failure and complete
session/server/process extinction.
...
permits implementation review; false is permanent failure for gen6.
...
3. grouped bind/listen/connect/close/unlink round trips for both 78-byte gen6
   short sockets, including exact live socket and `/tmp` parent identities.
...
M3 first publishes typed `failed_gen5.json`, then the inherited gen4/gen3/gen2
records. Failed-gen5 evidence binds the terminal gen5 subject, both impossible
path observations, complete absence of its capability/protocol/downstream
state, and the separately classified operator-disclosed non-experiment tmux
history. It says calibration GPU/model/tmux was not run; it does not erase the
disclosed containment tests.
...
Setup is a 23-receipt transaction. Each receipt binds the prior receipt, plan,
...
The registered order is provenance root; failed gen5/gen4/gen3/gen2; M4 data;
config; state; nonce; key staging; private payload; key binding; final key;
setup subject; public key; commitment; CPU trace; protocol; key-binding deletion;
key-payload deletion; key-staging removal; premanifest; final manifest.

The result is non-authorizing M3 state: exact four-file config, empty M4 data,
four failed records plus setup manifest in provenance, empty nonce directory,
...
After repeated idle checks and the UUID flock, the launcher creates
`launcher_intent.json`, mode-0600 `monitor.log` and `tmux_server.log`, a bound
broker socket, and one `AF_UNIX/SOCK_SEQPACKET` socketpair. It starts exactly one
source-only monitor in its own process group with a closed environment and four
distinct inherited FDs >=3: control, lease, monitor log, and server log. The
monitor verifies stdio, all four bindings, inheritable flags, closed unrelated
FDs, control credentials, environment, launcher intent, lease, code, and logs
before proceeding.

The launcher publishes canonical mode-0600 `tmux_monitor.json` with schema
`msae_v3_gen6_tmux_monitor_v1` and the exact plan-registered keys/nested
schemas. It binds the authorization, Stage A, capability, launcher/monitor,
code, control, four FDs, logs, GPU/lease, both socket names, and handoff/terminal
deadlines.
...
```text
/usr/bin/tmux -D -f /dev/null -S /tmp/m6t_<D>.sock
```

After the mode-0700 socket appears, separate captured process groups run bounded
15-second clients for:

- `new-session -d -s msae-independent-v3-gen6-calibration` with the exact
...
`m6b_*` family empty, then sends frame 6. The monitor independently reopens the
...
- complete gen6 CPU/static/source-only/publisher/mount/recovery tests;
- planted closure and namespace mutations;
- capability report/recovery negative paths;
- 23-receipt interruption/reentry matrix; and
- the one plan-authorized, non-experiment real-tmux containment command using
  ...
  deadline, fault boundaries, and exact final absence.
...
Only `launch-gen6`, after signed authorization, may invoke `nvidia-smi`. It
...
handoff/session information without waiting for calibration results. No gen6
