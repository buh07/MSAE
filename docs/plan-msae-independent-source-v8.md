# PLAN — MSAE independent source v8 and replacement blind payload

Status: prospective source contract; no Swedish Talbanken source blob or v8 payload has been acquired.
...
## Prior retained failures and v8 delta
...
No ATIS payload exists. V8 does not repair, rescore, or reinterpret ATIS.
...
V7 froze Finnish TDT at `bfaae13719f249573d940edda6a0d7aa8eec620f`, passed independent
plan, source-free implementation, and pre-acquisition authority reviews, and entered its one-shot
acquisition. It stopped at the first frozen `tree_file_set` gate: the hash-only 290-byte `ls-tree`
result reconstructed as the three requested CoNLL-U files plus `LICENSE.txt`, while the contract
required those four paths plus absent `README.md` (353 bytes if complete). Scratch cleanup
succeeded; no raw namespace, acquisition-success report, scientific artifact, or payload survived.
The retained entry, acquisition rejection, and independent terminal SHIP review have SHA-256 values
`a5ed3430c081704e2b9dd8e7c53e928ebee21841dea3f5dcf7b823632e9fe7fa`,
`e71be14d61b76965423f6c209f3049eef156dc1daa5fdccfae80e5c040701a7e`, and
`a9ffad68a78df7b3362ff3900bd7a4e0959a33a63742d29175ba1ccef580207b`.
The terminal snapshot truthfully recorded one unrelated external forbidden-token identity, while
the builder/runner initiated zero model, GPU, scoring, or training operations and all authorization
bits remained false. V7 is terminal and is not retried.

V8 changes both candidate and namespace. The sole candidate is Swedish Talbanken, fixed by one
metadata-only remote-head query before this plan; it inherits the globally sorted training-root
reconstruction and every applicable one-shot/custody safeguard from reviewed v7. It requests only
the three conventional CoNLL-U partitions plus `LICENSE.txt`, deliberately omitting README from the
tree contract and license predicate. It also freezes a candidate-independent license grammar and an
explicit document-group split before any Swedish source blob is acquired.

The exact predecessor set below is direct, not transitive: the v8 builder constant, source-free
...
The sole v8 candidate is the official Universal Dependencies repository
`https://github.com/UniversalDependencies/UD_Swedish-Talbanken.git` at commit
`c434778d9511be5c35a6a11531f0107a960fb5d6`. Before source acquisition, a
normalized alias scan of all 16,133 safely readable current project-history text files used the
contiguous token sequences `ud swedish talbanken`, `swedish talbanken`, `sv talbanken`,
`https github com universaldependencies ud swedish talbanken git`, and the commit. Input is strict UTF-8,
...
normalization goldens. The exact 16,133 sorted `{path,sha256,size}` records are
...
They include all safely readable v5/v6/v7 public/raw text as history, exclude only the exact enumerated
v8 authority/generated paths, and lstat but do not read the three Atlas quarantines. Candidate
...
`reports/provenance/msae_independent_source_v8/preacquisition_alias_screen.json`, SHA-256
`68a7536c95896087960dd4fc5882df85296baaa1a07aaf910e0e7c257b165e2f`, schema
`msae_independent_source_v8_preacquisition_alias_screen_v2`, with zero
quarantine reads, model operations, GPU queries, and training runs. It supports only
`project_source_use_unseen_before_v8` if the later digest, pedigree, and full-content gates pass—not
researcher-unawareness, pretraining independence, or content separation by itself. After the
screen, any new candidate alias is permitted only in the exact enumerated v8 authority artifacts;
all v4/v5/v6/v7 artifacts and raw files remain history, never v8 authority.

After this plan and its independent SHIP review, but still before source acquisition, the v8
...
`reports/provenance/msae_independent_source_v8/{preacquisition_alias_screen,historical_source_registry,preacquisition_authority_manifest,baseline_inventory}.json`;
...
`reports/verification/msae_independent_source_v8_source_free_checks.log`; and
`reports/adversarial/msae_independent_source_v8_preacquisition_implementation_review.md`; and
`reports/adversarial/msae_independent_source_v8_preacquisition_authority_review.md`.
Each listed path receives exactly `v8_authority`, while only the unpopulated
`data/msae_independent_source_v8/` namespace receives `v8_excluded`; no prefix-based exclusion of
other v8-named paths is allowed. The baseline inventory output path is absent during its own walk and
...
sorted `{path,sha256,size}` array contract above and must equal the registry's 16,133 `inputs`
byte-for-byte and reproduce
`8070a7cdd536a0dbd6104ff1dfb279ef01fb0acf98e969227625324155227564`. The frozen registry contains zero archive-member input
...
additions only the exact four acquired raw file paths and these exact post-baseline public-chain
...
The v8 namespace itself must contain exactly a non-symlink `0700` namespace, non-symlink `0700`
`raw/` parent, non-symlink `0555` commit directory, and four `0444` one-link regular files, with no
...
`sv_talbanken-ud-train.conllu`, `sv_talbanken-ud-dev.conllu`, `sv_talbanken-ud-test.conllu`, and
`LICENSE.txt` beneath ignored
`data/msae_independent_source_v8/raw/<commit>/`, with files `0444` and the directory `0555`.
...
The only network/process-capable artifact is `scripts/acquire_msae_independent_source_v8.py`; its
...
`reports/provenance/msae_independent_source_v8/source_acquisition.json` is canonical create-once
JSON with schema `msae_independent_source_v8_source_acquisition_v1`. It records authority manifest,
...
resolved commit and tree SHA-1; exact four path/blob SHA-1 pairs from `ls-tree`; each installed file's
...
The ignored v8 namespace, raw parents, commit directory, files, acquisition entry marker,
...
does not enter or resume a v8 acquisition attempt and requires a new reviewed protocol without
cleanup or mutation under v8. Every named temporary, whether apparently complete, partial, or
malformed, is case (c) and is never finished, removed, or promoted by this runner. This explicit
non-entry state is not claimed as a retained v8 outcome because no
...
`reports/provenance/msae_independent_source_v8/source_acquisition_entry.json` (schema
`msae_independent_source_v8_source_acquisition_entry_v1`) binding the authority manifest/review,
...
matches. The exact enumerated `v8_authority` paths are non-history and audited separately; their
...
16,133 current-history inputs and frozen at
`reports/provenance/msae_independent_source_v8/historical_source_registry.json`, SHA-256
`ff4a36996f58531353dda9ed8fcbedb4c3a615f2d877e8b31f1462ddc8581a97`, schema
`msae_independent_source_v8_historical_source_registry_v1`. Its exhaustive input rule is every
...
`separator_hex="00"` with domain `msae-v8/pedigree`, ordered steps, strip codepoints, regex flags/runtime/chunking, and five
...
`ee69f1b7839d9eaba43b743baa065b321c154e13577dc6dd2a21816b9b2aea65`. It contains 4,051 identifiers and binds its regex bytes, normalization,
...
Candidate pedigree extraction applies exactly that same frozen byte-regex, chunk/overlap,
strict-UTF-8, normalization, boundary, overlength, duplicate-suppression, occurrence-path, and
identifier-hash grammar to all four acquired raw files: `sv_talbanken-ud-train.conllu`,
`sv_talbanken-ud-dev.conllu`, `sv_talbanken-ud-test.conllu`, and `LICENSE.txt`. This includes
identifier-bearing CoNLL-U comment lines; none of the three partitions is treated as prose-only or
excluded from pedigree. It also injects the normalized frozen repo URL, commit, repo name,
`sv_talbanken`, and source filenames. Exact candidate
...
A source-free fixture independently places a prior-history URL and a prior-history `UD_*` token in
each of train, dev, and test CoNLL-U comments; every such fixture must terminate at candidate
pedigree before deduplication, support calculation, role/split construction, or payload creation.
A pass supports only `project_source_use_unseen_before_v8` plus later audited
...
`https://creativecommons.org/licenses/by-sa/4.0/`. The v8 grammar is frozen independently of the
candidate bytes. `LICENSE.txt` is strict UTF-8, NUL-free, NFKC-normalized, and
...
token and therefore cannot satisfy a window. The file must supply at least one
...
The public report records the file SHA-256,
...
for v8 namespaces, candidate paths, predecessor bindings, generated-directory exclusion, explicit
...
unchanged from v4.2, with lexical domains changed exactly to `msae-v8/token\0` and
`msae-v8/lemma\0`. `neutral_prefix_offset`, `entity_binary`, `entity_type`, and `source_genre` are
...
This rule is frozen before Swedish Talbanken content acquisition. It is a leakage-control transform, not a
...
V8 inherits the v4.2 manifest, payload, and custody byte contract with these enumerated split
changes: group domain `msae-independent-source-v8/C1C2-group`; schema literals
`msae_independent_source_v8_{split_manifest,role_manifest,payload}_v1`; Swedish Talbanken repo/commit,
...
`data/msae_independent_source_v8/private/blind_payload.jsonl`. Each row has exactly
...
`msae_independent_source_v8_scientific_artifact_inventory_v1`. Its ordered universe is every final
...
four-path checkout with a minimal `PATH`, `HOME` set to a fresh empty temporary, prompts and hooks
...
### V8-M1 — prospective plan and source-free implementation

- [ ] Independent plan review returns SHIP while all v8 source/payload paths remain absent.
- [ ] Implement and test every source-free parser, adapter, archive, dedup, split, custody, pedigree,
      ...
      v4/v4.1/v4.2 rejection chain, v5 control-plane rejection chain, v6
      plan/reviews/builder/runner/test/config/baseline/authority/acquisition/license/rejection chain,
      and the immediate v7 plan/plan-review/alias-screen/registry/builder/runner/test/config/
      source-free-transcript/implementation-review/baseline/authority-manifest/authority-review/
      acquisition-entry/acquisition-rejection/terminal-review chain exactly as enumerated in the
      predecessor table.
...
### V8-M2 — source and scientific gates
...
- [ ] The literal 15-step post-acquisition table above—structural recensus/snapshot, document
      ...
      order, or the first failure is retained and ends v8.

### V8-M3 — payload, review, and stop
...
- [ ] The reviewed v8 builder/runner initiated no model, tokenizer, GPU, scoring, K2/branch training,
...
- [ ] All v4/v4.1/v4.2/v5/v6/v7/v8 tests pass with plugin autoload disabled; `py_compile` and AST closure pass.
