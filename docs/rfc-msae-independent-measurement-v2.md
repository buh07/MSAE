# PLAN — MSAE exposed-source measurement remediation v2

> Prospective successor protocol, 2026-08-20. V1 remains immutable and failed.
> This v2 may use the same already-exposed AMALGUM partition only as an
> **exposed-source technical/measurement replication**, never as a pristine
> independent-source confirmation.

## Goal

Preserve the failed v1 record; replace its short-boilerplate-sensitive overlap
...
- Never mutate or reinterpret any v1 config, Stage A, overlap result, review, or
  ...
  `stage_ready=false`, and Stage B/C `not_run`. Before v2 implementation, freeze
  ...
  v2 build and launch rehashes that exhaustive protected manifest.
- “V1 byte-for-byte preserved” refers to those v1-owned protected roots/files.
  ...
  exposure incident, v2 deliberately makes no unverifiable current-content
...
- Do not call v2 pristine independent evidence, training-data novelty, or a
...
- Planning/review commands on 2026-08-20 recursively searched `data/` and therefore
  ...
  must be replaced before any future Stage C. This v2 calibration protocol never
...
`scripts/msae_measurement_remediation_v1.py`. V2 is additive and wraps those
...
substantive document independence from accessible Atlas text under the v2 gate,
...
text record under baseline `data/`, excluding only the exact v2 self-output root,
...
`data/msae_independent_measurement_v2/`; no other path containing `v2`, including
every `data/atlas_measurement_v2*` root, is excluded. That root must be absent at
...
`exposed_quarantined_not_scanned_by_v2`. Each file has distinct fields
`v1_status_at_v1_audit=excluded_sealed_unopened` and
`v2_current_status=exposed_quarantined_not_scanned`; v2 never copies the former
...
RFC and `TODO.md`; the three v2 entry points
`scripts/msae_independent_measurement_v2.py`,
`scripts/run_msae_independent_calibration_v2.py`, and
`scripts/launch_msae_independent_calibration_v2.sh`; one test
`tests/test_msae_independent_measurement_v2.py`; config files
`configs/msae_independent_measurement_v2/{protocol.json,authorization_commitment.json,ed25519_public.pem}`;
and data files under the sole self-output root named
`{data_baseline_manifest,protected_v1_manifest,source_exposure,source_alias_manifest,overlap_fixtures,history_census,history_overlap,v1_row_crosswalk,label_rows,task_manifest,support,prefix_templates,maps,finite_pass_matrix,protocol_imports,dependency_closure,endpoint_registry,environment_allowlist}.json`
(each expands to `.json` except that `label_rows` expands only to `.jsonl`). Prescore provenance is exactly
`reports/provenance/msae_independent_measurement_v2/{stage_a,status,prescore_candidate_manifest}.json`
and the external review transcript is exactly
`reports/adversarial/msae_independent_measurement_v2_prescore.md`.

The only live run root is
`pilot_runs/20260820_msae_independent_measurement_v2_calibration/`. Its cache is
...
directories `/tmp/msae_independent_measurement_v2_build_primary` and
`/tmp/msae_independent_measurement_v2_build_rebuild`, created with exclusive
...
this allowlist; planted `v2` lookalikes, undeclared output, symlink, and sealed-
designation escape attempts must fail.
The external-state write allowlist contains the private key
`/jumbo/lisp/f004ndc/.msae_keys/independent_measurement_v2_ed25519_private.pem`
...
`/tmp/msae_v2_prescore_trace_${M}.log` and
`/tmp/msae_v2_prescore_check_${M}.json`, where `M` is the validated
candidate-manifest digest; the initially absent mode-0700 CPU-test root
`/tmp/msae_independent_measurement_v2_verify`; and the tmux socket
`/tmp/msae_independent_measurement_v2_${A}.sock` where `A` is the validated
v2-Stage-A digest. Build/review/verification paths must be absent before
exclusive creation and are removed after their phase; the tmux socket persists only while the
fixed session `msae-independent-v2-calibration` lives. All Python commands use
...
M0 also installs `data_baseline_manifest.json` before any v2 data output. It
...
v2 self-output root absent. M1/M4/reviewer/signer/launcher revalidate it with a
closed projection that permits only the exact declared v2-root files created by
...
`same_frozen_source_copy` for every exclusion. V2
...
identity blocks. `.md` and `.diff` are each one document whose sentences are its
nonempty physical UTF-8 lines in encounter order after `\r\n`/`\n` terminators
are removed (markup/prefix characters are
...
5. “Short sentence” means <10 lexical tokens. For a document pair, form the set
   ...
   tuple once and sum its tuple length once in encoded-tuple byte order. Block
   only when the set has >=5 members and summed length >=20. Empty/punctuation-
   only strings are ignored. Fixtures cover 4/5 members, 19/20 tokens, and
   arbitrary repeats of one heading remaining one member.
6. Shared-passage coverage blocks when at least four distinct shared five-grams
   cover >=20 selected token positions and >=10% of that selected document.
...
7. Every threshold is inclusive. Fixtures cover 49/50 tokens, 19/20 document
   grams, 9/10 sentence tokens, 5/6 sentence grams, 19/20 positions,
   `0.10-1e-12`/0.10 coverage, and `threshold-1e-12`/threshold Jaccard.
8. Any parse failure, identity conflict, nonfinite weight, incomplete manifest,
   or additional sealed-payload content access by the v2 builder/audit invalidates
...
The v2-only `v1_row_crosswalk.json` is a
`v2_replay_crosswalk_of_frozen_v1_rows`, not a v1 reclassification. It preserves
every frozen legacy field/verdict verbatim and adds only v2 namespace/identity/
diagnostic fields; it is never written beneath a v1 root. Construction is
reproducible despite the legacy rows lacking IDs:
rerun the exact v1 algorithm over its frozen file/hash universe; assign selected
sentence and historical file/record/sentence zero-based indices; sort by
`(historical_path, selected_path, selected_sentence_index,
historical_record_index, historical_sentence_index, jaccard)`; and assign an
occurrence ordinal to duplicate visible signatures. Require the projected
legacy fields/multiplicities to equal all 206 frozen rows before applying v2.
...
- Every “u64-length-prefixed tuple” below uses one typed codec. A tuple begins
  ...
  Golden cap tuple
  `("msae_exposed_source_measurement_v2","C1","amalgum:academic","doc-1","sent-1",7,"row-1")`
  hashes to `43dcf75650846dcfabfee037c9190aaca3f1d10e936ee5c80ff2a836428ee399`;
  golden map tuple
  `("msae_exposed_source_measurement_v2","20260820","calibration","absolute_bucket","gum",0,0)`
  hashes to `a65d135b27ed6f9979c6ade8fd107828d62acc6e0b7e5bb1bec29f3d73111dca`.
...
- Each applicable task/role needs >=25 groups, every retained class >=20 groups,
  ...
  `("msae_exposed_source_measurement_v2","20260820",role,task,stratum,draw,slot)`
...
A canonical v2 protocol manifest binds path, size, mode, and SHA-256 for:

- this RFC; v1 failure/status/overlap; v2 source, overlap fixtures/results,
  row/task/support/template/map manifests; remediation/v2/replay/launcher code;
...
No other v1 scientific rule is imported. In particular, v2's genuine
...
an imported endpoint task ID is resolved only through the v2 task manifest, and
...
every public v2 entry point; observed requests must be a subset of the static
...
not proof of v2 readiness. A v2-only wrapper constructs each evidence category
...
and emits a distinct v2 Stage-A schema. The v2 runner and launcher accept only
...
`/jumbo/lisp/f004ndc/.msae_state/independent_measurement_v2/nonces`, expected device/inode,
...
v2 Stage-A digests, supplies those remaining fields, and is accepted by the
...
digest, closure/config/v2-Stage-A hashes, exact current user instruction and
...
`.../independent_measurement_v2`, and `.../nonces`, all exact mode `0700`, then
...
After `tmux -S /tmp/msae_independent_measurement_v2_${A}.sock new-session -d -s msae-independent-v2-calibration`,
...
action; every closure entry; v2 Stage-A and closure digests; and the complete
porcelain-v2 repository status with every pre-existing dirty/untracked path
...
`M=$(sha256sum reports/provenance/msae_independent_measurement_v2/prescore_candidate_manifest.json | awk '{print $1}')`
and requires `M` to full-match lowercase `[0-9a-f]{64}`. It then runs
`.venv-atlas/bin/python -B -I scripts/msae_independent_measurement_v2.py prescore-traced-check --manifest reports/provenance/msae_independent_measurement_v2/prescore_candidate_manifest.json --output /tmp/msae_v2_prescore_check_${M}.json --trace /tmp/msae_v2_prescore_trace_${M}.log`,
followed by
`.venv-atlas/bin/python -B -I scripts/msae_independent_measurement_v2.py verify-prescore-trace --manifest reports/provenance/msae_independent_measurement_v2/prescore_candidate_manifest.json --check /tmp/msae_v2_prescore_check_${M}.json --trace /tmp/msae_v2_prescore_trace_${M}.log`.
...
- [ ] **M0 — V1 preservation and v2 plan.** Hash v1 terminal artifacts; plan gets
  adversarial SHIP. Acceptance: the exhaustive config/data/provenance/RFC/
  review/scripts/tests v1-owned protected manifest is installed, every byte it owns is
  unchanged, the exact terminal failure remains explicit, and the complete
  pre-v2 `data/` baseline manifest is create-once with v2 root absent.
- [ ] **M1 — Overlap remediation.** Implement fixtures and v2 audit. Acceptance:
...
- V2 is explicitly exposed-source, uses unchanged documents, and passes a
...
- From the repository root run one `bash -euo pipefail` block that sets
  `V=/tmp/msae_independent_measurement_v2_verify`, requires it absent, creates it
  mode `0700`, installs `trap 'rm -rf -- "$V"' EXIT`, creates mode-0700
  `$V/tmp`, and exports `TMPDIR=$V/tmp`. Inside that block run
  `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv-atlas/bin/python -B -m pytest -q -p no:cacheprovider --basetemp=$V/pytest tests/test_msae_measurement_remediation_v1.py tests/test_msae_independent_measurement_v1.py tests/test_msae_independent_measurement_v2.py`;
  then in-memory syntax compilation (no `py_compile` output) with
  `.venv-atlas/bin/python -B -c 'from pathlib import Path; ps=["scripts/msae_measurement_remediation_v1.py","scripts/msae_independent_measurement_v1.py","scripts/run_msae_independent_calibration_v1.py","scripts/msae_independent_measurement_v2.py","scripts/run_msae_independent_calibration_v2.py"]; [compile(Path(p).read_bytes(),p,"exec") for p in ps]'`;
  then `bash -n scripts/launch_msae_independent_calibration_v1.sh scripts/launch_msae_independent_calibration_v2.sh` and `git diff --check`.
