# RFC: Atlas v3.1 attempt 5 discovery study of positional, contextual, lexical, and relational factors

**Status:** attempt-5 source-common adjusted-predictive-specificity prescore plan; no representation scoring has started  
**Date:** 2026-08-02  
...
The loader rejects every data path other than those two exact canonical CoNLL-U paths; it never opens a mixed-source parent merely to filter it. It also rejects any role/source/path containing `C1`, `C2`, `confirmation`, `final`, `blind`, `private`, `ESLSpok`, `LinES`, `WikiNeural`, `FewNERD`, `WNUT`, or `PUD`, resolves real paths before opening, rejects symlinks and digest mismatches, and restricts all writes to new `data/atlas_discovery_v3_1_attempt5` and `pilot_runs/20260803_atlas_discovery_v3_1_attempt5` roots. A pre-open process snapshot and an opened-input ledger are terminal artifacts.
...
The master seed is `20260803`. A document's five-fold assignment is the first eight SHA-256 bytes of `atlas_discovery_v3_1_attempt5|fold|20260803|{source}|{document_id}`, interpreted big-endian and reduced modulo five. CoNLL-U `newdoc id` is the document ID; a sentence before the first `newdoc id`, duplicate sentence ID, or duplicate document ID block is an error. For each `(source, task, document)`, candidate rows are ordered **without labels** by SHA-256 of `atlas_discovery_v3_1_attempt5|row-cap|20260803|{row_id}` and the first `128` are selected. Labels are opened only after this sample is frozen; support is then recomputed and cannot trigger reselection.

The 500 ordinary held-out bootstrap maps resample endpoint-local connected component IDs with replacement, drawing exactly the observed number of components. For component slot `j`, draw `b`, direction `d`, and endpoint `e`, the selected index is the first eight SHA-256 bytes of `atlas_discovery_v3_1_attempt5|bootstrap|20260803|{d}|{e}|{b}|{j}` modulo the sorted component count. All rows and linked donors in a sampled component receive its multiplicity. A draw is finite only when every frozen target class is present and every metric denominator is finite; no redraw-until-acceptance is allowed.
...
Cross-family adjusted predictive specificity is measured by source-held-out linear classification of normalized single-target-token delta directions among `relative_gap`, `true_context`, `unrelated_context`, and `proper_noun_substitution`. Rows are deterministically class-balanced within each source/fold; pooled multi-token means are not mixed with token deltas. A nuisance-only construct classifier receives exactly the four marginal categorical blocks and their joint-interaction block frozen below; intervention target count is fixed to one. The representation-delta classifier must improve held-out macro-F1 over that nuisance classifier by at least `0.02` with lower bound `>0` in both directions.

Attempt 4 was retired before inference because its GUM-fit to EWT-held-out
context classes had 27.33% unseen nuisance joint cells, above the frozen 20% gate.
Attempt 5 therefore defines a **source-common adjusted predictive estimand**. The
exact 94-cell allowlist is the mechanical intersection of joint nuisance cells
present in the two attempt-4 pooled label-only candidate populations; it is stored
and hashed in `configs/atlas_discovery_v3/attempt5_common_joint_cells.json`. Rows
outside this source-common set are excluded before balancing. This transductive
support definition uses both exploratory sources' nuisance support but no
activation or scientific outcome and is not a generalization claim.

The attempt-5 label-only joint population is frozen before inference and estimates
**adjusted predictive specificity**, not exact-matched or causal specificity. For
each source and fold separately, let `m` be the smallest remaining candidate count
among the four construct classes. Within `(source, fold, class)`, order rows by SHA-256 of
`atlas_discovery_v3_1_attempt5|joint-cap|20260803|{source}|{fold}|{class}|{joint_row_id}`
and retain the first `m`. No representation value, scientific outcome, or
fit/held-out direction changes this sample. Each class must retain at least 50 rows
and 25 distinct target documents overall, at least 5 rows in every fold, and all
four classes must appear in at least 490/500 shared-node bootstrap draws.

The exact nuisance design is four fit-source-only marginal categorical blocks
(start-distance, coarse UPOS, capitalization, and word-length) plus one categorical
joint-interaction block over their four-tuple. Every block includes a frozen
`__UNKNOWN__` column; unseen held-out marginal levels and joint cells map only to
the corresponding unknown column. Nuisance-only and representation-plus-nuisance
models use identical rows, columns, fit-source standardization, ridge penalty grid,
fit-only alpha selection, and held-out bootstrap maps. A nuisance-recoding planted
fixture must have zero incremental signal. The primary incremental result must be
positive both on the full held-out sample and on the subset whose joint cells were
observed in the fit source.

Common support is a prescore gate in both transfer directions. For every held-out
class, no more than 20% of retained rows may have a joint cell absent from the fit
source, and the fit-observed-cell subset must contain at least 25 target documents
and 40 rows. Failure makes the endpoint ineligible without relaxing these values.
The manifest reports candidate/retained counts, marginal and joint-cell
distributions, unseen rates, common-support rows/documents, folds, and exclusions
per class.
