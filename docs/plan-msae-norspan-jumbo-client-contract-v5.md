# PLAN — NORSPAN Jumbo client contract and integration v5
...
- Original declared process_limit512 is NOT proven enforced by UID-wide RLIMIT_NPROC
  ...
  hard limit.
- Formal prospective contract: docs/contract-msae-norspan-jumbo-client-storage-v3.md.
...
## Aggregate final-byte phase refinement
Prior v4 prospective review found that per-pair fences alone leave aggregates
metadata-only after the last sequential pair.validate. Specify an explicit final
ORIGINAL-FD expected-byte phase over ALL held pairs in _validate_all_pairs after
all existing pair validation/durability calls, then a separate all-original frozen
metadata/named-alias/ancestry sweep. No new expected fingerprints/hashes may be
harvested after a late mutation. Native bounded hashes use the same expected receipts.
This covers flat publisher/verifier, four-role publisher/verifier and opaque custody
recovery, which route through that helper. Keep caller root/cardinality checks too.
New test_msae_norspan_jpc_timestamp_aggregate.py has7red genuine earlier-byte
mutation cases with cached timestamps, after last EXISTING pair.validate, across
those5boundaries and both verifier durability settings. Actual-injection assertions
must follow rejection, and original donor/failure roots remain. A mutation during
the new last byte fence is NOT asserted impossible: all finite phases remain
conditional controlled-writer observations, not atomically current custody.
Independent prospective v5+contractv3 review precedes implementation.
