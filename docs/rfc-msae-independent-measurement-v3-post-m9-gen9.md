# MSAE v3 post-M9 gen9 execution protocol

Status: implementation candidate. This document implements the immutable gen9
...
- plan: `docs/plan-msae-independent-measurement-v3-post-m9-gen9.md`, SHA-256
  `135f313fdad7299a69b8b04268f4168e168a1be30ba5e80d408e43b7f5690a30`;
- plan review:
...
Gen9 preserves the reviewed M1/M2 science, unchanged AMALGUM split, calibration
...
2. add the mode-0700, 78-byte `/tmp/m9p_<stage-a-sha>.sock` monitor-to-pane
...
gen9 plan and plan review, the current seven-file implementation subject, and
...
No gen9 operation opens, hashes, or content-reads these objects. Every guarded
...
and filesystem identity. Every gate rejects any unregistered `m9t_*`, `m9b_*`,
`m9p_*`, `m9x_*`, `m9a_*`, `m9c_*`, or `m9s_*` entry. Creators set and verify
...
- tmux control: `/tmp/m9t_${D}.sock`;
- launcher-to-broker gate: `/tmp/m9b_${D}.sock`; and
- monitor-to-pane lease: `/tmp/m9p_${D}.sock`.
...
`/tmp/m9x_<test-token>.sock`, `/tmp/m9a_<test-token>.sock`,
`/tmp/m9c_<test-token>.sock`, and scratch root `/tmp/m9s_<test-token>`, plus
session `msae-independent-v3-gen9-containment`. `test-token` is the SHA-256 of
...
```text
.venv-atlas/bin/python -S -B -I scripts/msae_independent_measurement_v3_post_m2_gen9.py probe-gen9-capability --pre-capability-review-sha256 <64-lowerhex> --output reports/analysis/msae_independent_measurement_v3_post_m2_gen9/gen9_capability_reproduction.json
```
...
permits implementation review; false is permanent failure for gen9.
...
3. grouped bind/listen/connect/close/unlink round trips for all three 78-byte
   gen9 production sockets, including exact live socket and `/tmp` parent identities.
...
`msae_v3_gen9_tmux_monitor_v2` and the exact plan-registered keys/nested
...
`msae_v3_gen9_pane_lease_frame_v1` protocol carries monitor-to-pane
...
`m9b_*` family empty, then sends frame 6. The monitor independently reopens the
...
- complete gen9 CPU/static/source-only/publisher/mount/recovery tests;
...
`predecessor_remediation_v1`, `predecessor_gen4`, `gen9_cpu`,
`gen9_publisher`, and `gen9_real_tmux`.  Each object has schema
`msae_v3_gen9_pre_capability_check_v1` and exact keys `schema_version`, `id`,
...
`msae_v3_gen9_check_subject_v1` digest over the registered ordered path/SHA-256
...
Only `launch-gen9`, after signed authorization, may invoke `nvidia-smi`. It
...
handoff/session information without waiting for calibration results. No gen9
