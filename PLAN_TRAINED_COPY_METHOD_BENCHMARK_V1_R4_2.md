# Trained-Copy Method Benchmark v1 R4.2 — Pre-access Technical Launch Recovery

## Recovery boundary

R4.1 was frozen but blocked by the exact freeze adversarial review before launch because the launcher and worker disagreed on the GPU-lock basename. R4.1 is preserved unchanged and was never launched. R4.2 fixes that single integration defect, adds a launcher-compatible mocked lock-name regression, and binds the exact R4.1 rejection preservation. R4 terminated during launch preflight before any scientific payload was accessed because its `/proc/<pid>/stat` parser assumed a fixed whitespace field for PPID. R4 is preserved and has `no_retry_authorized=true`. R4.2 is a separately versioned, one-shot recovery: it preserves every scientific method, estimator, threshold, seed, offset, generated row, checkpoint, and confirmation rule; changes only mutable namespaces, executable lineage, and the process-ancestry parser; and binds the exact R4 failure preservation manifest. Its prepared JSONL payloads must be byte-identical to the unopened R4 payloads.

Acceptance additions:
- parse PPID from the substring after the final `)` in procfs stat records and test command names containing spaces and `)` characters;
- bind the exact R4 preservation manifest and terminal state before candidate, freeze, review, and launch;
- assert all three R4.2 payload hashes equal the corresponding unopened R4 freeze payload hashes;
- never resume or modify the R4 namespace.
...
- `python -m py_compile scripts/trained_copy_method_benchmark_v1_r4_2.py`
- `pytest -q tests/test_trained_copy_method_benchmark_v1_r4_2.py`
- `python scripts/trained_copy_method_benchmark_v1_r4_2.py candidate-preflight --device cpu`
- `python scripts/trained_copy_method_benchmark_v1_r4_2.py candidate-preflight --device cuda`
- `python scripts/trained_copy_method_benchmark_v1_r4_2.py mutation-suite`
- `python scripts/trained_copy_method_benchmark_v1_r4_2.py verify-freeze --opaque`
- `python scripts/trained_copy_method_benchmark_v1_r4_2.py verify-review-binding`
- `bash -n scripts/launch_trained_copy_method_benchmark_v1_r4_2_tmux.sh`
...
## R3 adversarial repair addendum

R2 candidate `1a9c7067...c90` was blocked and preserved before generator lock or payload creation. R3 prospectively adds exact registered-offset coverage audits, controller-output nonlinear Jacobian union rank, canonical single-line review-verdict parsing with an exact bound hash, configuration-derived launcher timeout, per-block tuple counts, and hostile no-access/artifact/signal mutations.


## R4.2 adversarial repair addendum

R3 candidate `cfc94705...6f4` was blocked and preserved before generator lock or payload creation. R4.2 prospectively binds the test and launcher through every lineage gate, defines analytic fit-dependent parameter counts, expands all protocol semantics in the hashed method table, exercises the actual fitted rank-64 output oracle and rank ordering, and adds unpaired-selector and zero/permutation negative mutations.
