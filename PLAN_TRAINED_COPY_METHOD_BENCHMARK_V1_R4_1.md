# Trained-Copy Method Benchmark v1 R4.1 — Pre-access Technical Launch Recovery

## Recovery boundary

R4 terminated during launch preflight before any scientific payload was accessed because its `/proc/<pid>/stat` parser assumed a fixed whitespace field for PPID. R4 is preserved and has `no_retry_authorized=true`. R4.1 is a separately versioned, one-shot recovery: it preserves every scientific method, estimator, threshold, seed, offset, generated row, checkpoint, and confirmation rule; changes only mutable namespaces, executable lineage, and the process-ancestry parser; and binds the exact R4 failure preservation manifest. Its prepared JSONL payloads must be byte-identical to the unopened R4 payloads.
...
- assert all three R4.1 payload hashes equal the corresponding unopened R4 freeze payload hashes;
...
- `python -m py_compile scripts/trained_copy_method_benchmark_v1_r4_1.py`
- `pytest -q tests/test_trained_copy_method_benchmark_v1_r4_1.py`
- `python scripts/trained_copy_method_benchmark_v1_r4_1.py candidate-preflight --device cpu`
- `python scripts/trained_copy_method_benchmark_v1_r4_1.py candidate-preflight --device cuda`
- `python scripts/trained_copy_method_benchmark_v1_r4_1.py mutation-suite`
- `python scripts/trained_copy_method_benchmark_v1_r4_1.py verify-freeze --opaque`
- `python scripts/trained_copy_method_benchmark_v1_r4_1.py verify-review-binding`
- `bash -n scripts/launch_trained_copy_method_benchmark_v1_r4_1_tmux.sh`
...
## R4.1 adversarial repair addendum

R3 candidate `cfc94705...6f4` was blocked and preserved before generator lock or payload creation. R4.1 prospectively binds the test and launcher through every lineage gate, defines analytic fit-dependent parameter counts, expands all protocol semantics in the hashed method table, exercises the actual fitted rank-64 output oracle and rank ordering, and adds unpaired-selector and zero/permutation negative mutations.
