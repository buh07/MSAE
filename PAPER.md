...
trained-copy SAE capacity study sharpened the mechanism: full-code SAE patching and rank-32 PCA [C094]
succeeded, whereas every registered fixed subset containing at most half of a 256-feature dictionary
failed joint control despite a mean full-code reconstruction error of 0.405%. [C094] Decoder-span
geometry nevertheless covered the successful rank-32 PCA subspace completely by budget 64, [C095] showing
that the failure was not explained by missing linear span alone. [C095] The supported conclusion is
local rather than universal: stable, decodable, and reconstructive representations at the tested sites
did not by themselves provide replicated, collateral-safe control. Encoding is not separability, and
potency is not precision.
...
### 4.16 Two-tier induction control v2: contribution without sufficient control

The separately frozen two-tier successor first patched the final residual state as a decoder-tail
identity skyline. Clean and sham patches reproduced their complete donor logit vectors byte-for-byte,
and normalized skyline recovery was 1.000 [1.000, 1.000]. [C070] This validates row alignment and the
registered capture--transport--patch--score path at that terminal site; because the normalization is
algebraically self-referential, the byte identities are the substantive technical check rather than
the ratio alone.

The externally fixed nine-head upstream set then produced a positive ablation advantage of 0.093 [0.049, 0.137]. [C071]
It also produced positive clean-donor recovery, but not enough to satisfy the prospective control gates.
Joint recovery was 0.166 [0.145, 0.188], clean-versus-sham specificity was 0.101 [0.081, 0.122], and the circuit-versus-control margin was 0.084 [0.069, 0.100]. [C071]

Thus the most informative contrast is between necessity-like contribution and sufficiency. Removing
the set hurt the behavior more than removing its matched controls, so the registered heads matter;
transplanting their clean state nevertheless recovered only a small fraction of the verified ceiling
and was insufficiently distinct from sham and control patches. The formal result was
`DEVELOPMENT_UPSTREAM_SET_STOP`. Confirmation remained null and its authorization record states that
the payload was not touched; no representation method was evaluated and no training occurred. [C072]

The defensible claim is therefore narrow: full-residual patching exactly recovered the registered
behavior, validating the intervention skyline, while the externally fixed upstream set showed a
reproducible ablation contribution and positive clean-donor recovery but failed the prespecified
recovery and specificity gates. This does not establish a complete GPT-2 induction circuit and does
not evaluate SAEs or linear representation methods.

### 4.17 Constructed complete-circuit positive control v1.1

The registered constructed-copy successor supplied the complete causal graph rather than expanding a
natural-model head set after observing failures. Its exact final status was
`GROUND_TRUTH_CIRCUIT_CONTROL_CONFIRMED`. [C073] The model was a single deterministic analytic
construction; no natural prompt, representation method, or training path was run. [C073]

On the first generated panel, all 256 development rows and every 32-row block entered the common
cohort. [C074] Circuit recovery, sham specificity, matched-control margin, and full-vocabulary
restoration were all 1.000, while collateral error was 0.000. [C074] Edge and source necessity were
each 0.9990234375. [C074] The disjoint generated confirmation panel reproduced those values on all
256 rows and passed every registered gate. [C075]

This positive control changes the interpretation of the preceding GPT-2 result without reversing it.
The externally fixed GPT-2 set contributed causally but did not supply sufficient or specific control;
the constructed complete circuit supplied sufficient, sham-specific, matched-control-specific, and
collateral-free control. [C071] [C074] Thus the evaluator is not structurally incapable of returning a
positive result. It recognizes complete modular control in the exact regime where that property is
known by construction. The post-result claim review nevertheless limits this validation to the exact
frozen path: the computation is analytic and nearly degenerate, the second panel changes generated
rows and token ranges rather than natural models, and soft attention, residual bypass, layer
normalization, superposition, noise, learned component discovery, and natural-model generalization
remain untested. [C073] [C075]

### 4.18 Transformer-realistic bridge and controller-capacity result

The separately frozen transformer-realistic successor extended the constructed graph through soft
attention, residual mixing and a bypass path, layer normalization, correlated private/shared
factors, overcomplete superposition, and controlled noise. The exact registered controller passed
all six regimes on both disjoint 192-row panels, producing the terminal status
`TRANSFORMER_REALISTIC_GROUND_TRUTH_CONFIRMED`. [C076] In the final noisy regime, ground-truth and
full-vocabulary recovery were 1.000 and collateral error was 0.000 on both panels. [C077] Confirmation sham
specificity was 0.943, matched-control margin was 0.924, and the exact-minus-best-incomplete gap was [C077]
0.193. [C077]

The conditional controller benchmark then completed all 18 registered paths. Under the frozen
matched-L2 estimand, the exact controller was the only family to pass every recovery, sham,
matched-control, collateral, necessity, full-vocabulary, support, and replication gate on both
development and confirmation. Random and PCA projections, DiffMean, LEACE-style removal, all three
TopK-SAE seeds, all three rank-8 behavior-supervised seeds, and every incomplete circuit/path control [C078]
failed the joint conjunction. [C078]

Those failures do not mean that every approximation was behaviorally inert. On confirmation,
DiffMean and LEACE-style controllers recovered 0.770 and 0.729 of the target margin, respectively, [C079]
but their sham-specificity points were approximately zero; their collateral errors were 0.143 and [C079]
0.313 and their full-vocabulary recoveries were 0.437 and 0.400. [C079] The SAE fits reached training
reconstruction losses near 0.0003 while their matched controllers had approximately zero mean target [C079]
recovery, whereas the privileged finite-step rank-8 supervised baseline recovered about 0.552 but [C079]
still lacked sham specificity and collateral safety. [C079]

> On a transformer-like constructed system, the evaluator recovered an exact complete controller,
> while registered low-rank, projection, SAE, and supervised approximations frequently recovered
> behavioral potency without recovering specificity, completeness, and collateral safety. [C076]
> [C078] [C079]

This is a controlled synthetic result, not a general negative claim about SAEs, supervision, K2, or
natural transformers. The full exact controller was not capacity matched to every rank-limited
baseline, and the learned failures conflate rank, optimization, objective design, feature selection,
and direction estimation. [C080] The next diagnostic must therefore vary controller rank and provide
a provably learnable full-rank supervised positive control before attributing failure specifically to
component discovery.

### 4.19 Capacity-controlled diagnostic and trained-copy positive control

The preregistered capacity diagnostic completed on three constructed generator instances, with [C081]
disjoint development and confirmation panels. Exact deltas and the provably learnable full-rank
closed-form map passed throughout. Within the global paired-linear class, rank 4 and rank 8 failed, [C081]
whereas ranks 16, 32, and 64 passed for every learned seed, instance, and panel; the corresponding
closed-form reduced-rank regressions showed the same registered threshold. [C081] This is a
family-specific capacity result, not a universal rank bound.

Capacity was not the only determinant. Full-rank unpaired PCA and full-rank random controls still
failed, while paired rank-16 controllers passed. [C082] Thus, in this construction, adequate global
rank and paired counterfactual alignment were both needed by the registered successful methods.
Per-target rank is not directly capacity-comparable: the target-indexed maps had union rank 64 at [C082]
every registered bottleneck.

The target-indexed rank-8 models made the potency--safety distinction especially clear. [C083] Their target
recovery was 1.073--1.079 and signed sham specificity was 0.861--0.869, yet collateral error remained [C083]
0.286--0.297 and full-vocabulary recovery only 0.609--0.617. [C083] Strong target movement therefore
did not imply restoration of the intended full behavioral state. The nonlinear target-conditioned
rank-16 family was also not fully stable: 16/18 model-seed/panel endpoints passed, while rank 32 [C084]
passed all endpoints. [C084]

After the exact and full-rank capacity controls authorized the conditional stage, three separately
trained synthetic copy transformers were evaluated at their architecture-fixed `head_pre_ln`
bottleneck. Every checkpoint passed development and confirmation on all 192 rows: recovery, [C085]
necessity, and full-vocabulary recovery were effectively 1, the smallest sham-specificity lower bound [C085]
was 0.973, and the largest collateral-error upper bound was (5.19\times10^{-8}). [C085] This validates
the scorer, patching path, and joint positive-control logic on trained synthetic models rather than
only an analytic generator.

> Complete causal control is measurable in constructed and trained synthetic systems. Within these
> registered constructions, recovering it required sufficient capacity and counterfactual alignment;
> targeted potency alone substantially overstated controller completeness and safety. [C081] [C082]
> [C083] [C085]

The trained-copy controller was the analytic clean-minus-corrupt head-state delta at a deliberately
complete, no-residual-bypass site. No estimated controller, SAE, K2 model, pretrained model, or
natural-language system was evaluated in that conditional stage. [C086] The result validates an
evaluator skyline, not controller discovery or population-level transformer generality.

### 4.20 Trained-copy method benchmark R4.2

The separately frozen R4.2 successor subsequently closed the estimated-controller gap left by the
R5 conditional stage. It completed prospectively on the same three qualified trained-copy
checkpoints, with estimated-method outcomes excluded from confirmation authorization and with every
method assessed on both development and confirmation under native and norm-matched estimands.
[C087] This supersedes the project-level implication of C086 without changing that historically
correct statement about R5 itself.

The exact and identity controls passed, as did output-oracle, paired-linear, and ambient-PCA donor
compressors at global ranks 32 and 64. [C088] The target-aware readout projection and target-nonlinear
rank-32 controller also passed the full all-checkpoint, both-panel conjunction. [C088] Because the
corresponding global families failed at rank 16 and passed at rank 32, the supported sufficient-rank
statement is the bracket `(16, 32]`, not proof that 32 is an intrinsic minimum. [C089]

This result narrows the earlier alignment interpretation. Ambient PCA was fit to unordered ambient
states, yet its rank-32 controller passed once that span covered the trained-copy value manifold. [C090] Counterfactual alignment was therefore necessary only in the earlier R5 constructed
diagnostic, not universally in this donor-compression setting. [C090] The rank-64 random projector was
algebraically identity-equivalent and is reported only as QA, not as meaningful random discovery.
[C091]

The capacity--safety separation remained visible even under supervision. Target-nonlinear rank 16
recovered 0.928--0.953 of the target effect but incurred 0.173--0.210 collateral error and did not [C092]
pass jointly, whereas target-nonlinear rank 32 recovered 0.993--0.996 with full-vocabulary recovery [C092]
0.931--0.943 and collateral error 0.056--0.068. [C092] Thus potency at an insufficient capacity was
not evidence of complete control.

The registered SAE result is deliberately narrower: a width-256 SAE with token TopK 16 and exactly [C091]
16 selected features failed, despite low ambient reconstruction loss. [C091] It does not establish a
general SAE failure because coding capacity, selected-feature budget, basis learning, and selection
were confounded. Moreover, all successful estimated methods in R4.2 received the clean/corrupt donor
states or donor delta at inference; none performed donor-free controller discovery. [C093]

### 4.21 Confirmed SAE capacity result and frozen geometry analysis

The separately frozen SAE-capacity R2 study evaluated five TopK settings, three SAE seeds, three
trained-copy checkpoints, four fixed selectors, both development and confirmation panels, and native
and norm-matched estimands. Every registered partial subset at budgets 16, 32, 64, and 128 failed:
0 of 2,880 estimand records passed all six joint-control gates. Full-code budget-256 patching passed
all 720 estimand records. [C094] Rank-32 PCA and paired-linear donor compressors also passed in the
same study. This is the clearest capacity contrast in the project.

The reconstruction result is informative but must be stated precisely. Full-code ambient relative-L2
reconstruction error averaged 0.405% across the 45 model×SAE-seed×TopK fits, [C094] with a range of
0.255--0.754%. [C094] Thus the most accurate summary is:

> Despite roughly 0.4% mean full-code reconstruction error, every registered fixed SAE subset
> containing at most half the dictionary failed joint control, whereas a rank-32 PCA subspace and
> full-code SAE patching succeeded across all seeds and panels. [C094]

![Recovery–collateral Pareto frontier for frozen TopK-16 SAE selectors.](reports/trained_copy_sae_geometry_v1/figures/recovery_collateral_pareto_topk16.png)

The no-forward companion analysis changes the causal diagnosis. For every frozen SAE and selector,
the decoder-row span at budget 64 already covered the successful ambient-PCA rank-32 subspace [C095] to
numerical precision (minimum coverage `0.9999999999999996`). [C095] Yet unweighted SAE-code
transplantation still failed at budgets 64 and 128. The registered failures therefore cannot be
explained by absent decoder span alone; intervention coefficients, code transplantation, selector
behavior, or conditioning within the redundant span remain plausible bottlenecks. [C095]

![Coverage of the successful rank-32 PCA subspace by frozen SAE decoder subsets.](reports/trained_copy_sae_geometry_v1/figures/pca_coverage_by_budget.png) [C095]

This geometry study used only frozen result JSON, decoder weights, selector orders, and linear bases;
it performed no model forwards. R2 did not cache per-example codes, so example-level active-set
overlap is unavailable, and feature indices were not compared across independently trained SAEs.
...
- the exact constructed-copy circuit passed complete selective-control evaluation on both generated
  panels. [C074] [C075]
- the exact controller remained complete, specific, and collateral-free through the six-regime
  transformer-realistic bridge. [C076] [C077]
- the global paired capacity sweep showed a registered rank-16 threshold while full-rank unpaired [C081] [C082]
  controls still failed. [C081] [C082]
- an exact complete controller qualified across three trained synthetic copy-transformer checkpoints
  and both fresh panels. [C085]
- estimated donor compressors also achieved complete joint control on those trained-copy checkpoints
  once their registered global capacity reached the sufficient `(16, 32]` bracket. [C087] [C088] [C089]
...
- in the transformer-realistic bridge, every registered estimated controller failed the full joint
...
10. Preserve canonical induction two-tier v2 unchanged. Its exact full-residual skyline validates the
    terminal intervention machinery, while its externally fixed upstream set contributed causally but
    did not provide sufficient recovery or specificity. Confirmation and method evaluation remain
    unopened. [C070] [C071] [C072]
11. Preserve constructed-copy v1.1 unchanged. It validates the exact analytic evaluator path, not
    K2, a natural circuit, or any representation method. Use a separately versioned transformer-like
    bridge before opening any component-discovery benchmark. [C073] [C075]
...
The subsequent canonical induction assays recovered strong edge attention and causal contributions,
but their donor patches missed the frozen material recovery and selectivity magnitudes. [C067] [C068]
[C069] Two-tier v2 then cleanly separated an exact full-residual pipeline skyline from an externally
fixed upstream intervention: the skyline passed exactly, while the fixed set remained insufficiently
potent and specific. [C070] [C071] [C072] Any further positive-control study must therefore use an
externally specified complete mechanism rather than add GPT-2 heads post hoc, and normalize circuit
recovery by the verified skyline. Single-model circuit reproduction remains technical validation only;
any general method claim still requires prospective replication in at least two model families.
The constructed-copy positive control now demonstrates that the frozen hierarchy can return complete
control when the causal graph is complete by construction. [C073] [C074] Its contrast with GPT-2 is
the key result: causal contribution without sufficient control in the natural model versus complete,
selective control in the analytic construction. It does not authorize K2 or method claims; the next
question is whether that success survives a separately frozen transformer-realistic ground-truth
bridge. [C071] [C073]

The completed bridge supplies that missing test: exact modular control survived every registered
transformer-like operation and replicated, while all registered approximations failed joint control.
[C076] [C078] The next experiment is consequently a capacity-controlled synthetic diagnostic—not a
K2 restart—and only a successful full-rank positive control can authorize a conditional trained-small-
transformer external-validity study. [C080]
...
- The constructed positive control is a single analytic generated model with deterministic, nearly
  degenerate metrics; it validates implementation behavior, not external validity. [C073] [C075]
- The transformer-realistic bridge uses one fixed untrained generator family. Its exact controller is
  not capacity matched to rank-limited estimators, and its panels do not replicate across natural
  models. [C080]
- The capacity diagnostic contains three constructed instances, and rank labels are not comparable
  across global and target-conditioned architectures. R5's conditional trained-copy stage evaluated
  only the exact donor controller, [C086] while R4.2 reused the same three already-qualified
  checkpoints from one synthetic architecture and evaluated estimated donor compressors rather than
  donor-free discovery. [C087] [C093]
...
- `results/canonical_induction_two_tier_v2_20260809/final/result.json`
- `results/canonical_induction_two_tier_v2_20260809/development/SUMMARY.json`
- `results/canonical_induction_two_tier_v2_20260809/development/QA.json`
- `reports/claim_review/canonical_induction_two_tier_v2_postresult.md`
- `results/constructed_copy_circuit_v1_1_20260809/final/result.json`
- `reports/claim_review/constructed_copy_circuit_v1_1_post_result_claim_review.md`
- `reports/provenance/constructed_copy_circuit_v1_1_postresult/PRESERVATION.json`
- `results/transformer_realistic_bridge_v1_20260810r2/final/result.json`
- `results/transformer_realistic_methods_v1_20260810r2/final/result.json`
- `reports/claim_review/transformer_realistic_bridge_v1_r2_post_result_claim_review.md`
- `reports/provenance/transformer_realistic_bridge_v1_r2_postresult/PRESERVATION.json`
...
The constructed complete-circuit control supplies the necessary positive contrast: the same hierarchy
returns full selective control when the mechanism is complete by construction, while the registered
GPT-2 circuit sets show contribution without comparable sufficiency. [C071] [C074]

The transformer-realistic bridge strengthens that contrast: the exact controller remained complete
through soft attention, bypass, layer normalization, superposition, correlation, and noise, while
approximate controllers often retained potency or necessity without joint precision. [C076] [C078]

The subsequent capacity-controlled result further localizes that gap. Sufficiently ranked paired
controllers succeeded in the constructed diagnostic, whereas full-rank unpaired controls did not;
the exact skyline then replicated across three trained synthetic copy-transformer checkpoints.
[C081] [C082] [C085] R4.2 then showed that output-oracle, paired-linear, ambient-PCA, readout-projection,
and target-nonlinear donor compressors could also pass at adequate capacity. [C087] [C088] Pairing was
not universally required: ambient PCA passed when its span covered the trained-copy manifold. [C090]
The remaining prospective question is donor-free controller discovery and whether SAE failure was a
