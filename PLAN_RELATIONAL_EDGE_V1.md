# PLAN — Relational Attention-Edge Atlas v1 (Discovery 2)
...
The new study is `relational_attention_edges_v1_discovery2`. It is not Attempt 14/15, not a retry of
the position/context atlas, and not an architecture-training authorization. Discovery 1 remains
preserved as a signed prescore-ineligible implementation candidate: before any candidate-source model
forward, its newly broadened audit mistakenly treated hashes of already excluded short-sentence
collisions copied into historical firewall reports as fresh source exposure. Its terminal is not
deleted or relabeled. Discovery 2 prospectively excludes those already removed rows from the set of
candidate exposure hashes, uses a new namespace/study key, and must pass a fresh exact-config rebuild,
freeze, and adversarial review. No scientific outcome motivated this repair.
The signed successor key must bind the literal Discovery-2 namespace, a canonical digest of sources,
model, matching, analysis, and exposure policy, the exact exposure-builder implementation digest,
and the exact signed Discovery-1 terminal and D1-only retirement hashes. To avoid a circular hash,
the D1 retirement records only the old key; the separately signed Discovery-2 source amendment
records both distinct keys and binds the immutable D1 retirement. This is the sole prescore-only
successor authorized because Discovery 1 had no source opening, model forward, activation, or
endpoint result. Any Discovery-2 prescore failure is terminal: no Discovery 3 or further audit repair
is authorized.

## Non-goals

- Do not modify or continue Attempt 13 or Attempt 14.
- Do not modify, resume, or relabel the Discovery-1 prescore terminal.
...
- The exposure audit order is frozen exactly: (1) index prior raw CoNLL-U sentence/document token
  hashes; (2) identify the prespecified long-document and repeated-short-collision exclusions; (3)
  remove those documents/sentences from the candidate sentence/token/document digest set; (4) retain
  all candidate raw-file hashes regardless; (5) scan every prior project artifact path and exact file
  hash, byte-scan aliases/literal digests (including binary artifacts), parse every JSON/JSONL
  `input_ids` sequence, and inspect explicit stage/exposure metadata; then (6) classify a hit as
  label-only only when an independently verified signed sidecar binds that exact artifact hash, record
  pointer, and zero-forward/zero-endpoint role. Ordinary JSON self-assertions and path-name heuristics
  cannot establish a role. Any ambiguous hit, scan/parse error in a
  purported structured artifact, missing required sidecar, or unexplained collision is blocking. The
  audit writes a deterministic exhaustive scanned/skipped/error ledger; “skipped without blocking” is
  forbidden.
- JSON/JSONL role metadata is attached only to the containing record/object; it may not be unioned
  across sibling records. File-wide inheritance requires an explicit `metadata_scope=FILE`. The same
  per-record classifier applies to tokenizer `input_ids`: explicit label-only/no-forward/no-endpoint
  hits are disclosed, true exposures block, and ambiguous hits block without being relabeled as model
  forwards. Every `.npy`, `.npz`, `.pt`, `.pkl`, or `.pyc` artifact requires an exact signed structural
  sidecar. The sidecar safely inspects NumPy dtypes/members, Torch ZIP pickle opcodes/members and a
  restricted `torch.load(weights_only=True, mmap=True)` object tree (with only NumPy ndarray
  reconstruction plus concrete NumPy dtype classes added for historical RNG-state compatibility and
  no unrestricted-pickle fallback), pickle integer/string opcodes, or
  Python bytecode constants without unrestricted pickle/bytecode execution; records normalized string
  hashes and every recovered integer payload path/dtype/shape; and accepts no unknown opaque type.
  Candidate-length integer arrays/tensors/opcode runs receive the same input-ID comparison. An
  identity-bearing field without a proven payload disposition blocks. All remaining unknown suffixes
  must decode as strict UTF-8 and receive line-normalized scanning or block.
- The declared project-artifact universe is every regular node under `data`, `pilot_runs`,
  `pilot_outputs`, `results`, `reports`, `configs`, `prereg`, `docs`, `analysis`, `experiments`,
  `scripts`, and `tests`, plus every top-level regular file. Git internals, virtual environments,
  tool caches, and generated `__pycache__` directories are outside the scientific artifact scope and
  must be absent/ignored rather than described as scanned. Symlinks, special nodes, unreadable files,
  and unknown non-UTF8 formats inside the declared scope block. Every in-scope path receives exactly
  one signed-universe disposition. Every universe entry's own SHA-256 is compared to every candidate
  raw-file hash regardless of suffix/path, and every entry assigned the CoNLL-U reader participates in
  sentence/document normalization. Structured strings and unstructured UTF-8/HTML/Markdown/log lines
  also receive boundary-aware multi-pattern search for candidate normalized sentences of at least
  eight characters, so prefixes or markup cannot hide embedded source text; shorter strings remain
  subject to exact whole-string/whole-line hashing.
- Token strings for content overlap use Unicode NFKC, Unicode case-folding, and internal-whitespace
  collapse before canonical-JSON SHA-256. Sentence text uses the same normalization for structured
  text search; raw `# text` SHA-256 is retained separately. A document digest is SHA-256 over the
  ordered canonical-JSON list of its normalized token-list sentence digests. The exact prespecified
  removal sequence is: an exact raw-file or normalized-document collision terminates the prescore;
  otherwise any collision sentence of at least eight syntactic words removes its document; at least
  two distinct shorter collision hashes remove their document; remaining singleton collision
  sentences are removed; only then are retained sentence/token/document content digests and tokenizer
  input-ID digests constructed. Raw source-file hashes are never removed. No source, threshold,
  normalizer, exclusion order, namespace, or fallback may change after the first Discovery-2 prescore
  build attempt.
...
- The signed lifecycle first reads and hashes the raw config bytes without validating schema,
  exploratory status, permissions, or protocol fields. It derives the declared pure study key when
  possible (otherwise a raw-config-digest provisional invalid-config identity), then uses code-fixed
  canonical lifecycle paths and a code-fixed expected Ed25519 signer to consume one global
  Discovery-2 prescore opening with `O_CREAT|O_EXCL` **before** every config, permission, amendment,
  universe, preservation, or source validation. Any malformed/invalid config then terminalizes under
  that canonical opening, so repairing it cannot retry. The primary destination is exactly the configured data root.
  Only after all primary artifacts are written and every final preservation/universe check passes is
  a separate signed `PRESCORE_PRIMARY_COMPLETE` record created with `O_CREAT|O_EXCL`; it binds the
  opening owner nonce and hash plus the exact eligible manifest hash. A rebuild is allowed only after
  verifying that completion record, writes only to the configured rebuild root, and uses the same
  byte-immutable config; it consumes a separate signed `O_CREAT|O_EXCL` rebuild claim. Opening
  ownership is bound to a random invocation nonce. Only the process that successfully creates the
  relevant claim may terminalize; concurrent/duplicate losers reject without mutating lifecycle
  state. Canonical lifecycle routing and signer identity are bound into the study key; mutable config
  routing cannot redirect an opening or terminal. The owner bootstrap context is retained in memory,
  every direct or CLI build call terminalizes post-claim exceptions internally, and terminalization
  never rereads config to locate its claim. The global terminal is itself signed and write-once; an existing terminal is accepted only
  when its complete signed payload exactly matches the owner's requested terminal. Universe and
  preservation checks occur only after the winning claim. Historical CoNLL-U overlap inputs are
  strict UTF-8 and every non-comment row must have ten columns and a valid CoNLL-U token ID. Sentence
  headers, monotone integer IDs, ranges, blank separators, and the final blank terminator are validated;
  malformed or truncated rows/sentences block rather than disappearing from normalization. Any owner primary or rebuild
  audit/support/exception failure writes a signed global no-retry terminal; arbitrary output roots are
  forbidden. The remaining lifecycle is either
  `PRESCORE_COMPLETE -> TERMINAL_PRESCORE_INELIGIBLE`, or
...
- Previous planning mentioned ParTUT/ATIS but rejected them because their one-sentence pseudo-document
  ...
  status.
- Preserve the current signed Discovery-2 draft amendment under its exact superseded path/hash. Before
  the exposure universe or first Discovery-2 build, write and sign a separately versioned, immutable
  final source-amendment artifact binding the exact
  ...
  support rejection,
  the train-only rule, a zero-model-load/zero-forward attestation, the old/new distinct study keys,
  exact plan/common/builder/config-projection/D1-retirement/opaque-sidecar/record-role-sidecar digests,
  and this plan
  digest. The carried counts are explicitly the already opened Discovery-1 label-only reference
  counts; exact Discovery-2 build/rebuild counts go only into the later rebuild attestation. The
  amendment and complete config are byte-immutable across universe creation and both builds. M2
  verifies this immutable historical artifact rather than regenerating the rationale.
